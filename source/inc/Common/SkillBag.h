...
#ifdef COMPACT_MEM
	std::map<short, SSkillGrid> m_SBagMap;
	std::map<short, SSkillGrid> m_ChangeMap;
#else
...
inline void CSkillBag::InitSkill() {
#ifdef COMPACT_MEM
	m_SBagMap.clear();
	m_ChangeMap.clear();
#else
...
inline void CSkillBag::SetChangeSkillFlag(bool bChange) {
	...
	if (!bChange)
		...
		for (std::map<short, SSkillGrid>::iterator it = m_SBagMap.begin(); it != m_SBagMap.end(); it++) {
			m_ChangeMap.insert(std::map<short, SSkillGrid>::value_type(it->first, it->second));
		}
	}
#else
...
inline bool CSkillBag::AddSkill(SSkillGrid* pGridCont) {
	...
	std::map<short, SSkillGrid>::iterator it = m_SBagMap.find(pGridCont->sID);

	pGridCont->lColdDownT = 0;

	// 新技能
	if (it == m_SBagMap.end())
		m_SBagMap.insert(std::map<short, SSkillGrid>::value_type(pGridCont->sID, *pGridCont));
	else {
	...
	if (it == m_ChangeMap.end())
		m_ChangeMap.insert(std::map<short, SSkillGrid>::value_type(pGridCont->sID, *pGridCont));
...
inline bool CSkillBag::DelSkill(short sSkillID) {
	...
	std::map<short, SSkillGrid>::iterator it = m_SBagMap.find(sSkillID);
	...
	if (it != m_SBagMap.end()) {
		...
		m_ChangeMap.insert(std::map<short, SSkillGrid>::value_type(sSkillID, grid));
	} else {
		it->second.chLv = 0;
	}
...
inline bool CSkillBag::SetSkillState(short sID, char chState) {
	...
	if (sID < 0) // 设置所有的技能
	{
		...
		if (sID > defMAX_SKILL_NO)
			...
			std::map<short, SSkillGrid>::iterator it = m_SBagMap.find(sID);
			if (it != m_SBagMap.end()) {
				if (it->second.chState != chState) {
					it->second.chState = chState;

					std::map<short, SSkillGrid>::iterator change = m_ChangeMap.find(sID);
...
inline char* SkillBagData2String(CSkillBag* pSkillBag, char* szStrBuf, int nLen) {
	...
	nBufLen += nDataLen;
	for (std::map<short, SSkillGrid>::iterator it = pSkillBag->m_SBagMap.begin(); it != pSkillBag->m_SBagMap.end(); it++) {
		pGridCont = &it->second;
