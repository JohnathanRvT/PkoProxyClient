...
public:
	...
	bool IsPlayerCha();		 // 是否玩家角色
	bool IsGMCha();				 // GM等级在0-10的GM角色
	bool IsGMCha2();			 // GM角色
	bool IsPlayerCtrlCha();  // 玩家当前控制的角色
	bool IsPlayerMainCha();  // 玩家的人角色
	bool IsPlayerFocusCha(); // 玩家当前控制的角色， 同IsPlayerCtrlCha例程
	bool IsPlayerOwnCha();   // 玩家角色，且其控制类型是玩家
	CCharacter* GetPlyCtrlCha();
	CCharacter* GetPlyMainCha();

	void WritePK(WPACKET& wpk) override; //写入玩家本身及其所有附加结构(如召唤兽等)的所有数据
	void WriteCharPartInfo(WPACKET& packet);
	void ReadPK(RPACKET& rpk) override; //重构玩家本身及其所有附加结构(如召唤兽等)
	...
	virtual void Run(uLong ulCurTick) override;
	...
	dbc::Short Cmd_BankOper(EKitBagType sourceBagType, dbc::Short sSrcGridID, dbc::Short sSrcNum, EKitBagType targetBagType, dbc::Short sTarGridID);
	...
	//生活技能
	void Cmd_LifeSkillItemAsk(ELifeSkillType type, SLifeSkillItem* pSItem);
	void Cmd_LifeSkillItemAsR(ELifeSkillType type, SLifeSkillItem* pSItem);
	//背包锁定
	void Cmd_LockKitbag();
	...
	virtual void ReflectINFof(Entity* srcent, WPACKET chginf) override;
	virtual CCharacter* IsCharacter() override { return this; }
	...
	// 与npc交易
	BOOL SafeSale(BYTE byIndex, WORD wCount, WORD& wItemID, DWORD& dwMoney);
	BOOL SafeBuy(WORD wItemID, WORD wCount, BYTE byIndex, DWORD& dwMoney);
	BOOL SafeSaleGoods(DWORD dwBoatID, BYTE byIndex, WORD wCount, WORD& wItemID, DWORD& dwMoney);
	BOOL SafeBuyGoods(DWORD dwBoatID, WORD wItemID, WORD wCount, BYTE byIndex, DWORD& dwMoney);
	...
	bool HasTradeAction() { return m_CSkillState.HasState(85); }
	//
	...
	bool AddSkillState(dbc::uChar uchFightID, dbc::uLong ulSrcWorldID, dbc::Long lSrcHandle, dbc::Char chObjType, dbc::Char chObjHabitat, dbc::Char chEffType,
					   dbc::uChar uchStateID, dbc::uChar uchStateLv, dbc::Long lOnTick, dbc::Char chType = enumSSTATE_ADD_UNDEFINED, bool bNotice = true) override; // 增加技能状态
#ifdef COMPACT_MEM_2
	virtual bool DelSkillState(std::map<unsigned char, SSkillStateUnit>::iterator& pUnit, bool bNotice = true);
#endif

	bool DelSkillState(dbc::uChar uchStateID, bool bNotice = true) override; // 删除状态
	...
	bool IsInPK() { return m_chPKCtrl & 0x01 ? true : false; }
	bool IsInGymkhana() { return m_chPKCtrl & 0x02 ? true : false; }
	void SetPKCtrl(dbc::Char chCtrl) { m_chPKCtrl = chCtrl; }
	dbc::Char GetPKCtrl() { return m_chPKCtrl; }
	bool CanPK() { return IsInPK() || IsInGymkhana(); }
	bool IsInArea(dbc::Short sAreaMask) { return GetAreaAttr() & sAreaMask ? true : false; }
	void SetRelive(Char chType = enumEPLAYER_RELIVE_ORIGIN, Char chLv = 0, cChar* szInfo = nullptr);
	void Reset();

	virtual void BreakAction(RPACKET pk = nullptr) override;
	virtual void EndAction(RPACKET pk = nullptr) override;
	// 角色事件处理函数
	virtual void AfterObjDie(CCharacter* pCAtk, CCharacter* pCDead) override;
	...
	virtual void OnLevelUp(USHORT sLevel) override;
	virtual void OnSailLvUp(USHORT sLevel) override;
	virtual void OnLifeLvUp(USHORT sLevel) override;
	...
	void SynPKCtrl();									 // 同步PK状态
	void SynAddItemCha(CCharacter* pCItemCha);
	void SynDelItemCha(CCharacter* pCItemCha);
	void CheckPing();
	void SendPreMoveTime();
	void SynSideInfo();
	void SynBeginItemRepair();
	void SynBeginItemForge();

	void SynBeginItemLottery();		 //Add by lark.li 20080513
	void SynBeginItemAmphitheater(); //Add by sunny.sun 20080723

	void SynBeginGetDisTime();	//Add by sunny.sun 20090310
	void SynBeginItemJewelryUp(); //Add by sunny.sun 20090401
	void SynGetCrystalState();	//Add by sunny.sun 20090519
	void SynBeginItemUnite();
	void SynBeginItemMilling();
	...
	void SetMotto(dbc::cChar* szMotto) {
		if (szMotto) /*strncpy(m_szMotto, szMotto, defMOTTO_LEN - 1)*/
			strncpy_s(m_szMotto, sizeof(m_szMotto), szMotto, _TRUNCATE);
	}
	dbc::cChar* GetMotto() { return m_szMotto; }
	void SetIcon(dbc::uShort usIcon) { m_usIcon = usIcon; }
	dbc::uShort GetIcon() { return m_usIcon; }
	void SetGuildName(dbc::cChar* szGuildName);
	dbc::cChar* GetGuildName();
	dbc::cChar* GetValidGuildName();
	void SetGuildMotto(dbc::cChar* szGuildMotto);
	dbc::cChar* GetGuildMotto();
	dbc::cChar* GetValidGuildMotto();
	...
	virtual void AfterAttrChange(int nIdx, LONG64 lOldVal, LONG64 lNewVal) override;
	virtual void Die() override;					// 处理重生
	...
	void BackToCity(bool Die = false, cChar* szCityName = nullptr, Long lMapCpyNO = -1, Char chSwitchType = enumSWITCHMAP_DIE);
	void BackToCityEx(bool Die = false, cChar* szCityName = nullptr, Long lMapCpyNO = -1, Char chSwitchType = enumSWITCHMAP_DIE);
	...
	void ResetScriptParam() { memset(m_lScriptParam, 0, sizeof(m_lScriptParam)); }
	...
	long GetKitbagRecDBID() { return m_lKbRecDBID; }

	//临时背包ID
	void SetKitbagTmpRecDBID(long lDBID) { m_lKbTmpRecDBID = lDBID; }
	long GetKitbagTmpRecDBID() { return m_lKbTmpRecDBID; }
	...
	long GetDBSaveInterval() { return m_timerDBUpdate.GetInterval(); }
	void ResetPosState() {
	...
	DWORD m_dwBoatCtrlTick{0}; // 熟练度记时用

	// AI使用的变量和接口函数----------------------------------------------------------------
	// 最好的方式是封装一个 CAICharacter, 交给Character继承
	BYTE m_AIType{0};
	CCharacter* m_AITarget{nullptr};
	CCharacter* m_HostCha{nullptr}; // 宠物的主人
	int m_nPatrolX{0};				// 巡逻点坐标
	int m_nPatrolY{0};
	short m_sChaseRange{1000}; // 角色的追踪范围, 超过这个范围角色就会回出生点休眠
	BYTE m_btPatrolState{0};   // 巡逻状态, 0 表示停留在点1
							   //           1 表示停留在点2
							   //           2 表示正从点1前往点2
							   //           3 表示正从点2前往点1
public:
...
protected:
private:
	virtual void OnBeginSee(Entity*) override;
	virtual void OnEndSee(Entity*) override;
	virtual void OnBeginSeen(CCharacter* pCCha) override;
	virtual void OnEndSeen(CCharacter* pCCha) override;
	virtual void AreaChange() override;
	...
	virtual void AfterStepMove() override;
	virtual void SubsequenceFight() override;
	virtual void SubsequenceMove() override;
...
#else
	CKitbag* m_pCKitbag{nullptr}; // 道具栏
#endif

public:
	CKitbag* m_pCKitbagTmp{nullptr}; // 临时背包
	...
	std::unique_ptr<CHateMgr> m_pHate{std::make_unique<CHateMgr>()}; // 仇恨度

	BOOL m_bRelive{false}; // 是否正在被接受复活

	BOOL m_bVol{false};		// 是否是志愿者
	BOOL m_bInvited{false}; // 是否正在被邀请
	...
	std::array<short, 9> m_sTigerItemID; // 老虎机的9个道具ID
	std::array<short, 3> m_sTigerSel;	// 投注标志
#ifdef NEW_MAPCOPY
...
private:
	...
	// 交易信息
	mission::CTradeData* m_pTradeData{nullptr};
...
#ifdef SHUI_JING
	DWORD m_lFlatMoney{0};	 //平台帐户
	DWORD m_cancelSaletick{0}; //取消挂单时间
	DWORD m_refreshtick{0};	//刷新时间限制
	DWORD m_getmoneytick{0};   //取平台帐户时间限制
	int m_buyNum{0};		   //买单个数
	int m_saleNum{0};		   //卖单个数
public:
...
public:
	...
	//Added by deguix
	WORD wCountLimit(WORD wCount, DWORD dwPrice);
};
