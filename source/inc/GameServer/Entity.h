...
public:
	...
	const Rect& GetRectTerritory() { return m_SRespawnTerritory; }
	void SetRectTerritory(Rect& STerritory) {
		m_DoesRespawnTerritoryExist = true;
		m_SRespawnTerritory = STerritory;
	}
	void NotiChgToEyeshot(WPACKET chginf, bool bIncludeOwn = true);
	void SetLogEnable(bool bEnable = true) { m_CLog.SetEnable(bEnable); }
	void SetName(dbc::cChar* cszName) { /*strncpy(m_name, cszName, defENTITY_NAME_LEN - 1);*/
		strncpy_s(m_name, sizeof(m_name), cszName, _TRUNCATE);
		m_name[defENTITY_NAME_LEN - 1] = '\0';
	}
	const char* GetName() { return m_name; }
	const char* GetLogName() { return m_CLog.GetLogName(); }
	void SetBirthCity(dbc::cChar* cszName) { /*strncpy(m_szBirthCity, cszName, MAX_MAPNAME_LENGTH - 1);*/
		strncpy_s(m_szBirthCity, sizeof(m_szBirthCity), cszName, _TRUNCATE);
		m_szBirthCity[MAX_MAPNAME_LENGTH - 1] = '\0';
	}
	const char* GetBirthCity() { return m_szBirthCity; }
	void SetBirthMap(dbc::cChar* cszName) { /*strncpy(m_szBirthMap, cszName, MAX_MAPNAME_LENGTH - 1)*/
		strncpy_s(m_szBirthMap, sizeof(m_szBirthMap), cszName, _TRUNCATE);
	...
	dbc::Short GetEyeshotWidth();
	dbc::Short GetEyeshotHeight() { return GetEyeshotWidth(); }
	bool IsInEyeshot(Entity* pCTarEnti);

	// 实体切换地图单元网格事件处理函数
	virtual void GotoMapUnit(){};

	virtual CFightAble* IsFightAble() { return nullptr; }
	virtual CMoveAble* IsMoveAble() { return nullptr; }
	virtual CItem* IsItem() { return nullptr; }
	virtual Monster* IsMonster() { return nullptr; }
	virtual CCharacter* IsCharacter() { return nullptr; }
	virtual CAttachable* IsAttachable() { return nullptr; }
	virtual mission::CNpc* IsNpc() { return nullptr; }
	virtual mission::CEventEntity* IsEvent() { return nullptr; }
...
public:
	...
	bool m_DoesRespawnTerritoryExist{false};
	Rect m_SRespawnTerritory; // 领地
	short m_sAngle;
	/**
	 * @label 当前所在的地图 
