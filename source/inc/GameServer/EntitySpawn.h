...
public:
	...
	long GetChaCount() { return m_lCount; }

protected:
private:
	long m_lRegionNum{0};
	SMonInfo* m_pSMonInfo{nullptr};

	char m_szSpawnTable[_MAX_FNAME];
	long m_lRecordNum;
	CMonRefRecordSet* m_pCMonRefRecordSet{nullptr};
...
private:
	...
	long m_lRecordNum{0};
	SubMap* m_pCMap;
	CSwitchMapRecordSet* m_pCSwitchMapRecSet{nullptr};
};
...
protected:
	...
	// 招唤的NPC列表
	std::array<mission::CNpc*, ROLE_MAXNUM_MAPNPC> m_NpcList;
	USHORT m_sNumNpc;
};
