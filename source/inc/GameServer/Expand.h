...
inline int lua_AddItemAttr(lua_State* pLS) {
	...
	}
	{
		...
		if ((lAttrID == ITEMATTR_VAL_PARAM1) || (lAttrID == ITEMATTR_VAL_PARAM2) || (lAttrID == ITEMATTR_VAL_LEVEL)) {
...
inline int lua_SetMapEntryTime(lua_State* pLS) {
	...
	const int nParaNum = lua_gettop(pLS); // 取得参数个数
	...
	}
	{
		...
		if (n != 5) {
			...
			MessageBox(nullptr, szTime, RES_STRING(GM_EXPAND_H_00111), MB_OK);
			bSuccess = false;
			goto End;
		}
...
inline int lua_SetMapEntryEventName(lua_State* pLS) {
	...
	{
		auto pEntry = static_cast<CDynMapEntryCell*>(lua_touserdata(pLS, 1));
		if (!pEntry) {
#ifdef defPARSE_LOG
			//g_pCLogObj->Log("\t入口对象不存在，调用失败\n");
			g_pCLogObj->Log("\t entrance object is inexistence,transfer failed\n");
...
inline int lua_Lua(lua_State* pLS) {
	...
	luaL_handled_dostring(pLS, lua_tostring(pLS, 1)); //CHANGED: better lua error handling (deguix)
...
inline int lua_ReloadCal(lua_State* L) {
	//luaL_handled_dofile(L, GetResPath("script\\calculate\\skilleffect.lua")); //CHANGED: better lua error handling (deguix)
	return 1;
}
...
inline int lua_ChangeExternValue(lua_State* pLS) {
	try {
		...
		if (pCCha) {
			...
			WRITE_CMD(wpk, CMD_MM_EXCHEXTERNVALUE);
			WRITE_LONG(wpk, pCCha->GetID());
			WRITE_STRING(wpk, szScriptName);
			WRITE_LONG(wpk, m_lValue);
