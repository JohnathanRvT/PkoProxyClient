...
public:
	...
	bool IsOpen() { return m_bValid && m_chState == enumMAP_STATE_OPEN; }
	bool SetCopyNum(dbc::Short sCpyNum);
	dbc::Short GetCopyNum() { return m_sMapCpyNum; }
	SubMap* GetCopy(dbc::Short sCpyNO = -1);
	void SetCopyPlyNum(dbc::Short sPlyNum) { m_sCopyPlyNum = sPlyNum; }
	dbc::Short GetCopyPlyNum() { return m_sCopyPlyNum; }
	bool InitCtrl();
	...
	bool HasDynEntry() { return strcmp(m_szEntryMapName, "") != 0 ? true : false; }
	void SetCanSavePos(bool bCan = true) { m_bCanSavePos = bCan; }
	bool CanSavePos() { return m_bCanSavePos; }
	void SetCanPK(bool bCan = true) { m_bCanPK = bCan; }
	bool CanPK() { return m_bCanPK; }
	...
	bool CanTeam() { return m_bCanTeam; }
	bool CanStall() { return m_bCanStall; } //能否摆摊
	bool CanGuild() { return m_bCanGuild; }
	void SetType(dbc::Char chType = enumMAPTYPE_NORMAL) { m_chType = chType; }
	dbc::Char GetType() { return m_chType; }
	void SetCopyStartType(dbc::Char chStartType = enumMAPCOPY_START_PLAYER) { m_chCopyStartType = chStartType; }
	dbc::Char GetCopyStartType() { return m_chCopyStartType; }
	void SetCopyStartCondition(dbc::Char chType, dbc::Long lVal) {
		m_chCopyStartCdtType = chType;
		m_lCopyStartCdtVal = lVal;
	}
	dbc::Char GetCopyStartCdtType() { return m_chCopyStartCdtType; }
	dbc::Long GetCopyStartCdtVal() { return m_lCopyStartCdtVal; }

	void SetName(dbc::cChar* cszName) { m_strMapName = cszName; }
	const char* GetName() { return m_strMapName.c_str(); }
	const Rect& GetRange() { return m_SRange; }
	...
	bool IsRepatriateDie() { return m_bRepatriateDie; }

	void BeginGetUsedCopy() { m_sUsedCopySearch = 0; }
	SubMap* GetNextUsedCopy();

	mission::CNpc* FindNpc(const char szName[]);

	// Add by lark.li 20090331 begin for 地图状态检查
	void CheckMapState(std::string& str);
	...
	struct
	{
		...
		std::string m_strEntryFile; // 入口文件脚本
							   // End
	};
