...
protected:
	void Initially();
	void Finally();
	void WritePK(WPACKET& wpk) override;
	void ReadPK(RPACKET& rpk) override;

	virtual bool overlap(long& xdist, long& ydist) override;
	...
	dbc::Short GetMoveStopState() { return m_SMoveInit.sStopState; }
	const Point& GetMoveEndPos() { return m_SMoveInit.SInflexionInfo.SList[m_SMoveInit.SInflexionInfo.sNum - 1]; }
	bool SetMoveOnInfo(SMoveInit* pSMoveI);

private:
	virtual void AfterStepMove(){}; // 处理地图切换，该函数没有放在AttemptMove例程中，是因为AfterStepMove可能进行地图切换而导致当前地图为空，而AttemptMove之后可能进行视野通告，地图不能为空
	virtual void SubsequenceMove(){};

	void BeginMove(dbc::uLong ulElapse = 0);
	void EndMove();
	void OnMoveBegin() { m_bOnMove = true; }
	void OnMoveEnd() { m_bOnMove = false; }
