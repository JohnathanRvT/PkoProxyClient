...
public:
	...
	// 类的类型信息
	virtual CNpc* IsNpc() override { return this; }
...
public:
	...
	// 类的类型信息
	virtual void SetType() override { m_byType = TALK; }

	// 装载npc配置信息
	virtual BOOL Load(const CNpcRecord& recNpc, const CChaRecord& recChar) override;

	// 装载脚本信息
	virtual BOOL InitScript(const char szFunc[], const char szName[]);

	//
	virtual HRESULT MsgProc(CCharacter& character, RPACKET& packet) override;

	//
	virtual BOOL MissionProc(CCharacter& character, BYTE& byState) override;

	// 判定npc的地图属性
	virtual BOOL IsMapNpc(const char szMap[], USHORT sID) override;

	// 添加触发器信息
	virtual BOOL AddNpcTrigger(WORD wID, mission::TRIGGER_EVENT e, WORD wParam1, WORD wParam2, WORD wParam3, WORD wParam4) override;

	// 触发器事件处理
	virtual BOOL EventProc(TRIGGER_EVENT e, WPARAM wParam, LPARAM lParam) override;

	// NPC被召唤
	virtual void Summoned(USHORT sTime) override;
