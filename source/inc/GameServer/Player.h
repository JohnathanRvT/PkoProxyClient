...
public:
	...
	void SetPassword(const char szPassword[]) { /*strncpy( m_szPassword, szPassword, ROLE_MAXSIZE_PASSWORD2 );*/
		strncpy_s(m_szPassword, sizeof(m_szPassword), szPassword, _TRUNCATE);
	}
	...
	dbc::Long GetID() { return m_lID; }
	void SetHandle(dbc::Long lHandle) { m_lHandle = lHandle; }
	dbc::Long GetHandle() { return m_lHandle; }
	void SetHoldID(dbc::Long lID) { m_lHoldID = lID; }
	dbc::Long GetHoldID() { return m_lHoldID; }

	bool IsPlayer() { return bIsValid && (GetGate() ? true : false); }

	virtual void OnLogoff() override {} // 添加这个player的数据库存盘代码
	...
	DWORD GetDBActId() { return m_dwDBActId; }
	void SetActName(dbc::cChar* szActName) { /*strncpy(m_chActName, szActName, ACT_NAME_LEN - 1);*/
		strncpy_s(m_chActName, sizeof(m_chActName), szActName, _TRUNCATE);
		m_chActName[ACT_NAME_LEN - 1] = 0;
	}
	dbc::cChar* GetActName() { return m_chActName; }
	void SetGMLev(dbc::Char chGMLev) { m_chGMLev = chGMLev; }
	dbc::uChar GetGMLev() { return (dbc::uChar)m_chGMLev; }
	void SetMapMaskDBID(long lID) { m_lMapMaskDBID = lID; }
	long GetMapMaskDBID() { return m_lMapMaskDBID; }
	...
	dbc::uLong GetLoginChaType() { return m_ulLoginCha[0]; }
	dbc::uLong GetLoginChaID() { return m_ulLoginCha[1]; }
	...
	bool HasTeam() { return getTeamLeaderID() != 0 ? true : false; }
	void BeginGetTeamPly() { m_sGetTeamPlyCount = 0; }
	CPlayer* GetNextTeamPly();

	void SetChallengeType(dbc::Char chType) { m_chChallengeType = chType; }
	dbc::Char GetChallengeType() { return m_chChallengeType; }
	bool SetChallengeParam(dbc::Char chParamID, dbc::Long lParamVal);
	dbc::Long GetChallengeParam(dbc::Char chParamID);
	bool HasChallengeObj();
	void ClearChallengeObj(bool bAll = true);
	void StartChallengeTimer() { m_timerChallenge.Begin(30 * 1000); }

	bool OpenRepair(CCharacter* pCNpc);
	CCharacter* GetRepairman() { return m_pCRepairman; }
	bool SetRepairPosInfo(dbc::Char chPosType, dbc::Char chPosID);
	SItemGrid* GetRepairItem() { return m_pSRepairItem; }
	bool CheckRepairItem() { return (m_SRepairItem == *m_pSRepairItem) ? true : false; }
	bool IsRepairEquipPos() { return m_chRepairPosType == 1 ? true : false; }
	dbc::Char GetRepairPosID() { return m_chRepairPosID; }
	bool IsInRepair() { return m_bInRepair; }
	...
	CCharacter* GetForgeman() { return m_pCForgeman; }
	bool IsInForge() { return m_bInForge; }
	bool IsInLifeSkill() { return m_bInLiftSkill; }
	...
	void SetLifeSkillInfo(ELifeSkillType type, SLifeSkillItem* pLifeSkill) { m_lLifeSkillType = type, m_pSLifeSkillItem = *pLifeSkill; }
	dbc::Char GetForgeType() { return m_chForgeType; }
	SForgeItem* GetForgeItem() { return &m_SForgeItem; }
	...
	CCharacter* GetMainCha() { return m_pMainCha; }
	CCharacter* GetCtrlCha() { return m_pCtrlCha; }
	...
	void SetMaskMapName(const char* szMapName) { /*strncpy(m_szMaskMapName, szMapName, MAX_MAPNAME_LENGTH - 1);*/
		strncpy_s(m_szMaskMapName, sizeof(m_szMaskMapName), szMapName, _TRUNCATE);
	}
	const char* GetMaskMapName() { return m_szMaskMapName; }
	bool IsMapMaskChange() { return m_chMapMaskChange >= 3 ? true : false; }
	void SetMapMaskChange() { m_chMapMaskChange++; }
	void ResetMapMaskChange() { m_chMapMaskChange = 0; }
	float GetMapMaskOpenScale(const char* szMapName) { return m_CMapMask.GetMapMaskOpenScale(szMapName); }

	// 银行
	char GetCurBankNum() { return m_chBankNum; }
	...
	void CloseBank();
	CCharacter* GetBankNpc() { return m_pCBankNpc; }
...
private:
	...
	{
		bool m_bInLiftSkill;
		ELifeSkillType m_lLifeSkillType;
