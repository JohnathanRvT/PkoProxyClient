...
#define MONITOR_DUMP                                             \
	if (ProfileMonitor::MonitorManager::Instance()->GetEnable()) \
...
public:
	...
	inline void Register(std::string& funcsig_, LONGLONG span) {
		...
		try {
			// Step 1
			auto it = FunNumMap_.find(funcsig_);
			...
			// Step 2
			auto it1 = FunSpanList_.end();
			auto it2 = FunSpanList_.end();

			for (auto it = FunSpanList_.begin(); it != FunSpanList_.end(); it++) {
				if (span > it->second) {
					it1 = it;
				}
...
private:
	...
	using NumSpanT = std::pair<int, LONGLONG>;
	using FunNumMapT = std::map<std::string, NumSpanT>;
	using FunNumMapIt = std::map<std::string, NumSpanT>::iterator;

	using FunSpanT = std::pair<std::string, LONGLONG>;
	using FunSpanListT = std::list<FunSpanT>;
	using FunSpanIt = std::list<FunSpanT>::iterator;
