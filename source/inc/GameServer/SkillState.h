...
//TODO: "TODO modification, each character does not save all skill states, only save the skill state that they own"
// "Role initialization from 8200Byte to 1084Byte, maximum reduction (8200-1084) * 24000/1024/1024 = 163M memory"
// "Map initialization from 8200Byte to 1084Byte, maximum reduction (8200-1084) *11600/1024/1024= 79M memory"
class CSkillState {
public:
	...
	unsigned char GetStateNum(void) {
		return (unsigned char)m_SStateMap.size();
	}
#else
	SSkillStateUnit* GetSStateByNum(unsigned char uchNum);
	unsigned char GetStateNum() { return m_uchStateNum; }
...
#ifdef COMPACT_MEM_2
	unsigned char GetSkillStateChangeNum(void) {
		return (unsigned char)m_SStateChangeMap.size();
	}
#else
	void SetSkillStateChangeBitFlag(long lBit);
	unsigned char GetSkillStateChangeNum() { return m_uchChangeNum; }

	void BeginGetState() { m_uchCurGetNo = m_uchStateNum; }
	SSkillStateUnit* GetNextState();
...
#ifdef COMPACT_MEM_2
public:
	std::map<unsigned char, SSkillStateUnit> m_SStateMap;
	std::map<unsigned char, SSkillStateUnit> m_SStateChangeMap;
...
inline void CSkillState::ResetSkillState() {
#ifdef COMPACT_MEM_2
	m_SStateMap.clear();
	m_SStateChangeMap.clear();
#else
...
inline bool CSkillState::AddSkillState(unsigned char uchFightID, unsigned long ulSrcWorldID, long lSrcHandle, char chObjType, char chObjHabitat, char chEffType,
	...
	try {
		...
		std::map<unsigned char, SSkillStateUnit>::iterator it = m_SStateMap.find(uchStateID);
		// 如果不存在
		if (it == m_SStateMap.end()) {
			...
			m_SStateMap.insert(std::map<unsigned char, SSkillStateUnit>::value_type(uchStateID, unit));
...
#ifdef COMPACT_MEM_2
		// lBit 即为状态ID
		std::map<unsigned char, SSkillStateUnit>::iterator it = m_SStateChangeMap.find((const unsigned char)lBit);
		// 如果不存在
...
#ifdef COMPACT_MEM_2
#else
inline SSkillStateUnit* CSkillState::GetNextState() {
	if (m_uchCurGetNo <= m_uchStateNum)
...
inline bool CSkillState::WriteState(WPACKET& pk) {
	...
	for (std::map<unsigned char, SSkillStateUnit>::iterator it = m_SStateMap.begin(); it != m_SStateMap.end(); it++) {
		pUnit = &it->second;
