...
public:
	CSkillTempData(size_t ulSize = 1) : PreAllocStru(1) {
		sRange[0] = enumRANGE_TYPE_NONE;
		sStateParam[0] = SSTATE_NONE;
	}

	short sUseEndure{0}; // 对“耐久度“的消耗
	short sUseEnergy{0}; // 对“能量“的消耗
	short sUseSP{0};	 // 对“SP“的消耗

	long lResumeTime{0};						 // 再释放需要的时间（毫秒）
