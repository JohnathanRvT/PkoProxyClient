...
public:
	...
	const char* GetName() { return m_pCMapRes->GetName(); }
	...
	CCharacter* GetNextCharacterInRange();
	...
	CCharacter* ChaSpawn(dbc::Long lChaInfoID, dbc::Char chCtrlType, dbc::Short sAngle, Point* pSPos, bool bEyeshotAbility = false,
						 dbc::cChar* cszChaName = nullptr, const long clSearchRadius = 120 * 100);
	...
	dbc::Short GetEyeshotCellWidth() const { return m_pCMapRes->m_csEyeshotCellWidth; }
	dbc::Short GetEyeshotCellHeight() const { return m_pCMapRes->m_csEyeshotCellHeight; }
	dbc::Short GetEyeshotCellLin() const { return m_pCMapRes->m_sEyeshotCellLin; }
	dbc::Short GetEyeshotCellCol() const { return m_pCMapRes->m_sEyeshotCellCol; }
	dbc::Short GetStateCellWidth() const { return m_pCMapRes->m_csStateCellWidth; }
	dbc::Short GetStateCellHeight() const { return m_pCMapRes->m_csStateCellHeight; }
	dbc::Short GetStateCellLin() const { return m_pCMapRes->m_sStateCellLin; }
	dbc::Short GetStateCellCol() const { return m_pCMapRes->m_sStateCellCol; }
	dbc::Short GetBlockCellWidth() const { return m_pCMapRes->m_csBlockUnitWidth; }
	dbc::Short GetBlockCellHeight() const { return m_pCMapRes->m_csBlockUnitHeight; }
	...
	const Rect& GetRange() const { return m_pCMapRes->GetRange(); }
	BYTE GetMapID() { return m_pCMapRes->GetMapID(); }
	dbc::Short GetEyeshotWidth() const { return m_pCMapRes->m_eyeshotwidth; }
	bool CanPK() { return m_pCMapRes->CanPK(); }
	bool CanSavePos() { return m_pCMapRes->CanSavePos(); }
	void BeforePlyOutMap(CCharacter* pCCha);
	void AfterPlyEnterMap(CCharacter* pCCha);
	dbc::Long GetMonsterNum() { return m_pCMapRes->m_pCMonsterSpawn->GetChaCount(); }
	CMapRes* GetMapRes() { return m_pCMapRes; }
	void SetCopyNO(dbc::Short sCopyNO) { m_sCopyNO = sCopyNO; }
	dbc::Short GetCopyNO() { return m_sCopyNO; }
	dbc::Long GetPlayerNum() { return m_lPlayerNum; }
	...
	CEyeshotCell** m_pCEyeshotCell{nullptr}; // 视野单元
	CStateCell*** m_pCStateCell{nullptr};	// 状态单元
	COutMapCha m_COutMapCha;				 // 地图外角色
	//供随机访问的地图信息数组
	CActEyeshotCell m_CEyeshotCellL; // 激活的视野单元列表
	CActStateCell m_CStateCellL;	 // 激活的状态单元列表

	bool m_bIsRun{false};	 // 标示本地图是否在运行
