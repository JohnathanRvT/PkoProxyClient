...
public:
	~CWeatherMgr();
	void AddWeatherRange(std::unique_ptr<CWeather> pWeather);
	void RemoveWeather(std::unique_ptr<CWeather>& pWeather);
	void Run(SubMap* pMap); // 被定时器调用, 内部会调用每一个Weather的RandLocation
	void ClearAll();

protected:
	std::list<std::unique_ptr<CWeather>> _WeatherList;
};

inline void CWeatherMgr::AddWeatherRange(std::unique_ptr<CWeather> pWeather) {
	_WeatherList.emplace_back(std::move(pWeather));
}

inline void CWeatherMgr::RemoveWeather(std::unique_ptr<CWeather>& pWeather) {
