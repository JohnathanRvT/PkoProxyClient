...
inline int lua_handled_pcall(lua_State* L, int nargs, int nresults, int errfunc) {
	try {
		int nStatus = lua_pcall(L, nargs, nresults, errfunc);
		if (nStatus != 0) {
			switch (nStatus) {
			case LUA_ERRRUN:
...
inline int luaL_handled_dostring(lua_State* L, const char* str) {
	try {
		int nStatus = luaL_loadstring(L, str);
		switch (nStatus) {
		case 0:
...
inline int luaL_handled_dofile(lua_State* L, const char* filename) {
	try {
		int nStatus = luaL_loadfile(L, filename);
		switch (nStatus) {
		case 0:
			return (nStatus || lua_handled_pcall(L, 0, LUA_MULTRET, 0));
		case LUA_ERRSYNTAX:
...
inline void lua_FrameMove() {
	try {
		luaL_handled_dostring(g_pLuaState, "RunTimer()"); //CHANGED: better lua error handling (deguix)
	}
	T_E
}
...
inline void lua_AIRun(CCharacter* pCha) {
	try {
		...
		//CHANGED: better lua error handling (deguix) - also removed useless int
		lua_handled_pcall(g_pLuaState, 1, 0, 0);
