...
inline int lua_AddWeatherRegion(lua_State* L) {
	...
	auto weather = std::make_unique<CWeather>(btType);
	weather->SetFrequence(rng.uniform(dwFre + 10, dwFre + 10 + 20 - 1));
	weather->SetRange(sx, sy, w, h);
	weather->SetStateLastTime(dwLastTime);

	g_pScriptMap->m_WeatherMgr.AddWeatherRange(std::move(weather));
...
inline int lua_SummonCha(lua_State* L) {
	...
	if (sType == 1) // 放置水雷
	{
		pCha = pHost->GetSubMap()->ChaSpawn(sChaInfoID, enumCHACTRL_PLAYER_PET, rng.uniform(0, 360 - 1), &Pos);
		if (pCha) {
	...
	{
		Pos.move(rng.uniform(0, 360), 3 * 100);
		pCha = pHost->GetSubMap()->ChaSpawn(sChaInfoID, enumCHACTRL_PLAYER_PET, rng.uniform(0, 360 - 1), &Pos);
		if (pCha) {
			pCha->m_HostCha = pHost;
			pCha->SetPlayer(pHost->GetPlayer());
