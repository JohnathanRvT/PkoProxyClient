...
public:
	...
	CActionState* GetCurState() { return _pCurState.get(); }
...
public:																			  // CActionState 使用
	bool SwitchState(std::unique_ptr<CActionState> pState);						  // 清除现有队列,中断当前操作,执行现有操作
	bool InsertState(std::unique_ptr<CActionState> pState, bool IsFront = false); // 将现有操作压入队列,以后执行
	bool AddState(std::unique_ptr<CActionState> pState);
...
protected:
	...
	using synchro = std::list<CStateSynchro*>;
	void _ExecSynchro(synchro& s);
	void _ClearSynchro(synchro& s);

protected:
	std::unique_ptr<CActionState> _pCurState; // 当前状态
	using states = std::list<std::unique_ptr<CActionState>>;
	states _statelist;
...
private:
	using fights = std::vector<CServerHarm*>;
	fights _fights;

	using dies = std::vector<CActorDie*>; // 网络包：怪物掉料
	dies _dies;
...
//TODO: Should this be a sink function?
