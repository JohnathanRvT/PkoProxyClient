...
public:
	...
	void FrameMove(DWORD dwTailTime) override;
...
public:
	int m_iType{-1};
	SCameraMode* m_pModel{nullptr};

	BOOL m_bEnableRota{false};
	BOOL m_bEnableUpdown{true};
	BOOL m_bEnableTemp{true};

	BOOL m_bChangeModel{false};
	...
	bool m_bReset{false};
	float m_InitAngle{0.0f};  //INIT_CAMERA_ANGLE;
	float m_fResetRotatVel;
	float m_fResetRotatAccl;

	bool m_bToDefault{false};
