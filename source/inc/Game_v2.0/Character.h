...
private:
	virtual BOOL _Create(int nCharTypeID, int nType) override;

public:
	virtual bool GetRunTimeMatrix(MPMatrix44* mat, DWORD dummy_id) override;
...
public:
	...
	void StopMove();
	CActor* GetActor() { return _pActor.get(); }
...
private: // 移动
	...
	bool _isArrive{true};
	bool _isStopMove;
	float _fMapHeight{0.0f}; // 在地图上的总高度，单位：米
	std::unique_ptr<CSceneHeight> _pSceneHeight{std::make_unique<CSceneHeight>(this)};
...
private:
	...
	CChaRecord* _pDefaultChaInfo{nullptr};
	std::unique_ptr<CActor> _pActor{std::make_unique<CActor>(this)};

private:
	char _szName[33]{"Player"}; // 名字
	char _szHumanName[33]{};	// 第一个角色的名字
	char _szGuildName[33]{};	// 公会名称
	char _szGuildMotto[101]{};  // 公会座右铭
	int _nGuildID{0};			// 公会ID
	...
	long _lSideID{0};				 // 所属那一方,红方,绿方,用于两方作战时使用
	CShadeEff* _pSideShade{nullptr}; // 用于显示红方,绿方
...
public:
	...
	void RefreshUI(int nParam = 0) override;
...
public:
	...
	int ReCreate(DWORD type_id) override;
...
public: // 技能运行状态
	CChaStateMgr* GetStateMgr() { return _pChaState.get(); }
...
private:
	std::unique_ptr<CChaStateMgr> _pChaState{std::make_unique<CChaStateMgr>(this)};
...
protected:
	BOOL _bEnableAI{false};
	DWORD _dwAIInterval;
	DWORD _dwLastAITick;

	std::unique_ptr<GUI::CHeadSay> _pHeadSay{std::make_unique<GUI::CHeadSay>(this)};
...
#ifdef _LOG_NAME_
	char _szLogName[128]{"LogName"};

public:
	void setLogName(const char* str) { strncpy_s(_szLogName, sizeof(_szLogName), str, _TRUNCATE); }

	static bool IsShowLogName;
#endif

private:
	float _fMoveSpeed{1.0f};
	char* _pszFootMusic{nullptr};
	char* _pszWhoopMusic{nullptr};
	float _fMaxOpacity{1.0f}; // 客户端透明度最大值,1.0f为实心
	...
	CSceneItem* _pNpcStateItem{nullptr};
	CSceneItem* _pShopItem{nullptr};

	int _nNpcType{0};
	bool _IsForUI{false}; // 这个角色是画在UI上的

	long _nLeaderID{0};			 // 所属组队的ID
	char _szSecondName[41]{""};  // 第二个名字,即座右铭
	BOOL _bShowSecondName{true}; // 是否显示第二个名字
	short _sPhotoID{0};			 // 头像ID

	EChaModalType _eChaModalType{enumMODAL_MAIN_CHA}; // 模型类型
	EChaCtrlType _eChaCtrlType{enumCHACTRL_PLAYER};   // 操作类型
	eMainChaType _eMainType{enumMainNone};			  // 主角类型
	int _nDanger{0};								  // 危险程序，用于玩家鼠标选择时，按危险程序排序

	DWORD _dwHumanID{0}; // 用于与Group Server通信的ID

	char _chGMLv{0};
	CEvent* _pEvent{nullptr};

	xShipInfo* _pShipInfo{nullptr};
	bool _bUpdate{false};
	CEffectObj* _pBoatFog{nullptr}; // 船的烟雾,与血有关
...
private:
	...
	// 怪物摆放所需要的参数记录-----------------------------------
	short _sReliveTime{60}; // 放怪时用来记录重生时间
	int _nPatrolX{0};		// 巡逻点x
	int _nPatrolY{0};		// 巡逻点y
	// -----------------------------------------------------------

	int _nServerX{0}, _nServerY{0};
...
protected:
	CCharacter* mParentNode{nullptr};
	int mParentBoneID{0};
...
inline void CCharacter::HitEffect(int nAngle) {
	if (!_hits.empty()) {
		for (auto& hit : _hits) {
			SelfEffect(hit.nEffectID, hit.nDummy, false, -1, nAngle);
		}
	}
}
