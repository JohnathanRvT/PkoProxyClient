...
class MPDrunkCtrl {
private:
	MPSceneItem* dummy_obj{nullptr};
	MPCharacter* cha_obj{nullptr};
...
public:
	...
	virtual int LoadShip(DWORD type_id, DWORD ship_type, const DWORD* part_buf);
	virtual int LoadShip(DWORD type_id, const DWORD* part_buf);
	...
	CSceneItem* GetLinkItem(DWORD link_id) { return link_id < 0 || link_id >= LINK_ID_NUM ? nullptr : _LinkItem[link_id]; }
