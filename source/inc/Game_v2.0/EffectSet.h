...
//	CEffectInfo()
//	{
//	   strncpy_s(szName,sizeof(szName), "无名特效", _TRUNCATE);
//	   nPhotoTexID = 0;
//	   nHeightOff  = 0;
//	   nEffType	= 0;
//	   nScaleX	= 100;
...
//
//		strncpy_s(pInfo->szName,sizeof(pInfo->szName), ParamList[0].c_str(), _TRUNCATE);
//
//		pInfo->nHeightOff = Str2Int(ParamList[1].c_str());
//
//		char szPhoto[72]; _snprintf_s(szPhoto, _TRUNCATE, "texture/photo/%s.bmp", pInfo->szName);
...
protected:
	...
	virtual BOOL _ReadRawDataInfo(CRawDataInfo* pRawDataInfo, std::vector<std::string>& ParamList) override {
		...
		//strncpy_s(pInfo->szName, sizeof(pInfo->szName), ParamList[0].c_str(),_TRUNCATE);
		//strncpy_s(pInfo->szPhotoName, sizeof(pInfo->szPhotoName), ParamList[1].c_str(),_TRUNCATE);
	...
	virtual void _ProcessRawDataInfo(CRawDataInfo* pRawDataInfo) override {
		CMagicInfo* pInfo = (CMagicInfo*)pRawDataInfo;
		char szPhoto[72];
		_snprintf_s(szPhoto, _TRUNCATE, "texture/photo/%s.bmp", pInfo->szPhotoName);
		pInfo->nPhotoTexID = GetTextureID(szPhoto);
	}
};
...
protected:
	...
	virtual BOOL _ReadRawDataInfo(CRawDataInfo* pRawDataInfo, std::vector<std::string>& ParamList) override {
		...
		//strncpy_s(pInfo->szName, ParamList[0].c_str(),_TRUNCATE);
		strncpy_s(pInfo->szName, sizeof(pInfo->szName), ConvertResString(ParamList[0].c_str()), _TRUNCATE);
		// End
