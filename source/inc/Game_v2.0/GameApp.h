...
public:
	...
	*/
	...
	void ShowStaticNotify(const char* szStr, DWORD dwColor);
	void ShowScrollingNotify(const char* szStr, int setnum, DWORD dwColor); //Add by sunny.sun20080804
...
private: // 背景音乐的切换,经历三个阶段:1.当前音乐音量变小,2.为零时切换到新音乐,并且音乐变大,3.正常播放新音乐
	...
	std::unique_ptr<CTextHint> CreateStaticNotify();
	std::unique_ptr<CTextScrollHint> CreateScrollingNotify();
	std::unique_ptr<CTextHint> staticNotify = CreateStaticNotify();
	std::unique_ptr<CTextScrollHint> scrollingNotify = CreateScrollingNotify();
