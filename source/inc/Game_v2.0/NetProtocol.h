...
{
	EKitBagType chSrcType; // 源格类型 0，道具栏 1，银行
	EKitBagType chTarType; // 目标格类型
	short sSrcID;  // 物品栏位置
	short sSrcNum; // 物品操作数
	short sTarID;  // 物品栏位置
};
...
struct stNetKitbag {		// 整个道具栏更新
	EKitBagType bagType;	// 0，道具栏 1，银行
	ESynKitbagType synType; // 更新类型（参考CompCommand.h的ESynKitbagType）
	int capacity; // Inventory slots
	struct stGrid {
		short sGridID; // 表格ID
...
struct stLookEnergy {
	std::array<short, enumEQUIP_NUM> sEnergy;
};
...
#define HELPINFO_DESPSIZE 16384
...
struct SForgeCell {
	SForgeCell() : sCellNum(0), pCell(nullptr) {}
	~SForgeCell() {
		delete[] pCell;
	}
...
struct SLotteryCell {
	SLotteryCell() : sCellNum(0), pCell(nullptr) {}
	~SLotteryCell() {
		delete[] pCell;
	}
...
extern void NetLoginSuccess(char byPassword, size_t iCharNum, NetChaBehave chabehave[]); //加密Key//角色数目//角色信息
extern void NetLoginFailure(unsigned short Errno);									   //登陆错误信息
extern void NetBeginPlay(unsigned short Errno);
extern void NetEndPlay(size_t iCharNum, NetChaBehave chabehave[]);
...
// 与npc交易结果
extern void NetTradeResult(BYTE byCmd, BYTE byIndex, WORD wCount, USHORT sItemID, DWORD dwMoney);
...
extern void NetStallAddBoat(BYTE byGrid, USHORT sItemID, WORD wCount, DWORD dwMoney, NET_CHARTRADE_BOATDATA& Data);
extern void NetStallAddItem(BYTE byGrid, USHORT sItemID, WORD wCount, DWORD dwMoney, NET_CHARTRADE_ITEMDATA& Data);
extern void NetStallDelGoods(DWORD dwCharID, BYTE byGrid, WORD wCount);
