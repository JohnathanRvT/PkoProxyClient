...
public:
	...
	bool Create(LPDIRECT3DDEVICEX pDev, RECT rcwnd, CGameScene* pScene = nullptr, int bksize = 256);
...
public:
	...
	BOOL Init(LPDIRECT3DDEVICEX pDev) {
		m_pDev = pDev;
		D3DXVECTOR2 vUV[4] = {
			D3DXVECTOR2(0, 0),
			D3DXVECTOR2(1, 0),
...
#ifdef MGR
			if (FAILED(lwLoadTex(&_pTex[n], res_mgr, pszName[n].c_str(), nullptr, D3DFMT_A4R4G4B4))) {
				LG("ERROR", "msg%s", pszName[n].c_str());
				return FALSE;
			}
#else
	...
	void Begin(IDirect3DVertexShaderX* shader, IDirect3DVertexDeclaration9* declaration, DWORD dwVs) {
#ifdef MGR
		_pVB->BeginSet();

		if (shader != nullptr) {
			g_Render.SetVertexShader(shader);
			g_Render.SetVertexDeclaration(declaration);
		} else {
			g_Render.SetVertexShader(nullptr);
			g_Render.SetFVFX(dwVs);
		}
#endif
	}
	...
	void Render() {
		...
		for (int i = 0; i < 4; ++i) {
			...
			g_Render.SetVertexShaderConstantX(9 + i, &tv, 1);
		}
#ifdef MGR
...
private:
	...
	//CEffectBox  box;
	CSMCha _Cha;
	IDirect3DVertexShaderX* _dwVShader;
#if (defined LW_USE_DX9)
	IDirect3DVertexDeclaration9* _dwVShaderD;
#endif
...
public:
	...
	virtual void FrameMove(DWORD dwDailTime) override;
...
public:
	...
	BOOL Init() {
		constexpr std::array<const char*, TEXNUM> pszName = {
			R"(texture\ui\minimap\0.tga)",
			R"(texture\ui\minimap\1.tga)",
			R"(texture\ui\minimap\2.tga)",
			R"(texture\ui\minimap\3.tga)",
			R"(texture\ui\minimap\4.tga)",
			R"(texture\ui\minimap\5.tga)",
			R"(texture\ui\minimap\6.tga)",
			R"(texture\ui\minimap\7.tga)",
			R"(texture\ui\minimap\8.tga)",
			R"(texture\ui\minimap\9.tga)",
			R"(texture\ui\minimap\10.tga)",
			R"(texture\ui\minimap\11.tga)",
			R"(texture\ui\minimap\12.tga)",
			R"(texture\ui\minimap\13.tga)",
			R"(texture\ui\minimap\14.tga)",
			R"(texture\ui\minimap\15.tga)",
			R"(texture\ui\minimap\arraw.tga)",
			R"(texture\ui\minimap\shop.tga)"};
	...
	void setAngle(int z) {
		...
		//if(z != 0)
		{
			...
			GetMatrixRotation(&mat, &tvpos,
							  &v, Angle2Radian(float(z)) + D3DX_PI);
