...
public:
	...
	bool GetPickPos(int nPosX, int nPosY, D3DXVECTOR3& vPickPos) {
		return _pTerrain ? _pTerrain->GetPickPos(nPosX, nPosY, vPickPos)
						 : false;
	}
...
public: // 报告场景状态
	...
	DWORD m_dwValidSceneObjCnt{0};
	DWORD m_dwRenderSceneObjCnt;
	DWORD m_dwSceneObjPolyCnt{0};
...
protected:
	BOOL _bEnableCamDrag{false};
	MPTerrain* _pTerrain{nullptr};
	DWORD _dwMapID{1};

	CCharacter* _pChaArray{nullptr};
	CSceneObj* _pSceneObjArray{nullptr};
	CSceneItem* _pSceneItemArray{nullptr};
	CEffectObj* _pEffectArray{nullptr};
	CShadeEff* _pShadeArray{nullptr};

	FreeArray<CCharacter> _free_chas;
	FreeArray<CSceneItem> _free_items;
	FreeArray<CEffectObj> _free_effs;

	CPugMgr* _pcPugMgr{nullptr};

	BOOL _bLightEnable[3]; // 0: parallel light, 1&2: point light
	std::array<std::list<int>, MAX_SCENEOBJ_TYPE> _SceneObjIdx;
	...
	bool _bShowTerrain{false};
	DWORD _dwChaTEXFilter{D3DTEXF_LINEAR};
	bool _bShowSceneObj{true};
	bool _bShowSceneItem{true};
	bool _bEnableSceneObjCulling{true};

	bool _nShowChair{false};
	POINT _HitBuf;
...
protected:
	...
	CCharacter* _pSelCha{nullptr};
	CBoolSet _UserLeve; // 用户可以操作的级别

	bool _IsShowItemName;				// 是否显示道具名字
	CSceneItem* _pMouseInItem{nullptr}; // 鼠标移动在这个道具上,这道具需要显示名字

	// 地面的特效
	using areaeffs = std::list<CEffectObj*>;
	areaeffs _areaeffs;

	CSceneSign _cSceenSign;
	std::unique_ptr<CEventMgr> _pEventMgr;
	CMapInfo* _pMapInfo{nullptr};
...
public:
	...
	using RBOIterator = std::set<CSceneObj*>::iterator;
