...
public:
	...
	//显示现有角色
	bool SelectCharacters(NetChaBehave* chabehave, size_t num);
	//新增角色
...
protected:
	...
	virtual void _RenderUI() override;
	...
	static void _evtCreateDoublePwdEvent(CCompent* pSender, int nMsgType,
										 int x, int y, MouseClickState dwKey); // 询问是否要创建二次密码

	static void _evtWelcomeNoticeEvent(CCompent* pSender, int nMsgType,
									   int x, int y, MouseClickState dwKey); // 欢迎界面 事件处理

	static void _evtCreateOKNoticeEvent(CCompent* pSender, int nMsgType,
										int x, int y, MouseClickState dwKey); // 首次创建角色成功提示界面 事件处理
...
private:
	...
	std::vector<SelectableCharacter*> selectableCharacters; //³¡¾°ÖÐµÄ½ÇÉ«(°üÀ¨×ÖÌå)
	...
	//UI
	CForm* frmSelectCha{nullptr};
	CTextButton* btnDel{nullptr};
	CTextButton* btnYes{nullptr};
	CTextButton* btnCreate{nullptr};
	CTextButton* btnExit{nullptr};
	CTextButton* btnAlter{nullptr};
