...
protected:
	...
	virtual BOOL _ReadRawDataInfo(CRawDataInfo* pRawDataInfo, std::vector<std::string>& ParamList) override {
		...
		strncpy_s(pInfo->szRegion, ParamList[0].c_str(), _TRUNCATE);
		for (int i = 0; i < MAX_GROUP_GATE; i++) // 读入5个可能的gate ip
		{
			strncpy_s(pInfo->szGateIP[i], ParamList[i + 1].c_str(), _TRUNCATE);
		...
		strncpy_s(pInfo->szComment, ParamList[6].c_str(), _TRUNCATE);
		strncpy_s(pInfo->szState, ParamList[7].c_str(), _TRUNCATE);

		return TRUE;
	}

	virtual void _ProcessRawDataInfo(CRawDataInfo* pRawDataInfo) override {
		CServerGroupInfo* pInfo = (CServerGroupInfo*)pRawDataInfo;
...
inline int GetCurServerGroupCnt(size_t nRegionNo) {
...
inline size_t GetReginIndex(const char* ReginText) {
	const ReginList& reginList = CServerSet::I()->m_ReginList;
	for (size_t i = 0; i < reginList.size(); i++) {
		if (reginList[i] == ReginText) {
			return i;
		}
	}
	return -1;
...
// 通过组编号, 选中一个GateIP
inline const char* SelectGroupIP(size_t nRegionNo, size_t nGroupNo) {
	...
	int nGateNo = rng.uniform(0, (int)(pGroup->cValidGateCnt - 1));
...
inline const char *SelectGroupIP(const char *pszGroupName)
{
	...
	int nNo = rng.uniform(0,(int)(pGroup->cValidGateCnt - 1)); 
