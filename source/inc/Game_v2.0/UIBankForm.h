...
protected:
	virtual bool Init() override;	  // 用户界面银行信息初始化
	virtual void CloseForm() override; //关闭表单

private:
	static void _MoveItemsEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey); // 移动多个物品
	static void _MoveAItemEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey); //移动单个物品
