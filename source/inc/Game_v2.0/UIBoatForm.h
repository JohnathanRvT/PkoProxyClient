...
namespace GUI {
constexpr int defMaxBoat = 3; // 可获得最大船只数
...
public:
	...
	bool GetIsValid() const { return _chtBoat != nullptr; } // 获得角色信息
...
private:
	...
	CCharacter* _chtBoat{nullptr};
	CForm* _frmShipRoom{nullptr};
	CGoodsGrid* _grdHold{nullptr};
};
...
protected:
	virtual bool Init() override; //用户船信息初始化
	virtual void End() override;
	virtual void SwitchMap() override { Clear(); } // 跳转地图时清除船信息

private:
	CBoat* GetFreeBoat(); // 获得免费船只

private:
	CCharacter* _pHuman{nullptr};
	std::array<CBoat, defMaxBoat> _cBoats; // 当前角色可拥有的船数量
