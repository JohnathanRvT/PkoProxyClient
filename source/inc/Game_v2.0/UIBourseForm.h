...
private:
	...
	using ItemList = std::vector<ItemInfo_T*>;
	using ItemListIter = ItemList::iterator;
...
private:
	...
	using BlackTradeVec = std::vector<stBlackTrade>;
	using BlackTradeVecIter = BlackTradeVec::iterator;
	...
	// 兑换确认
	static void _TradeExchangeEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
