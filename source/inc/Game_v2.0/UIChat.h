...
public:
	static constexpr int MAX_MEMBER = {4}; // 最多成员数
...
private:
	...
	static void _MemberMouseDownEvent(CGuiData* pSender, int x, int y, MouseClickState key);

public:
	// Add by lark.li 20080804 begin
	CTreeGridNode* GetFrndGroupNode(const char* szGroupName, CTreeGridNode* pNode = nullptr);
...
private:
	...
	std::vector<std::pair<std::string, CTreeGridNode*>> vFrndNode; // 好友节点集合
	...
	// Add by lark.li 20080804 begin
	static void _OnMouseFrndGroupMenu(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _OnMouseAddFrndGroupMenu(CGuiData* pSender, int x, int y, MouseClickState key);
	// End

	static void _MainMouseClick(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _OnMouseFrndMenu(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _OnMouseGroupMenu(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _OnMouseRoadMenu(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _OnMouseSessMenu(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _OnMouseMasterMenu(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _OnMouseStudentMenu(CGuiData* pSender, int x, int y, MouseClickState key);

	static void _evtQQMainShow(CGuiData* pSender);
	static void _evtQQMainAddFrnd(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _evtQQMainChangeStyle(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _evtAddFrnd(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _evtChangeStyle(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _evtQQMainEditMotto(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _evtChangeMotto(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _evtRefreshInfo(CGuiData* pSender, int x, int y, MouseClickState key);

	// Add by lark.li 20090325 begin
	static void _evtGostMainEditMotto(CGuiData* pSender, int x, int y, MouseClickState key);
	// End
	//	Add by alfred.shi 20080903	begin
	static void _evtAddFriend(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _evtViewFriend(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _evtViewDetailInfo(CGuiData* pSender, int x, int y, MouseClickState key);
	//	End.
	...
	static void _OnDragEnd(CForm* pTargetForm, CTreeView* pTree, CTreeNodeObj* pNode1, CTreeNodeObj* pNode2, CItemObj* pItem, int x, int y, MouseClickState key);
	static void _OnDragMouseMove(CForm* pTargetForm, CTreeView* pTree, CTreeNodeObj* pNode1, CTreeNodeObj* pNode2, CItemObj* pItem, int x, int y, MouseClickState key);
	// End

	static void _OnFrndDeleteConfirm(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
	static void _OnMasterDeleteConfirm(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
	static void _OnStudentDeleteConfirm(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
	static void _OnStudentAskConfirm(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
	static void _OnMasterAskConfirm(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);

	// Add by lark.li 20080804 begin
	static void _evtAddFrndGroup(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _evtEditFrndGroup(CGuiData* pSender, int x, int y, MouseClickState key);

	static void _evtAskDelGroupEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
	// End

	// Add by lark.li 20080707 begin
	static void _OnCaptainAskConfirm(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
	// End
};
