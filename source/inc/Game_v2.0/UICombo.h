...
protected:
	static void _OnButtonClick(CGuiData* pSender, int x, int y, MouseClickState key) {
		((CCombo*)(pSender)->GetParent())->_OnButtonClick(x, y, key);
	}
	void _OnButtonClick(int x, int y, MouseClickState key);

	static void _OnListDown(CGuiData* pSender, int x, int y, MouseClickState key) {
