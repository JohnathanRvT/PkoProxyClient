...
private:
	using lstTextInfoType = std::list<sTextInfo>;
	struct sChannelInfo {
		eChannel enumChannel;
		//std::string						strChannelName;
		DWORD dwChannelColor;
		DWORD dwChannelPrefixColor;
...
public:
	...
	static void SetChannelPrefixColor(eChannel channel, DWORD dwColor);
	static DWORD GetChannelPrefixColor(eChannel channel);
...
private:
	...
	using lstInstanceType = std::list<CCharMsg*>;
...
private:
	using lstCardType = std::list<std::string>;
...
public:
	...
	void OnSightMsg(std::string strName, std::string strMsg);
	void OnSightMsg(CCharacter* pChar, std::string strMsg);
	void OnPrivateMsg(std::string strFromName, std::string strToName, std::string strMsg);
	void OnWorldMsg(std::string strName, std::string strMsg);
	void OnTradeMsg(std::string strName, std::string strMsg);
	void OnTeamMsg(std::string strName, std::string strMsg);
	void OnTeamMsg(DWORD dwCharID, std::string strMsg);
	void OnGuildMsg(std::string strName, std::string strMsg);
	void OnSystemMsg(std::string strMsg);
	void OnSideMsg(std::string strName, std::string strMsg);
...
protected:
	...
	static void EventMainListKeyDown(CGuiData* pSender, int x, int y, MouseClickState key);
	static void EventSendChannelSwitchClick(CGuiData* pSender, int x, int y, MouseClickState key);
	static void EventSendChannelChange(CGuiData* pSender);
	static void EventMainPageDragBegin(CGuiData* pSender, int x, int y, MouseClickState key);
	static void EventMainPageDragging(CGuiData* pSender, int x, int y, MouseClickState key);
	static void EventMainPageDragEnd(CGuiData* pSender, int x, int y, MouseClickState key);
	static void EventSystemPageDragBegin(CGuiData* pSender, int x, int y, MouseClickState key);
	static void EventSystemPageDragging(CGuiData* pSender, int x, int y, MouseClickState key);
	static void EventSystemPageDragEnd(CGuiData* pSender, int x, int y, MouseClickState key);
	static void EventChannelSwitchCheck(CGuiData* pSender);
	static void EventCallingCardSwitchClick(CGuiData* pSender, int x, int y, MouseClickState key);
	//static void EventCallingCardLostFocus(CGuiData *pSender);
	static void EventCardSelected(CGuiData* pSender);
	static void EventFacePanelSwitchClick(CGuiData* pSender, int x, int y, MouseClickState key);
	//static void EventFacePanelLostFocus(CGuiData *pSender);
	static void EventFaceSelected(CGuiData* pSender);
	static void EventBrowPanelSwitchClick(CGuiData* pSender, int x, int y, MouseClickState key);
	static void EventBrowSelected(CGuiData* pSender);
	static void EventActionPanelSwitchClick(CGuiData* pSender, int x, int y, MouseClickState key);
...
public:
	...
	static void AddNameToCard(std::string strName);
...
private:
	...
	static void _evtShowChannelList(CGuiData *pSender, int x, int y, MouseClickState key);
	static void _evtForbidCheck(CGuiData *pSender);
	static void _evtCardClick(CGuiData *pSender, int x, int y, MouseClickState key);

	static void _MainOnLstKeyDownEvent(CGuiData *pSender, int x, int y, MouseClickState key);
	static bool _MainOnSayKeyDownEvent( int& key );

	static void _DragBeginOnSystemTitle(CGuiData *pSender, int x, int y, MouseClickState key);
	static void _DragMoveOnSystemTitle(CGuiData *pSender, int x, int y, MouseClickState key);
	static void _DragEndOnSystemTitle(CGuiData *pSender, int x, int y, MouseClickState key);

	static void _DragBeginOnSay(CGuiData *pSender, int x, int y, MouseClickState key);
	static void _DragMoveOnSay(CGuiData *pSender, int x, int y, MouseClickState key);
	static void _DragEndOnSay(CGuiData *pSender, int x, int y, MouseClickState key);
	...
	//static void ShowLstOnSay(std::string str , DWORD color);
	static void PrintToList(eChannel channel, std::string name, std::string str, DWORD color);
...
private:
	std::string		oldSay[MAX_PAGE_SAY];			// 以前说的话
