...
public: // Get,Set
	...
	void SetMaxNum(unsigned v) { _nMaxNum = v; }

	int GetMaxVisible() { return _nMaxNumVisible; }
	void SetMaxNumVisible(unsigned v) { _nMaxNumVisible = v; }
	...
	virtual void SetTextColor(DWORD color) override;
...
protected:
	...
	unsigned _nMaxNum{64};	  // 最大字数
	unsigned _nMaxNumVisible; //控件一次可以容纳的字符数目
