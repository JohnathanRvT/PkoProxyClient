...
public:
	...
	virtual void Parse(CEditStrategy* pParse) override;
...
public:
	...
	const char* GetCaption() override { return _szChar; }
	DWORD GetWordCount() override { return _szChar[1] == 0 ? 1 : 2; }
...
public:
	...
	virtual void Parse(CEditStrategy* pParse) override;
	virtual DWORD GetWordCount() override { return 1; }
...
public:
	...
	void SetCaption(const char* str) override { _szString = str; }
	const char* GetCaption() override { return _szString.c_str(); }

	void AddCaption(const char* str) override { _szString += str; }

	bool AddChar(DWORD dwIndex, CEditChar* pChar);

private:
	using chars = std::vector<CEditChar*>;
	chars _chars;
...
private:
	...
	using memorys = std::vector<CEditObj*>;
...
inline bool CEditArticle::AddChar(char c1, char c2) {
	auto* pChar = new CEditChar(c1, c2);
...
inline bool CEditSentence::AddChar(DWORD dwIndex, CEditChar* pChar) {
	...
	if (dwIndex >= _chars.size()) {
		...
		auto i = std::find(_chars.begin(), _chars.end(), r);
