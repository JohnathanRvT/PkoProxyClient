...
public:
	...
	static CGuiPic* GetLifePic() { return _pImgLife.get(); }
	static CGuiPic* GetLeaderPic() { return _pImgTeamLeaderFlag.get(); }
...
private:
	int _nShowTime{0};   // 目前显示的时间
	float _fScale{0.1f}; //显示的比例
	CItemEx* _pObj;
	std::string _str; //聊天时候显示在头像上面的文字信息
	CCharacter* _pOwn;

private:						  // 显示攻击效果时，血量变化
	static int _nMaxShowLifeTime; // 最大显示多长时间

	static std::unique_ptr<CGuiPic> _pImgLife;
	static std::unique_ptr<CGuiPic> _pImgTeamLeaderFlag; // 显示在队长头上的标志

	float _fLifeW{1.0f};

	static unsigned int _nFaceFrequency;

	static std::unique_ptr<CGuiPic[]> _pImgFace;
	static DWORD _dwBkgColor;

	int _nFaceTime{0}; // 表情已经显示多长时间
	int _nCurFaceID;   //目前显示的表情ID号
	DWORD _dwNameColor{COLOR_WHITE};

	CGuiPic* _pCurFace{nullptr}; // 目前显示的是那张表情
	unsigned int _nCurFaceFrame; // 目前显示的表情第几帧
	unsigned int _nCurFaceCycle;

	bool _IsShowLife{false}; // 是否显示血条
	bool _IsShowName{false}; // 是否显示角色的名字。added by billy
	int _nChaNameOffX{0};	// 玩家名字的X偏移
