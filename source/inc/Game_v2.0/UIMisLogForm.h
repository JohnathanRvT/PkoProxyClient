...
protected:
	...
	typedef struct MisData //added by deguix
	{
		BYTE byRet;
		BYTE byType;
		char* pszName;
	} MisData;
	MisData GetMisData(WORD wMisID);
	BOOL AddNode(WORD wMisID, BYTE byState, BYTE& byType);
	void ClearAllNode();

private:
	// 窗口处理消息函数
	static void _MouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
	static void _MouseDown(CGuiData* pSender, int x, int y, MouseClickState key);
	static void _Show(CGuiData* pSender);
	static void _ItemClickEvent(std::string strItem);
	// 确认是否中断任务
	static void _evtBreakYesNoEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
