...
private:
	void _SetSelf();
	static void _OnTitleDown(CGuiData* pSender, int x, int y, MouseClickState key);
	void _OnTitleDown();
...
public:
	...
	virtual CCompent* GetHintCompent(int x, int y) override;
	virtual CGuiPic* GetImage() override { return _pImage; }
	virtual CCompent* GetHitCommand(int x, int y) override;
...
protected:
	...
	using items = std::vector<CPageItem*>;
	items _pages;
...
// 内联函数
inline void CPageItem::_OnTitleDown(CGuiData* pSender, int x, int y, MouseClickState key) {
