...
public: // 以下坐标为屏幕上的坐标
	...
	MPTexRect* GetImage(unsigned int v = 0) const {
		if (v < _max) {
			return &_pImage[v];
		}
		return nullptr;
	...
	int GetWidth(unsigned int frame = 0) const { return (int)(_pImage[frame].fScaleX * _pImage[frame].nTexW); }
	int GetHeight(unsigned int frame = 0) const { return (int)(_pImage[frame].fScaleY * _pImage[frame].nTexH); }
	...
	bool IsNull() const { return _pImage[0].nTextureNo == -1; }

	void UnLoadImage(int frame = -1);

private:
	std::unique_ptr<MPTexRect[]> _pImage{nullptr};
	unsigned int _frame{0};
	unsigned int _max{0};
	CGuiData* _pParent{nullptr};
	bool _bIsScale{false}; // 缩放到窗口同样大小
};
...
public:
	...
	CGuiData* GetParent() const { return _pOwn; }
...
private:
	std::unique_ptr<MPTexRect[]> _pImage{nullptr};
	CGuiData* _pOwn;
	std::array<int, ppEnd> _nX{};
	std::array<int, ppEnd> _nY{};
	bool _bIsTitle{true};	  // ppClient是否平铺
	bool _bIsShowFrame{false}; // 是否显示边线
};
