...
public:
	CUIPanel();
	~CUIPanel();
	bool Create(LPDIRECT3DDEVICEX pDev);

	void Draw(LPDIRECT3DTEXTUREX pSrcTexture,
...
public:
	...
	bool Init();
	void RenderSprite(LPTEXTURE tex, RECT* rc, D3DXVECTOR2* vscale, D3DXVECTOR2* vdest, DWORD dwColor);
	...
	LPD3DXSPRITE GetSprite() { return _p2DSprite; }
...
private:
	LPD3DXSPRITE _p2DSprite{nullptr};
	//CUIPanel		_p2DSprite;

	float _fScreenScaleX{1.0f}, _fScreenScaleY{1.0f};
	float _fDrawScaleX{1.0f}, _fDrawScaleY{1.0f};

	float _fOldScreenScaleX, _fOldScreenScaleY;
	float _fOldDrawScaleX, _fOldDrawScaleY;

	int _nGuiWidth{1024}, _nGuiHeight{768}; // GUI系统宽高,即屏幕大小
	bool _IsFullScreen;
...
private:
	...
	int _nOutLine; // 私人框
	RECT _rtFrame; // 纹理大小
	D3DXVECTOR2 _vesLevel;
	D3DXVECTOR2 _vesVertical;

	D3DXVECTOR2 _vesLevelThin;
	D3DXVECTOR2 _vesVerticalThin;
...
inline void UIRender::RenderSprite(LPTEXTURE tex, RECT* rc, D3DXVECTOR2* vscale, D3DXVECTOR2* vdest, DWORD dwColor) {
	...
	if (_p2DSprite)
		...
		if (FAILED(_p2DSprite->Draw(tex, rc, vscale, nullptr, 0, vdest, dwColor))) {
			LG("ERROR", "drawing texture failed");
		}
#elif (defined LW_USE_DX9)
		_p2DSprite->Begin(D3DXSPRITE_ALPHABLEND);
		D3DXMATRIX m;
	    D3DXMatrixTransformation2D(&m, nullptr, 0, vscale, nullptr, 0, vdest);
		_p2DSprite->SetTransform(&m);
		switch (_p2DSprite->Draw(tex, rc, nullptr, nullptr, dwColor)) {
