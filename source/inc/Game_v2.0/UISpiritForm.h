...
public:
	...
	static constexpr int NO_USE = -1;
	static constexpr int SPIRIT_MARRY_TYPE = 6; // 精灵融合类型（与服务器交互用）

	static constexpr int SPIRIT_MARRY_CELL_COUNT = 3; // 格子个数
	static constexpr int SPIRIT_MARRY_ITEM = 0;		  // 恶魔果实
	static constexpr int SPIRIT_MARRY_ONE = 1;		  // 精灵1
	static constexpr int SPIRIT_MARRY_TWO = 2;		  // 精灵2

	static constexpr int ERNIE_IMAGE_COUNT = 5; // 老虎机图片数
	static constexpr int ERNIE_SPEED = 50;		// 翻转速度（毫秒）
	static constexpr int ERNIE_COIN_COUNT = 5;  // 5 个精灵硬币
	static constexpr int ERNIE_EMPTY_COUNT = 5; // 5 个背包空格
...
private:
	...
	static void _evtMainMouseButton(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey); // 鼠标点击按钮事件
...
private:
	...
	static void _evtErnieMouseButton(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey); // 鼠标点击按钮事件
