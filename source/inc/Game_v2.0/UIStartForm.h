...
public:
	...
	void RefreshLeftExpName(short sSelectTime, std::string BaseExp, std::string AddExp, std::string TotalExp, std::string UpLevel, std::string addPercent, short sNum, std::string amplitude, short coin);
	...
	void ShowCurrentHelper(const char* mapName, bool isShow /*= true*/);
	void ShowNPCHelper(const char* mapName, bool isShow /*= true*/);
	void ShowMonsterHelper(const char* mapName, bool isShow /*= true*/);
	void ShowBossHelper(const char* mapName, bool isShow /*= true*/);
...
private:
	static void _evtStartFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);

	static void _evtFormMouseClick(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey); //	挂机

	static void _evtReliveFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);

	static void _evtTaskMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);

	// 队伍单挑的对话框回调
	static void _evtAskTeamFightMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);

	static void _evtChaActionChange(CGuiData* pSender); //改变角色的动画
	static void _evtChaHeartChange(CGuiData* pSender);  //改变角色的心情

	static void _evtPopMenu(CGuiData* pSender, int x, int y, MouseClickState key);

	static void _evtSelfMouseDown(CGuiData* pSender, int x, int y, MouseClickState key); // 给自己加血事件

	static void _evtOriginReliveFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);

	static void _evtShowBoatAttr(CGuiData* pSender, int x, int y, MouseClickState key); // 显示船只属性

	static void _NewFrmMainMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);

	static void _HelpFrmMainMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);

	static void _CloseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
...
private:
	...
	static void _OnSelfMenu(CGuiData* pSender, int x, int y, MouseClickState key);

	static void _evtHelpListChange(CGuiData* pSender);
	static void _evtNPCListChange(CGuiData* pSender);   // add by alfred.shi 20080709
	static void _evtMonsterListChange(CGuiData* pSender);
	static void _evtBossListChange(CGuiData* pSender);
	...
	static void _evtOOXXEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
};
