...
private:
	...
	BOOL WriteInteger(const char* szSection, const char* szKey, int value, const char* szFileName) {
		char szbuf[50] = {0};
		_itoa_s(value, szbuf, _countof(szbuf), 10);
...
private:
	...
	static void _evtMainMusicMouseDown(CGuiData* pSender, int x, int y, MouseClickState key); //处理音效

	static void _evtVideoFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
	static void _evtSystemFromMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
	static void _evtAudioFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);

	static void _evtAskReloginFormMouseDown(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
	static void _evtAskExitFormMouseDown(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
	static void _evtAskChangeFormMouseDown(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
	static void _evtGameOptionFormMouseDown(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
	static void _evtGameOptionFormBeforeShow(CForm* pForm, bool& IsShow);

private:
	CForm* frmSystem{nullptr};

	CForm* frmAudio{nullptr};
	CProgressBar* proAudioMusic{nullptr};
	CProgressBar* proAudioMidi{nullptr};

	CForm* frmVideo{nullptr};
	CCheckGroup* cbxTexture{nullptr};
	CCheckGroup* cbxMovie{nullptr};
	CCheckGroup* cbxCamera{nullptr};
	//CCheckGroup      *cbxView;        //取消视野远近(Michael Chen 2005-04-22
	CCheckGroup* cbxTrail{nullptr};
	CCheckGroup* cbxColor{nullptr};
	CCheckGroup* cbxSize{nullptr};
	CCheckGroup* cbxModel{nullptr};
	CCheckGroup* cbxQuality{nullptr};

	CForm* frmGameOption{nullptr};
	CCheckGroup* cbxRunMode{nullptr};
	CCheckGroup* cbxLockMode{nullptr};
	CCheckGroup* cbxHelpMode{nullptr};

	CForm* frmAskRelogin{nullptr};
	CForm* frmAskExit{nullptr};
	CForm* frmAskChange{nullptr};
