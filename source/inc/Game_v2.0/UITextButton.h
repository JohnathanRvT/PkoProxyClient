...
public:
	...
	virtual void SetAlpha(BYTE alpha) override { _pImage->SetAlpha(alpha); }
	...
	virtual void SetIsEnabled(bool v) override;
	...
	void SetTextColor(DWORD color) override { _textColor = color; }
	bool GetState() { return _isDown; }

	void SetFormModal(CForm::eModalResult v);
	CForm::eModalResult GetFormModal() { return _eFormModal; }

	void DoClick(MouseClickState state = MouseClickState::LUp); // 外部模拟点击事件
