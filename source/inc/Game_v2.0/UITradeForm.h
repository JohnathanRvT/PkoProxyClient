...
public:
	...
	void DragItemToTrade(DWORD dwCharID, USHORT sItemID, BYTE byIndex, WORD wCount, BYTE byItemIndex, SItemGrid* pGrid, const NET_CHARTRADE_BOATDATA* const pBoat);
...
protected:
	...
	static void _evtGoldFormEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
	static void _MainMousePlayerTradeEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
...
private:
	...
	static void _evtLocalSaleEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey);
