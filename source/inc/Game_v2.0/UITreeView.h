...
namespace GUI {
class CTreeNodeObj;
using TreeNodes = std::vector<CTreeNodeObj*>;
...
using GuiTreeDragEvent = void (*)(CForm* pTargetForm, CTreeView* pSourceTree, CTreeNodeObj* pNode1, CTreeNodeObj* pNode2, CItemObj* pItem, int x, int y, MouseClickState key);
// End
...
public:
	...
	virtual void MouseRunSelf(int x, int y, MouseClickState key) {}
...
public:
	...
	bool RootMouseRun(int x, int y, MouseClickState key);
...
public:
	...
	virtual void MouseRunSelf(int x, int y, MouseClickState key) override;

	virtual const char* GetCaption() override { return _pItem->GetString(); }

	int GetHeight() override { return _pItem->GetHeight(); }
	int GetWidth() override { return _pItem->GetWidth(); }
	CItemObj* GetItem() { return _pItem; }

	virtual void SetCaption(const char* str) override { _pItem->SetString(str); }

protected:
	virtual CItemObj* _GetSelfHitItem(int x, int y) override { return _pItem; }
...
public:
	...
	virtual void RenderSelf() override;
	virtual void ShowFocus() override;
	virtual void MouseRunSelf(int x, int y, MouseClickState key) override;
	virtual void RefreshNode() override;
	virtual int GetFocusX() override { return _nFocusX1; }
	virtual int GetFocusY() override { return _nFocusY1; }
	...
	int GetWidth() override { return _nColMaxNum * _nUnitWidth; }
...
protected:
	...
	virtual bool _HasPage() override { return true; }
	virtual CItemObj* _GetSelfHitItem(int x, int y) override;

protected:
	CItemObj* _pItem; // 显示在表格上面,相当于表格的注释

	using vitems = std::vector<CItemObj*>;
	vitems _items;
...
protected:
	...
	void _DragMove(int x, int y, MouseClickState key);
	// End
