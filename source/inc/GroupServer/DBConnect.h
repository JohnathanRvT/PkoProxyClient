...
public:
	...
	const char* GetActName() const { return m_buf[0].c_str(); }
	int GetActID() const { return atoi(m_buf[0].c_str()); }
	char GetGM() const { return atoi(m_buf[1].c_str()); }
	const char* GetChaIDs() const { return m_buf[2].c_str(); }
	const char* GetPassword() const { return m_buf[3].c_str(); }
	const char* GetLast_IP() const { return m_buf[4].c_str(); }
	const char* GetDisc_Reason() const { return m_buf[5].c_str(); }
	const char* GetLast_Leave() const { return m_buf[6].c_str(); }

private:
	static constexpr int ACT_MAXCOL = 10;
...
public:
	...
	bool ZeroAddr();
	bool SetAddr(long cha_id, uintptr_t addr);
	...
	int Getcha_id() const { return atoi(m_buf[0].c_str()); }
	const char* GetMottonByName() const { return m_buf[1].c_str(); }
	short GetIconByName() const { return atoi(m_buf[2].c_str()); }
	//For普通登录
	int FetchRowByChaID(int cha_id);
	//Add for frnd ablum
	int FetchActNameRowByChaID(int cha_id, std::string& act_nam);

	const char* GetChaName() const { return m_buf[0].c_str(); } //角色名
	const char* GetMotto() const { return m_buf[1].c_str(); }
	short GetIcon() const { return atoi(m_buf[2].c_str()); }	  //小图标
	int GetGuildID() const { return atoi(m_buf[3].c_str()); }	 //所属公会ID
	const char* GetGuildName() const { return m_buf[4].c_str(); } //所属公会名
	const char* GetJob() const { return m_buf[5].c_str(); }		  //职业
	short GetDegree() const { return atoi(m_buf[6].c_str()); }	//等级
	const char* GetMap() const { return m_buf[7].c_str(); }		  //地图名
	int GetMap_X() const { return atoi(m_buf[8].c_str()); }
	int GetMap_Y() const { return atoi(m_buf[9].c_str()); }
	const char* GetLook() const { return m_buf[10].c_str(); }
	int GetStr() const { return atoi(m_buf[11].c_str()); }
	int GetDex() const { return atoi(m_buf[12].c_str()); }
	int GetAgi() const { return atoi(m_buf[13].c_str()); }
	int GetCon() const { return atoi(m_buf[14].c_str()); }
	int GetSta() const { return atoi(m_buf[15].c_str()); }
	int GetLuk() const { return atoi(m_buf[16].c_str()); }
...
private:
	static constexpr int CHA_MAXCOL = 24;
...
private:
	static constexpr int CHA_MAXCOL = 20;
	std::string m_buf[CHA_MAXCOL];
};
...
private:
	static constexpr int CHA_MAXCOL = 3;
	std::string m_buf[CHA_MAXCOL];
};
// End
...
public:
	...
	int GetFriendsCount(long cha_id1, long cha_id2, const char* groupname = nullptr);
...
private:
	static constexpr int FRD_MAXCOL = 3;
...
private:
	static constexpr int MASTER_MAXCOL = 6;
...
public:
	...
	int Getguild_id() const { return atoi(m_buf[0].c_str()); }
	...
	bool GetGuildMemberNum(uLong gldid, std::atomic<long>& memberNum); //Add by sunny.sun 20090611
private:
	bool SendGuildInfo(Player* ply);
	static constexpr int GLD_MAXCOL = 50;
...
private:
	static constexpr int CRYSTAL_MAXCOL = 3;
	std::string m_buf[CRYSTAL_MAXCOL];
};
#endif
//end
struct ORDERINFO {
...
protected:
	static constexpr int PARAM_MAXCOL = 7;
