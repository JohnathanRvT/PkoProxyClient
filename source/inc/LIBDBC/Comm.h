...
private:
	...
	std::atomic<long> m_selexit;

	ThreadPool *const __communicator, *const __communicator1, *const __processor, *const __processor1;
	std::atomic<long> m_TickCount;
	uLong const __delay;
	uLong m_keepalive;

	std::atomic<long> __atnotconn, __conntotal, __deltotal;
...
public:
	...
	*/
	DataSocket* Connect(cChar* hostname, uShort port, SOCKET* sock_out = nullptr);

private:
	virtual void TaskDispatcher(Task*) override;
};
...
protected:
	virtual void ShutDown(uLong ulMilliseconds) override;
