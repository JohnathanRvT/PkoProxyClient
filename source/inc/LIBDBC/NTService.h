...
private:
	virtual void ServiceStart() override = 0;
	virtual void ServiceStop() override = 0;
	virtual void ServicePause() override{};
	virtual void ServiceContinue() override{};
	virtual long ServiceProcess() override { return 1000; }; //返回服务需要的调用间隔时间,缺省为1000毫秒(1秒),后裔类有信号等待应该返回0,函数中等待不能太久(应小于3秒),以保持对用户动作响应的及时性

	virtual cChar* SetSvcName() const override = 0;
	virtual cChar* SetDispName() const override { return SetSvcName(); }; //显示名缺省等于服务名
	virtual cChar* SetDependencies() const override { return nullptr; }		 //缺省服务没有依赖,返回格式："MSSQL\0Server\0"
	virtual cChar* SetRCFile() const override { return nullptr; };				 //缺省使用可执行文件做资源文件
	virtual DWORD SetNumOfEvtCat() const override { return 0; }			 //CategoryMessageFile文件中的目录总数,后裔可以丢弃以提供目录.

	virtual bool CanPaused() const override { return true; };	   //缺省支持暂停和继续操作
	virtual bool RelyMainThread() const override { return false; }; //尽量不依赖主线程,除非特殊需要就override返回true;
	virtual bool HasAppEvent() const override { return true; };
	virtual bool HasSecEvent() const override { return false; }; //一般来说没有安全事件
	virtual bool HasSysEvent() const override { return false; }; //一般来说没有系统事件
public:
...
#define CATCH_COM_ERROR                                                      \
		...
		sprintf(l_buf, "Code meaning = %s", (char*)e.ErrorMessage());       \
		sprintf(l_buf, "Source = %s", (char*)e.Source());                   \
