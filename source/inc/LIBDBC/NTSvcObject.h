...
typedef class NTSvcObject {
	...
	//数据成员:
	long m_delay{0}; //ServiceProcess返回的延迟
	Sema m_semSignal1, m_semSignal2;
	HANDLE m_hAppEvtSrc{nullptr}, m_hSecEvtSrc{nullptr}, m_hSysEvtSrc{nullptr};
	volatile bool m_EndSvcFlag{false}, m_EndMainFlag{false}, m_ServiceExitFlag{false};

	LPSERVICE_MAIN_FUNCTION m_ServiceMain;
	SERVICE_STATUS_HANDLE m_hServiceStatus{nullptr};
	volatile DWORD m_dwCurrentState;
	DWORD m_dwCheckPoint{0};
	DWORD m_SvcArgc{0};
	LPTSTR* m_SvcArgv{nullptr};
	NTSvcObject* m_nextservice{nullptr};
} * PNTSVC;
//=======NTServiceSet====================================================================
class NTServiceSet {
public:
...
private:
	...
	Mutex m_svcHandle;
	long m_svclen{0};
	PNTSVC m_svchead{nullptr}, m_svctail{nullptr};

	int m_argc{0}; //argc参数
	char** m_argv{nullptr}; //argv参数
};
...
VOID WINAPI GenNTService<T>::SvcMain(DWORD dwArgc, LPTSTR* lpszArgv) {
	...
	if (m_svc->HasAppEvent()) {
		...
		m_svc->m_hAppEvtSrc = RegisterEventSource(nullptr, l_buf);
	}
	if (m_svc->HasSecEvent()) {
		strcpy(l_buf, m_svc->SetDispName());
		strcat(l_buf, ".SEC");
		m_svc->m_hSecEvtSrc = RegisterEventSource(nullptr, l_buf);
	}
	if (m_svc->HasSysEvent()) {
		strcpy(l_buf, m_svc->SetDispName());
		strcat(l_buf, ".SYS");
		m_svc->m_hSysEvtSrc = RegisterEventSource(nullptr, l_buf);
	}
	m_svc->m_hServiceStatus = RegisterServiceCtrlHandler(
		m_svc->SetSvcName(), GenNTService<T>::Handler);

	SERVICE_STATUS l_svcstat;
	SC_HANDLE l_sc = nullptr, l_sv = nullptr;
	if (m_svc->RelyMainThread()) { //依赖主线程
		l_sc = OpenSCManager(nullptr, nullptr, SC_MANAGER_ALL_ACCESS);
