...
public:
	...
	WPacket(const TcpCommApp* tca = nullptr);
	...
	bool WriteLongLong(ULONG64 ll);
	bool WriteAddress(uintptr_t loll);
	...
	size_t Size() const { return (*this) ? (*this)->Size() : 0; }
...
public:
	...
	RPacket(const TcpCommApp* tca = nullptr);
	...
	cChar* ReadString(uShort* len = nullptr);
	float ReadFloat();

	uChar ReverseReadChar();
	uShort ReverseReadShort();
	uLong ReverseReadLong();
	ULONG64 ReverseReadLongLong();
#ifdef _WIN64
	uintptr_t ReverseReadAddress();
#elif _WIN32
	uintptr_t ReverseReadAddress();
#endif
