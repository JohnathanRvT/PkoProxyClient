...
public:
	TFNCall(size_t size) : PreAllocStru(size) {
		m_semBlock.Create(0, 1, nullptr);
		m_mtx.Create(false);
	};
	~TFNCall() {
		delete m_nextfnc;
...
private:
	...
	uLong volatile m_sessid{0}; //调用的sessid
	uLong volatile m_oldsessid{0}; //备份的sessid
	bool volatile m_isSync{false};

	RPacket m_retbuf{nullptr}; //同步需要
	void* volatile m_pointer{nullptr}; //异步需要

	Sema m_semBlock;
	Mutex m_mtx;

	TFNCall* volatile m_nextfnc{nullptr};
};
//==============================================================================
class RPCInfo {
	friend class DataSocket;
	friend class RPCMGR;
	RPCInfo() : m_fncall(nullptr) {
...
public:
	OnServeCall(size_t size) : PreAllocTask(size), m_rpk(nullptr), m_sessid(0){};
...
public:
	OnAsynReturn(size_t size) : PreAllocTask(size), m_rpk(nullptr), m_pointer(nullptr){};
	long Process() override;
	Task* Lastly() override { return PreAllocTask::Lastly(); }
