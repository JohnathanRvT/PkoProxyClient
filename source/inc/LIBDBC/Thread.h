...
class TaskQue {
	...
	TaskQue(long max) : m_max(max), m_taskcount(0), m_head(0), m_tail(0) { //池任务队列初始化
		...
		try {
			m_semQueAdd.Create(0, m_max, nullptr);
			m_semQueGet.Create(m_max, m_max, nullptr);
...
public:
	TaskWait(size_t size = 0) : PreAllocStru(size) {
		//if(!m_semWait.Create(0,1,0))THROW_EXCP(excpSync,"任务等待结构建立操作系统同步器失败");
		if (!m_semWait.Create(0, 1, 0))
			THROW_EXCP(excpSync, "Ceeate sync mutex failed(task wait)!");
	};

private:
	Sema m_semWait;
	long volatile m_retval{0};
	TaskWait* volatile next{nullptr}; //不需要维护的链表指针
};
...
private:
	...
	bool volatile m_freeflag{true};		   //线程是否空闲标志
	HANDLE m_handle;						   //线程的HANDLE句柄
	DWORD m_threadid;						   //线程的ID标志
	ThreadPool* volatile m_pool{nullptr};	   //池指针
	Thread* volatile m_last{nullptr}, *volatile m_next{nullptr}; //队列指针，m_last队列上一个，m_next队列下一个
};
