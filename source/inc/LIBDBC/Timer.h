...
class Timekeeper {
public:
	Timekeeper(bool mode = false) : m_mode(mode) //mode=false使用GetTickCount()定位时间，mode=true使用QueryPerformance[X]函数定位时间。
	{
		if (m_mode) {
			if (dwFrequency == 1) {
...
private:
	...
	bool const m_mode;
	LARGE_INTEGER m_liStartTime{0};
	uLong m_ulStartTime{0};

	uLong m_ulTimeCount{0};
};
//==============定时器======================================
class Timer {
...
private:
	...
	void Free() override;
	Mutex m_mtxtime;
	Timer* volatile m_timer[100]{0};
	uLong volatile m_timercount{0};
	uLong m_starttick;
};
