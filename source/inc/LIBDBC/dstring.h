...
public:
	dstrbuf(size_t size) : BaseBuf(size) {}
	void Free() override { PreAllocStru::Free(); }

private:
	void Initially() override { *(short*)getbuf() = 0; }
};
//=================================================================
class dstring {
public:
	dstring() : m_buf(nullptr), m_overwrite(true){}; //缺省构造
	dstring(dstring& ds) : m_buf(nullptr), m_overwrite(true) {
		SetSize(ds.length() + 1);
		if (m_buf)
			strcpy(m_buf->getbuf(), ds);
	}
	dstring(cChar* str) : m_buf(nullptr), m_overwrite(true) //串构造
	{
		if (!str)
			return;
	...
	dstring(cwChar* str) : m_buf(nullptr), m_overwrite(true) //宽串构造
	{
		if (!str)
			return;
		SetSize(wctmbcpy((char*)nullptr, str) + 1);
		if (m_buf)
			wctmbcpy(m_buf->getbuf(), str);
	}
	explicit dstring(int i) : m_buf(nullptr), m_overwrite(true) //整数构造
	{
		char l_str[21];
		itoa(i, l_str, 10);
	...
	size_t Size() const { return !m_buf ? 0 : m_buf->Size(); }
	inline size_t SetSize(size_t) const;
	size_t length() const { return !m_buf ? 0 : strlen(m_buf->getbuf()); }
...
//==内联成员定义============================================================
inline size_t dstring::SetSize(size_t size) const {
	if (Size() < size) {
