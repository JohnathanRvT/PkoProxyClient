...
class rstring {
public:
	rstring() : m_buf(nullptr), m_overwrite(true){};		  //缺省构造
	rstring(cChar* str) : m_buf(nullptr), m_overwrite(true) //串构造
	{
		SetSize(strlen(str) + 1);
		if (m_buf)
			strcpy(m_buf->getbuf(), str);
	}
	rstring(cwChar* str) : m_buf(nullptr), m_overwrite(true) //宽串构造
	{
		SetSize(wctmbcpy((char*)nullptr, str) + 1);
		if (m_buf)
			wctmbcpy(m_buf->getbuf(), str);
	}
	explicit rstring(int i) : m_buf(nullptr), m_overwrite(true) //整数构造
	{
		char l_str[21];
		itoa(i, l_str, 10);

		SetSize(strlen(l_str) + 1);
		if (m_buf)
			strcpy(m_buf->getbuf(), l_str);
	}
	rstring(const rstring& str) : m_buf(nullptr), m_overwrite(false) //拷贝构造
	{
		m_buf = str.m_buf;
		if (m_buf)
			m_buf->adopt();
	...
	size_t Size() const { return !m_buf ? 1 : m_buf->Size(); }
...
//==内联成员定义============================================================
inline size_t rstring::SetSize(size_t size) const {
	if (Size() < size) {
		rstrbuf* l_buf;
