...
{
	DWORD dwTileInfo{0}; // 保存3个layer的tex no和alpha no
	BYTE btTileInfo{0}; // 最下层的tex no
	short sColor{0};	 // 32bit的颜色保存为565
	char cHeight{0};	// 每10cm为一级来保存高度
	short sRegion{0};
	BYTE btIsland{0};
	BYTE btBlock[4]{0, 0, 0, 0};
};

struct SFileTile {
	BYTE t[8];
	short sHeight{0}; // 以厘米为单位的地表顶点高度
	DWORD dwColor{0};
	short sRegion{0};
	BYTE btIsland{0};
	BYTE btBlock[4]{0, 0, 0, 0}; // 4 x 1的障碍和高度记录
};
