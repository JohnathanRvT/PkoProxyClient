...
struct SSceneObjInfo {
	short sTypeID{0}; // 高2位是type(0: 场景物件, 1: 特效物件), 其余是ID
	unsigned long nX{0};
	unsigned long nY{0};
	short sHeightOff;
	short sYawAngle;
...
class CSceneObjData : public CSectionDataMgr {
	...
	struct SFileHead {
		char tcsTitle[16]{"HF Object File!"}; // "HF Object File!"
		int lVersion{0};
		long lFileSize{0};

		int iSectionCntX{0}; // 地图的横向区域数
		int iSectionCntY{0}; // 地图的纵向区域数
		int iSectionWidth{0}; // 区域的宽度（单位：Tile）
		int iSectionHeight{0}; // 区域的高度（单位：Tile）
		int iSectionObjNum{MAX_SECTION_OBJ}; // 区域允许的最大物件数
	};
...
protected:
	int _GetSectionDataSize() override { return 20 * MAX_SECTION_OBJ; }
	BOOL _ReadFileHeader() override;
	void _WriteFileHeader() override;
	DWORD _ReadSectionIdx(DWORD dwSectionNo) override;
	void _WriteSectionIdx(DWORD dwSectionNo, DWORD dwOffset) override;
	void _AfterReadSectionData(SDataSection* pSection, int nSectionX, int nSectionY) override;
...
inline void CSceneObjData::TrimFile(const char* pszTarget) {
	...
	// 写入内容
	auto* pIdx = new SSectionIndex[_nSectionCntX * _nSectionCntY];
