...
using MAP_LOADING_PROC = long(CALLBACK*)(int nFlag, int nSectionX,
...
protected:
	...
	char _szDataName[32]{"noname"}; // 地图名称
	int _nSectionCntX;	 // 地图水平方向上Section的块数
	int _nSectionCntY;	 // 地图垂直方向上Section的块数
	int _nLoadCenterX{0}; // 动态读入的中心位置
	int _nLoadCenterY{0};
	int _nLoadWidth{0}; // 可视范围
	int _nLoadHeight{0};

	int _nSectionStartX{0};
	int _nSectionStartY{0};

	MAP_LOADING_PROC _pfnProc{nullptr}; // 地图动态读入回调通知函数

	FILE* _fp{nullptr}; // 地图文件句柄
	BOOL _bEdit{FALSE}; // 是否可编辑
	BOOL _bDebug{FALSE};
