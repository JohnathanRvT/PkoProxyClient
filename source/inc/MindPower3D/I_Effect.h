...
//常用的struct or type.
//--------------------------------------------------------------------------------------
#define LIST_string list<std::string>
...
// Default meshes
constexpr auto MESH_TRI = "Triangle";
constexpr auto MESH_PLANETRI = "TrianglePlane";
constexpr auto MESH_RECT = "Rect";
constexpr auto MESH_RECTZ = "RectZ";
constexpr auto MESH_PLANERECT = "RectPlane";
constexpr auto MESH_CYLINDER = "Cylinder";
constexpr auto MESH_CONE = "Cone";
constexpr auto MESH_SPHERE = "Sphere";
// ~ Default meshes

inline bool IsTobMesh(const std::string& strName) {
	return ((strName == MESH_CYLINDER) || (strName == MESH_CONE) || (strName == MESH_SPHERE));
}
inline bool IsCylinderMesh(const std::string& strName) {
	return (strName == MESH_CYLINDER);
}
inline bool IsDefaultMesh(const std::string& strName) {
	static constexpr std::array<std::string_view, 8> str =
		{
			MESH_TRI,
			MESH_PLANETRI,
			MESH_RECT,
	...
	return std::any_of(str.begin(), str.end(), [&](auto& str) { return str == strName; });
}
...
#ifdef USE_RENDER
	CEffectModel(MPRender* pDev, lwIResourceMgr* pRes = nullptr);
#else
	CEffectModel(LPDIRECT3DDEVICEX pDev, lwIResourceMgr* pRes = NULL);
#endif
...
public:
...
#ifdef USE_RENDER
	void InitDevice(MPRender* pDev, lwIResourceMgr* pRes = nullptr);
#else
	void InitDevice(LPDIRECT3DDEVICEX pDev, lwIResourceMgr* pRes = NULL);
#endif
	...
	//创建可变形的模型
	bool CreateTob(const std::string& str, int nSeg, float fHei, float fTopRadius, float fBottomRadius) {
	...
	void Lock(BYTE** pvEffVer) {
		...
		if (LW_FAILED(_lpSVB->Lock(0, 0, (void**)pvEffVer, 0))) {
			MessageBox(nullptr, "lock error msglock error", "error", 0);
			*pvEffVer = nullptr;
			assert(false);
		}
#else
...
/* 纹理坐标变换*/
/************************************************************************/
using TEXCOORD = std::vector<D3DXVECTOR2>;
...
public:
	void SetTextureName(const std::string& pszName);
...
public:
	...
	//!纹理名
	std::string m_vecTexName;
	//!当前的纹理指针
	LPDIRECT3DTEXTUREX m_lpCurTex;
...
public:
	...
	void AddTexture(const std::string& pszName);
...
public:
	...
	float m_fFrameTime;
	//!纹理名
	std::vector<std::string> m_vecTexName;
	//!当前的纹理指针
	lwITex* m_lpCurTex;
...
#else
	bool CreateEffectFont(LPDIRECT3DDEVICEX pDev,
						  CMPResManger* pCResMagr, int iTexID, D3DXCOLOR dwColor, bool bUseBack = false, bool bmain = false);
#endif
...
protected:
	...
	//背景图
	bool _bUseBack;
	std::string _strBackBmp;
#ifdef USE_RENDER
	lwITex* _lpBackTex;
#else
	LPDIRECT3DTEXTUREX _lpBackTex;
#endif
...
#else
	virtual void Init(LPDIRECT3DDEVICEX pDev, EFFECT_TYPE eType,
					  WPARAM wParam, LPARAM lParam);
#endif
...
public:
	...
	std::string GetTextureName() { return m_CTextruelist.m_vecTexName; }

	void SetTextureName(const std::string& pszName) { m_CTextruelist.SetTextureName(pszName); }

	void SetModel(CEffectModel* pCModel);
	//！替换纹理
	void ChangeTexture(const std::string& pszName) {
	...
	//!得到模型名称
	std::string& getEffectModelName();
	void setEffectModelName(const std::string& strModelName) { m_strModelName = strModelName; }
	...
	void AddFrameTex(const std::string& str) {
		if (_eEffectType == EFFECT_FRAMETEX) {
...
protected:
	...
	DWORD _iVSIndex;
	IDirect3DVertexShaderX* _pdwVShader;
