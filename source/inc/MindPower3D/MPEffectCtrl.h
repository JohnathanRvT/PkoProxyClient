...
struct MINDPOWER_API EFF_Param {
	int m_iModelNum;
	std::vector<std::string> m_vecModelName;
	int m_iPartNum;
	float m_fvel;
	std::vector<std::string> m_vecParName;
...
public:
	...
	bool LoadParamFromName(EFF_Param* pParam, const std::string& strName);

	bool LoadParamFromName(Group_Param* pParam, const std::string& strName);
...
public:
	...
	void Emission(const D3DXVECTOR3* vStart, const D3DXVECTOR3* vTarget);
	void CalculateEmission(const D3DXVECTOR3* vStart, const D3DXVECTOR3* vTarget);
	void Stop() {
		if (_pPartResult) {
			_bStop = true;
