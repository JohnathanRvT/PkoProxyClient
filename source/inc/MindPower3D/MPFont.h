...
inline bool GetIntersectRect(RECT* pdest, const RECT* psrc, const RECT* pclip) {
...
public:
	...
	BYTE GetClipState() const { return m_byState; }
	RECT& GetClipRect() { return m_rectClip; }

	static UIClip* GetCliper() { return m_pClip; }

	bool GetIntersectRect(RECT* pdest, const RECT* psrc, BYTE* byFill = NULL) {
		int x1 = 0, x2 = 0, y1 = 0, y2 = 0;
...
/*
	bool CreateFont( LPDIRECT3DDEVICEX pd3dDevice, char* szFontName, int nSize = 16, int nLevel = 3 );
	\param1		3D设备
	\param2		字体名称
...
public:
...
#else
	bool CreateFont(LPDIRECT3DDEVICEX pd3dDevice, char* szFontName, int nSize = 16, int nLevel = 3, DWORD dwFlag = 0);
#endif
	...
	bool Draw3DText(const char* szText, D3DXVECTOR3& vPos, D3DXCOLOR color = 0xFFFFFFFF, float fScale = 0.3f);

	SIZE* GetTextSize(const char* szText, SIZE* pSize, float fScale = 1.0f) const;
	int GetHzLength(float fscale = 1.0f) const;
	int GetAscLength(float fscale = 1.0f) const;

	void ReleaseFont();

	void UseSoft(bool bUseSoft) { _bUseSoft = bUseSoft; }
	bool IsUseSoft() const { return _bUseSoft; }
#ifdef USE_RENDER
	lwITex* GetTexture() {
		return _pTex;
	}
#else
	LPDIRECT3DTEXTUREX GetTexture() {
		return _pTexFast;
	}
#endif

	void SetRenderIdx(int iIdx) {
		_iRenderIdx = iIdx;
	}
...
private:
	...
	void FillTextToTex(const char* szText);
...
private:
	...
	LPDIRECT3DTEXTUREX _pTex;
#else
	lwIMesh* _pVB{nullptr};
	lwITex* _pTex{nullptr};
	...
	bool _bUseSoft;
	IDirect3DVertexShaderX* _pdwVS{nullptr};

	D3DXMATRIX* _pmatBBoard;
	D3DXMATRIX* _pmatViewProj;
	D3DXMATRIX* _pmat2DViewProj;
	int* _iBufWidth{nullptr};
	int* _iBufHeight{nullptr};

#ifndef USE_RENDER
	LPDIRECT3DTEXTUREX _pTexFast;
#else
	lwITex* _pTexFast{nullptr};
#endif
	...
	int _RowNumFast{0};
	UINT _MaFast{0};
	int _TextureSizeFast{0};

#ifndef USE_RENDER
	LPDIRECT3DTEXTUREX _lpCurTex;
#else
	lwITex* _lpCurTex;
#endif
	...
	//////////////////////////////////////////////////////////////////////////
	FONT_VER* _vecVer{nullptr};
	FONT_VER* _vecAsh{nullptr};
};
