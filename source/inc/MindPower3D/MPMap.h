...
class MPMap;
using MAP_PROC = long(CALLBACK*)(int nFlag, int nSectionX,
...
public:
	DWORD m_dwTerrainRenderTime{0};
	DWORD m_dwSeaRenderTime{0};
	DWORD m_dwLoadingTime[3]{0}; // 最近3次不为0的读盘时间
	DWORD m_dwMaxLoadingTime{0};
	DWORD m_dwActiveSectionCnt{0};
	DWORD m_dwBatchTime{0};

public:
	bool m_bCullTile{false};
...
protected:
...
#else
	LPDIRECT3DVERTEXBUFFERX _pVB{nullptr};
#endif

	LPDIRECT3DVERTEXBUFFERX _pLandVB{nullptr};
	...
	MAP_PROC _pfnProc{nullptr}; // 地图动态读入回调通知函数

	FILE* _fp{nullptr}; // 地图文件句柄
	BOOL _bEdit{TRUE};	// 是否可编辑
	BOOL _bSeaVisible{TRUE};
	BOOL _bShowCenterPoint{FALSE}; // 是否显示焦点和焦点所在Section
...
private:
	...
	IDirect3DVertexShaderX* m_SkyDoomVertexShaderHandle{nullptr};
	IDirect3DPixelShaderX* m_SkyDoomPixelShaderHandle{nullptr};
#elif (defined LW_USE_DX8)
	IDirect3DVertexShaderX m_SkyDoomVertexShaderHandle{0};
	IDirect3DPixelShaderX m_SkyDoomPixelShaderHandle{0};
#endif
