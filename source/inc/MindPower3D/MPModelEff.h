...
public:
	...
	void GetTransformMatrix(D3DXMATRIX* pSOut, D3DXMATRIX* pRota = nullptr) {
...
public:
	...
	void BindingRes(CMPResManger* pResMagr) {
		...
		if (idx == -1) {
			char szData[128];
			_snprintf_s(szData, _TRUNCATE, "获取特效信息(ID%d)名称失败！", idx);
...
public:
	...
	~CMPStrip();
#ifdef USE_RENDER
	void CreateStrip(MPRender* pDev, const std::string& strTexName, CMPResManger* pResMagr)
#else
	void CreateStrip(LPDIRECT3DDEVICEX pDev, const std::string& strTexName, CMPResManger* pResMagr)
#endif
	{
		m_pDev = pDev;
...
public:
	...
	std::string GetTexName() { return _strTexName; }
	void SetTexName(const std::string& str, CMPResManger* pResMagr) {
		_strTexName = str;
		int id = pResMagr->GetTextureID(_strTexName);
		if (id < 0)
			_pTex = nullptr;
