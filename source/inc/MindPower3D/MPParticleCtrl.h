...
public:
	...
	bool LoadScript(const std::string& strModel);
...
public:
	...
	CMPStrip* NewStrip(const std::string& strTex, CMPResManger* pResMagr);
	CMPStrip* GetStrip(int iIdx);
	int GetStripNum() { return m_iStripNum; }

	CChaModel* NewModel(const std::string& strID, CMPResManger* pResMagr);
	...
	void GetHitRes(CMPResManger* pResMagr, std::vector<std::string>& vecPar);
	...
	//!装入粒子从文件
	bool LoadFromFile(const char* pszName);
