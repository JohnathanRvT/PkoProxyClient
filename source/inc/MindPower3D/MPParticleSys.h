...
inline int Rand(int _ix) {
	...
	if (_ix == 0)
		return 0;
	return rng.uniform(0, _ix);
}
inline int Rand(int _imin, int _imax) {
	
	if (_imin == 0 && _imax == 0)
		return 0;
	return rng.uniform(_imin, _imax);
}
inline float Randf(float _fx) {
	
	if (_fx == 0)
		return 0;
	return rng.uniform((float)0.0, _fx);
}
inline float Randf(float _f1, float _f2) {
	
	if (_f1 == _f2)
		return _f2;
	return rng.uniform(_f1, _f2);
}
...
inline D3DXMATRIX* GetMatrixRotation(D3DXMATRIX* pout, D3DXVECTOR3* Point,
	...
	D3DXMatrixRotationAxis(&r2, axis, angle);
	r = r1 * r2;
...
public:
	...
	bool Create(int iType, const std::string& strPartName, int iNumPart,
...
public:
	...
	std::string GetSysName() { return _strPartName; }
	void SetSysName(const std::string& strPartName) { _strPartName = strPartName; }
	...
	std::string GetTextureName() { return _strTexName; }
	void SetTextureName(const std::string& strTexName, CMPResManger* pCResMagr);

	std::string GetModelName() { return _strModelName; }
	void SetModelName(const std::string& strModelName, CMPResManger* pCResMagr);
...
public:
	...
	std::string& GetHitEff() { return m_strHitEff; }
	void SetHitEff(const std::string& streff) { m_strHitEff = streff; }
...
protected:
	...
	std::string _strPartName;

	int _iParNum;
	S_BVECTOR<CMPParticle> _vecParticle;

	std::string _strTexName;
	LPDIRECT3DTEXTUREX _lpCurTex;
	lwITex* _pTex;
	//lwITex*									_oldtex;

	std::string _strModelName;
	...
	std::string _strVirualModel;			 //用来查找的虚拟模型的名称
