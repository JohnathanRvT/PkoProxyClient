...
public:
	...
	void SetSamplerState(int nStage, D3DSAMPLERSTATETYPE, int nValue);
#endif
...
#if (defined LW_USE_DX9)
	void SetSamplerStateForced(int nStage, D3DSAMPLERSTATETYPE, int nValue);
#endif
...
public:
	...
	void AddLine(const D3DXVECTOR3& v1, const D3DXVECTOR3& v2, DWORD dwColor = 0xFFFFFFFF);
	lwInterfaceMgr* GetInterfaceMgr() { return &_IMgr; }

	void InitRender(IDirect3DX* pd3d, LPDIRECT3DDEVICEX pDev) {
		_pD3D = pd3d;
		_pD3DDevice = pDev;
...
protected:
	...
	CMPFont* _pFont;
	//@}
	LPDIRECT3DDEVICEX _pD3DDevice{nullptr};
	D3DVIEWPORTX _view;
	int _nScrWidth;  // 屏幕宽度
	...
	DWORD _dwBackgroundColor{D3DCOLOR_XRGB(128, 128, 128)}; // 屏幕背景清除颜色
	DWORD _dwClearFlag;
...
#if (defined LW_USE_DX9)
inline void MPRender::SetFVFX(DWORD fvf) {
	_IMgr.dev_obj->SetFVFX(fvf);
}
...
#if (defined LW_USE_DX9)
inline void MPRender::SetSamplerState(int nStage, D3DSAMPLERSTATETYPE nState, int nValue) {
#if (defined USE_MANAGED_RES)
	_IMgr.dev_obj->SetSamplerState(nStage, nState, nValue);
#else
	_pD3DDevice->SetSamplerState(nStage, nState, nValue);
#endif
}
#endif
...
#if (defined LW_USE_DX9)
inline void MPRender::SetSamplerStateForced(int nStage, D3DSAMPLERSTATETYPE nState, int nValue) {
	_IMgr.dev_obj->SetSamplerStateForced(nStage, nState, nValue);
}
#endif
...
#if (defined LW_USE_DX9)
inline BOOL MPRender::GetSoftwareVertexProcessing() {
	return _IMgr.dev_obj->GetSoftwareVertexProcessing();
}
#endif
