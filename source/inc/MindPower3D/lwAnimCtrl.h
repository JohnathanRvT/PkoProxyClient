...
public:
	...
	// == base method
	virtual LW_DWORD GetCtrlType() const override {
		return lwAnimCtrl::_GetCtrlType();
	}
	virtual void SetResFile(const lwResFileAnimData* info) override {
		lwAnimCtrl::_SetResFile(info);
	}
	virtual lwResFileAnimData* GetResFileInfo() override {
		return lwAnimCtrl::_GetResFileInfo();
	}
	virtual LW_VOID SetRegisterID(LW_DWORD id) override {
		lwAnimCtrl::_SetRegisterID(id);
	}
	virtual LW_DWORD GetRegisterID() const override {
		return lwAnimCtrl::_GetRegisterID();
	}
	virtual lwIPoseCtrl* GetPoseCtrl() override {
		return lwAnimCtrl::_GetPoseCtrl();
	}
	// end

	LW_RESULT LoadData(const void* data) override;
	LW_RESULT Destroy();

	LW_RESULT SetFrame(float frame, DWORD start_frame, DWORD end_frame);

	lwMatrix44* GetBoneRTMSeq() override { return _rtmat_ptr; }
	lwIndexMatrix44* GetDummyRTMSeq() override { return _dummy_rtmat_seq; }
	lwMatrix44* GetDummyRTM(DWORD id) override;
	DWORD GetDummyNum() override { return _data._dummy_num; }
	DWORD GetBoneNum() override { return _data._bone_num; }

	virtual LW_RESULT UpdatePose(lwPlayPoseInfo* info) override;
	virtual LW_RESULT UpdateAnimData(const lwPlayPoseInfo* info) override;
	virtual LW_RESULT UpdatePoseKeyFrameProc(lwPlayPoseInfo* info) override;

	LW_RESULT CreateRunTimeDataBuffer() override;
	LW_RESULT EnableRunTimeFrameBuffer(DWORD frame, DWORD flag) override;

	LW_RESULT ExtractAnimData(lwIAnimDataBone* out_data) override;
	LW_RESULT DumpRunTimeBoneData(const char* file) override;
	LW_RESULT DumpInitInvMat(const char* file) override;
};
...
public:
	...
	virtual LW_RESULT LoadData(const void* data) override;
	...
	LW_RESULT ExtractAnimData(lwIAnimDataMatrix* out_data) override;
};
...
public:
	...
	LW_RESULT LoadData(const void* data) override;
	LW_RESULT LoadData(const lwMatrix44* mat_seq, DWORD mat_num) override;
	...
	virtual LW_RESULT UpdatePose(lwPlayPoseInfo* info) override;
	virtual LW_RESULT UpdateAnimData(const lwPlayPoseInfo* info) override;
	virtual void SetAnimKeySetPRS(lwIAnimKeySetPRS* keyset) override;

	LW_RESULT GetRunTimeMatrix(lwMatrix44* mat) override;
	LW_RESULT ExtractAnimData(lwIAnimDataTexUV* out_data) override;
};
...
public:
	...
	LW_RESULT ExtractAnimData(lwIAnimDataTexImg* out_data) override;
...
public:
	...
	LW_RESULT LoadData(const void* data) override;
	LW_RESULT UpdatePose(lwPlayPoseInfo* info) override;
	LW_RESULT UpdateAnimData(const lwPlayPoseInfo* info) override;

	LW_RESULT ExtractAnimData(lwIAnimDataMtlOpacity* out_data) override;
