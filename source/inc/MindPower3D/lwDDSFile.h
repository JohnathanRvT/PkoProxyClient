...
public:
	...
	void SetDevice(IDirect3DDeviceX* dev) override { _dev = dev; }
	LW_RESULT Clear() override;
	//LW_RESULT Load(BYTE* data);
	//LW_RESULT Load(const char* file);
	LW_RESULT LoadOriginTexture(const char* file, DWORD mip_level, D3DFORMAT format, DWORD colorkey) override;
	LW_RESULT Convert(const char* file, D3DFORMAT src_fmt, D3DFORMAT dds_fmt, DWORD mip_level, DWORD src_colorkey) override;
