...
struct lwTexInfo_0001 {
	...
	lwColorValue4b colorkey;
	char file_name[LW_MAX_NAME];
	DWORD padding; //Changed from void* data (no issues here since data gets transfered to lwTexInfo)
...
struct lwMeshInfo_0000 {
	...
	lwMeshInfo_0000()
		: vertex_seq(nullptr), normal_seq(nullptr), vercol_seq(nullptr), index_seq(nullptr), blend_seq(nullptr),
...
struct lwHelperInfo : public lwIHelperInfo {
	...
	lwHelperInfo()
		: type(HELPER_TYPE_INVALID), mesh_seq(nullptr), box_seq(nullptr), dummy_seq(nullptr), bbox_seq(nullptr), bsphere_seq(nullptr),
...
struct lwBoneKeyInfo {
	...
	lwBoneKeyInfo()
		: mat43_seq(nullptr), mat44_seq(nullptr), quat_seq(nullptr), pos_seq(nullptr) {
	}
...
public:
	...
	DWORD GetFrameNum() override { return _frame_num; }
...
struct lwAnimKeySetPRS {
	...
	lwAnimKeySetPRS()
		: pos_seq(nullptr), rot_seq(nullptr), sca_seq(nullptr), pos_num(0), rot_num(0), sca_num(0), frame_num(0) {
	}
...
public:
	...
	LW_RESULT GetValue(lwMatrix44* mat, float frame);
	LW_RESULT SetData(const lwMatrix44* mat_seq, DWORD mat_num) override {
...
public:
	lwAnimDataMtlOpacity() : _aks_ctrl(nullptr) {}
	virtual ~lwAnimDataMtlOpacity() { LW_SAFE_RELEASE(_aks_ctrl); }

	LW_RESULT Clone(lwIAnimDataMtlOpacity** obj) override;
	LW_RESULT Load(FILE* fp, DWORD version);
	LW_RESULT Save(FILE* fp);
	DWORD GetDataSize();
	lwIAnimKeySetFloat* GetAnimKeySet() override { return _aks_ctrl; }
	void SetAnimKeySet(lwIAnimKeySetFloat* aks_ctrl) override { _aks_ctrl = aks_ctrl; }
};
...
class lwAnimDataInfo : public lwIAnimDataInfo {
	...
	enum {
		ANIM_DATA_NUM = 2 + LW_MAX_SUBSET_NUM + LW_MAX_SUBSET_NUM * LW_MAX_TEXTURESTAGE_NUM * 2
	};
...
struct lwGeomObjInfo : public lwIGeomObjInfo {
	...
	LW_RESULT Save(FILE* fp);
	LW_RESULT Load(const char* file) override;
	LW_RESULT Save(const char* file) override;
	DWORD GetDataSize() const;

	lwMeshInfo* GetMeshInfo() override { return &mesh; }
	lwMtlTexInfo* GetMtlTexInfo() { return mtl_seq; }
};

using lwGeomObjInfoHeader = lwGeomObjInfo::lwGeomObjInfoHeader;
...
public:
	...
	virtual ~lwModelObjInfo() {
		DWORD i = 0;
		for (i = 0; i < LW_MAX_MODEL_GEOM_OBJ_NUM; i++) {
...
using lwModelObjInfoHeader = lwModelObjInfo::lwModelObjInfoHeader;
