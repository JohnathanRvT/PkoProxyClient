...
public:
	...
	using lwHeapTCompareProc = BOOL (*)(const T& i, const T& j);
	using lwHeapTFilterProc = void (*)(const T& t, DWORD i);
...
class lwHeap : public lwIHeap {
	...
	using T = lwHeapT<void*>;

private:
	T _heap;

public:
	LW_RESULT Reserve(DWORD size) override {
		return _heap.Reserve(size);
	}
	LW_RESULT Clear() override {
		return _heap.Clear();
	}
	LW_RESULT Push(const void* t) override {
		return _heap.Push((T::value_type_ref)t);
	}
	LW_RESULT Push(const void* buf, DWORD size) override {
		return _heap.Push((T::value_type_ptr)buf, size);
	}
	LW_RESULT Pop() override {
		return _heap.Pop();
	}
	LW_RESULT Top(void** t) override {
		return _heap.Top(t);
	}
	LW_RESULT Find(DWORD* id, const void* t) override {
		return _heap.Find(id, (T::value_type_ref)t);
	}
	LW_RESULT Update(DWORD id) override {
		return _heap.Update(id);
	}
	LW_RESULT Remove(DWORD id) override {
		return _heap.Remove(id);
	}

	void SetCompareProc(lwHeapCompProc proc) override {
		_heap.SetCompareProc((T::lwHeapTCompareProc)proc);
	}
	void SetFilterProc(lwHeapFilterProc proc) override {
		_heap.SetFilterProc((T::lwHeapTFilterProc)proc);
	}
	lwHeapCompProc GetCompareProc() override {
		return (lwHeapCompProc)_heap.GetCompareProc();
	}
	lwHeapFilterProc GetFilterProc() override {
		return (lwHeapFilterProc)_heap.GetFilterProc();
	}

	DWORD GetHeapNum() override {
		return _heap.GetHeapNum();
	}
	DWORD GetHeapCapacity() override {
	...
	LW_RESULT Clone(lwIHeap** out_heap) override;
};
