...
public:
	virtual LW_RESULT LoadData(const void* data) override PURE_METHOD;
...
public:
	virtual std::string SetPath(DWORD type, std::string path) PURE_METHOD;
	virtual std::string GetPath(DWORD type) PURE_METHOD;
};
...
public:
...
#if (defined LW_USE_DX9)
	virtual LW_RESULT SetFVFX(DWORD fvf) PURE_METHOD;
...
#if (defined LW_USE_DX9)
	virtual BOOL GetSoftwareVertexProcessing() PURE_METHOD;
#endif
	...
	virtual LW_RESULT CreateCubeTexture(IDirect3DCubeTextureX** o_tex, UINT edge_length, UINT levels, DWORD usage, D3DFORMAT format, D3DPOOLX pool, HANDLE* handle) PURE_METHOD;
	virtual LW_RESULT CreateOffscreenPlainSurface(IDirect3DSurfaceX** surface, UINT width, UINT height, D3DFORMAT format, D3DPOOLX pool, HANDLE* handle) PURE_METHOD;
...
public:
	...
	virtual LW_RESULT AddStateToSet(DWORD state, DWORD value) PURE_METHOD;
...
public:
	...
	virtual lwIAnimCtrl* GetAnimCtrl() override PURE_METHOD;
	virtual lwPlayPoseInfo* GetPlayPoseInfo() override PURE_METHOD;
	virtual LW_RESULT SetTypeInfo(lwAnimCtrlObjTypeInfo* info) override PURE_METHOD;
	virtual LW_RESULT GetTypeInfo(lwAnimCtrlObjTypeInfo* info) override PURE_METHOD;
	virtual LW_RESULT PlayPose(const lwPlayPoseInfo* info) override PURE_METHOD;
	virtual BOOL IsPlaying() override PURE_METHOD;

	virtual LW_RESULT UpdateAnimCtrl() override PURE_METHOD;
...
public:
	virtual std::tuple<DWORD*, DWORD> LoadFileShader(std::string file, DWORD binary_flag) PURE_METHOD;
	...
	virtual LW_RESULT RegisterVertexShader(DWORD type, std::string file, DWORD usage, DWORD* decl, DWORD decl_size, DWORD file_flag) PURE_METHOD;
	...
	virtual LW_RESULT RegisterVertexShader(DWORD type, DWORD* data, DWORD size) PURE_METHOD;
	virtual LW_RESULT RegisterVertexShader(DWORD type, std::string file, DWORD binary_flag);
...
public:
	...
	virtual LW_RESULT Update() override PURE_METHOD;
	virtual LW_RESULT Render() override PURE_METHOD;
	virtual LW_RESULT RenderSubset(DWORD subset) PURE_METHOD;
	virtual LW_RESULT RenderHelperObject() PURE_METHOD;
...
public:
	...
	virtual LW_RESULT GetOpenFileName(HWND hwnd, char* buf, int num, const char* dir, const char* title = nullptr,
	...
	virtual LW_RESULT GetSaveFileName(HWND hwnd, char* buf, int num, const char* dir, const char* title = nullptr,
									  const char* filter = "all files(*.*)\0*.*\0\0", const char* ext = nullptr, int flag = OFN_PATHMUSTEXIST | OFN_EXPLORER) PURE_METHOD;
};
...
public:
	...
	virtual LW_RESULT Create(DWORD desired_access = GENERIC_READ | GENERIC_WRITE, DWORD share_mode = 0, const SECURITY_ATTRIBUTES* security_attr = nullptr) PURE_METHOD;
	virtual LW_RESULT Destroy() PURE_METHOD;
...
public:
	...
	virtual LW_RESULT SeekWord(const char* str, const char* termi = nullptr) PURE_METHOD;
