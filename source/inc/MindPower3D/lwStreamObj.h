...
public:
	lwStreamEntity()
		: size(0), offset(0), stride(0), data(nullptr),
...
public:
	...
	LW_RESULT CreateStreamEntitySeq(DWORD entity_vb_num, DWORD entity_ib_num) override;
	LW_RESULT CreateVertexBufferStream(DWORD stream_id, DWORD stream_size) override;
	LW_RESULT CreateIndexBufferStream(DWORD stream_id, DWORD stream_size) override;

	LW_RESULT RegisterVertexBuffer(LW_HANDLE* out_handle, void* data, DWORD size, DWORD stride) override;
	LW_RESULT RegisterIndexBuffer(LW_HANDLE* out_handle, void* data, DWORD size, DWORD stride) override;
	LW_RESULT UnregisterVertexBuffer(LW_HANDLE handle) override;
	LW_RESULT UnregisterIndexBuffer(LW_HANDLE handle) override;
	LW_RESULT BindVertexBuffer(LW_HANDLE handle, UINT channel) override;
	LW_RESULT BindIndexBuffer(LW_HANDLE handle) override;
	DWORD GetVertexEntityOffset() override { return _vertex_entry_offset; }
	DWORD GetIndexEntityOffset() override { return _index_entry_offset; }
	LW_RESULT LoseDevice() override;
	LW_RESULT ResetDevice() override;
	LW_RESULT GetDebugInfo(lwStaticStreamMgrDebugInfo* info) override;
};
...
public:
	...
	LW_RESULT Create(DWORD vb_size, DWORD ib_size) override;
	LW_RESULT DrawPrimitive(D3DPRIMITIVETYPE pt_type, UINT count, const void* vert_data, UINT vert_stride, D3DFORMAT fvf) override;
	LW_RESULT DrawIndexedPrimitive(D3DPRIMITIVETYPE pt_type, UINT min_vert_index, UINT num_vert_indices, UINT count,
								   const void* index_data, D3DFORMAT index_format, const void* vertex_data, UINT vert_stride, D3DFORMAT fvf) override;

	LW_RESULT LoseDevice() override;
	LW_RESULT ResetDevice() override;

	LW_RESULT BindDataVB(DWORD channel, const void* data, DWORD size, UINT stride) override;
	LW_RESULT BindDataIB(const void* data, DWORD size, DWORD stride) override;
	LW_RESULT DrawPrimitive(D3DPRIMITIVETYPE pt_type, UINT start_vert, UINT count) override;
	LW_RESULT DrawIndexedPrimitive(D3DPRIMITIVETYPE pt_type, INT base_vert_index, UINT min_index, UINT num_vert, UINT start_index, UINT count) override;
};
...
public:
	lwLockableStream()
		: _size(0), _usage(0), _fvf(0), _data(nullptr), _lock_offset(0), _lock_size(0), _lock_flag(0), _lock_cnt(0) {
	}
	inline void Reset() {
		_size = 0;
		_usage = 0;
...
public:
	...
	LW_RESULT Destroy();
	LW_RESULT Lock(UINT offset, UINT size, void** data, DWORD flag) override;
	LW_RESULT Unlock() override;
...
class lwLockableStreamMgr : public lwILockableStreamMgr {
	...
	using lwPoolStreamVB = lwObjectPoolVoidPtr1024;
	using lwPoolStreamIB = lwObjectPoolVoidPtr1024;
	using lwPoolEntity = void*;
...
public:
	...
	lwILockableStreamVB* GetStreamVB(LW_HANDLE handle) override;
	lwILockableStreamIB* GetStreamIB(LW_HANDLE handle) override;

	LW_RESULT RegisterVertexBuffer(LW_HANDLE* handle, void* data, DWORD size, DWORD usage, DWORD fvf) override;
	LW_RESULT RegisterIndexBuffer(LW_HANDLE* handle, void* data, DWORD size, DWORD usage, DWORD fvf) override;
	LW_RESULT UnregisterVertexBuffer(LW_HANDLE handle) override;
	LW_RESULT UnregisterIndexBuffer(LW_HANDLE handle) override;
	LW_RESULT BindVertexBuffer(LW_HANDLE handle, UINT channel, UINT offset_byte, UINT stride) override;
	LW_RESULT BindIndexBuffer(LW_HANDLE handle, UINT base_vert_index) override;
	LW_RESULT LoseDevice() override;
	LW_RESULT ResetDevice() override;
};
...
public:
	...
	LW_RESULT CreateRenderTarget(UINT width, UINT height, D3DFORMAT format, D3DMULTISAMPLE_TYPE multi_sample, DWORD multi_sample_quality, BOOL lockable, HANDLE* handle) override;
	LW_RESULT CreateDepthStencilSurface(UINT width, UINT height, D3DFORMAT format, D3DMULTISAMPLE_TYPE multi_sample, DWORD multi_sample_quality, BOOL discard, HANDLE* handle) override;
	...
	IDirect3DSurfaceX* GetSurface() override { return _surface; }
};

class lwSurfaceStreamMgr : public lwISurfaceStreamMgr {
	LW_STD_DECLARATION()

	using lwPoolSurface = lwObjectPoolVoidPtr1024;
...
public:
	...
	LW_RESULT CreateRenderTarget(LW_HANDLE* ret_handle, UINT width, UINT height, D3DFORMAT format, D3DMULTISAMPLE_TYPE multi_sample, DWORD multi_sample_quality, BOOL lockable, HANDLE* handle) override;
	LW_RESULT CreateDepthStencilSurface(LW_HANDLE* ret_handle, UINT width, UINT height, D3DFORMAT format, D3DMULTISAMPLE_TYPE multi_sample, DWORD multi_sample_quality, BOOL discard, HANDLE* handle) override;
	LW_RESULT UnregisterSurface(LW_HANDLE handle) override;

	LW_RESULT LoseDevice() override;
	LW_RESULT ResetDevice() override;

	lwIResourceMgr* GetResourceMgr() override { return _res_mgr; }
	lwISurfaceStream* GetSurfaceStream(LW_HANDLE handle) override;
};
...
public:
	...
	LW_RESULT Create(UINT length, DWORD usage, DWORD fvf, D3DPOOL pool, DWORD stride, HANDLE* handle) override;
	LW_RESULT Destroy() override;
	LW_RESULT Lock(UINT offset, UINT size, D3DLOCK_TYPE** out_data, DWORD flag) override { return _buf->Lock(offset, size, out_data, flag); }
	LW_RESULT Unlock() override { return _buf->Unlock(); }
	LW_RESULT LoadData(const void* data_seq, DWORD data_size, UINT offset, DWORD lock_flag) override;
	LW_RESULT LoadDataDynamic(const void* data_seq, DWORD data_size) override;
	LW_RESULT BindDevice(DWORD stream_id, UINT offset_byte) override { return _dev_obj->SetStreamSource(stream_id, _buf, offset_byte, _buf_info.stride); }
	LW_RESULT SetDeviceFVF() override { return _dev_obj->SetFVFX(_buf_info.fvf); }
	lwVertexBufferInfo* GetBufferInfo() override { return &_buf_info; }
	IDirect3DVertexBufferX* GetStreamBuffer() override { return _buf; }
};
...
public:
	...
	LW_RESULT Create(UINT length, DWORD usage, D3DFORMAT format, D3DPOOL pool, HANDLE* handle) override;
	LW_RESULT Destroy() override;
	LW_RESULT Lock(UINT offset, UINT size, D3DLOCK_TYPE** out_data, DWORD flag) override { return _buf->Lock(offset, size, out_data, flag); }
	LW_RESULT Unlock() override { return _buf->Unlock(); }
	LW_RESULT LoadData(const void* data_seq, DWORD data_size, UINT offset, DWORD lock_flag) override;
	LW_RESULT LoadDataDynamic(const void* data_seq, DWORD data_size) override;
	LW_RESULT BindDevice() override { return _dev_obj->SetIndices(_buf, 0); }
	lwIndexBufferInfo* GetBufferInfo() override { return &_buf_info; }
	IDirect3DIndexBufferX* GetStreamBuffer() override { return _buf; }
};
