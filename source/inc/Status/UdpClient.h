...
private:
	using gates = std::vector<CGateInfo*>;
	gates Gates;
	CRegionInfo* pOwner;
};
...
private:
	using groups = std::vector<CGroupInfo*>;
	groups Groups;
	CRegionMgr* pOwner;
};

using GateChangeEvent = void (*)(CGateInfo* pGate);
...
private:
	using regions = std::vector<CRegionInfo*>;
	...
	using gates = std::map<SOCKET, CGateInfo*>;
