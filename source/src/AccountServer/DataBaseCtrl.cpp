...
bool CDataBaseCtrl::CreateObject() {
	//读取配置
	char buf[512];
	dbc::IniFile inf(g_strCfgFile.data());
	dbc::IniSection& is = inf["db"];
	std::string strTmp = "";
	try {
		//sprintf(buf, "dbserver");
		_snprintf_s(buf, sizeof(buf), "dbserver");
		m_strServerIP = is[buf].c_str();
		//sprintf(buf, "db");
		_snprintf_s(buf, sizeof(buf), "db");
		m_strServerDB = is[buf].c_str();
		//sprintf(buf, "userid");
		_snprintf_s(buf, sizeof(buf), "userid");
		m_strUserID = is[buf].c_str();
		//sprintf(buf, "passwd");
		_snprintf_s(buf, sizeof(buf), "passwd");
		strTmp = is[buf].c_str();
	} catch (dbc::excp& e) {
...
bool CDataBaseCtrl::UserLogin(int nUserID, std::string strUserName, std::string strIP) {
	if (!strUserName.c_str() || strUserName.empty()) {
...
{
	if (!strActName.c_str() || strActName.empty()) {
