...
BOOL CChaRecordSet::_ReadRawDataInfo(CRawDataInfo* pRawDataInfo, std::vector<std::string>& ParamList) {
	try {
		if (ParamList.empty())
			return FALSE;
		...
		//pInfo->szName[defCHA_NAME_LEN - 1] = '\0';
		...
		// End
		//pInfo->szIconName[defCHA_ICON_NAME_LEN - 1] = '\0';
		// 模型类型
		...
		//pInfo->szGuild[defCHA_GUILD_NAME_LEN - 1] = '\0';
		// 角色称号
