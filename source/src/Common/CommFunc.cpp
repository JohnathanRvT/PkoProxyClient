...
bool CTextFilter::Add(const eFilterTable eTable, const char* szFilterText) {
	...
	m_FilterTable[eTable].emplace_back(szFilterText); //释放内存 Modify by Waiting 2009-06-18
...
bool CTextFilter::IsLegalText(const eFilterTable eTable, const std::string strText) {
...
bool CTextFilter::Filter(const eFilterTable eTable, std::string& strText) {
	bool ret = false;
	std::vector<std::string>::iterator iter;
	for (iter = m_FilterTable[eTable].begin(); iter != m_FilterTable[eTable].end(); iter++) {
...
bool CTextFilter::ReplaceText(std::string& strText, const std::string* pstrFilterText) {
	//_W64 nPos=strText.find(*pstrFilterText);
	std::string::size_type nPos = strText.find(*pstrFilterText);

	static const std::basic_string<char>::size_type errPos = -1;
	while (nPos != errPos) {
...
bool CTextFilter::bCheckLegalText(const std::string& strText, const std::string* pstrIllegalText) {
	//_W64 nPos=strText.find(*pstrIllegalText);
	std::string::size_type nPos = strText.find(*pstrIllegalText);
	static const std::basic_string<char>::size_type errPos = -1;
	if (nPos != errPos) return false;
	return true;
}
...
bool CTextFilter::LoadFile(const char* szFileName, const eFilterTable eTable) {
	...
	std::ifstream filterTxt(szFileName, std::ios::in);
	if (!filterTxt.is_open())
		return false;

	char buf[500] = {0};
	while (filterTxt.getline(buf, 500)) {
		m_FilterTable[eTable].emplace_back(buf);
	}
...
char* LookData2String(const stNetChangeChaPart* pLook, char* szLookBuf, int nLen, bool bNewLook) {
	...
	*/
	...
	for (size_t i = 0; i < pLook->SLink.size(); i++) {
		auto pGridCont = static_cast<const SItemGrid*>(&pLook->SLink[i]);
		if (bNewLook && !g_IsValidLook(pLook->sTypeID, (int)i, pGridCont->sID)) {
			return nullptr; // 数据不合法
		}
	...
	_snprintf_s(szData, _TRUNCATE, ";%lld", lnCheckSum);
	nDataLen = (int)strlen(szData);
	if (nBufLen + nDataLen >= nLen)
		return nullptr;
	//strcat(szLookBuf, szData);
...
const char* LookData2StringEx(const stNetChangeChaPart* pLook, StringPoolL& lookString, int index, bool bNewLook) {
	...
	for (int i = 0; i < pLook->SLink.size(); i++) {
		auto pGridCont = static_cast<const SItemGrid*>(&pLook->SLink[i]);
...
bool Strin2LookData(stNetChangeChaPart* pLook, std::string& strData) {
	...
	*/
	...
	short sTCount;
	size_t i;
	for (i = 0; i < pLook->SLink.size(); i++) {
		sTCount = 0;
		SItemGrid* pItem = &pLook->SLink[i];
	...
	if (!bIsOldVer) {
		...
		_snprintf_s(szCheckSum, _TRUNCATE, "%lld", lnCheckSum);
