...
BOOL CNpcRecordSet::_ReadRawDataInfo(CRawDataInfo* pRawDataInfo, std::vector<std::string>& ParamList) {
	if (ParamList.empty())
		return FALSE;
	...
	//pInfo->szName[NPC_MAXSIZE_NAME - 1] = '\0';
	...
	strncpy_s(pInfo->szNpc, sizeof(pInfo->szNpc), ParamList[m++].c_str(), _TRUNCATE);
	//pInfo->szNpc[NPC_MAXSIZE_NAME - 1] = '\0';

	// npc对话处理
	//_tcsncpy(pInfo->szMsgProc, ParamList[m++].c_str(), NPC_MAXSIZE_MSGPROC);
	strncpy_s(pInfo->szMsgProc, sizeof(pInfo->szMsgProc), ParamList[m++].c_str(), _TRUNCATE);
	//pInfo->szMsgProc[NPC_MAXSIZE_MSGPROC - 1] = '\0';

	// npc对话处理
	//_tcsncpy(pInfo->szMisProc, ParamList[m++].c_str(), NPC_MAXSIZE_MSGPROC);
	strncpy_s(pInfo->szMisProc, sizeof(pInfo->szMisProc), ParamList[m++].c_str(), _TRUNCATE);
	//pInfo->szMisProc[NPC_MAXSIZE_MSGPROC - 1] = '\0';
