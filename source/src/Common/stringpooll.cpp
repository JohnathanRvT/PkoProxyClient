...
void StringPoolL::Clear() {
	...
	if (Offsets)
		memset(Offsets, 0, Num * sizeof(*Offsets));
...
void StringPoolL::ReserveBuf(unsigned int newsize) {
	...
	if (Buffer2 == nullptr)
		Error(3, newsize); // Allocation failed
...
void StringPoolL::ReserveNum(unsigned int newsize) {
	...
	if (Offsets)
		memcpy(Offsets2, Offsets, OffsetsSize * sizeof(*Offsets));
...
void StringPoolL::Error(int message, int i) const {
	...
	if ((unsigned int)message >= NumErrorTexts)
		message = 0;
