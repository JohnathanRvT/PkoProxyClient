...
CAuctionSystem::~CAuctionSystem() {
	for (auto [id, pAucItem] : m_mapItemList) {
		SAFE_DELETE(pAucItem);
	}
	m_mapItemList.clear();
}

BOOL CAuctionSystem::StartAuction(short sItemID, const std::string& strName, short sCount, uInt nBasePrice, uInt nMinBid) {
	auto it = m_mapItemList.find(sItemID);
	...
	auto* pAucItem = new CAuctionItem(sItemID, strName, sCount, nBasePrice, nMinBid);
	if (pAucItem == nullptr) {
...
void CAuctionSystem::ListAuction(CCharacter* pCha, CCharacter* pNpc) {
	...
	WRITE_CMD(wpk, CMD_MC_LISTAUCTION);

	for (const auto& [id, pAucItem] : m_mapItemList) {
		WRITE_SHORT(wpk, pAucItem->GetItemID());
		WRITE_STRING(wpk, pAucItem->GetName().c_str());
		WRITE_STRING(wpk, pAucItem->GetCurChaName().c_str());
		WRITE_SHORT(wpk, pAucItem->GetItemCount());
		WRITE_LONG(wpk, pAucItem->GetBasePrice());
		WRITE_LONG(wpk, pAucItem->GetMinBid());
		WRITE_LONG(wpk, pAucItem->GetCurPrice());
	}
	WRITE_SHORT(wpk, m_mapItemList.size());
...
void CAuctionSystem::NotifyAuction(CCharacter* pCha, CCharacter* pNpc) {
	...
	WRITE_SHORT(wpk, m_mapItemList.size());
	pNpc->NotiChgToEyeshot(wpk, false);
}
