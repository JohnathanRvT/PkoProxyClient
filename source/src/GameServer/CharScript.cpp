...
inline int lua_AddSailExp(lua_State* L) {
	...
	try {
		...
		if (dwMinExp >= dwMaxExp) {
			...
			dwValue = rng.uniform(dwMinExp, dwMinExp + (dwMaxExp - dwMinExp) - 1);
		}
...
inline int lua_Garner2GetWiner(lua_State* L) {
	...
	WRITE_CMD(wpk, CMD_MP_GARNER2_CGETORDER);
	pChar->ReflectINFof(pChar, wpk);
...
inline int lua_DealAllActivePlayerInMap(lua_State* L) //2006.10.12wsj
{
	...
	std::string strfun(pfunctionname);
...
inline int lua_LifeSkillBegin(lua_State* L) {
	if (auto bValid = lua_gettop(L) == 3 && lua_islightuserdata(L, 1) && lua_islightuserdata(L, 2) && lua_isnumber(L, 3);
		!bValid) {
		E_LUAPARAM;
		return 0;
	}
	...
	auto pTalk = static_cast<CTalkNpc*>(lua_touserdata(L, 2));
	auto type = static_cast<char>(lua_tonumber(L, 3));
	if (!pChar || !pTalk) {
		E_LUANULL;
		return 0;
	}
	...
	pChar->ForgeAction();
	WPacket wpk = GETWPACKET();
	WRITE_CMD(wpk, CMD_MC_LIFESKILL_BGING);
	WRITE_CHAR(wpk, type);
	pChar->ReflectINFof(pChar, wpk);
	lua_pushnumber(L, LUA_TRUE);
	return 1;
