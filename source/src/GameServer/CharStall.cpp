...
CStallData::CStallData(size_t lSize)
	: PreAllocStru(1) {
	Clear();
}
...
void CStallSystem::StartStall(CCharacter& staller, RPACKET& packet) {
	...
	for (BYTE i = 0; i < pData->m_byNum; i++) {
		...
		pData->m_Goods[i].wCount = packet.ReadShort();
		...
		__int64 n64Temp = (__int64)(pData->m_Goods[i].dwMoney) * pData->m_Goods[i].wCount;
		...
		if (pData->m_Goods[i].wCount == 0) {
			pData->m_Goods[i].wCount = 1;
		}

		if (staller.GetKitbag()->GetNum(pData->m_Goods[i].byIndex) < pData->m_Goods[i].wCount) {
		...
		if (pItem->sType == EItemType::Boat) {
			...
			if (!pBoat) {
				...
				//sprintf( szTemp, "，%d艘船只《%s》，ID[0x%X]", pData->m_Goods[i].wCount, pBoat->GetName(), pData->m_Goods[i].wCount, dwBoatID );
				//sprintf( szTemp, RES_STRING(GM_CHARSTALL_CPP_00024), pData->m_Goods[i].wCount, pBoat->GetName(), pData->m_Goods[i].wCount, dwBoatID );
				//_snprintf_s( szTemp,sizeof(szTemp),_TRUNCATE, RES_STRING(GM_CHARSTALL_CPP_00024), pData->m_Goods[i].wCount, pBoat->GetName(), pData->m_Goods[i].wCount, dwBoatID );
				CFormatParameter param(3);
				param.setDouble(0, pData->m_Goods[i].wCount);
				param.setString(1, pBoat->GetName());
				param.setLong(2, dwBoatID);
				RES_FORMAT_STRING(GM_CHARSTALL_CPP_00024, param, szTemp);
			}
		} else {
			//sprintf( szTemp, RES_STRING(GM_CHARSTALL_CPP_00025), pData->m_Goods[i].wCount, pItem->szName, pData->m_Goods[i].dwMoney );
			//_snprintf_s( szTemp,sizeof(szTemp),_TRUNCATE, RES_STRING(GM_CHARSTALL_CPP_00025), pData->m_Goods[i].wCount, pItem->szName, pData->m_Goods[i].dwMoney );
			CFormatParameter param(3);
			param.setDouble(0, pData->m_Goods[i].wCount);
...
void CStallSystem::BuyGoods(CCharacter& character, RPACKET& packet) {
	...
	if (wCount > pData->m_Goods[byIndex].wCount) {
		wCount = pData->m_Goods[byIndex].wCount;
	}

	__int64 n64Temp = (__int64)(pData->m_Goods[byIndex].dwMoney) * wCount;
	if (n64Temp > MAX_STALL_MONEY) {
	...
	if (pData->m_Goods[byIndex].dwMoney * wCount > (DWORD)character.getAttr(ATTR_GD)) {
	...
	if (Bag.GetNum(pData->m_Goods[byIndex].byIndex) < wCount) {
	...
	if (pItem->sType == EItemType::Boat && character.GetPlayer()->GetNumBoat() >= MAX_CHAR_BOAT) {
	...
	// 更新物品数量
	pData->m_Goods[byIndex].wCount -= wCount;
	...
	character.setAttr(ATTR_GD, character.getAttr(ATTR_GD) - pData->m_Goods[byIndex].dwMoney * wCount);
	character.SynAttr(enumATTRSYN_TRADE);
	character.SyncBoatAttr(enumATTRSYN_TRADE);
	pStaller->setAttr(ATTR_GD, pStaller->getAttr(ATTR_GD) + pData->m_Goods[byIndex].dwMoney * wCount);
	/*pStaller->SystemNotice( "“%s”买走了你的%d个物品《%s》，得到了%d金钱！单价（%d），总额（%d）。", character.GetName(), wCount, pItem->szName, 
			wCount * pData->m_Goods[byIndex].dwMoney, pData->m_Goods[byIndex].dwMoney, pStaller->getAttr( ATTR_GD ) );*/

	CFormatParameter param(6);
	param.setString(0, character.GetName());
	param.setLong(1, wCount);
	param.setString(2, pItem->szName);
	param.setLong(3, pData->m_Goods[byIndex].dwMoney * wCount);
	...
	DelGoods(*pStaller, pData->m_Goods[byIndex].byGrid, wCount);
	...
	/*character.SystemNotice( "购买了“%s”的%d个物品《%s》,花费了%d金钱！单价（%d）,余额（%d）。", pStaller->GetName(), wCount, pItem->szName,
			pData->m_Goods[byIndex].dwMoney * wCount, pData->m_Goods[byIndex].dwMoney, character.getAttr( ATTR_GD ) );*/
	...
	/*sprintf( szTemp, "%s，单价（%d）× 数量（%d）= 交易额（%d），出售方：金钱总额（%d），购买方：金钱总额（%d）。", 
			szLog, pData->m_Goods[byIndex].dwMoney, wCount, pData->m_Goods[byIndex].dwMoney * wCount, 
	...
	// 更新摆摊物品信息
	if (pData->m_Goods[byIndex].wCount == 0) {
...
void CStallSystem::DelGoods(CCharacter& staller, BYTE byGrid, WORD wCount) {
	WPACKET packet = GETWPACKET();
...
void CStallSystem::SyncData(CCharacter& character, CCharacter& staller) {
	...
	for (BYTE i = 0; i < pData->m_byNum; ++i) {
		...
		WRITE_SHORT(packet, pData->m_Goods[i].wCount);
