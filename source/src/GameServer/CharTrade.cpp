	...
	for (int i = 0; i < ROLE_MAXNUM_TRADEDATA; i++) {
		if (pTradeData->ReqTradeData.ItemArray[i].sItemID != 0) {
			...
			if (cir) {
				...
				//
				//_snprintf(	sTextEx,	512,"%s-%d",	cir->szName,	pTradeData->ReqTradeData.ItemArray[i].wCount	);
				_snprintf_s(sTextEx, sizeof(sTextEx), _TRUNCATE, "%s-%d", cir->szName, pTradeData->ReqTradeData.ItemArray[i].wCount);
	...
	for (int i = 0; i < ROLE_MAXNUM_TRADEDATA; i++) {
		if (pTradeData->AcpTradeData.ItemArray[i].sItemID != 0) {
			...
			if (cir) {
				...
				//
				//_snprintf(	sTextEx,	512,"%s-%d",	cir->szName,	pTradeData->AcpTradeData.ItemArray[i].wCount	);
				_snprintf_s(sTextEx, sizeof(sTextEx), _TRUNCATE, "%s-%d", cir->szName, pTradeData->AcpTradeData.ItemArray[i].wCount);
				//strncat(	sText,	sTextEx,	512	);
				strncat_s(sText, sizeof(sText), sTextEx, _TRUNCATE);
			};
		};
...
CTradeData::CTradeData(size_t lSize)
	: PreAllocStru(1) {
	try {
	}
	T_E
...
// 放置或者取走物品到交易栏
BOOL CTradeSystem::AddItem(BYTE byType, CCharacter& character, DWORD dwCharID, BYTE byOpType, BYTE byIndex, BYTE byItemIndex, WORD wCount) {
	try {
		CCharacter* pMain = &character;
		...
		if (byOpType == TRADE_DRAGTO_ITEM) {
			...
			if (wCount > ROLE_MAXNUM_ITEMTRADE) {
				wCount = ROLE_MAXNUM_ITEMTRADE;
			}

			if (wCount > Bag.GetNum(byItemIndex)) {
				wCount = (BYTE)Bag.GetNum(byItemIndex);
			}

			pItemData->ItemArray[byIndex].sItemID = Bag.GetID(byItemIndex);
			pItemData->ItemArray[byIndex].wCount = wCount;
		...
		if (pTradeData->bAcpTrade == 1 && pTradeData->bReqTrade == 1 &&
			...
			for (int i = 0; i < ROLE_MAXNUM_TRADEDATA; i++) {
				...
				if (pTradeData->AcpTradeData.ItemArray[i].sItemID != 0) {
					...
					if (pItem == nullptr) {
						...
						AcpGrid[i].sNum = pTradeData->AcpTradeData.ItemArray[i].wCount;
			...
			for (int i = 0; i < ROLE_MAXNUM_TRADEDATA; i++) {
				if (pTradeData->ReqTradeData.ItemArray[i].sItemID != 0) {
					...
					if (pItem == nullptr) {
						...
						ReqGrid[i].sNum = pTradeData->ReqTradeData.ItemArray[i].wCount;
			...
			for (int i = 0; i < ROLE_MAXNUM_TRADEDATA; i++) {
				if (pTradeData->AcpTradeData.ItemArray[i].sItemID != 0) {
					...
					if (sPushRet != enumKBACT_ERROR_FULL && pItem->sType == EItemType::Boat) {
			...
			for (int i = 0; i < ROLE_MAXNUM_TRADEDATA; i++) {
				if (pTradeData->AcpTradeData.ItemArray[i].sItemID != 0) {
					...
					BYTE byNum = pTradeData->AcpTradeData.ItemArray[i].wCount - AcpGrid[i].sNum;
					if (byNum) {
				...
				if (pTradeData->ReqTradeData.ItemArray[i].sItemID != 0) {
					...
					BYTE byNum = pTradeData->ReqTradeData.ItemArray[i].wCount - ReqGrid[i].sNum;
					if (byNum) {
...
BOOL CStoreSystem::PushOrder(DWORD cha_id, std::string cha_name, long long lOrderID, long lComID, long lNum) {
	try {
		SOrderData OrderInfo;
		OrderInfo.lOrderID = lOrderID;
...
SOrderData CStoreSystem::PopOrder(long long lOrderID) {
	try {
		...
		std::vector<SOrderData>::iterator vec_it;
...
BOOL CStoreSystem::HasOrder(long long lOrderID) {
	try {
		auto it = std::find_if(m_OrderList.begin(), m_OrderList.end(),
							   [&](const SOrderData& o) { return o.lOrderID == lOrderID; });
		return it != m_OrderList.end() ? true : false;
	}
	T_E
}
...
long CStoreSystem::GetClassId(long lComID) {
	try {
		auto it = m_ItemSearchList.find(lComID);
		return it != m_ItemSearchList.end() ? it->second : 0;
	}
	T_E
}
...
cChar* CStoreSystem::GetClassName(long lClsID) {
	try {
		auto it = std::find_if(m_ItemClass.begin(), m_ItemClass.end(),
							   [&](const ClassInfo& o) { return o.clsID == lClsID; });
		return it != m_ItemClass.end() ? it->clsName : nullptr;
	}
	T_E
}
...
SItemData* CStoreSystem::GetItemData(long lComID) {
	try {
		...
		if (lClsID != 0) {
			for (auto& itemData : m_ItemList[lClsID]) {
				if (itemData.store_head.comID == lComID)
					return &itemData;
			}
		}
		return nullptr;
...
BOOL CStoreSystem::DelItemData(long lComID) {
	try {
		...
		auto cls_it = m_ItemSearchList.find(lComID);
		...
		if (lClsID != 0) {
			std::vector<SItemData>::iterator it;
...
BOOL CStoreSystem::Request(CCharacter* pCha, long lComID) {
	try {
		...
		auto pNm = new NetMessage();
		auto* pChaInfo = new RoleInfo();
...
BOOL CStoreSystem::RequestItemList(CCharacter* pCha, long lClsID, short sPage, short sNum) {
	try {
		std::map<long, std::vector<SItemData>>::iterator map_it = m_ItemList.find(lClsID);
		if (map_it != m_ItemList.end()) {
			...
			std::vector<SItemData>::iterator it = m_ItemList[lClsID].begin();
			int i;
...
BOOL CStoreSystem::SetItemList(void* pItemList, long lNum) {
	try {
		...
		int i;
		auto* pStore = (StoreInfo*)pItemList;
		...
		for (i = 0; i < lNum; i++) {
			...
			if (map_it != m_ItemList.end()) {
				...
				std::vector<SItemData> vecItem;
				vecItem.push_back(ItemNode);
				std::pair<long, std::vector<SItemData>> MapNode(lClsID, vecItem);
				m_ItemList.insert(MapNode);
			}

			std::pair<long, long> SearchNode(lComID, lClsID);
		...
		std::vector<ClassInfo>::iterator cls_it = m_ItemClass.begin();
		{
			while (cls_it != m_ItemClass.end()) {
				short sClsID = (*cls_it).clsID;
				std::map<long, std::vector<SItemData>>::iterator itemList_it = m_ItemList.find(sClsID);
				if (itemList_it != m_ItemList.end()) {
					std::vector<SItemData>::iterator item_it = m_ItemList[sClsID].begin();
...
BOOL CStoreSystem::SetItemClass(ClassInfo* pClassList, long lNum) {
	try {
		...
		std::vector<ClassInfo>::iterator it = m_ItemClass.begin();
...
//买单
BOOL CStoreSystem::RequestCrystalAdd(CCharacter* pCha, std::string act_tName, DWORD cha_tid, DWORD dwrplMoney, DWORD dcslPrice, DWORD iCslNum, DWORD dcslnum, int type, char quicktype, DWORD m_TAX) {
	try {
		pNetMessage pNm = new NetMessage();
...
//取消卖单
BOOL CStoreSystem::RequestCrystalAdd(std::string act_Name, DWORD cha_id, std::string cha_name, DWORD dwrplMoney, DWORD dcslPrice, DWORD iCslNum, DWORD dcslnum, DWORD m_TAX) {
	try {
		pNetMessage pNm = new NetMessage();
...
//根据玩家和交易者 修改相应水晶
BOOL CStoreSystem::RequestCrystalDel(CCharacter* pCha, std::string act_tName, DWORD cha_tid, DWORD dwrplMoney, DWORD iCslPrice, DWORD iCslNum, DWORD dcslnum, int type, char quicktype, DWORD m_TAX) {
	try {
		pNetMessage pNm = new NetMessage();
