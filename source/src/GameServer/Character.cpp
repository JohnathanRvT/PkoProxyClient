...
CCharacter::CCharacter()
	...
	try {
		...
		for (size_t i = 0; i < enumACTCONTROL_MAX; i++) {
			SetActControl(i);
		}
	}
	T_E
...
CCharacter::~CCharacter() {
	SAFE_DELETE(m_pCKitbagTmp);

#ifndef ALL_HAS_KITBAG
	SAFE_DELETE(m_pCKitbag);
#endif
}
...
bool CCharacter::IsPlayerMainCha() {
	return m_pCPlayer && (m_pCPlayer->GetMainCha() == this);
}

bool CCharacter::IsPlayerCtrlCha() {
...
inline bool CCharacter::IsPlayerFocusCha() {
...
void CCharacter::SwitchMap(SubMap* pCSrcMap, cChar* szTarMapName, Long lTarX, Long lTarY, bool bNeedOutSrcMap, Char chSwitchType, Long lTMapCpyNO) {
	try {
		...
		if (!strcmp(pCSrcMap->GetName(), szTarMapName)) { // 同地图间切换
			...
			WRITE_CMD(wpk, CMD_MT_SWITCHMAP);
			WRITE_STRING(wpk, pCSrcMap->GetName());
			WRITE_LONG(wpk, pCSrcMap->GetCopyNO());
			WRITE_LONG(wpk, GetShape().centre.x);
			WRITE_LONG(wpk, GetShape().centre.y);
			WRITE_STRING(wpk, szTarMapName);
			WRITE_LONG(wpk, lTMapCpyNO);
			WRITE_LONG(wpk, lTarX);
			WRITE_LONG(wpk, lTarY);
...
void CCharacter::ReflectINFof(Entity* srcent, WPACKET chginf) {
	try {
		...
		WRITE_ADDRESS(chginf, GetPlayer()->GetGateAddr());
...
void CCharacter::OnBeginSeen(CCharacter* pCCha) {
	try {
		...
		if (pEntity) {
			unsigned short usEventID = pEntity->GetInfoID();
...
void CCharacter::SetRelive(Char chType, Char chLv, const Char* szInfo) {
	...
	WRITE_LONG(pk, GetID());
	szInfo ? WRITE_STRING(pk, szInfo) : WRITE_STRING(pk, "");
...
SItemGrid* CCharacter::GetItem(Char chPosType, Long lItemID) {
	SItemGrid* pSItemCont{nullptr};

	if (chPosType == 1) {
		for (size_t i = enumEQUIP_HEAD; i < enumEQUIP_NUM; i++) {
...
bool CCharacter::SetKitbagItemValid(dbc::Short sPosID, bool bValid, bool bRecheckAttr, bool bSyn) {
	...
	if (sPosID == sEspeGridID) {
		...
		if (pItem->sType == EItemType::Pet) { // 宠物
			if (bSyn) {
				m_CChaAttr.ResetChangeFlag();
				SetBoatAttrChangeFlag(false);
			}
...
void CCharacter::SetEspeItemChangeFlag(bool bChange) {
	...
	if (pGrid) {
		...
		if (pItem && pItem->sType == EItemType::Pet) { // 宠物道具
			GetKitbag()->SetSingleChangeFlag(sEspeGridID);
		}
	}
}

Char CCharacter::GetLookChangeNum() {
	Char chNum = 0;
	for (size_t i = enumEQUIP_HEAD; i < enumEQUIP_NUM; i++) {
...
bool CCharacter::CheckForgeItem(SForgeItem* pSItem) {
	...
	for (size_t i = 0; i < defMAX_ITEM_FORGE_GROUP; i++) {
		for (size_t j = 0; j < pSItem->SGroup[i].sGridNum; j++) {
			SItemGrid* pSGridCont = GetKitbag()->GetGridContByID(pSItem->SGroup[i].SGrid[j].sGridID);
...
// 加载特殊道具
void CCharacter::CheckEspeItemGrid() {
	Short sEspeGridID = 1;
	SItemGrid* pGrid = GetKitbag()->GetGridContByID(sEspeGridID);
	if (pGrid) {
		CItemRecord* pItem = GetItemRecordInfo(pGrid->sID);
...
Short CCharacter::KbPushItem(bool bRecheckAttr, bool bSynAttr, SItemGrid* pGrid, Short& sPosID, Short sType, bool bCommit, bool bSureOpr) {
	...
	short sEspeGridID = 1;
	const bool b2HasItem = GetKitbag()->GetGridContByID(sEspeGridID) ? true : false;
	const short sPushRet = GetKitbag()->Push(pGrid, sPosID, sType, bCommit, bSureOpr);
...
Short CCharacter::KbPopItem(bool bRecheckAttr, bool bSynAttr, SItemGrid* pGrid, Short sPosID, Short sType, bool bCommit) {
	...
	short sEspeGridID = 1;
	const short sPushRet = GetKitbag()->Pop(pGrid, sPosID, sType, bCommit);
...
Short CCharacter::KbClearItem(bool bRecheckAttr, bool bSynAttr, Short sPosID, Short sType) {
	...
	if (sPosID == sEspeGridID) { // 特殊格
		...
		const short sRet = GetKitbag()->Clear(sPosID, sType);
...
Short CCharacter::KbRegroupItem(bool bRecheckAttr, bool bSynAttr, Short sSrcPosID, Short sSrcNum, Short sTarPosID, Short sType) {
	...
	if (sSrcPosID == sEspeGridID || sTarPosID == sEspeGridID) {
		...
		if (pSEspeGridOld) {
			SEspeGridOld = *pSEspeGridOld;
		}
		const short sRet = GetKitbag()->Regroup(sSrcPosID, sSrcNum, sTarPosID, sType);
...
void CCharacter::CheckBagItemValid(CKitbag* pCBag) {
	...
	SItemGrid* pGridCont;
	const short sUsedNum = pCBag->GetUseGridNum();
	for (size_t i = 0; i < sUsedNum; i++) {
		pGridCont = pCBag->GetGridContByNum(i);
		CheckItemValid(pGridCont);
	}
}

void CCharacter::CheckLookItemValid() {
	for (auto& item : m_SChaPart.SLink) {
		CheckItemValid(&item);
	}
}
...
bool CCharacter::String2KitbagTmpData(std::string& strData) {
	...
	m_pCKitbagTmp->Init(defMAX_KBITEM_NUM_PER_TYPE); //CHANGED: From default 16. Associate it with max items from reg inventory. Based on: https://pkodev.net/index.php?/topic/21-how-to-increase-temporary-bag-capacity/&page=1
...
bool CCharacter::DoForgeLikeScript(dbc::cChar* cszFunc, dbc::Long& lRet) {
	...
	const int nState = lua_handled_pcall(g_pLuaState, nParamNum, LUA_MULTRET, 0);
	if (nState != 0) {
...
bool CCharacter::DoLifeSkillcript(dbc::cChar* cszFunc, dbc::Long& lRet) {
	...
	for (size_t i = 0; i < pSItem->sbagCount; i++) {
...
bool CCharacter::DoTigerScript(dbc::cChar* cszFunc) {
	...
	if (strcmp(cszFunc, "TigerStart") == 0) {
		int nParamNum = 0;
		short sRet = 0;
		lua_getglobal(g_pLuaState, cszFunc);
		if (!lua_isfunction(g_pLuaState, -1)) { // 不是函数名
		...
		for (size_t i = 0; i < m_sTigerItemID.size(); i++) {
			...
			if (sRet <= 0) {
				...
				m_sTigerSel = {};
				LG("lua_err", "DoString %s\n", cszFunc);
				//lua_callalert(g_pLuaState, nState); //CHANGED: I don't think this is needed here
				lua_settop(g_pLuaState, 0);
				return false;
			}
		...
		lua_settop(g_pLuaState, 0);
	} else if (strcmp(cszFunc, "TigerStop") == 0) {
		int nParamNum = 0;
		lua_getglobal(g_pLuaState, cszFunc);
		if (!lua_isfunction(g_pLuaState, -1)) // 不是函数名
		{
		...
		for (const auto& i : m_sTigerItemID) {
			lua_pushnumber(g_pLuaState, i);
			nParamNum++;
		}
		for (const auto& i : m_sTigerSel) {
			lua_pushnumber(g_pLuaState, i);
			nParamNum++;
		}
		//CHANGED: better lua error handling (deguix)
...
void CCharacter::AfterStepMove() {
	...
	if (IsBoat()) {
		...
		if (strcmp(pMap->GetName(), "garner") == 0) {
		...
		} else if (strcmp(pMap->GetName(), "magicsea") == 0) {
			if (GetPos().x >= area.rbtm.x - clSwitchDist) {
				bSwitch = true;
		...
		} else if (strcmp(pMap->GetName(), "darkblue") == 0) {
...
void CCharacter::SubsequenceMove() {
	try {
		...
		if (GetMoveState() & enumMSTATE_CANCEL ||
			!(GetMoveState() & enumMSTATE_INRANGE)) {
			m_SMoveRedu.ulStartTick = GetTickCount();
		}
...
void CCharacter::Die() {
	...
	try {
		...
		m_chReliveLv = 0;
		if (IsPlayerOwnCha() &&
			pCMap && !pCMap->GetMapRes()->IsRepatriateDie()) {
		...
		if (GetPlayer() && (this == GetPlyMainCha() || IsBoat())) {
			...
			const BYTE byNumBoat = GetPlayer()->GetNumBoat();
			for (BYTE i = 0; i < byNumBoat; i++) {
				CCharacter* pBoat = GetPlayer()->GetBoat(i);
				if (pBoat) {
					if (pBoat->m_pTradeData) {
						const DWORD dwCharID = (pBoat == pBoat->m_pTradeData->pRequest) ? pBoat->m_pTradeData->pAccept->GetID() : pBoat->m_pTradeData->pRequest->GetID();
		...
		if (!IsPlayerCha() && pCMap) {
			if (InOutMapQueue()) {
				...
				Point SPos;
				if (!g_Config.m_bRandomizedRespawns || !m_DoesRespawnTerritoryExist) {
					SPos = m_STerritory.centre;
					do { // Remember, initial spawn is already valid, so no matter what this will eventually succeed quickly.
						SPos.move(rng.uniform(0, 360 - 1), m_STerritory.radius);
					} while (!pCMap->IsValidPos(SPos.x, SPos.y));
				} else {
					do {
						SPos = {
							rng.uniform(m_SRespawnTerritory.ltop.x, m_SRespawnTerritory.rbtm.x - 1),
							rng.uniform(m_SRespawnTerritory.ltop.y, m_SRespawnTerritory.rbtm.y - 1)};
					} while (!pCMap->IsValidPos(SPos.x, SPos.y));
				}
				m_STerritory.centre = SPos;
...
void CCharacter::MoveCity(cChar* szCityName, Long lMapCopyNO, Char chSwitchType) {
	...
	SBirthPoint* pSBirthP{nullptr};
	if (strcmp(szCityName, "") == 0) {
	...
	if (pSBirthP) {
		SwitchMap(GetSubMap(), pSBirthP->szMapName, (pSBirthP->x + 2 - rng.uniform(0, 4 - 1)) * 100, (pSBirthP->y + 2 - rng.uniform(0, 4 - 1)) * 100, true, chSwitchType, lMapCopyNO);
	} else {
...
void CCharacter::BackToCity(bool bDie, cChar* szCityName, Long lMapCpyNO, Char chSwitchType) {
	...
	if (!szCityName || strcmp(szCityName, "") == 0) {
		pCMainCha->ResetBirthInfo();
	} else {
		SBirthPoint* pSBirthP = GetRandBirthPoint(GetLogName(), szCityName);
...
BOOL CCharacter::ConvoyNpc(WORD wRoleID, BYTE byIndex, WORD wNpcCharID, BYTE byAiType) {
	...
	try {
		if (GetPlayer()) {
			Point pos;
			pos = GetPos();
			pos.x += rng.uniform(0, 100 - 1);
			pos.y += rng.uniform(0, 100 - 1);
			CCharacter* pNpc = this->GetSubMap()->ChaSpawn(wNpcCharID, enumCHACTRL_NPC, rng.uniform(0, 360 - 1), &pos, true);
			if (!pNpc) {
				return false;
			}
...
BOOL CCharacter::SafeSale(BYTE byIndex, WORD wCount, WORD& wItemID, DWORD& dwMoney) {
	try {
		MONITOR if (GetPlyMainCha()->GetKitbag()->IsPwdLocked()) {
		...
		if (GetKitbag()->GetNum(byIndex) < wCount) {
			//SystemNotice( "出售物品《%s》数量(%d)不足，总数(%d)!", pItem->szName, wCount, wItemID );
			//SystemNotice( RES_STRING(GM_CHARACTER_CPP_00008), pItem->szName, wCount, wItemID );
			char szData[128];
			CFormatParameter param(3);
			param.setString(0, pItem->szName);
			param.setDouble(1, wCount);
		...
		if (Grid.sEndure[0] == Grid.sEndure[1]) {
			tempdwPrice = (dwPrice >> 1);
		} else {
			tempdwPrice = (dwPrice >> 3);
		}

		wCount = wCountLimit(wCount, tempdwPrice);
		dwMoney = tempdwPrice * wCount;
		...
		//SystemNotice( "你出售了%d个《%s》物品，获得了(%d)金钱，总额(%d)!", wCount, pItem->szName, dwMoney, dwCharMoney );
		//SystemNotice( RES_STRING(GM_CHARACTER_CPP_00011), wCount, pItem->szName, dwMoney, dwCharMoney );
		char szData[128] = "";
		CFormatParameter param(4);
		param.setDouble(0, wCount);
		...
		SystemNotice(szData);
		char szLog[128] = "";
		//sprintf( szLog, "%d个%s", wCount, pItem->szName );
		_snprintf_s(szLog, sizeof(szLog), _TRUNCATE, "%d %s", wCount, pItem->szName);
		...
		//::lgtool_PrintItem(	GetPlyMainCha()->GetName(),	GetPlyMainCha()->m_ID,	wCount,	cir,	&Grid	);
...
//FIXED: Money integer wrapping exploit
WORD CCharacter::wCountLimit(WORD wCount, DWORD dwPrice) {
	try {
		WORD wCountLimitVar = USHRT_MAX;
		if (dwPrice > 0) { //takes care of division by 0
			DWORD dwCountLimit = ULONG_MAX / dwPrice;
			if (dwCountLimit < USHRT_MAX) {
				wCountLimitVar = (WORD)dwCountLimit; //handled DWORD -> WORD
			}
		}
		return (wCountLimitVar > wCount) ? wCount : wCountLimitVar; //if it's USHRT_MAX, this will just be wCount
	}
	T_E
}

BOOL CCharacter::SafeBuy(WORD wItemID, WORD wCount, BYTE byIndex, DWORD& dwMoney) {
	try {
		MONITOR if (GetPlyMainCha()->GetKitbag()->IsPwdLocked()) {
			//SystemNotice("道具栏已锁定!");
			SystemNotice(RES_STRING(GM_CHARACTER_CPP_00002));
		...
		wCount = wCountLimit(wCount, pItem->lPrice);
		__int64 n64Temp = (__int64)(pItem->lPrice) * wCount;
		dwMoney = pItem->lPrice * wCount;
		USHORT sSize = GetKitbag()->GetCapacity();
		if (byIndex >= sSize) {
		...
		if (dwCharMoney < dwMoney) {
			//SystemNotice( "你的金钱(%d)不够购买%d个物品《%s》!单价(%d)", dwCharMoney,wCount, pItem->szName, pItem->lPrice );
			//SystemNotice( RES_STRING(GM_CHARACTER_CPP_00018), dwCharMoney,wCount, pItem->szName, pItem->lPrice );
			char szData[128];
			CFormatParameter param(4);
	...
	try {
		if (m_pCPlayer) {
			...
			for (BYTE i = 0; i < byNumBoat; i++) {
				...
				if (pBoat && pBoat->GetID() == dwBoatID /*&& sBerthID == pBoat->getAttr( ATTR_BOAtry {ERTH )*/) {
...
BOOL CCharacter::SafeSaleGoods(DWORD dwBoatID, BYTE byIndex, WORD wCount, WORD& wItemID, DWORD& dwMoney) {
	try {
		MONITOR if (m_pCPlayer) {
			USHORT sBerthID, sxPos, syPos, sDir;
			m_pCPlayer->GetBerth(sBerthID, sxPos, syPos, sDir);
			...
			for (BYTE i = 0; i < byNumBoat; i++) {
				...
				if (pBoat && pBoat->GetID() == dwBoatID /* && sBerthID == pBoat->getAttr( ATTR_BOAtry {ERTH )*/) {
					...
					if (pBoat->GetKitbag()->GetNum(byIndex) < wCount) {
						//SystemNotice( "出售物品《%s》数量(%d)不足，总数(%d)!", pItem->szName, wCount, wItemID );
						//SystemNotice( RES_STRING(GM_CHARACTER_CPP_00008), pItem->szName, wCount, wItemID );
						char szData[128];
						CFormatParameter param(3);
					...
					if (pBoat->KbPopItem(true, false, &Grid, byIndex) != enumKBACT_SUCCESS) {
						...
						//LG( "trade_error", "提取角色%s船只%s船舱出售货物失败!ID[%d], Count[%d]", GetName(), pBoat->GetName(), wItemID, wCount );
						LG("trade_error", "distill character %s boat %s cabin sale goods failed!ID[%d], Count[%d]\n", GetName(), pBoat->GetName(), wItemID, wCount);
						return FALSE;
					}
					...
					wCount = wCountLimit(wCount, dwPrice);
					dwMoney = dwPrice * wCount;
					DWORD dwCharMoney = (long)getAttr(ATTR_GD);
					dwCharMoney += dwMoney;
...
BOOL CCharacter::SafeBuyGoods(DWORD dwBoatID, WORD wItemID, WORD wCount, BYTE byIndex, DWORD& dwMoney) {
	try {
		MONITOR if (m_pCPlayer) {
			USHORT sBerthID, sxPos, syPos, sDir;
			m_pCPlayer->GetBerth(sBerthID, sxPos, syPos, sDir);
			...
			for (BYTE i = 0; i < byNumBoat; i++) {
				...
				if (pBoat && pBoat->GetID() == dwBoatID /* && sBerthID == pBoat->getAttr( ATTR_BOAtry {ERTH )*/) {
					...
					DWORD dwCharMoney = (long)getAttr(ATTR_GD);
					if (dwCharMoney < dwMoney) {
						//SystemNotice( "你的金钱(%d)不够购买%d个物品《%s》!单价(%d)", dwCharMoney,wCount, pItem->szName, dwPrice );
						//SystemNotice( RES_STRING(GM_CHARACTER_CPP_00018), dwCharMoney,wCount, pItem->szName, dwPrice );
						char szData[128];
						CFormatParameter param(4);
						param.setLong(0, dwCharMoney);
						param.setDouble(1, wCount);
...
bool CCharacter::GiveItem2KitbagTemp(USHORT sItemID, USHORT sCount, ItemInfo* pItemAttr, BYTE bySoundType) {
	try {
		...
		if (!pItemAttr) {
			...
			for (size_t i = 0; i < defITEM_INSTANCE_ATTR_NUM; i++) {
			...
			const short sHole = static_cast<short>(ulForgeP / 1000000000);
		...
		short sPushPos = defKITBAG_DEFPUSH_POS;
		const short sPushRet = m_pCKitbagTmp->Push(&SGridCont, sPushPos);
		if (sPushRet == enumKBACT_ERROR_LOCK) { // 道具栏被锁定
			ItemOprateFailed(enumITEMOPT_ERROR_KBLOCK);
			return false;
		} else if (sPushRet == enumKBACT_ERROR_PUSHITEMID) { // 道具不存在
			...
			// 获得物品触发事件
			const unsigned short sNum = sCount - SGridCont.sNum;
			if (sNum > 0) {
				AfterPeekItem(sItemID, sNum);
			}
...
BOOL CCharacter::HasTradeItemLevel(BYTE byLevel) {
	...
	for (int i = 0; i < sNum; i++) {
		...
		if (pGridCont) {
			...
			if (pItem && pItem->sType == EItemType::Trade) {
...
// 删除状态
#ifdef COMPACT_MEM_2
bool CCharacter::DelSkillState(std::map<unsigned char, SSkillStateUnit>::iterator& pUnit, bool bNotice) {
	try {
		...
		if (pState) {
			...
			for (std::map<unsigned char, SSkillStateUnit>::iterator it = m_CSkillState.m_SStateMap.begin(); it != m_CSkillState.m_SStateMap.end(); it++) {
	...
	try {
		...
		case enumEVENTT_RANGE: {
			...
			if (usEventEType == enumEVENTE_SMAP_ENTRY) {
				...
				std::string strScript1 = "begin_enter_";
	...
	try {
		if (GetPlayer()) {
			...
			if (sLevel == 41) {
				...
				WPacket wpk = GETWPACKET();
				WRITE_CMD(wpk, CMD_MP_MASTER_FINISH);
...
#ifdef COMPACT_MEM
		for (std::map<short, SSkillGrid>::iterator it = pCSkillBag->m_SBagMap.begin(); it != pCSkillBag->m_SBagMap.end(); it++) {
	...
	if (Boat.OnBoatDie(Attacker)) {
		...
		for (int i = 0; i < sNumGird; i++) {
			...
			if (pGridCont) {
				...
				if (pItem->sType == EItemType::Boat && dwBoatID == pGridCont->GetDBParam(enumITEMDBP_INST_ID)) {
...
BOOL CCharacter::BoatSelected(BYTE byType, BYTE byIndex) {
	...
	if (byType == mission::BERTH_REPAIR_LIST) {
		...
		//CHANGED: better lua error handling (deguix)
		if (int nStatus = lua_handled_pcall(g_pLuaState, 3, 1, 0); nStatus != 0) {
...
BOOL CCharacter::PackBag(CCharacter& boat, BYTE byType, BYTE byLevel) {
	...
	//CHANGED: better lua error handling (deguix)
	int nStatus = lua_handled_pcall(g_pLuaState, 4, 1, 0);
	if (nStatus) {
...
dbc::cChar* CCharacter::GetGuildName() {
	if (GetPlayer())
...
dbc::cChar* CCharacter::GetValidGuildName() {
...
dbc::cChar* CCharacter::GetGuildMotto() {
	if (GetPlayer())
...
dbc::cChar* CCharacter::GetStallName() {
	if (GetPlayer())
...
dbc::cChar* CCharacter::GetValidGuildMotto() {
	if (GetPlayer() && GetGuildState() == emGldMembStatNormal)
		return GetPlayer()->m_szGuildMotto;
	else
		return "";
...
void CCharacter::SynKitbagCapacity() {
	WPACKET packet = GETWPACKET();
...
#define RAND_IN_NUM(x) (rng.uniform(0,x))
...
#ifdef COMPACT_MEM_2
	for (std::map<unsigned char, SSkillStateUnit>::iterator it = pSState->m_SStateMap.begin(); it != pSState->m_SStateMap.end(); it++) {
