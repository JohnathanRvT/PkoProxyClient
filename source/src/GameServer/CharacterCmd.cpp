...

			{
				...
				if (bNewCha) // 新角色
				{
					...
					if (pSBirthP) {
						...
						l_x = rng.uniform(pSBirthP->x + 2, pSBirthP->x + 2 + 4 - 1) * 100;
						l_y = rng.uniform(pSBirthP->y + 2, pSBirthP->y + 2 + 4 - 1) * 100;
					}
				} else {
					if (chLogin == 0) // 角色上线
					{
...

				{
					...
					for (size_t i = 0; i < m_SChaPart.SLink.size(); i++) {
...

				{
					...
					{
						CLevelRecord *pCLvRec = nullptr, *pNLvRec = nullptr;
...

				{
					...
					WRITE_ADDRESS(pkret, ToAddress(pCPlayer)); //告诉GateServer自己的Player描述结构地址
...
Short CCharacter::Cmd_UseEquipItem(Short sKbPage, Short sKbGrid, bool bRefresh) {
	try {
		...
		const short sItemId = pSEquipIt->sID;
		const short sItemNum = pSEquipIt->sNum;
		...
		//if( sItemId >= 5000 && pCItemRec->sType != EItemType::Boat && pSEquipIt->GetFusionItemID() )
		CItemRecord* pItem = GetItemRecordInfo(sItemId);
		if (CItemRecord::IsVaildFusionID(pItem) && pCItemRec->sType != EItemType::Boat && pSEquipIt->GetFusionItemID()) { // ning.yan  end
			const short sEquipCon = CanEquipItemNew(sItemId, (Short)pSEquipIt->GetFusionItemID());
			if (sEquipCon != enumITEMOPT_SUCCESS) {
				return sEquipCon;
			}
		} else {
			const short sEquipCon = CanEquipItem(sItemId);
			if (sEquipCon != enumITEMOPT_SUCCESS) {
				return sEquipCon;
			}
		}
		...
		Char chEquipPos = -1;
		for (size_t i = 0; i < pCItemRec->szAbleLink.size(); i++) {
		...
		if (pCItemRec->szNeedLink[0] == -1) {
			...
			for (auto chNeedL : pCItemRec->szNeedLink) {
...
Short CCharacter::Cmd_UseExpendItem(Short sKbPage, Short sKbGrid, Short sTarKbPage, Short sTarKbGrid, bool bRefresh) {
	try {
		...
		if (pCItemRec->sType == EItemType::Scroll) {
...
Short CCharacter::Cmd_UnfixItem(Char chLinkID, Short* psItemNum, Char chDir, Long lParam1, Long lParam2, bool bPriority, bool bRefresh, bool bForcible) {
	try {
		...
		const bool bOptKb = chDir == 1 ? true : false;
		if (bRefresh) {
			m_CChaAttr.ResetChangeFlag();
			SetBoatAttrChangeFlag(false);
			m_CSkillState.ResetSkillStateChangeFlag();
...
Short CCharacter::Cmd_PickupItem(uLong ulID, Long lHandle) {
	try {
		...
		const unsigned short usAreaAttr = pCMap->GetAreaAttr(pCEnt->GetPos());
		if ((g_IsLand(usAreaAttr) != g_IsLand(GetAreaAttr())) &&
			(g_IsLand(usAreaAttr))) { //角色所在的区域不同于道具
		...
		const short sPushRet = pKitbagCha->KbPushItem(true, true, &pCItem->m_SGridContent, sPushPos);
		...
		// 给角色添加物品携带的船只
		if (pItem->sType == EItemType::Boat) {
			const DWORD dwBoatID = pCItem->m_SGridContent.GetDBParam(enumITEMDBP_INST_ID);
			// 保存数据库操作
			if (SaveAssets()) {
...
Short CCharacter::Cmd_ThrowItem(Short sKbPage, Short sKbGrid, Short* psThrowNum, Long lPosX, Long lPosY, bool bRefresh, bool bForcible) {
	try {
		...
		const unsigned short sItemID = GetKitbag()->GetID(sKbGrid, sKbPage);
		CItemRecord* pItem = GetItemRecordInfo(sItemID);
		if (!pItem) {
			return enumITEMOPT_ERROR_NONE;
		}
		...
		// 判断丢弃船长证明
		if (pItem->sType == EItemType::Boat) {
			const DWORD dwBoatID = GetKitbag()->GetDBParam(enumITEMDBP_INST_ID, sKbGrid);
		...
		const unsigned short usAreaAttr = pCMap->GetAreaAttr(lPosX, lPosY);
		...
		if (*psThrowNum != 0) {
			const short sCurNum = GetKitbag()->GetNum(sKbGrid, sKbPage);
			if (sCurNum <= 0) {
				return enumITEMOPT_ERROR_NONE;
			}
		...
		const short sRet = KbPopItem(bRefresh, bRefresh, &GridCont, sKbGrid, sKbPage); // 弹出操作不成功
...
Short CCharacter::Cmd_ItemSwitchPos(Short sKbPage, Short sSrcGrid, Short sSrcNum, Short sTarGrid) {
	try {
		...
		GetKitbag()->SetChangeFlag(false, sKbPage);
		const short sKbOptRet = KbRegroupItem(true, true, sSrcGrid, sSrcNum, sTarGrid);
		if (sKbOptRet == enumKBACT_SUCCESS ||
			sKbOptRet == enumKBACT_ERROR_FULL) { // 换位成功，通知主角道具栏更新
			SynKitbagNew(enumSYN_KITBAG_SWITCH);
...
Short CCharacter::Cmd_DelItem(Short sKbPage, Short sKbGrid, dbc::Short* psThrowNum, bool bRefresh, bool bForcible) {
	try {
		...
		const unsigned short sItemID = GetKitbag()->GetID(sKbGrid, sKbPage);
		const unsigned short sItemNum = GetKitbag()->GetNum(sKbGrid, sKbPage);
		// 判断是否船长证明道具
		CItemRecord* pItem = GetItemRecordInfo(sItemID);
		if (!pItem) {
			return enumITEMOPT_ERROR_NONE;
...
*/
		//if(pItem->sType == EItemType::Mission)
		//	return enumITEMOPT_ERROR_UNTHROW;
...
Short CCharacter::Cmd_BankOper(EKitBagType sourceBagType, dbc::Short sSrcGridID, dbc::Short sSrcNum, EKitBagType targetBagType, dbc::Short sTarGridID) {
	try {
		CKitbag *pCSrcBag{nullptr}, *pCTarBag{nullptr};
		CCharacter* pCMainCha = GetPlyMainCha();
		if (sourceBagType == EKitBagType::player_or_boat) {
			pCSrcBag = pCMainCha->GetKitbag();
		} else {

			pCSrcBag = GetPlayer()->GetBank();
		}
		if (sourceBagType == targetBagType) {
			pCTarBag = pCSrcBag;
		} else {
			if (targetBagType == EKitBagType::player_or_boat) {
		...
		if (pCSrcBag == pCTarBag) { // 交换位置
			...
			const short sSrcItemID = pCSrcBag->GetID(sSrcGridID);

			CItemRecord* pItem = GetItemRecordInfo(sSrcItemID);
			if (!pItem) {
				return enumITEMOPT_ERROR_NONE;
			}
			if (sourceBagType == EKitBagType::player_or_boat && targetBagType == EKitBagType::bank) { // 从道具栏到银行
				if (pItem->sType == EItemType::Boat ||
					pItem->sType == EItemType::Trade ||
					pItem->sType == EItemType::Bravery) { // 船长证，贸易证，勇者之证
			...
			param.setString(0, sourceBagType == EKitBagType::player_or_boat ? RES_STRING(GM_CHARACTERCMD_CPP_00013) : RES_STRING(GM_CHARACTERCMD_CPP_00014));
			param.setString(1, targetBagType == EKitBagType::player_or_boat ? RES_STRING(GM_CHARACTERCMD_CPP_00013) : RES_STRING(GM_CHARACTERCMD_CPP_00014));
...
void CCharacter::Cmd_ReassignAttr(RPACKET& pk) {
	try {
		...
		for (size_t i = ATTR_BSTR; i <= ATTR_BLUK; i++) {
	...
	try {
		...
		if (chFromType == 2 || chFromType == 0) { // 从道具栏移除
			...
			if (sFromID >= 0 && sFromID < GetKitbag()->GetCapacity()) {
				...
				const short sUseGNum = GetKitbag()->GetUseGridNum();
				Short sOptNum;
				Short sPosID;
				if (sUseGNum > 0) {
...
void CCharacter::Cmd_FightAsk(dbc::Char chType, dbc::Long lTarID, dbc::Long lTarHandle) {
	try {
		...
		std::string strScript = "check_can_enter_";
...
void CCharacter::Cmd_FightAnswer(bool bFight) {
	try {
		...
		const long lID = pCPly->GetChallengeParam(2);
		const long lHandle = pCPly->GetChallengeParam(3);
		char lFightType = pCPly->GetChallengeType();

		CPlayer *pCSrcPly, *pCTarPly;
		pCSrcPly = g_pGameApp->IsValidPlayer(lID, lHandle);
		if (!pCSrcPly || !pCSrcPly->HasChallengeObj()) {
			LG("teamPK_flow", "this request is invalid1!\n");
			SystemNotice(RES_STRING(GM_CHARACTERCMD_CPP_00031));
			return;
		}
		const char chObjNum = (Char)pCSrcPly->GetChallengeParam(0);
		...
		if (!pCTarPly || !pCTarPly->HasChallengeObj()) {
			LG("teamPK_flow", "this request is invalid2!\n");
			SystemNotice(RES_STRING(GM_CHARACTERCMD_CPP_00031));
			return;
		}
		...
		if (!pCTeamFightEntry) { // 没有队伍PK地图
			const std::string strNoti = RES_STRING(GM_CHARACTERCMD_CPP_00034);
			WPACKET WtPk = GETWPACKET();
			WRITE_CMD(WtPk, CMD_MC_SYSINFO);
			WRITE_SEQ(WtPk, strNoti.c_str(), (Short)strNoti.length() + 1);
			pCSrcPly->GetCtrlCha()->ReflectINFof(this, WtPk);
		...
		if (!(pCMCpyCell = pCTeamFightEntry->AddCopy(&CMCpyCell))) {
			const std::string strNoti = RES_STRING(GM_CHARACTERCMD_CPP_00035);
			WPACKET WtPk = GETWPACKET();
			WRITE_CMD(WtPk, CMD_MC_SYSINFO);
			WRITE_SEQ(WtPk, strNoti.c_str(), (Short)strNoti.length() + 1);
			ReflectINFof(this, WtPk);
		...
		if (!pCMCpyCell->HasFreePlyCount((Short)pCSrcPly->GetChallengeParam(0))) { // 数量不足
			...
			const std::string strNoti = RES_STRING(GM_CHARACTERCMD_CPP_00036);
			WPACKET WtPk = GETWPACKET();
			WRITE_CMD(WtPk, CMD_MC_SYSINFO);
			WRITE_SEQ(WtPk, strNoti.c_str(), (Short)strNoti.length() + 1);
			ReflectINFof(this, WtPk);
		...
		std::string strScript1 = "after_get_map_copy_";
		...
		std::string strScript = "begin_enter_";
		strScript += pCTeamFightEntry->GetTMapName();
		std::string strScript2 = "check_can_enter_";
...
void CCharacter::Cmd_ItemForgeAsk(dbc::Char chType, SForgeItem* pSItem) {
	try {
		...
		pCPly->SetForgeInfo(chType, pSItem);
		{
			...
			if (chType == 4) {
				for (size_t i = 0; i < defMAX_ITEM_FORGE_GROUP; i++) {
			...
			const char* szCheckCanScript{nullptr};
			const char* szGetMoneyScript{nullptr};
			switch (chType) {
			case 1: { // 精炼
	...
	try {
		...
		}
		{
			...
			}
			{
				...
				for (size_t i = 1; i < defMAX_ITEM_LOTTERY_GROUP; i++) {
					SItemGrid* pItemGrid = this->GetKitbag()->GetGridContByID(pSItem->SGroup[i].SGrid[0].sGridID);
					if (!pItemGrid) {
						goto EndItemLotteryAsk;
					}
				...
				if (pItemLottery->sID = 5828) {
					const int issue = this->GetLotteryIssue();
...
void CCharacter::Cmd_ItemAmphitheaterAsk(int ReID) {
	...
	if (GetKitbag()->HasItem(ID)) {
		...
		SItemGrid* pItemGrid = this->GetKitbag()->GetGridContByID(8);
...
void CCharacter::Cmd_ItemAmphitheaterAnswer(bool bUpdate) {
	SynKitbagNew(enumSYN_KITBAG_EQUIP);
	std::vector<std::vector<std::string>> dataPromotion;
	...
	if (flag) {
		...
		for (auto& i : dataPromotion) {
			// cha_name, id，winnum
			WRITE_STRING(packet, i[0].c_str());
			WRITE_CHAR(packet, atoi(i[1].c_str()));
			WRITE_LONG(packet, atoi(i[2].c_str()));
		}

		WRITE_CHAR(packet, (uChar)dataRelive.size());
		for (auto& i : dataRelive) {
			// A.id, A.captain, B.cha_name, A.relivenum
			//WRITE_CHAR( packet,atoi(dataRelive[i][0].c_str()));
			//cha_name,relivenum
			WRITE_STRING(packet, i[0].c_str());
			WRITE_LONG(packet, atoi(i[1].c_str()));
			WRITE_LONG(packet, atoi(i[2].c_str()));
		}
	} else {
		WRITE_CHAR(packet, 0);
...
//End
void CCharacter::Cmd_LifeSkillItemAsk(ELifeSkillType type, SLifeSkillItem* pSItem) {
	try {
		if (GetKitbag()->IsPwdLocked()) {
		...
		std::string& strLifeSkillinfo = GetPlayer()->GetLifeSkillinfo();
		...
		pCPly->SetLifeSkillInfo(type, pSItem);
		...
		default: {
			SystemNotice(RES_STRING(GM_CHARACTERCMD_CPP_00055), static_cast<int>(type));
			goto EndItemForgeAsk;
			break;
		}
		}
	...
	EndItemForgeAsk:
		...
		WRITE_CHAR(WtPk, static_cast<unsigned char>(type));
		WRITE_SHORT(WtPk, (short)lOptMoney);
		if (type == ELifeSkillType::analyze) {
			std::string strVer[2];
...
void CCharacter::Cmd_LifeSkillItemAsR(ELifeSkillType type, SLifeSkillItem* pSItem) {
	try {
		if (GetKitbag()->IsPwdLocked()) {
			SystemNotice(RES_STRING(GM_CHARACTERCMD_CPP_00057));
			return;
		...
		const char* cszFunc = [type]() {
			switch (type) {
		case ELifeSkillType::manufacturing:
			return "begin_manufacture_item";
		case ELifeSkillType::analyze:
			return "begin_manufacture3_item";
		case ELifeSkillType::crafting: 
			return "begin_manufacture1_item";
		case ELifeSkillType::cooking: 
			return "begin_manufacture2_item";
		} }();
		...
		int nState = lua_handled_pcall(g_pLuaState, nParamNum, LUA_MULTRET, 0);
		if (nState != 0) {
		...
		WRITE_CMD(wpk, CMD_MC_LIFESKILL_ASR);
		WRITE_CHAR(wpk, static_cast<unsigned char>(type));
		WRITE_SHORT(wpk, stime);

		if (ELifeSkillType::analyze == type) {
			std::string strVer[2];
			Util_ResolveTextLine(cszContent, strVer, 2, ',');
			WRITE_STRING(wpk, strVer[0].c_str());
		} else
			WRITE_STRING(wpk, "");
...
void CCharacter::Cmd_SelectTimeExperience_Ask(char cIsGet, short sSeltime, short sGridPos, short sNum) {
	try {
		...
		if (cIsGet == '0') {
			...
			std::string check = g_CParser.GetReturnString(0);
			if (check == "1") {
				std::string BaseExp = g_CParser.GetReturnString(1);			  //基础经验
				std::string AddExp = g_CParser.GetReturnString(2);			  //增幅经验
				std::string AllExp = g_CParser.GetReturnString(3);			  //总经验
				std::string Lev = g_CParser.GetReturnString(4);				  //等级
				std::string Percentage = g_CParser.GetReturnString(5);		  //比例
				std::string AmplitudeMultiple = g_CParser.GetReturnString(6); //增幅器倍数
				std::string AmplitudeSum = g_CParser.GetReturnString(7);	  //增幅器使用个数
...
void CCharacter::Cmd_CrystalBuyAndSaleList() {
	try {
		...
		std::vector<std::vector<std::string>> dataBuyTrade;
		std::vector<std::vector<std::string>> dataSaleTrade;
		...
		if (flag) {
			...
			//玩家挂单
			std::vector<std::vector<std::string>> chaTradeData;
			...
			//挂买单和卖单的个数
			if (!game_db.GetHangNum(CrystalTradeType::enumbuy, m_buyNum) || !game_db.GetHangNum(CrystalTradeType::enumsale, m_saleNum)) {
				return;
			}
...
BOOL CCharacter::Cmd_ChangeFlatAfterTrade(DWORD iCslNum, DWORD dcslnum, int type, char* act_name, DWORD cha_id) {
	try {
		...
		if (type == CrystalTradeType::enumbuy) {
			//改挂单
			if (iCslNum < dcslnum) {
				if (game_db.UpdateHangTrade((char*)this->GetPlayer()->GetActName(), this->GetID(), dtradeMoney, CrystalTradeType::enumbuy, CrystalTradeType::enumend) && game_db.UpdateHangTrade(act_name, cha_id, dtradeMoney, CrystalTradeType::enumsale, CrystalTradeType::enumhang))
					return true;
				...
				if (game_db.UpdateHangTrade((char*)this->GetPlayer()->GetActName(), this->GetID(), dtradeMoney, CrystalTradeType::enumbuy, CrystalTradeType::enumend) && game_db.UpdateHangTrade(act_name, cha_id, dtradeMoney, CrystalTradeType::enumsale, CrystalTradeType::enumend))
					return true;
			}
		}
		//添加卖单
			...
			if (iCslNum < dcslnum) {
				if (game_db.UpdateHangTrade((char*)this->GetPlayer()->GetActName(), this->GetID(), dtradeMoney, CrystalTradeType::enumsale, CrystalTradeType::enumend) && game_db.UpdateHangTrade(act_name, cha_id, dtradeMoney, CrystalTradeType::enumbuy, CrystalTradeType::enumhang))
					return true;
			} else if (iCslNum > dcslnum) {
				...
				if (game_db.UpdateHangTrade((char*)this->GetPlayer()->GetActName(), this->GetID(), dtradeMoney, CrystalTradeType::enumsale, CrystalTradeType::enumend) && game_db.UpdateHangTrade(act_name, cha_id, dtradeMoney, CrystalTradeType::enumbuy, CrystalTradeType::enumend))
					return true;
			}
		}
		return false;
...
BOOL CCharacter::Cmd_ChangeFlatForTradeFailed(DWORD iCslNum, DWORD dcslnum, int type, char* act_name, DWORD cha_id) {
	try {
		...
		if (type == CrystalTradeType::enumbuy) {
			//改挂单
			if (iCslNum < dcslnum) {
				if (game_db.UpdateHangTradeForFailed((char*)this->GetPlayer()->GetActName(), this->GetID(), dtradeMoney, CrystalTradeType::enumbuy, CrystalTradeType::enumend) && game_db.UpdateHangTradeForFailed(act_name, cha_id, dtradeMoney, CrystalTradeType::enumsale, CrystalTradeType::enumhang))
					ret = true;
			} else if (iCslNum > dcslnum) {
				...
				if (game_db.UpdateHangTradeForFailed((char*)this->GetPlayer()->GetActName(), this->GetID(), dtradeMoney, CrystalTradeType::enumbuy, CrystalTradeType::enumend) && game_db.UpdateHangTradeForFailed(act_name, cha_id, dtradeMoney, CrystalTradeType::enumsale, CrystalTradeType::enumend))
					ret = true;
			}
		} else if (type == CrystalTradeType::enumsale) {
			if (iCslNum < dcslnum) {
				if (game_db.UpdateHangTradeForFailed((char*)this->GetPlayer()->GetActName(), this->GetID(), dtradeMoney, CrystalTradeType::enumsale, CrystalTradeType::enumend) && game_db.UpdateHangTradeForFailed(act_name, cha_id, dtradeMoney, CrystalTradeType::enumbuy, CrystalTradeType::enumhang))
					ret = true;
			} else if (iCslNum > dcslnum) {
				...
				if (game_db.UpdateHangTradeForFailed((char*)this->GetPlayer()->GetActName(), this->GetID(), dtradeMoney, CrystalTradeType::enumsale, CrystalTradeType::enumend) && game_db.UpdateHangTradeForFailed(act_name, cha_id, dtradeMoney, CrystalTradeType::enumbuy, CrystalTradeType::enumend))
					ret = true;
			}
		} else
			return false;
...
void CCharacter::Cmd_CheckHasAccordForTrade(DWORD iCslNum, DWORD iCslPrice, int type, char quicktype) {
	try {
		...
		std::string act_tName; //交易玩家的账号名称
		//如果是买单，查看有没有符合条件卖单
		if (type == CrystalTradeType::enumbuy) {
				...
				BOOL Cmd_ChangeFlatForTradeFailed(DWORD iCslNum, DWORD dcslnum, int type, char* act_name, DWORD cha_id, char* actName, DWORD chaID) {
					try {
						...
						if (type == CrystalTradeType::enumbuy) {
							//改挂单
							if (iCslNum < dcslnum) {
								if (game_db.UpdateHangTradeForFailed(actName, chaID, dtradeMoney, CrystalTradeType::enumbuy, CrystalTradeType::enumend) && game_db.UpdateHangTradeForFailed(act_name, cha_id, dtradeMoney, CrystalTradeType::enumsale, CrystalTradeType::enumhang))
									ret = true;
							} else if (iCslNum > dcslnum) {
								//if( game_db.UpdateHangTradeForFailed( (char*)this->GetPlayer()->GetActName(), dtradeMoney, CrystalTradeType::enumbuy, CrystalTradeType::enumhang )
								//	&& game_db.UpdateHangTradeForFailed( act_name, dtradeMoney, CrystalTradeType::enumsale, CrystalTradeType::enumend ))
								...
								if (game_db.UpdateHangTradeForFailed(actName, chaID, dtradeMoney, CrystalTradeType::enumbuy, CrystalTradeType::enumend) && game_db.UpdateHangTradeForFailed(act_name, cha_id, dtradeMoney, CrystalTradeType::enumsale, CrystalTradeType::enumend))
									ret = true;
							}
						} else if (type == CrystalTradeType::enumsale) {
							if (iCslNum < dcslnum) {
								if (game_db.UpdateHangTradeForFailed(actName, chaID, dtradeMoney, CrystalTradeType::enumsale, CrystalTradeType::enumend) && game_db.UpdateHangTradeForFailed(act_name, cha_id, dtradeMoney, CrystalTradeType::enumbuy, CrystalTradeType::enumhang))
									ret = true;
							} else if (iCslNum > dcslnum) {
								//if( game_db.UpdateHangTradeForFailed( (char*)this->GetPlayer()->GetActName(), dtradeMoney, CrystalTradeType::enumsale, CrystalTradeType::enumhang )
								//&& game_db.UpdateHangTradeForFailed( act_name, dtradeMoney, CrystalTradeType::enumbuy, CrystalTradeType::enumend ))
								...
								if (game_db.UpdateHangTradeForFailed(actName, chaID, dtradeMoney, CrystalTradeType::enumsale, CrystalTradeType::enumend) && game_db.UpdateHangTradeForFailed(act_name, cha_id, dtradeMoney, CrystalTradeType::enumbuy, CrystalTradeType::enumend))
									ret = true;
							}
						} else
							return false;
