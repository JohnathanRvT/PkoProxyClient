...
void CCharacter::WriteSkillbag(WPACKET& pk, int nSynType) {
	SSkillGrid* pSkillGrid = nullptr;
	CSkillTempData* pSkillTData = nullptr;
...
#ifdef COMPACT_MEM
	for (std::map<short, SSkillGrid>::iterator it = pCCtrlCha->m_CSkillBag.m_ChangeMap.begin(); it != pCCtrlCha->m_CSkillBag.m_ChangeMap.end(); it++) {
...
void CCharacter::WriteLookData(WPACKET& WtPk, Char chLookType, Char chSynType) {
	...
	if (m_CChaAttr.GetAttr(ATTR_CHATYPE) == enumCHACTRL_PLAYER && IsBoat()) {
		...
		if (chLookType == LOOK_TEAM)
			nItemCnt = 3; // 组队类型的外观通知, 只需要上半身信息

#ifdef TEAM_BUFF
		WRITE_SHORT(WtPk, nItemCnt); //Add by sunny.sun 20090810 和客户端统一
#endif
		for (size_t i = 0; i < nItemCnt; i++) {
