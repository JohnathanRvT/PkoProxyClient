...
void CCharacter::DoCommand(cChar* cszCommand, uLong ulLen) {
	try {
		...
		// 检查执行GM指令
		DoGMCommand(szComHead, szComParam)
			? LG("DoCommand", "[execution succeed]%s:%s\n", GetLogName(), strPrint.c_str())
			: LG("DoCommand", "[execution failed]%s:%s\n", GetLogName(), strPrint.c_str());
	}
	T_E
}
...
BOOL CCharacter::DoGMCommand(const char* pszCmd, const char* pszParam) {
	...
	try {
		...
		if (!PrivilegeCheck::Instance()->IsAdmin(this->m_pCPlayer->GetActName())) {
			...
			for (std::vector<int>::iterator it = g_Config.m_vGMCmd.begin(); it != g_Config.m_vGMCmd.end(); it++) {
				if (TM.tm_wday == *it) {
					gmOK = true;
					break;
				}
		...
		} else if (!strcmp(szComHead, "pet")) // 召唤宠物
		{
			...
			Point Pos = GetPos();
			Pos.move(rng.uniform(0, 360 - 1), 3 * 100);
			CCharacter* pCha = m_submap->ChaSpawn(lChaInfoID, enumCHACTRL_PLAYER_PET, rng.uniform(0, 360 - 1), &Pos);
			if (pCha) {
				pCha->m_HostCha = this;
		...
		} else if (!strcmp(szComHead, "summon")) // 召唤怪物
		{
			...
			if (n >= 1) {
				...
				Point Pos = GetPos();
				Pos.move(rng.uniform(0, 360 - 1), 3 * 100);
				CCharacter* pCha = m_submap->ChaSpawn(lChaInfoID, enumCHACTRL_NONE, rng.uniform(0, 360 - 1), &Pos);
				if (pCha) {
					if (n >= 2) {
		...
		} else if (!strcmp(szComHead, "summonex")) // 召唤怪物扩展指令，格式:角色编号，个数，是否激活视野，ＡＩ类型
		{
			...
			if (n >= 1) {
				...
				for (long i = 0; i < lChaNum; i++) {
					Pos = GetPos();
					Pos.move(rng.uniform(0, 360 - 1), rng.uniform(0, 20 - 1) * 100);
					pCha = m_submap->ChaSpawn(lChaInfoID, enumCHACTRL_NONE, rng.uniform(0, 360 - 1), &Pos, bActEyeshot);
					if (pCha) {
						if (n > 3)
							pCha->m_AIType = (BYTE)nAIType; // 设置怪物的AI类型
					} else {
		...
		} else if (!strcmp(szComHead, "kill")) // 杀死召唤怪物，指令格式:怪物名称，范围（米，默认值8米），个数（默认为范围内的所有怪物）
		{
			...
			if (n >= 1) {
				...
				CCharacter *pCCha = nullptr, *pCFreeCha = nullptr;
		...
		{
			luaL_handled_dostring(g_pLuaState, szComParam); //CHANGED: better lua error handling (deguix)
			LG("ServerRunLog", "ChaID: %i, ChaName: %s, CMD: %s, Param: %s\n", GetPlayer()->GetID(), GetName(), pszCmd, pszParam);
			return TRUE;
...
void CCharacter::DoCommand_CheckStatus(cChar* pszCommand, uLong ulLen) {
	try {
		...
		std::string strCmd = szComHead;
		std::string strParam = szComParam;
