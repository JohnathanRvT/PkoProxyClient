...
Entity::Entity() : m_cat(0), m_ID(0) {
	try {
		m_pCStateCellHead = nullptr;
		m_pCStateCellTail = nullptr;
		m_submap = nullptr;
		m_sAngle = 0;
		memset(&m_STerritory, 0, sizeof(Circle));
		memset(&m_SRespawnTerritory, 0, sizeof(Rect));
...
void Entity::Initially() {
	try {
		...
		m_pCStateCellHead = nullptr;
		m_pCStateCellTail = nullptr;
		m_pCEyeshotCellNext = nullptr;
		m_pCEyeshotCellLast = nullptr;
		SetBirthCity("");
		SetBirthMap("");
		m_sAngle = 0;
		m_CEvent.Init();
...
dbc::Short Entity::GetEyeshotWidth() {
	if (!GetSubMap())
		return 0;
