...
Entity* CEntityAlloc::GetEntity(long lID) {
	try {
		const long lType = lID & 0xff000000;
		const long lEntiID = lID & 0x00ffffff;

		switch (lType) {
		case defENTI_ALLOC_TYPE_CHA: {
			return m_ChaAlloc.getinfo(lEntiID);
		} break;
		case defENTI_ALLOC_TYPE_ITEM: {
			return m_ItemAlloc.getinfo(lEntiID);
		} break;
		case defENTI_ALLOC_TYPE_TNPC: {
			return m_TalkNpcAlloc.getinfo(lEntiID);
		} break;

		case defENTI_ALLOC_TYPE_ENTBERTH: {
			return m_BerthAlloc.getinfo(lEntiID);
		} break;
		case defENTI_ALLOC_TYPE_ENTRESOURCE: {
