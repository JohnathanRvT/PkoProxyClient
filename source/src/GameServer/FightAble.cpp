			...
			for (int i = 0; i < ATTR_CLIENT_MAX; i++) {
				if (m_CChaAttr.GetChangeBitFlag(i)) // 如果该属性有变，则压包
				{
					if (sTempChangeNum >= sAttrChangeNum)
						...
						MessageBox(nullptr, RES_STRING(GM_FIGHTALBE_CPP_00001), RES_STRING(GM_FIGHTALBE_CPP_00002), MB_OK);
					WRITE_CHAR(pk, i);
...
#ifdef COMPACT_MEM_2
		for (std::map<unsigned char, SSkillStateUnit>::iterator it = pSFireSrc->pCFightSrc->m_CSkillState.m_SStateMap.begin(); it != pSFireSrc->pCFightSrc->m_CSkillState.m_SStateMap.end(); it++) {
			pSStateUnit = &it->second;
#else
...
void CFightAble::SynSkillStateToUnit(CFightAble* pCObj) {
	try {
		...
		if (m_CLog.GetEnable()) {
			...
			SSkillStateUnit* pSStateUnit;
#ifdef COMPACT_MEM_2
			for (std::map<unsigned char, SSkillStateUnit>::iterator it = pCObj->m_CSkillState.m_SStateMap.begin(); it != pCObj->m_CSkillState.m_SStateMap.end(); it++) {
				pSStateUnit = &it->second;
#else
...
void CFightAble::WriteLookEnergy(WPACKET& pk) {
	for (auto& item : IsCharacter()->m_SChaPart.SLink) {
		if (!g_IsRealItemID(item.sID)) {
			WRITE_SHORT(pk, 0);
		} else {
			WRITE_SHORT(pk, item.sEnergy[0]);
		}
	}
}
...
bool CFightAble::SkillExpend(Short sExecTime) {
	...
	if (sNeedEnergy > 0) {
		for (size_t i = 0; i < defSKILL_ITEM_NEED_NUM; i++) {
			if (m_SFightInit.pCSkillRecord->sConchNeed[i][0] == cchSkillRecordKeyValue) {
				break;
			}

			SItemGrid* pGrid = &pCMainCha->m_SChaPart.SLink[m_SFightInit.pCSkillRecord->sConchNeed[i][0]];
...
bool CFightAble::SkillGeneral(Long lDist, Short sExecTime) // 普通技能
{
	...
	} else if (m_SFightInit.chTarType == 1) // 对象是ID
	{
									  ...
									  m_SFightInit.pCSkillRecord->chApplyTarget, m_SFightInit.pCSkillRecord->chTarType, m_SFightInit.pCSkillRecord->chHelpful)) // 目标不合法
		{
			...
			NotiSkillSrcToEyeshot();
			m_SFightInit.chTarType = 0;
			IsCharacter()->m_AITarget = nullptr;
...
void CFightAble::SpawnResource(CCharacter* pCAtk, dbc::Long lSkillLv) {
	try {
		...
		//CHANGED: better lua error handling (deguix) - extra note which won't be repeated as follows:
		//comes with lua_settop(g_pLuaState, 0); on nStatus != 0
		int nStatus = lua_handled_pcall(g_pLuaState, 4 + i, 0, 0);
		if (nStatus) {
...
bool CFightAble::GetTrowItemPos(Long* plPosX, Long* plPosY) {
	...
	Pos.move(rng.uniform(0, 360 - 1), 150);
...
void CFightAble::ItemCount(CCharacter* pAtk) {
	try {
		...
		{
			//TODO: Uncomment this section (not tested boats yet)
		...
		int nState = lua_handled_pcall(g_pLuaState, 2 + lItemNum, LUA_MULTRET, 0);
		if (nState != 0) {
		...
		//CHANGED: better lua error handling (deguix)
		nState = lua_handled_pcall(g_pLuaState, 2 + lItemNum, LUA_MULTRET, 0);
		if (nState != 0) {
			LG("lua_err", "DoString %s\n", szItemScript);
			return;
		}
...
void CFightAble::OnSkillState(DWORD dwCurTick) {
	try {
		...
		if (m_CSkillState.GetStateNum() > 0) {
			...
			m_CChaAttr.ResetChangeFlag();
			m_CSkillState.ResetSkillStateChangeFlag();
			CSkillStateRecord* pCSStateRec = nullptr;
			SSkillStateUnit* pSStateUnit;
			Short sExecTime = 0;
			Long lOldHP;
			bool bIsDie;
#ifdef COMPACT_MEM_2
			for (std::map<unsigned char, SSkillStateUnit>::iterator it = m_CSkillState.m_SStateMap.begin(); it != m_CSkillState.m_SStateMap.end();) {
				pSStateUnit = &it->second;
#else
			m_CSkillState.BeginGetState();
			while (pSStateUnit = m_CSkillState.GetNextState()) {
