...
const auto szChaInfoName = "CharacterInfo";
const auto szSkillInfoName = "skillinfo";
const auto szSkillStateInfoName = "skilleff";
const auto szChaLvUpName = "character_lvup";
const auto szChaSailLvUpName = "saillvup";
const auto szChaLifeLvUpName = "lifelvup";
const auto szItemInfoName = "ItemInfo";
const auto szInitChaItName = "Int_Cha_Item";
const auto g_szForgeTable = "forgeitem"; //精炼用别的表
const auto szIslandInfoName = "AreaSet";
const auto szHairInfoName = "Hairs"; // 可更换的发型记录表
const auto szNoLockItemName = "NoLock";
const auto szMapCopyInfoName = "MapCopySet";
...
DWORD WINAPI g_GameLogicProcess(LPVOID lpParameter) {
	...
	while (!g_bGameEnd) {
		try {
			...
			if (g_pGameApp->m_bExecLuaCmd) {
				luaL_handled_dofile(g_pLuaState, "tmp.txt"); //CHANGED: better lua error handling (deguix)
			...
			if (g_Config.m_ResetFlag) {
				...
				if (g_pGameApp->m_ResetTimer.GetDayOfWeek() - 1 == 0 && g_pGameApp->m_ResetTimer.GetHour() == 3 && g_pGameApp->m_ResetTimer.GetMinute() == 0 && g_pGameApp->m_ResetTimer.GetSecond() == 0) {
					begin = true;
				}
				if (begin) {
	...
	const char* GetResPath(const char* pszRes) {
		...
		std::string str = g_Config.m_szResDir;
		if (!str.empty()) {
			str += "/";
		}
		str += pszRes;
	...
	BOOL LoadTable(CRawDataSet * pTable, const char* pszTableName) {
		std::string str = GetResPath(pszTableName);
	...
	BOOL CGameApp::Init() {
		try {
			...
			case 1: {
				strncat_s(buf, sizeof(g_Config.m_szDBPass), "==", _TRUNCATE);
			} break;
			case 2: {
				strncat_s(buf, sizeof(g_Config.m_szDBPass), "=", _TRUNCATE);
			} break;
			...
			m_CChaRecordSet = new CChaRecordSet(0, 2500);
			...
			//m_CSkillStateRecordSet = new CSkillStateRecordSet(0, SKILL_STATE_MAXID);
			...
			m_CNoLockRecordSet = new CNoLockRecordSet(0, 300);
			LoadTable(m_CNoLockRecordSet, szNoLockItemName);

			m_CMapCopyRecordSet = new CMapCopyRecordSet(0, 30);
			LoadTable(m_CMapCopyRecordSet, szMapCopyInfoName);
				...
				if (bIsDie || !bSavePos) { // 角色已经死亡
					if (bIsDie) {
						g_CParser.DoString("Relive", enumSCRIPT_RETURN_NONE, 0,
										   enumSCRIPT_PARAM_LIGHTUSERDATA, 1, pCCha, DOSTRING_PARAM_END);
					}
	...
	BOOL CGameApp::InitMap() {
		try {
			...
			//初始化地图数据结构
			std::fill(m_MapList.begin(), m_MapList.end(), nullptr);
			for (short i = 0; i < m_mapnum; i++) {
				m_MapList[i] = std::make_unique<CMapRes>();
				...
				strncat_s(m_MapList[i]->m_szMonsterCofFile, sizeof(m_MapList[i]->m_szMonsterCofFile), "init_copy.lua", _TRUNCATE); //CHANGED: From monster_conf to init_copy.
				...
				strncat_s(m_MapList[i]->m_szCtrlFile, sizeof(m_MapList[i]->m_szCtrlFile), "init.lua", _TRUNCATE);
				...
				strncat_s(m_MapList[i]->m_szEntryFile, sizeof(m_MapList[i]->m_szEntryFile), "init.lua", _TRUNCATE);
			...
			//CHANGED: These 2 are not needed (already has initial.lua).
			//luaL_handled_dofile(g_pLuaState, GetResPath("script/help/AddHelpNPC.lua")); //CHANGED: better lua error handling (deguix)

			//luaL_handled_dofile(g_pLuaState, GetResPath("script/monster/mlist.lua")); //CHANGED: better lua error handling (deguix)

			lua_getfield(g_pLuaState, LUA_GLOBALSINDEX, "MapInit_Callback"); //break;
			lua_handled_pcall(g_pLuaState, 0, 0, NULL);						 //call with 1 parameter, 1 result
	...
	CMapRes* CGameApp::FindMapByName(const char* mapname, bool bIncUnRun) {
		...
		*/
				if (strcmp(pMap->GetName(), mapname) == 0) {
					break;
				}
			}
			if (i < m_mapnum) {
				return m_MapList[i].get();
			} else {
				return nullptr;
			}
		}
	...
	mission::CTalkNpc* CGameApp::GetNextTNpc() {
		try {
	...
	int CGameApp::LogPlayer() {
		for (const auto& [id, player] : _PlayerIdx) {
			if (player) {
				LG("LogPlayers", "Key = %d, DB_ID = %d, Name = %s, Valid = %d \n",
				   id, player->GetDBActId(), player->GetActName(), player->IsValid());
			} else {
				LG("LogPlayers", "Key = %d \n", id);
			}
		}
	...
	void CGameApp::ResetHangState() {
		try {
			...
			for (int i = 0; i < (int)dataHangTrade.size(); i++) {
				...
				std::string curr_cha_name = dataHangTrade[i][1].c_str();
				std::string curr_act_name = dataHangTrade[i][2].c_str();
