...
long EnterServerCall::Process() {
	CPlayer* l_player = g_pGameApp->CreateGamePlayer(pszPassword, l_dbid, l_worldid, l_map, chLogin == 0 ? 0 : 1);
	if (!l_player) {
...
GameServerApp::GameServerApp(ThreadPool* proc, ThreadPool* comm)
	: TcpClientApp(this, proc, comm), RPCMGR(this), /*m_count(0), */ PKQueue(false),
	  m_UpdateProc(1, 100), m_GoOutProc(1, 100), m_EnterProc(1, 100) {
	try {
...
void GameServerApp::OnDisconnect(DataSocket* datasock, int reason) {
	try {
		...
		do {
			...
			GatePlayer* tmp = pGate->m_playerlist;
			GatePlayer* tmpNext;
			...
			//pkt.WriteAddress(ToAddress(gt->m_playerlist));
...
void GameServerApp::OnProcessData(DataSocket* datasock, RPacket& pk) {
	try {
		...
		const unsigned long total = this->GetPkTotal();
		const unsigned long l_tick = GetCurrentTick();
		if (l_tick - m_lastNetCheckTick < 1000 * 10) {
			m_netNum++;
		} else {
			static char buffer[64];
			const unsigned short cmd = pk.ReadCmd();
...
void GameServerApp::ProcessData(DataSocket* datasock, RPacket& pk) {
	try {
		...
		default: { msg_id = NETMSG_PACKET; } break;
		}
...
bool GameServerApp::IsValidGate(int i) const {
	try {
		if (i < 0 || i > m_gtnum) {
			throw std::logic_error("gate index out of range!");
		}
...
bool GameServerApp::KickPlayer2(GatePlayer* gtplayer) {
	try {
		...
		pkt.WriteAddress(gtplayer->GetGateAddr());
		pkt.WriteShort(1);
	...
	try {
		...
		for (int i = 0; i < m_gtnum; ++i) {
			...
			if (pGate->IsValid()) {
				...
				GatePlayer* pPlayer = pGate->m_playerlist;
				while (pPlayer != nullptr) {
					CChginf.WriteLong(pPlayer->GetDBChaId());
					CChginf.WriteAddress(pPlayer->GetGateAddr());
					sCount++;
...
bool GameServerApp::SendToClient(WPacket& pkt, GatePlayer* playerlist) {
	try {
		...
		while (tmp != nullptr) {
			if (!tmp->GetGate()) {
#ifdef defCOMMU_LOG
				LG("SendToClient", "WARNING! pGate = NULL, cha_id=%d, gt_addr=0x%zx\n",
				...
				for (int i = 0; i < sValidGateNum; ++i) {
					if (tmp->GetGate() == pValidGate[i]) {
						...
						CChginf[i].WriteAddress(tmp->GetGateAddr());
						break;
					}
				}
			}
		...
		for (int i = 0; i < array_cnt; ++i) {
			...
			for (j = 0; j < sValidGateNum; ++j) {
				if (uplayer_array[i].pGate == pValidGate[j]) {
					...
					CChginf[j].WriteAddress(uplayer_array[i].m_ulGateAddr);
					break;
				}
			}
		}
