...
void CItem::OnBeginSeen(CCharacter* pCMainCha) {
	try {
		...
		wpk.WriteCmd(CMD_MC_ITEMBEGINSEE);
		// 基本数据
		wpk.WriteLong(m_ID); // world ID
		wpk.WriteLong(m_lHandle);
		wpk.WriteLong(m_pCItemRecord->lID);  // ID
		wpk.WriteLong(GetShape().centre.x);  // 当前x位置
		wpk.WriteLong(GetShape().centre.y);  // 当前y位置
		wpk.WriteShort(m_sAngle);			 // 方向
		wpk.WriteShort(m_SGridContent.sNum); // 个数
		//
		wpk.WriteChar(m_chSpawType);
		wpk.WriteLong(m_lFromEntityID);
		// 事件信息
		WriteEventInfo(wpk);

		pCMainCha->ReflectINFof(this, wpk); //通告
	}
	T_E
}
