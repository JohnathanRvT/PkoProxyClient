...
CMapRes::CMapRes()
	: m_pCLeftMap(nullptr), m_pCTopMap(nullptr), m_pCRightMap(nullptr), m_pCBelowMap(nullptr), m_pCMonsterSpawn(nullptr), m_pCMapSwitchEntitySpawn(nullptr), m_pNpcSpawn(nullptr), m_csEyeshotCellWidth(800), m_csEyeshotCellHeight(800), m_csStateCellWidth(200), m_csStateCellHeight(200), m_csBlockUnitWidth(50), m_csBlockUnitHeight(50), m_byMapID(0), m_bCanStall(true), m_bCanGuild(true) {
	try {
		m_bValid = false;
...
bool CMapRes::Init() {
	try {
		...
		if (m_CTerrain.Init((char*)GetResPath(m_szSectionFile)) == 0) {
...
// 根据地图控制脚本初始化一些控制信息
bool CMapRes::InitCtrl() {
	try {
		//if (g_IsFileExist(GetResPath(m_szCtrlFile))) //CHANGED: already handles non-existing file.
		luaL_handled_dofile(g_pLuaState, GetResPath(m_szCtrlFile)); //CHANGED: better lua error handling (deguix)
...
bool CMapRes::CreateEntry() {
	try {
		...
		if (m_SEntryPos.x == -1 && m_SEntryPos.y == -1) {
			std::string strScript = "get_map_entry_pos_";
			strScript += GetName();
...
void CMapRes::Run(DWORD dwCurTime) {
	try {
		...
		if (bOpenEntry) {
			std::string strScript = "can_open_entry_";
			strScript += GetName();
			if (g_CParser.StringIsFunction(strScript.c_str())) {
				g_CParser.DoString(strScript.c_str(), enumSCRIPT_RETURN_NUMBER, 1, enumSCRIPT_PARAM_LIGHTUSERDATA, 1, this, DOSTRING_PARAM_END);
...
// 取现一个正在运行的副本
SubMap* CMapRes::GetNextUsedCopy() {
	try {
		if (!m_pCMapCopy)
			return nullptr;
...
void CMapRes::CheckMapState(std::string& str) {
	try {
...
Long CAreaData::Init(char* chFile) {
	try {
