...
BOOL CCharMission::MisGetDataEx(StringPoolL& pool, int index) {
	try {
		pool[index]
			.Printf("%d,%d,", ROLE_MIS_MISINFO_EDITION, m_byNumMission);
...
BOOL CCharMission::MisGetRecordEx(StringPoolL& pool, int index) {
	try {
		pool[index]
			.Printf("%d,", ROLE_MIS_RECORD_EDITION);
...
BOOL CCharMission::MisGetTriggerEx(StringPoolL& pool, int index) {
	try {
		pool[index]
			.Printf("%d,%d,", ROLE_MIS_TRIGGER_EDITION, m_byNumTrigger);
...
BOOL CCharMission::MisGetMissionCountEx(StringPoolL& pool, int index) {
	try {
		pool[index]
			.Printf("%d,%d,", ROLE_MIS_MISCOUNT_EDITION, m_byNumMisCount);
		...
		for (int i = 0; i < m_byNumTrigger; i++) {
			if (m_Trigger[i].byType == TE_KILL) {
				...
				if (sWareID == m_Trigger[i].wParam1) {
					...
					int nStatus = lua_handled_pcall(g_pLuaState, 6, 1, 0);
					if (nStatus) {
...
void CCharMission::MisGetMisLogInfo(WORD wMisID) {
	try {
		...
		//CHANGED: better lua error handling (deguix)
		int nStatus = lua_handled_pcall(g_pLuaState, 2, 0, 0);
		if (nStatus) {
...
BOOL CCharMission::CancelRole(WORD wRoleID, WORD wScriptID) {
	...
	//CHANGED: better lua error handling (deguix)
	int nStatus = lua_handled_pcall(g_pLuaState, 3, 1, 0);
	if (nStatus) {
