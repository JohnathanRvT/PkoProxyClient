		...
		for (Short x = sUnitSX; x <= sUnitEX; x++) {
			for (Short y = sUnitSY; y <= sUnitEY; y++) {
				...
				if (!g_IsMoveAble((char)m_CChaAttr.GetAttr(ATTR_CHATYPE), m_pCChaRecord->chTerritory, usAreaAttr)) {
					...
					if (CRangeBase::LineIntersection(v0, v1, p1, p2, FALSE) || CRangeBase::LineIntersection(v0, v1, p2, p3, FALSE) || CRangeBase::LineIntersection(v0, v1, p3, p4, FALSE) || CRangeBase::LineIntersection(v0, v1, p4, p1, FALSE)) {
						xdist = 0, ydist = 0;
						return true;
					}
				}
...
void CMoveAble::BeginMove(uLong ulElapse) {
	try {
		if (GetPos() != m_SMoveInit.SInflexionInfo.SList[0] && IsCharacter()->IsRangePoint2(m_SMoveInit.SInflexionInfo.SList[0], 25 * 25 * 2)) // 客户端会把当前位置调整到单元中点
		{
...
//=============================================================================
// "Find the point closest to the point pSReference on the line segment (pSPort1, pSPort2)"
//=============================================================================
