...
inline int lua_Rand(lua_State* L) {
	...
	try {
		...
		int dwMax = lua_tonumber(L, 1);
		int dwRand = (dwMax > 0) ? rng.uniform(0, dwMax - 1) : 0;
...
inline int lua_SafeBuy(lua_State* L) {
	try {
		...
		DWORD dwMoney;
		BOOL bRet = pChar->SafeBuy(wItemID, wCount, byIndex, dwMoney);
...
inline int lua_SafeSale(lua_State* L) {
	try {
		...
		WORD wItemID;
		DWORD dwMoney;
		BOOL bRet = pChar->SafeSale(byIndex, wCount, wItemID, dwMoney);
		lua_pushnumber(L, (bRet) ? LUA_TRUE : LUA_FALSE);
...
inline int lua_SafeSaleGoods(lua_State* L) {
	...
	WORD wItemID;
	BOOL bRet = pChar->SafeSaleGoods(dwBoatID, byIndex, wCount, wItemID, dwMoney);
...
inline int lua_SafeBuyGoods(lua_State* L) {
	...
	WORD wCount = (WORD)lua_tonumber(L, 5);
	DWORD dwMoney = (DWORD)lua_tonumber(L, 6);

	BOOL bRet = pChar->SafeBuyGoods(dwBoatID, wItemID, wCount, byIndex, dwMoney);
...
BOOL lua_ChaPlayEffect(lua_State* L) {
	...
	WRITE_CMD(wpk, CMD_MC_CHAPLAYEFFECT);
	WRITE_LONG(wpk, pChar->GetID());
	WRITE_LONG(wpk, nEffectID);

	pChar->NotiChgToEyeshot(wpk);
	return 1;
}
