...
void CPlayer::NoticeTeamLeaderID() {
	int nTMemberCnt = GetTeamMemberCnt();
	if (nTMemberCnt == 0)
		return;
...
CPlayer* CPlayer::GetNextTeamPly() {
...
void CPlayer::SystemNotice(const char szData[], ...) {
	try {
		...
		WRITE_ADDRESS(packet, GetGateAddr());
...
bool CPlayer::SetBankChangeFlag(char chBankNO, bool bChange) {
	...
	for (char i = chStart; i <= chEnd; i++) {
		if (CKitbag* pCBank = GetBank(i); pCBank) {
...
void CPlayer::CheckChaItemFinalData() {
	...
	for (auto& item : pCMainCha->m_SChaPart.SLink) {
		if (g_IsRealItemID(item.sID)) {
			if (item.InitAttr()) {
				g_CParser.DoString(szScript, enumSCRIPT_RETURN_NONE, 0, enumSCRIPT_PARAM_LIGHTUSERDATA, 1, &item, DOSTRING_PARAM_END);
			}
		}
	}
