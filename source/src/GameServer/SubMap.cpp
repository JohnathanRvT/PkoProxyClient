...
bool SubMap::Init(CMapRes* pCMapRes, dbc::Short sCopyNO) {
	try {
		...
		luaL_handled_dofile(g_pLuaState, GetResPath(m_pCMapRes->m_szMonsterCofFile)); //resok //CHANGED: better lua error handling (deguix)
...
CCharacter* SubMap::ChaSpawn(Long lChaInfoID, Char chCtrlType, Short sAngle, Point* pSPos, bool bEyeshotAbility, dbc::cChar* cszChaName, const long clSearchRadius) {
	try {
		...
		Circle l_STerritory = {*pSPos, pCCha->m_pCChaRecord->lVision/2};
...
void SubMap::Notice(const char* szString) {
	try {
		...
		CPlayer* pHeadPlayer = nullptr;
		CPlayer* pLastPlayer = nullptr;
...
dbc::Long SubMap::CountEyeshotPlyActiveNum(dbc::Long lCellX, dbc::Long lCellY) {
	try {
		...
		CCharacter* pCCha = m_pCEyeshotCell[lCellY][lCellX].m_pCChaL;
...
BOOL SubMap::LoadEventEntity() {
	try {
		g_pScriptMap = this;
		//CHANGED: Made it not call map entity.lua file (calling main file should be enough).
...
void SubMap::Add(Entity* pCEnt) {
	try {
		...
		if (pCCha) {
			...
			const Long l_radius = pCEnt->GetRadius();
			l_pt = pCEnt->GetPos();
...
void SubMap::MoveInStateCell(CCharacter* pCCha, const Point& SSrcPos, const Point& STarPos) {
	try {
		Point l_src = SSrcPos;
		Point l_dst = STarPos;
		const Long lRadius = pCCha->GetRadius();
...
bool SubMap::Enter(Square* pSEntShape, Entity* ent, cLong clSearchRadius) {
	try {
			...
			if (ent->overlap(l_xdlt, l_ydlt)) {
				...
				while (l_overflag) { //直到确定位置为止
					...
					for (short dir = 0; dir < 4; dir++) {
						...
						const long l_radius = GetBlockCellWidth() * l_dist;
		...
		//beginsee
		if (l_retval) {
			const bool bActive = ent->GetEyeshotAbility();
...
void SubMap::GoOut(Entity* ent) {
	try {
		...
		const Rect l_rect = GetEyeshot(l_pt);
		//删除
		Delete(ent);
		//endsee
...
void SubMap::BeginSearchInRange(Long* plRangeBParam, Long* plRangeEParam, bool bIncludeHideEnti) {
	try {
		...
		case enumRANGE_TYPE_STICK: {
			const Long lRadius = plRangeEParam[1];
			const Long lWidth = plRangeEParam[2];
		...
		case enumRANGE_TYPE_FAN: {
			const Long lRadius = plRangeEParam[1];
			const Long lAngleD = plRangeEParam[2];
...
CCharacter* SubMap::GetNextCharacterInRange() {
	try {
		if (!m_pCBaseRange) {
			return nullptr;
		}
...
bool SubMap::RangeAddState(uChar uchFightID, uLong ulSrcWorldID, Long lSrcHandle,
	...
	try {
		...
		CSkillStateRecord* pSStateR = nullptr;
...
void SubMap::NotiStateCellToEyeshot(Short sCellX, Short sCellY) {
	try {
		...
		wpk.WriteCmd(CMD_MC_ASTATEBEGINSEE);
		wpk.WriteShort(sCellX);
		wpk.WriteShort(sCellY);
		m_pCStateCell[sCellY][sCellX]->m_CSkillState.WriteState(wpk);
...
void SubMap::MoveTo(Entity* pCEnt, const Point& STar) {
	try {
		if (pCEnt->GetPos() != pCEnt->m_lastpos) {
			...
			_snprintf_s(szMess, sizeof(szMess), _TRUNCATE,
						"character %s move originality coordinate error，last time position[%d, %d]，current position[%d, %d]，aim position[%d, %d]",
			...
			for (long y = l_ymin; y <= l_ymax; y++) {
				for (long x = l_xmin; x <= l_xmax; x++) {
					...
					if (l_srcx <= GetEyeshotWidth() && l_srcy <= GetEyeshotWidth() &&
						...
						continue;
					} else if (abs(x - l_src.x) <= GetEyeshotWidth() &&
							   abs(y - l_src.y) <= GetEyeshotWidth()) { //源块附近的，endsee
						if (bActive) {
							InactiveEyeshotCell(x, y);
						}
				...
				if (pCProcCha->IsPlayerFocusCha()) {
					...
					if (!pCProcCha->IsLiveing()) {
						g_CParser.DoString("Relive", enumSCRIPT_RETURN_NONE, 0,
										   enumSCRIPT_PARAM_LIGHTUSERDATA, 1, pCProcCha, DOSTRING_PARAM_END);
...
void SubMap::Run(DWORD dwCurTime) {
	try {
		...
		if (m_timeScriptRun.IsOK(dwCurTime)) {
			std::string strScript = "map_copy_run_";
			strScript += GetName();
		...
		if (m_timeSpecialRun.IsOK(dwCurTime)) {
			std::string strScript = "map_copy_run_special_";
			strScript += GetName();
			g_CParser.DoString(strScript.c_str(), enumSCRIPT_RETURN_NONE, 0,
							   enumSCRIPT_PARAM_LIGHTUSERDATA, 1, this, DOSTRING_PARAM_END);
		}
...
void SubMap::Close() {
	try {
		...
		std::string strScript = "map_copy_before_close_";
		strScript += GetName();
		g_CParser.DoString(strScript.c_str(), enumSCRIPT_RETURN_NONE, 0,
						   enumSCRIPT_PARAM_LIGHTUSERDATA, 1, this, DOSTRING_PARAM_END);
...
void SubMap::BeforePlyOutMap(CCharacter* pCCha) {
	try {
		...
		std::string strScript = "before_leave_";
		strScript += GetName();
		g_CParser.DoString(strScript.c_str(), enumSCRIPT_RETURN_NONE, 0,
						   enumSCRIPT_PARAM_LIGHTUSERDATA, 2, pCCha, this, DOSTRING_PARAM_END);
	}
	T_E
}
...
void SubMap::AfterPlyEnterMap(CCharacter* pCCha) {
	try {
		std::string strScript = "after_enter_";
		strScript += GetName();
...
CCharacter* SubMap::GetNextPlyCha() {
	try {
		...
		if (m_pCMapRes->GetCopyStartType() == enumMAPCOPY_START_CONDITION) {
			if (m_pCMapRes->GetCopyStartCdtType() == enumMAPCOPY_START_CDT_PLYNUM) {
				if (GetPlayerNum() >= m_pCMapRes->GetCopyStartCdtVal()) {
					...
					std::string strScript = "map_copy_first_run_";
					strScript += GetName();
					g_CParser.DoString(strScript.c_str(), enumSCRIPT_RETURN_NONE, 0,
									   enumSCRIPT_PARAM_LIGHTUSERDATA, 1, this, DOSTRING_PARAM_END);
...
void SubMap::DealActivePlayer(std::string& function) {
	try {
		CCharacter* pCha;
		BeginGetPlyCha();
		...
		while (pCha = GetNextPlyCha()) {
			if (pCha->IsPlayerCha() && pCha->IsLiveing()) {
				g_CParser.DoString(function.c_str(), enumSCRIPT_RETURN_NONE, 0,
...
void SubMap::DealPlayer(std::string& function) {
	try {
		CCharacter* pCha = nullptr;
		BeginGetPlyCha();
