...
INT_PTR CALLBACK DlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	try {
		...
		case WM_INITDIALOG: {
			...
			::SetWindowPos(hwndDlg, nullptr, 0, 0, 392, 584, SWP_SHOWWINDOW);
		...
		case WM_COMMAND: {
			if (wParam == IDC_BUT_ZOOM1) {
				...
				if (flag)
					::SetWindowPos(hwndDlg, nullptr, rect.left, rect.top, 682, 584, SWP_SHOWWINDOW);
				else
					::SetWindowPos(hwndDlg, nullptr, rect.left, rect.top, 392, 584, SWP_SHOWWINDOW);
...
void CreateMainDialog(HINSTANCE hInst, HWND hParent) {
	try {
		...
		std::string strCaption = "KOPOL-GS(Ver2.00.003)";
		strCaption += "[";
		strCaption += __DATE__;
		strCaption += " ";
		strCaption += __TIME__;
