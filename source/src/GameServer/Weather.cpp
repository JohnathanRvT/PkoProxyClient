...
void CWeather::RandLocation(SubMap* pMap) {
	...
	int sx = rng.uniform(_sx, _sx + _w - 1);
	int sy = rng.uniform(_sy, _sy + _h - 1);
	int nw = rng.uniform(1, 1 + 3 - 1); // 每一次产生的地面范围的尺寸, 单位为2米
	int nh = rng.uniform(1, 1 + 3 - 1);
...
void CWeatherMgr::Run(SubMap* pMap) {
	for (auto& weather : _WeatherList) {
		weather->RandLocation(pMap);
	}
}
