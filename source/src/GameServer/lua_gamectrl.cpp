...
mission::CNpc* pTradeNPC = nullptr;
...
void ReloadAISdk() {
	//luaL_handled_dofile( g_pLuaState, GetResPath("script/ai/ai.lua")); //CHANGED: better lua error handling (deguix)
}
...
std::map<std::string, std::string> g_HelpList;
...
const char* FindHelpInfo(const char* pszKey) {
	std::map<std::string, std::string>::iterator it = g_HelpList.find(pszKey);
