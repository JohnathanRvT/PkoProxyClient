...
void CChaStateMgr::ChaDestroy() {
	for (auto* state : _states) {
		if (state->pEffect) {
			LG(_pCha->getLogName(), RES_STRING(CL_LANGUAGE_MATCH_29), state->pEffect->getIdxID(), state->pEffect);

			state->pEffect->SetValid(FALSE);
			state->pEffect = nullptr;
		}
	}
	...
	for (unsigned int i = 0; i < nTmpCount; i++) {
		...
		if (pChaState->IsDel) {
			...
			if (pInfo->sBitEffect > 0) {
				_hits.emplace_back(pInfo->sBitEffect, pInfo->sDummy2);
			}
