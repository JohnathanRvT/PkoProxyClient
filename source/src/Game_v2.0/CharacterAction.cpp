	...
	while (!feof(fFile)) {
		...
		while (!feof(fFile)) {
			...
			if (tcsLine[lIndex] == '\t') // tab键
			{
				...
				// 临时代码， 用于提示更新文件版本。
				if (_ttoi(tcsTemp) < 1) {
					MessageBox(nullptr, RES_STRING(CL_LANGUAGE_MATCH_24), RES_STRING(CL_LANGUAGE_MATCH_25), 0);
					goto end;
				}
				//
		...
		while (!feof(fFile)) {
			...
			if (tcsLine[lIndex] == '\t') // tab键
			{
				StringSkipCompartment(tcsLine, &lIndex, "\x09", 1);
				StringGet(tcsTemp, 1023, tcsLine, &lIndex, " ,\x09\x0a\x0d", 5);
...
void CGameCharacterAction::Free() {
	//short	i, j;
	short i;
...
// 从给定字符窜(char *in)的给定位置(long *in_from)获取字符窜，
// 直到遇到字符窜列表(char *end_list)中的任一字符时结束
...
// 从给定字符窜(char *in)的给定位置(long *in_from)剔除字符窜列表(char *end_list)中的任一字符
