...
struct wield_state {
	...
	constexpr wield_state(BYTE m, BYTE l, BYTE r)
...
constexpr static int __wield_state_num = 15;
constexpr static wield_state __wield_state[__wield_state_num] =
	{
...
int CCharacterModel::LoadCha(DWORD type_id, DWORD bone_id, const DWORD* part_buf) {
	...
	StartLoad();
	char str[260];
	_snprintf_s(str, _TRUNCATE, "%04d.lab", bone_id);
...
int CCharacterModel::LoadShip(DWORD type_id, DWORD ship_type, const DWORD* part_buf) {
	_TypeID = type_id;
...
int CCharacterModel::LoadShip(DWORD type_id, const DWORD* part_buf) {
	_ModelType = MODEL_WARSHIP;
	_TypeID = type_id;
	_ShipType = 0;
...
int CCharacterModel::LoadPart(DWORD part_id, DWORD id) {
	...
	char file[260];
	_snprintf_s(file, _TRUNCATE, "%010d.lgo", id);
...
int CCharacterModel::ChangePart(DWORD part_id, DWORD tab_id) {
	...
	char str[260];
	_snprintf_s(str, _TRUNCATE, "%s.lgo", sx);
	...
	if (_ModelType == MODEL_WARSHIP) {
		if (part_id == 0) {
			if (ir->sShipFlag > 0) {
				...
				_snprintf_s(str, _TRUNCATE, "%s.lgo", ir->chModule[0]);
				LoadPart(3, str);
			...
			for (DWORD i = 0; i < 4; i++) {
				...
				_snprintf_s(str, _TRUNCATE, "%s.lgo", iiir->chModule[0]);
...
int CCharacterModel::LoadPose() {
	...
	if (_TypeID == 1) {
		...
		if (anim_agent) {
			...
			auto* ctrl_obj_bone = (MPIAnimCtrlObjBone*)anim_agent->GetAnimCtrlObj(&type_info);
			auto* ctrl_bone = (MPIAnimCtrlBone*)ctrl_obj_bone->GetAnimCtrl();
...
int CCharacterModel::Lit(DWORD part_id, DWORD lit_id) {
	...
	if (_ModelType == MODEL_CHARACTER) {
		...
		char file[260];
		_snprintf_s(file, _TRUNCATE, "%s.lgo", _PartFile[part_id]);
		//_snprintf_s(file, _TRUNCATE, "%s.lgo", "0000810002");
		...
		lwPrimitiveTexLitA(p, l->mask, l->str_buf[lit_id], R"(.\texture\lit\)", l->anim_type);
		DWORD vst = p->GetRenderCtrlAgent()->GetVertexShader();
