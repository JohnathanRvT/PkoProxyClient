...
int LoginScene_CreateCha::_InitChaObjSeq() {
	...
	for (DWORD i = 0; i < _cha_num; i++) {
		auto* bone_ctrl = static_cast<lwINodeBoneCtrl*>(_cha_obj[i]->GetParent());
...
void LoginScene_CreateCha::Update() {
	...
	for (auto& ArrowMark : _ArrowMarks) {
		if (ArrowMark)
			ArrowMark->Update();
	}
...
int LoginScene_CreateCha::LoadModelLXO(const char* file) {
	...
	_res_mgr->SetTexturePath(R"(.\texture\scene\)");

	char path[260];
	_snprintf_s(path, _TRUNCATE, R"(.\model\scene\%s)", file);
	_model_lxo->Load(path, MODELOBJECT_LOAD_RESET, nullptr);
	...
	lwITreeNode* tn_bonectrl = nullptr;
	lwITreeNode* tn_dummy = nullptr;
	lwINodeBoneCtrl* node_bonectrl = nullptr;
	lwINodeDummy* node_dummy = nullptr;
	...
	if (node_bonectrl && node_dummy) {
		...
		_model_lxo->InsertTreeNode(tn_dummy, nullptr, tn_bonectrl);
	}
...

	{
		...
		for (DWORD i = 0; i < pci_num; i++) {
			...
			if (pci[i].anim_type == ANIM_CTRL_TYPE_BONE) {
				...
				if (LW_SUCCEEDED(_model_lxo->QueryTreeNode(&mnqi))) {
					auto* nb = static_cast<lwINodeBoneCtrl*>(mnqi.node->GetData());
...
void LoginScene_CreateCha::OnMouseMove(int flag, int x, int y) {
	...
	for (DWORD i = 0; i < _act_num; i++) {
		if ((pri = _cha_obj[_act_obj[i]]) == nullptr)
			continue;
		...
		auto* bone_ctrl = (lwINodeBoneCtrl*)pri->GetParent();
		bone_ctrl->GetAnimCtrlObj()->PlayPose(&play_info);

		if (i == 2) // octopus
		{
			auto* bone_ctrl = (lwINodeBoneCtrl*)_cha_obj[4]->GetParent();
			bone_ctrl->GetAnimCtrlObj()->PlayPose(&play_info);
		}
	}

	{ // dummy 99 obj
		auto* node_dummy = (lwINodeDummy*)_cha_obj[_act_obj[3]]->GetParent()->GetParent();
		auto* ctrl_mat = (lwIAnimCtrlObjMat*)node_dummy->GetAnimCtrlObj();
...
int LoginScene_CreateCha::OnLButtonDown(int flag, int x, int y) {
	...
	auto* pCreateChaScene = dynamic_cast<CCreateChaScene*>(g_pGameApp->GetCurScene());
	if (pCreateChaScene) {
...
CForm* CCreateChaScene::frmChaFound = nullptr;
CEdit* CCreateChaScene::edtName = nullptr;
CMemo* CCreateChaScene::memChaDescribe = nullptr;
CLabel* CCreateChaScene::labHair = nullptr;
CLabel* CCreateChaScene::labFace = nullptr;

CForm* CCreateChaScene::frmChaCity = nullptr;
CMemo* CCreateChaScene::memChaDescribe2 = nullptr;
CLabel* CCreateChaScene::labCity = nullptr;

CForm* CCreateChaScene::frmQuit = nullptr;
...
CTextButton* CCreateChaScene::imgCitiesBlock[MAX_CITY_NUM] = {nullptr, nullptr, nullptr};
...
bool CCreateChaScene::_InitUI() {
	// 角色创建界面
	{
		...
		auto* btnLeftHair = static_cast<CTextButton*>(frmChaFound->Find("btnLeftHair"));
		if (!btnLeftHair) {
		...
		auto* btnRightHair = static_cast<CTextButton*>(frmChaFound->Find("btnRightHair"));
		if (!btnRightHair) {
			Error(RES_STRING(CL_LANGUAGE_MATCH_45),
		...
		auto* btnLeftFace = static_cast<CTextButton*>(frmChaFound->Find("btnLeftFace"));
		if (!btnLeftFace) {
			Error(RES_STRING(CL_LANGUAGE_MATCH_45),
		...
		auto* btnRightFace = static_cast<CTextButton*>(frmChaFound->Find("btnRightFace"));
		if (!btnRightFace) {
			Error(RES_STRING(CL_LANGUAGE_MATCH_45),
		...
		auto* btnLeft3d = static_cast<CTextButton*>(frmChaFound->Find("btnLeft3d"));
		if (!btnLeft3d) {
			Error(RES_STRING(CL_LANGUAGE_MATCH_45),
		...
		auto* btnRight3d = static_cast<CTextButton*>(frmChaFound->Find("btnRight3d"));
		if (!btnRight3d) {
			Error(RES_STRING(CL_LANGUAGE_MATCH_45),
		...
		auto* ui3dCreateCha = static_cast<C3DCompent*>(frmChaFound->Find("ui3dCreateCha"));
		if (ui3dCreateCha) {
	...
	// 角色城市选择界面
	{
		...
		for (int i(0); i < MAX_CITY_NUM + 1; ++i) {
			for (int j(0); j < CITY_PICTURE_NUM; ++j) {
				_snprintf_s(szPicName, _TRUNCATE, szPicNameBase, i, j);
		...
		for (int i(0); i < MAX_CITY_NUM; ++i) {
			_snprintf_s(szPicName, _TRUNCATE, szCityBlockName, i + 1);
	...
	// 新手流程界面
	{
		...
		for (int i = 0; i < ROLE_ALL_INFO_COUNT; ++i) {
			_snprintf_s(szChaView, _TRUNCATE, "imgChaView%d", i + 1);
...
void CCreateChaScene::_ChaFoundFrmMouseEvent(CCompent* pSender, int nMsgType,
	...
	if ("frmFound"sv != pSender->GetForm()->GetName()) {
		return;
	}

	if (const std::string_view name = pSender->GetName(); name == "btnYes") {
...
void CCreateChaScene::__gui_event_left_face(CGuiData* sender,
											int x, int y, MouseClickState key) {
...
void CCreateChaScene::ChangeCity(eDirectType enumDirect) {
	...
	auto* labCityShow = static_cast<CLabelEx*>(frmChaCity->Find("labCityShow"));
	if (!labCityShow)
		return;
...
bool CCreateChaScene::IsValidCheckChaName(const char* name) {
	if (strlen(name) <= 0) {
		CGameApp::MsgBox(RES_STRING(CL_LANGUAGE_MATCH_49));
		return false;
	}

	if (strlen(name) <= 0) { //if (std::any_of(name, name + strlen(name), std::isspace)) { //Will revert once I figure out how to get it working on c++20
		CGameApp::MsgBox(RES_STRING(CL_LANGUAGE_MATCH_50));
		return false;
	}
...
CCreateChaScene& CCreateChaScene::GetCurrScene() {
	auto* pScene = dynamic_cast<CCreateChaScene*>(g_pGameApp->GetCurScene());
...
void CCreateChaScene::_evtRoleInfoFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
// 蓝崎界面 （1 ~ 7）
void CCreateChaScene::_evtLanchInfoFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	const std::string strName = pSender->GetName();
	auto* pCreateChaScene = dynamic_cast<CCreateChaScene*>(g_pGameApp->GetCurScene());
...
// 艾米界面 （19 ~ 24）
void CCreateChaScene::_evtAimiInfoFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string strName = pSender->GetName();
	auto* pCreateChaScene = dynamic_cast<CCreateChaScene*>(g_pGameApp->GetCurScene());
...
// 菲利尔界面 （11 ~ 18）
void CCreateChaScene::_evtFelierInfoFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	const std::string strName = pSender->GetName();
	auto* pCreateChaScene = dynamic_cast<CCreateChaScene*>(g_pGameApp->GetCurScene());
...
// 卡西斯界面 （8 ~ 10）
void CCreateChaScene::_evtCaxiusInfoFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	const std::string strName = pSender->GetName();
	auto* pCreateChaScene = dynamic_cast<CCreateChaScene*>(g_pGameApp->GetCurScene());
...
void CCreateChaScene::_evtRoleAllInfoFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	const std::string strName = pSender->GetName();
	auto* pCreateChaScene = dynamic_cast<CCreateChaScene*>(g_pGameApp->GetCurScene());
