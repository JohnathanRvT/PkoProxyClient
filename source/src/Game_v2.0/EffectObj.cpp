...
void (*GroupList[])(CMagicEff* pEffCtrl, const D3DXVECTOR3* pStart, const D3DXVECTOR3* pEnd) =
	{
		Part_fan,
		Part_sequence,
};
...
BOOL CMagicEff::CreateGroupMagic(int iIdxID) {
	...
	char pszID[10];
	_snprintf_s(pszID, _TRUNCATE, "%d", iIdxID);
...
void CMagicEff::Render() {
	...
	if (_bMagic || _bGroupMagic) {
		...
		if (_pTerrain) {
			for (auto* p : _pMagicCtrl) {
				if (p->IsPlaying()) {
					if (_pTerrain->IsPointVisible(p->GetPos()->x, p->GetPos()->y)) {
						bov = false;
					}
				}
			}
...
void CMagicEff::Emission(int iID, const D3DXVECTOR3* vBegin, const D3DXVECTOR3* vEnd, int iTime) {
	try {
...
bool CShadeEff::Create(std::string strTexName, float fSize,
...
void CShadeEff::Emission(WORD wID, const D3DXVECTOR3* vBegin, const D3DXVECTOR3* vEnd) {
	_vPos = *vBegin;
	MoveTo(&_vPos);
...
void CNavigationBar::Render() {
	...
	std::string strName = pScene->GetTerrainName();
