...
PATH_LINK* SearchPath(BYTE* block_buf, short width, short height, short sx, short sy, short tx, short ty, long STEP_LIMIT) {
	auto* node_list = new NODE_SHARED[STEP_LIMIT]; //jze
	auto* path_link = new PATH_LINK[STEP_LIMIT];
...
skip:
	...
	if (found_flag) {
		auto* path_return = new PATH_LINK[current_path];
...
BYTE* CFindPath::GetTempTerrain(CGameScene* pScene, CCharacter* pCha, int iCurX, int iCurY) {
	...
	//	{
	//		_snprintf_s(buf, _TRUNCATE,"%d",_byTempBlock[i * _nWidth + j]);
...
void CFindPathEx::Move() {
	...
	auto pMoveState = dynamic_cast<COneMoveState*>(pScene->GetMainCha()->GetActor()->GetCurState());
