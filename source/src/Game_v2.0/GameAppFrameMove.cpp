...
//		m_pAudioPlayer->Update( TRUE );
	...
	case BackgroundMusic::OldMusic:
		...
		if (_nCurMusicSize <= 1) {
			...
			if (_szBkgMusic[0] == '\0') {
				_eSwitchMusic = BackgroundMusic::NoMusic;
			} else {
	...
	case BackgroundMusic::NewMusic:
		...
		if (_nCurMusicSize >= _nMusicSize) {
			_eSwitchMusic = BackgroundMusic::MusicPlay;
		} else {
			//::bkg_snd_vol( _nCurMusicSize );
			AudioSDL::get_instance()->volume(g_dwCurMusicID, _nCurMusicSize);
		}
...
void CGameApp::_PreMouseRun(MouseClickState dwMouseKey) {
	...
	case eMouseAction::None:
		_MouseInScene = true;
		break;
	case eMouseAction::Gui:
	case eMouseAction::Skill:
		_MouseInScene = false;
		break;
	case eMouseAction::Drill:
