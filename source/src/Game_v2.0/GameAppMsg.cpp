...
long CALLBACK TerrainNotice(int nFlag, int nSectionX, int nSectionY, unsigned long dwParam, MPTerrain* pThis) {
	...
	// "These four constants should be obtained from the .obj or .ifl file headers."
	constexpr long clSectionWidth = 8;
	constexpr long clSectionHeight = 8;
	constexpr long clTileWidth = 2;
	constexpr long clTileHeight = 2;
	//
	constexpr long lSectionUnitNum = clSectionWidth * clTileWidth * clSectionHeight * clTileHeight;

	// SUnitBlockInfo	SUnitBlock[lSectionUnitNum] = {0};
	// SUnitHeightInfo	SUnitHeight[lSectionUnitNum] = {0};

	constexpr long lSectionTileNum = clSectionWidth * clSectionHeight;
	CGameScene* pScene{nullptr};
	std::array<CSceneObj*, MAX_MAP_SECTION_OBJ> pObjList{};
	CEffectObj* pEffObj;
	std::array<CEffectObj*, MAX_MAP_SECTION_OBJ> pEffObjList{};
	...
	if (nFlag == 0) { //"Read in Section data"
		int nSceneObj = 0;
		...
		if (g_ObjFile.ReadSectionObjInfo(nSectionNO, infoex, &nSectionObjCnt)) {
			for (int i = 0; i < nSectionObjCnt; i++) {
				...
				if (infoex[i].GetType() == 0) { // "Scene object"
					// "Modify the display of the extra large scene to eliminate the bug." by clp
					CSceneObjInfo* pInfo = GetSceneObjInfo(infoex[i].GetID());
					if (pInfo && pInfo->bIsReallyBig) {
						// "Extra large objects are managed by other methods."
						continue;
					}

					nSceneObj++;
					if (CSceneObj* pObj = pScene->AddSceneObj(infoex[i].GetID()); pObj) {
						pObj->setHeightOff(infoex[i].sHeightOff);
						pObj->setPos(infoex[i].nX, infoex[i].nY);
						pObj->setYaw(infoex[i].sYawAngle);
					} else if (g_Config.m_bEnableLGMsg) { // "If the object is added incorrectly, consult to remove it"
						_snprintf_s(tcsPrint, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_101), infoex[i].GetID());
						if (IDYES == MessageBox(nullptr, tcsPrint, RES_STRING(CL_LANGUAGE_MATCH_25), MB_YESNO)) {
				...
				} else if (infoex[i].GetType() == 1) { //"Special effects"
					...
					if (pEffObj) {
						...
						_snprintf_s(tcsPrint, _TRUNCATE,
									RES_STRING("CL_LANGUAGE_MATCH_101"),
									infoex[i].GetID());
						if (IDYES == MessageBox(nullptr, tcsPrint, RES_STRING("CL_LANGUAGE_MATCH_356"), MB_YESNO)) {
							for (int j = i; j < nSectionObjCnt - 1; j++) {
								infoex[j] = infoex[j + 1];
							}
							i--;
	...
	} else if (nFlag == 1) // "Save and release the Section data"
	{
		pScene = CGameApp::GetCurScene();
		int n = 0;
		int iCount;
		const int nStartX = nSectionX * 8 * 100;
		const int nEndX = (nSectionX + 1) * 8 * 100;
		const int nStartY = nSectionY * 8 * 100;
		const int nEndY = (nSectionY + 1) * 8 * 100;
		int nPosX, nPosY;

		// Scene object
		for (int i = 0; i < pScene->GetSceneObjCnt(); i++) {
			CSceneObj* pObj = pScene->GetSceneObj(i);
...
void CGameApp::HandleKeyDown(DWORD dwKey) {
	...
	} else if (IsKeyDown(DIK_F12) && IsShiftPress()) // 激活LogView工具 add by cf
	{
		...
		if (pMain) {
			char buf[128] = {0};
			_snprintf_s(buf, _TRUNCATE, "%d,%d", (pMain->GetCurX() + 50) / 100, (pMain->GetCurY() + 50) / 100);
...
bool CGameApp::HandleWindowMsg(DWORD dwMsg, DWORD dwParam1, DWORD dwParam2) {
	...
	case WM_CHAR: {
		...
		if (g_Config.IsPower() || (CGameScene::GetMainCha() && CGameScene::GetMainCha()->getGMLv())) {
			if (GetConsole()->OnCharEvent((char)dwParam1, dwParam2))
				return false;
		}
		//g_InputBox.HandleWindowMsg(dwMsg, dwParam1, dwParam2);
		LG("key", "keychar:%d, %d\n", dwParam1, dwParam2);
...
const char* HandleMonsterCommand(std::string& strCmd, std::string& p1, std::string& p2) {
	...
	if (strCmd == "load") {
		...
		while (!in.eof()) {
			...
			std::string strValue = strList[0]; // 去掉右括号
...
void CheckSkillEffect(CSkillRecord* pSkill, int nEffectID) {
	...
	auto n = (int)name.find(RES_STRING(CL_LANGUAGE_MATCH_118));
	if (n >= 0) {
...
const char* ConsoleCallback(const char* pszCmd) {
	...
	std::string strRes = RES_STRING(CL_LANGUAGE_MATCH_120);
	...
	try {
		...
		} else if (strCmd == "up") // 站立
		{
			...
			*p = 0;
			...
			if (!p1.empty()) {
				auto* pItem = dynamic_cast<CItemCommand*>(CGuiData::GetHintItem());
				if (pItem) {
					std::ofstream out(p1.c_str(), std::ios::app);
					out << "ID:" << pItem->GetItemInfo()->lID << std::endl;
				}
			...
			extern void NetBeginRepairItem();
			NetBeginRepairItem();
		} else if (strCmd == "move") {
			...
			for (int j(0); j < nTestCount; j++) {
				...
				// 正在测试所有特效
				{
					...
					for (int i = nStart; i < nEnd; i++) {
						...
						pos.x = rng.uniform(pos.x / 1000.0f - 5.0f, pos.x / 1000.0f + 5.0f - 1);
						pos.y = rng.uniform(pos.y / 1000.0f - 5.0f, pos.y / 1000.0f + 5.0f - 1);
			...
			// 检测技能表中的特效是否正常
			{
				...
				for (int k = 0; k < nCount; k++) {
					...
					for (short i : _pSkillInfo->sActionEffect) {
						CheckSkillEffect(_pSkillInfo, i);
					}
			...
			for (int i = 0; i < n; i++) {
				...
				int nTestX = rng.uniform(pMain->GetCurX() - 500, pMain->GetCurX() - 500 + 1000 - 1);
				int nTestY = rng.uniform(pMain->GetCurY() - 500, pMain->GetCurX() - 500 + 1000 - 1);
				...
				if (pCha) {
					...
					pCha->setYaw(rng.uniform(0, RAND_MAX));
			...
			auto pState = std::make_unique<CHitAttackState>(pMain->GetActor());
			pState->SetPose(pose, nPoseCount);
			pState->SetSpeed(fPoseSpeed);
			pMain->GetActor()->AddState(std::move(pState));
			...
			auto pState = std::make_unique<CAllPoseState>(pMain->GetActor());
			if (nStart == 0 || nEnd == 0) {
			...
			for (int kkk = nStart; kkk < nEnd; kkk++) {
				...
				if (pInfo->chModalType == enumMODAL_MAIN_CHA) {
					...
					DWORD part_buf[5] =
						{
							(DWORD)pInfo->sSkinInfo[0],
							(DWORD)pInfo->sSkinInfo[1],
							(DWORD)pInfo->sSkinInfo[2],
							(DWORD)pInfo->sSkinInfo[3],
							(DWORD)pInfo->sSkinInfo[4],
						};
					...
					_snprintf_s(load_info.bone, _TRUNCATE, "%04d.lab", pInfo->sModel);

					for (DWORD i = 0; i < 5; i++) {
						if (pInfo->sSkinInfo[i] != 0) {
							DWORD file_id = pInfo->sModel * 1000000 + pInfo->sSuitID * 10000 + i;
							_snprintf_s(load_info.part[i], _TRUNCATE, "%010d.lgo", file_id);
						}
					}
	...
	bool CGameApp::IsEnableSuperKey() const {
