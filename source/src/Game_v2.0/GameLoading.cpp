...
bool DrawBMPFile(RECT rect, LPCTSTR pszFile, HDC hDC) {
	...
	auto old = (HBITMAP)SelectObject(hdcCompatible, hBitmap);
...
LRESULT CALLBACK LoadingProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_PAINT: {
		...
		auto height = (int)(rc.bottom - rc.top - (rc.right - rc.left) * 9.0 / 16.0);

		RECT rect;
		rect.left = rc.left;
		rect.right = rc.right;
		rect.top = rc.top;
		rect.bottom = (long)(rc.top + height / 2.0 + 1);

		FillRect(hdc, &rect, (HBRUSH)(COLOR_WINDOWTEXT));

		rect.left = rc.left;
		rect.right = rc.right;
		rect.top = (long)(rc.bottom - height / 2.0 - 1);
...
unsigned __stdcall LoadingThread(void* param) {
	...
	auto* load = static_cast<GameLoading*>(param);
	...
	if (g_stUISystem.m_sysProp.m_videoProp.bFullScreen) {
		...
		load->m_hLoading = CreateWindowEx(WS_EX_TOPMOST, "Loading", "Loading", WS_POPUP /*& ~WS_TILED*/,
										  ...
										  (int)((nSizeW - nWindowWidth) / 2.0), (int)((nSizeH - nWindowHeight) / 2.0 + nCaptionSize + nFrameSize - 1), nWindowWidth, nWindowHeight, nullptr, nullptr, g_hInstance, nullptr);
	}
