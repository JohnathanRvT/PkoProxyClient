...
bool CGameMovie::Play(HWND hWnd, const char* pszFileName) {
	...
	*/
	...
	_hResult = _pGraph->RenderFile(szFileNameW, nullptr);
#endif
	if (FAILED(_hResult)) {
		Cleanup();
		return false;
	...
	auto height = (int)((rc.right - rc.left) * 9.0 / 16.0);
