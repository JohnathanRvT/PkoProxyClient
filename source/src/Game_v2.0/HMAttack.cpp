...
void CAttackEffect::_Exec() {
	...
	if (_IsMiss && _pTarget->IsEnabled()) {
		...
		vdir = rng.uniform(0, 2 - 1) ? vdir : -vdir;
...
void CAttackEffect::ChaDied(CCharacter* pTarget, CCharacter* pAttack) {
	...
	auto st = std::make_unique<CReadyDieState>(pTarget->GetActor());
	st->SetState(enumDied);
	st->SetAttack(pAttack);
	pTarget->GetActor()->InsertState(std::move(st));
}
...
void CAttackEffect::ExecHarm(CSizeArray<stEffect>& Value, CCharacter* pTarget, CCharacter* pAttack) {
	...
	for (int i = 0; i < count; i++) {
		...
		//_i64toa_s(p[i].lVal, v1, _countof(v1), 10);
		//_i64toa_s( pAttr->get( (short)p[i].lAttrID ), v2, _countof(v2), 10);
		//_snprintf_s(str, _TRUNCATE,"%s == >%s %s %s\r\n",pAttack->getName(),pTarget->getName(),  v1, v2);
		...
		case ATTR_HP:
			...
			if (pTarget->IsEnabled()) {
				...
				//_snprintf_s( buf, _TRUNCATE, "%d", abs(val) );
...
CStateSynchro* CAttackEffect::Gouge(float fRate) {
	if (!_IsMiss) {
		...
		auto* pAttack = new CAttackEffect;
	...
	if (isMain && enumATTRSYN_INIT != _nType) {
		for (int i = 0; i < count; i++) {
			if (p[i].lAttrID == ATTR_CEXP) {
				val = p[i].lVal - pAttr->get((short)p[i].lAttrID);
				char buff[32];
				_i64toa_s(val, buff, _countof(buff), 10);
