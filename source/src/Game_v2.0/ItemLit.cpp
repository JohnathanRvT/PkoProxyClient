...
LW_RESULT lwItemLit::Load(const char* file) {
	...
	LW_FAILED_RET(fp->Read(&head, sizeof(head), nullptr));
	LW_FAILED_RET(fp->Read(&item_num, sizeof(item_num), nullptr));
	_item.reserve(item_num);

	for (i = 0; i < item_num; i++) {
		item_info = LW_NEW(lwItemLitInfo);
		LW_FAILED_RET(fp->Read(&item_info->id, sizeof(item_info->id), nullptr));
		LW_FAILED_RET(fp->Read(&item_info->descriptor, sizeof(item_info->descriptor), nullptr));
		LW_FAILED_RET(fp->Read(&item_info->file, sizeof(item_info->file), nullptr));
		LW_FAILED_RET(fp->Read(&lit_num, sizeof(lit_num), nullptr));
		item_info->lit.reserve(lit_num);
		for (j = 0; j < lit_num; j++) {
			lit_info = LW_NEW(lwLitInfo);
			LW_FAILED_RET(fp->Read(lit_info, sizeof(lwLitInfo), nullptr));
			lit_info->id = j;
...
LW_RESULT lwItemLit::Save(const char* file) {
	...
	head.version = 1L;
	head.type = 1;
	LW_FAILED_RET(fp->Write(&head, sizeof(head), nullptr));
	item_num = (DWORD)_item.size();
	LW_FAILED_RET(fp->Write(&item_num, sizeof(item_num), nullptr));

	for (i = 0; i < item_num; i++) {
		item_info = _item[i];
		LW_FAILED_RET(fp->Write(&item_info->id, sizeof(item_info->id), nullptr));
		LW_FAILED_RET(fp->Write(&item_info->descriptor, sizeof(item_info->descriptor), nullptr));
		LW_FAILED_RET(fp->Write(&item_info->file, sizeof(item_info->file), nullptr));
		lit_num = (DWORD)item_info->lit.size();
		LW_FAILED_RET(fp->Write(&lit_num, sizeof(lit_num), nullptr));
		for (j = 0; j < lit_num; j++) {
			lit_info = item_info->lit[j];
			LW_FAILED_RET(fp->Write(lit_info, sizeof(lwLitInfo), nullptr));
			lit_info = nullptr;
		}
		item_info = nullptr;
	}
