...
CForm* CLoginScene::frmArea = nullptr;
...
static void _GoBack(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (auto pLoginScene = dynamic_cast<CLoginScene*>(CGameApp::GetCurScene()); pLoginScene) {
		g_cooperate.code ? pLoginScene->ShowServerSelection()
						 : pLoginScene->ShowLoginForm();
	}
}
...
bool CLoginScene::_Init() {
	...
	// 删除自动更新程序的复本
	const char szUpdateFileName[] = "_Update.exe";
...
void CLoginScene::_FrameMove(DWORD dwTimeParam) {
	...
	if (_eState == eLoginState::Connect) {
		...
		case Connection::Connection_Status::INVALID:
		case Connection::Connection_Status::FAILURE: {
			if (g_TomServer.bEnable) {
				MessageBox(nullptr, RES_STRING(CL_LANGUAGE_MATCH_169), "Info", 0);
		...
		case Connection::Connection_Status::TIMEOUT:
			_eState = eLoginState::Init;
...
// 界面相关Routines
//-----------------
void CLoginScene::CallbackUIEvent_LoginScene(CCompent* pSender, int state, int x, int y, MouseClickState key) {
	auto pSceneLogin = dynamic_cast<CLoginScene*>(CGameApp::GetCurScene());
	if (!pSceneLogin) {
		return;
	}
...
void CLoginScene::_evtRegionFrm(CCompent* pSender, int state, int x, int y, MouseClickState key) {
	auto pLoginScene = dynamic_cast<CLoginScene*>(CGameApp::GetCurScene());
	if (!pLoginScene) {
		return;
	}

	if (const std::string name = pSender->GetName(); name == "btnNo") {
		//关闭服务器列表表单,显示通告表单
...
void CLoginScene::_evtLoginFrm(CCompent* pSender, int state, int x, int y, MouseClickState key) {
	auto pLoginScene = dynamic_cast<CLoginScene*>(CGameApp::GetCurScene());
	if (!pLoginScene) {
		return;
	}

	if (const std::string strName = pSender->GetName(); strName == "btnYes") {
		if (frmKeyboard->GetIsShow()) {
			frmKeyboard->SetIsShow(false);
		}
		pLoginScene->LoginFlow();
	} else if (strName == "btnNo") {
		if (frmKeyboard->GetIsShow()) {
			frmKeyboard->SetIsShow(false);
		}
		...
		pSender->GetForm()->SetIsShow(false);
		pLoginScene->ShowServerSelection();
	} else if (strName == "btnKeyboard") {
		const bool bShow = frmKeyboard->GetIsShow();
		ShowKeyboard(!bShow);
	}
}

void CLoginScene::_evtRegionLDBDown(CGuiData* pSender, int x, int y, MouseClickState key) {
	auto pkScene = dynamic_cast<CLoginScene*>(CGameApp::GetCurScene());
	if (!pkScene) {
		return;
	}

	auto activeRegion = dynamic_cast<CList*>(pSender);
	if (!activeRegion) {
		return;
	}
	...
	for (auto region : lstRegion) {
		...
		size_t RegionIndex = GetReginIndex(info->szRegion);
...
void CLoginScene::_evtEnter(CGuiData* pSender) {
...
void CLoginScene::InitRegionServerLists() {
	...
	for (int Index = 1; Index <= MAX_REGION_LIST; ++Index) {
		...
		if (iter != ListMap.end()) {
			...
			// Initialize every server names in the list
			for (const auto& it : regionList) {
...
BOOL CLoginScene::_InitUI() {
	...
	{ // Initialize login UI
		...
		char szChkID[128] = {0};
		std::string strChkID;
		std::ifstream inCheck("user\\checkid.txt");
		if (inCheck.is_open()) {
			while (!inCheck.eof()) {
		...
		char szName[128] = {0};
		std::ifstream in("user\\username.txt");
...
bool CLoginScene::_CheckAccount() {
	// "Check the username and password locally"
	const auto username_length = strlen(edtID->GetCaption());
	if (!CommConstants::Login::UsernameLengthOK(username_length)) {
...
//			return true;
//		} else if (10004 == ret) {
//			CBoxMgr::ShowMsgBox(nullptr, RES_STRING(CL_LANGUAGE_MATCH_176), true);
//		} else {
//			std::string ret = cpai.LastError();
//			if (ret == "[Player_Missing]") {
//				CBoxMgr::ShowMsgBox(nullptr, RES_STRING(CL_LANGUAGE_MATCH_177), true);
//			} else if (ret == "[Player_Failure]") {
//				CBoxMgr::ShowMsgBox(nullptr, RES_STRING(CL_LANGUAGE_MATCH_178), true);
//			} else {
...
void CLoginScene::_evtVerErrorFrm(CCompent* pSender, int nMsgType, int x, int y, MouseClickState key) {
	g_pGameApp->SetIsRun(false);
...
void CLoginScene::ShowServerSelection() {
	CS_Disconnect(DS_DISCONN);
...
// 软键盘的鼠标点击按钮事件
void CLoginScene::_evtKeyboardFromMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (!edtFocus) {
		return;
	}
...
void CLoginScene::_evtAccountFocus(CGuiData* pSender) {
	if (auto edtTemp = dynamic_cast<CEdit*>(pSender); edtTemp) {
		edtFocus = edtTemp;
	}
}
