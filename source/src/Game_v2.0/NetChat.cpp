...
void NetFrndStart(stNetFrndStart& self, stNetFrndStart arrCha[], unsigned short arrnum, std::string groupNames[], unsigned short groupNum)
// End
{
...
void NetSessCreate(unsigned long newsessid, stNetSessCreate cha[], short chanum) {
	auto** pArrName = new stNetSessCreate*[chanum - 1];
...
extern void NetSessLeave(unsigned long sessid, unsigned long chaid) {
	...
	if (g_stUIChat._dwSelfID != chaid) {
		if (!pSession) {
			//char szBuf[2048] = { 0 };
			//_snprintf_s( szBuf, _TRUNCATE, "NetSessLeave(sessid:%d, chaid:%d, selfid:%d) error, pSession==NULL SessionCount=%d\n", sessid, chaid, g_stUIChat._dwSelfID, CTalkSessionFormMgr::GetSessionCount() );
