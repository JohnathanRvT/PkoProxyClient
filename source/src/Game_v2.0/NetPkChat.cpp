...
void CS_Say2Team(const char* pszContent) {
	...
	wpk.WriteCmd(CMD_CP_SAY2TEM);
	wpk.WriteString(pszContent);
	g_NetIF->SendPacketMessage(wpk);
}

void CS_Say2Guild(const char* pszContent) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CP_SAY2GUD);
	wpk.WriteString(pszContent);
	g_NetIF->SendPacketMessage(wpk);
}
...
void CS_Team_Invite(const char* chaname) {
	...
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CP_TEAM_INVITE);
...
void CS_Team_Refuse(unsigned long chaid) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CP_TEAM_REFUSE);
	wpk.WriteLong(chaid);
	g_NetIF->SendPacketMessage(wpk);
}
...
void CS_Team_Confirm(unsigned long chaid) {
	...
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CP_TEAM_ACCEPT);
	wpk.WriteLong(chaid);
	g_NetIF->SendPacketMessage(wpk);
}
...
void CS_Team_Kick(DWORD dwKickedID) {
	...
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CP_TEAM_KICK);
	wpk.WriteLong(dwKickedID);
	g_NetIF->SendPacketMessage(wpk);
}
...
void CP_Frnd_Del_Group(const char* groupname) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CP_FRND_DEL_GROUP);
	wpk.WriteString(groupname);
	g_NetIF->SendPacketMessage(wpk);
}

void CP_Frnd_Add_Group(const char* groupname) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CP_FRND_ADD_GROUP);
	wpk.WriteString(groupname);
	g_NetIF->SendPacketMessage(wpk);
}
...
void CP_Frnd_Move_Group(unsigned long chaid, const char* oldgroup, const char* newgroup) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CP_FRND_MOVE_GROUP);
	wpk.WriteLong(chaid);
	wpk.WriteString(oldgroup);
	wpk.WriteString(newgroup);
	g_NetIF->SendPacketMessage(wpk);
}
// End
...
void CP_Frnd_Change_Group(const char* oldgroup, const char* newgroup) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CP_FRND_CHANGE_GROUP);
	wpk.WriteString(oldgroup);
	wpk.WriteString(newgroup);
	g_NetIF->SendPacketMessage(wpk);
}
void CP_Frnd_Refresh_Info(unsigned long chaid) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CP_FRND_REFRESH_INFO);
	wpk.WriteLong(chaid);
	g_NetIF->SendPacketMessage(wpk);
}
...
void CP_Query_PersonInfo(stQueryPersonInfo* pInfo) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CP_QUERY_PERSONINFO);

	wpk.WriteString(pInfo->sChaName);		 // 名字
	wpk.WriteChar(pInfo->bHavePic);			 // 是否显示头像
	wpk.WriteString(pInfo->cSex);			 // 性别	  1是男 0是女的
	wpk.WriteLong(pInfo->nMinAge[0]);		 // 年龄查询的m_nMinAge[1]最大和m_nMinAge[0]最小值
	wpk.WriteLong(pInfo->nMinAge[1]);		 //
	wpk.WriteString(pInfo->szAnimalZodiac);  // 生肖
	wpk.WriteLong(pInfo->iBirthday[0]);		 // 查询出生年的m_nBirth[1]最大和m_nBirth[0]最小值
	wpk.WriteLong(pInfo->iBirthday[1]);		 //
	wpk.WriteString(pInfo->szState);		 // 省份
	wpk.WriteString(pInfo->szCity);			 // 城市
	wpk.WriteString(pInfo->szConstellation); // 星座
	wpk.WriteString(pInfo->szCareer);		 // 工作
	wpk.WriteLong(pInfo->nPageItemNum);		 // 每一页显示的个数
	wpk.WriteLong(pInfo->nCurPage);			 // 当前页数
...
//	//WPacket wpk	=g_NetIF->GetWPacket();
//	//wpk.WriteCmd(CMD_CP_CHANGE_PERSONINFO);
//	//wpk.WriteString(motto);
//	//wpk.WriteShort(icon);
//	//wpk.WriteChar(refuse_sess?1:0);
//
//	//g_NetIF->SendPacketMessage(wpk);
//
//	/*stPersonInfo info;
//strncpy_s(info.szMotto, sizeof(info.szMotto), pinfo,_TRUNCATE);
//	info.bShowMotto = true;
//strncpy_s(info.szSex, sizeof(info.szMotto), "性别",_TRUNCATE);
//	info.sAge = 30;
//strncpy_s(info.szName, sizeof(info.szMotto), "名字",_TRUNCATE);
//strncpy_s(info.szAnimalZodiac, sizeof(info.szMotto), "属相",_TRUNCATE);
//strncpy_s(info.szBloodType, sizeof(info.szMotto), "血型",_TRUNCATE);
//	info.iBirthday = 8<<4 + 8;
//strncpy_s(info.szState, sizeof(info.szMotto), "州（省）",_TRUNCATE);
//strncpy_s(info.szCity, sizeof(info.szMotto), "城市（区）",_TRUNCATE);
//strncpy_s(info.szConstellation, sizeof(info.szMotto), "星座",_TRUNCATE);
//strncpy_s(info.szCareer, sizeof(info.szMotto), "职业",_TRUNCATE);
//	info.iSize = 0;*/
//
//	//FILE* file;
//  //fopen_s(&file, "c:\\lark.jpg","rb");
//	//if(file)
//	//{
...
//	query.nCurPage = 1;
//strncpy_s(query.sChaName, sizeof(query.sChaName), "名字",_TRUNCATE);		// 名字
//
...
void CP_Change_PersonInfo(stPersonInfo* pInfo, const char* pOldMotto /*座右铭*/) {
	...
	wpk.WriteString(pInfo->szMotto);				 // 签名
	wpk.WriteChar(pInfo->bShowMotto);				 // 显示签名开关
	wpk.WriteString(pInfo->szSex);					 // 性别
	wpk.WriteLong(pInfo->sAge);						 // 年龄
	wpk.WriteString(pInfo->szName);					 // 名字
	wpk.WriteString(pInfo->szAnimalZodiac);			 // 属相
	wpk.WriteString(pInfo->szBloodType);			 // 血型
	wpk.WriteLong(pInfo->iBirthday);				 // 生日
	wpk.WriteString(pInfo->szState);				 // 州（省）
	wpk.WriteString(pInfo->szCity);					 // 城市（区）
	wpk.WriteString(pInfo->szConstellation);		 // 星座
	wpk.WriteString(pInfo->szCareer);				 // 职业
	wpk.WriteLong(pInfo->iSize);					 // 头像大小
	wpk.WriteSequence(pInfo->pAvatar, pInfo->iSize); // 头像
	wpk.WriteChar(pInfo->bPprevent);				 // 是否阻止消息
	wpk.WriteLong(pInfo->iSupport);					 // 鲜花数
	wpk.WriteLong(pInfo->iOppose);					 // 臭鸡蛋数

	wpk.WriteString(pOldMotto); // 座右铭
...
void CS_Sess_Create(const char* chaname[], unsigned char chanum) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CP_SESS_CREATE);
	wpk.WriteChar(chanum);
	for (char i = 0; i < chanum; i++) {
		wpk.WriteString(chaname[i]);
	}
...
void CS_Sess_Add(unsigned long sessid, const char* chaname) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CP_SESS_ADD);
	wpk.WriteLong(sessid);
	wpk.WriteString(chaname);
...
void CS_Sess_Leave(unsigned long sessid) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CP_SESS_LEAVE);
	wpk.WriteLong(sessid);
...
void CS_Sess_Say(unsigned long sessid, const char* word) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CP_SESS_SAY);
	wpk.WriteLong(sessid);
	wpk.WriteString(word);
...
BOOL PC_TEAM_REFRESH(LPRPACKET pk) {
	...
	for (unsigned char i = 0; i < l_pcteam.count; i++) {
		...
		strncpy_s(l_pcteam.cha_name[i], sizeof(l_pcteam.cha_name[i]), pk.ReadString(), _TRUNCATE);
