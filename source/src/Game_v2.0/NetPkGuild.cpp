...
void CM_GUILD_PUTNAME(bool confirm, cChar* guildname, cChar* passwd) {
	...
	wpk.WriteCmd(CMD_CM_GUILD_PUTNAME);
	wpk.WriteChar(confirm ? 1 : 0);
	wpk.WriteString(guildname);
	wpk.WriteString(passwd);
...
{
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CM_GUILD_TRYFOR);
	wpk.WriteLong(guildid);
...
{
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CM_GUILD_TRYFORCFM);
	wpk.WriteChar(confirm ? 1 : 0);
...
{
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CM_GUILD_LISTTRYPLAYER);
...
void CM_GUILD_APPROVE(uLong chaid) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CM_GUILD_APPROVE);
	wpk.WriteLong(chaid);
...
void CM_GUILD_REJECT(uLong chaid) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CM_GUILD_REJECT);
	wpk.WriteLong(chaid);
...
void CM_GUILD_KICK(uLong chaid) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CM_GUILD_KICK);
	wpk.WriteLong(chaid);
...
void CM_GUILD_LEAVE() {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CM_GUILD_LEAVE);

	g_NetIF->SendPacketMessage(wpk);
}
void CM_GUILD_DISBAND(cChar* passwd) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CM_GUILD_DISBAND);
	wpk.WriteString(passwd);
...
void CM_GUILD_MOTTO(cChar* motto) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CM_GUILD_MOTTO);
...
void CM_GUILD_CHALL(BYTE byLevel, DWORD dwMoney) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CM_GUILD_CHALLENGE);
	wpk.WriteChar(byLevel);
...
void CM_GUILD_LEIZHU(BYTE byLevel, DWORD dwMoney) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CM_GUILD_LEIZHU);
	wpk.WriteChar(byLevel);
	wpk.WriteLong(dwMoney);
