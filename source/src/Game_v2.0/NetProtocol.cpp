...
// 网络函数
//----------------------------------------------------------------------------
void NetLoginSuccess(char byPassword, size_t iCharNum, NetChaBehave chabehave[]) //角色数目 角色信息
{
#ifdef _TEST_CLIENT
	static int i = 0;
	i++;
	...
	for (size_t i = 0; i < iCharNum; i++) {
		auto part = static_cast<const stNetChangeChaPart*>(chabehave[i].sLook);
		LG("select", RES_STRING(CL_LANGUAGE_MATCH_248),
		   chabehave[i].sCharName, chabehave[i].sJob, chabehave[i].iDegree, part->sTypeID,
		   part->SLink[0].sID, part->SLink[1].sID, part->SLink[2].sID, part->SLink[3].sID, part->SLink[4].sID);
	}
...
void NetLoginFailure(unsigned short Errno) {
	...
	case ERR_AP_INVALIDPWD: {
		...
		// 以下代码用于判断密码输入错误处理
		using times = std::vector<DWORD>;
		static times error_time;
		error_time.push_back(CGameApp::GetCurTick());
		const DWORD dwCount = (DWORD)error_time.size() - 1;
		if (dwCount >= 3) {
			const DWORD dwLast = error_time[dwCount];
			const DWORD dwFirst = error_time[dwCount - 3];
...
void NetEndPlay(size_t iCharNum, NetChaBehave chabehave[]) {
#ifdef _TEST_CLIENT
	static int i = 0;
	i++;
...
void NetCreatePassword2(unsigned short Errno) {
	...
	if (Errno == ERR_SUCCESS) {
		...
		auto pSelChaScene = dynamic_cast<CSelectChaScene*>(g_pGameApp->GetCurScene());
		if (pSelChaScene) {
...
void NetActorMove(unsigned int id, stNetNotiMove& list) {
	...
	if (cha && cha->getAttachID() == id) {
		...
		if (!g_state) {
			...
			if (!cha->GetActor()->SwitchState(std::unique_ptr<CWaitMoveState>(dynamic_cast<CWaitMoveState*>(g_state)))) {
...
void stNetActorCreate::SetValue(CCharacter* pCha) {
	...
	_snprintf_s(pszLogName, sizeof(pszLogName), _TRUNCATE, "%s+%u", szName, ulWorldID);
...
void NetSynSkillState(DWORD dwCharID, stNetSkillState* pSSkillState) {
	...
	if (CCharacter* pCha = CGameApp::GetCurScene()->SearchByID(dwCharID); pCha != nullptr) {
		auto* pSynchro = new CSkillStateSynchro;
...
void NetActorSkillRep(unsigned int nID, stNetNotiSkillRepresent& SSkillRep) {
	const bool isRep = g_IsValidFightState(SSkillRep.sState);
		...
		if (!g_state) {
			...
			if (!isRep) {
				if (SSkillRep.SEffect.GetCount() > 0 || SSkillRep.SState.GetCount() > 0) {
					auto* eff = new CAttackRepSynchro;
					eff->SetSkill(p);
			...
			if (!pCha->GetActor()->AddState(std::unique_ptr<CWaitAttackState>(state))) {
...
void NetActorSkillEff(unsigned int nID, stNetNotiSkillEffect& SkillEff) {
	...
	if (SkillEff.SSrcEffect.GetCount() > 0 || SkillEff.SSrcState.GetCount() > 0 || (SkillEff.sSrcState & enumFSTATE_DIE)) {
		auto* rep = new CAttackRepSynchro;
		rep->SetSkill(pSkill);
...
void NetActorLean(unsigned int nID, stNetLeanInfo& lean) {
	// 坐,靠
	if (lean.chState == 0) {
		if (CCharacter* pCha = CGameApp::GetCurScene()->GetMainCha(); pCha && pCha->getAttachID() == nID) {
		} else {
			if (pCha = CGameApp::GetCurScene()->SearchByID(nID); !pCha) {
			...
			{
				auto st = std::make_unique<CSeatState>(pCha->GetActor());
				st->SetIsSend(false);
				...
				if (!pCha->GetActor()->AddState(std::move(st))) {
		...
		if (CCharacter* cha = GetCharacter(nID, "NetActorLean"); cha) {
			if (CActionState* st = cha->GetActor()->GetCurState(); st) {
...
void NetSwitchMap(stNetSwitchMap& switchmap) {
	...
	delete[] pMemory;
	pMemory = new char[rng.uniform(1, 777 - 1)];
	...
	if (auto pScene = dynamic_cast<CWorldScene*>(CGameApp::GetCurScene()); !pScene) { //非游戏场景
		stSceneInitParam init;
		...
		if (pScene = dynamic_cast<CWorldScene*>(g_pGameApp->CreateScene(&init)); !pScene) {
			LG("switchmap", "msgNetSwitchMap(%s) CreateScene return NULL\n", switchmap.szMapName);
			return;
		}
		g_pGameApp->GotoScene(pScene, true, switchmap.chEnterType == enumENTER_MAP_CARRY);
...
CSceneItem* NetCreateItem(stNetItemCreate& info) {
	...
	auto* pThrowItem = new CMonsterItem;
...
void NetItemDisappear(unsigned int nID) {
	...
	if (CSceneItem* pItem = CGameApp::GetCurScene()->SearchItemByID(nID); pItem) {
		pItem->SetValid(FALSE);
	}
}
...
void NetChangeChaPart(unsigned int nID, stNetLookInfo& SLookInfo) {
	if (CCharacter* pCha = CGameApp::GetCurScene()->SearchByID(nID); pCha) {
...
void NetChangeChaLookEnergy(unsigned int nID, stLookEnergy& SLookEnergy) {
	...
	for (int i = 0; i < enumEQUIP_NUM; i++) {
		if (CItemCommand* pItem = g_stUIEquip.GetEquipItem(i); pItem) {
...
void NetTempChangeChaPart(unsigned int nID, stTempChangeChaPart& SPart) {
	if (CCharacter* cha = GetCharacter(nID, "NetTempChangeChaPart"); cha) { // 临时协议，换装备
...
void NetCreateBoat(const xShipBuildInfo& Info) {
	xShipFactory* pShipFactory = static_cast<CWorldScene*>(g_pGameApp->GetCurScene())->GetShipMgr()->_factory;
	if (!pShipFactory) {
		return;
	}

	//如果已经有打开船属性窗口,将它关闭
	if (pShipFactory->sbf.wnd->GetIsShow()) {
		pShipFactory->sbf.wnd->SetIsShow(false);
	}
	pShipFactory->SetState(xShipFactory::STATE_CREATE);
	pShipFactory->SetBoatID(-1);
	pShipFactory->UpdateBoatCreate(&Info, 1, nullptr);
}

void NetUpdateBoat(const xShipBuildInfo& Info) {
	static_cast<CWorldScene*>(g_pGameApp->GetCurScene())->GetShipMgr()->_factory->Update(&Info, 1, nullptr);
}
...
void NetBoatInfo(DWORD dwBoatID, const char szName[], const xShipBuildInfo& Info) {
	...
	const xShipFactory::eState oldState = pShipFactory->GetState();
	const DWORD oldBoatID = pShipFactory->GetBoatID();

	pShipFactory->SetState(xShipFactory::STATE_INFO);
	pShipFactory->SetBoatID(dwBoatID);
	if (!pShipFactory->UpdateBoatInfo(&Info, 0, szName)) {
		pShipFactory->SetState(oldState);
		pShipFactory->SetBoatID(oldBoatID);
	}
}

void NetShowBoatList(DWORD dwNpcID, BYTE byNumBoat, const BOAT_BERTH_DATA& Data, BYTE byType) {
	auto pShipManager = static_cast<CWorldScene*>(g_pGameApp->GetCurScene())->GetShipMgr();
	pShipManager->_launch_list->SetType(byType);
	pShipManager->_launch_list->SetNpcID(dwNpcID);

	// ShowBoatList( byNumBoat, Data );
	switch (byType) {
	case mission::BERTH_LUANCH_LIST: {
		pShipManager->_launch_list->Update(byNumBoat, &Data);
	} break;
	case mission::BERTH_TRADE_LIST: {
		pShipManager->_launch_list->Update(byNumBoat, &Data, xShipLaunchList::eTrade);
	} break;
	case mission::BERTH_BAG_LIST: {
		pShipManager->_launch_list->Update(byNumBoat, &Data, xShipLaunchList::eBag);
	} break;
	case mission::BERTH_REPAIR_LIST: {
		pShipManager->_launch_list->Update(byNumBoat, &Data, xShipLaunchList::eRepair);
	} break;
	case mission::BERTH_SALVAGE_LIST: {
		pShipManager->_launch_list->Update(byNumBoat, &Data, xShipLaunchList::eSalvage);
	} break;
	case mission::BERTH_SUPPLY_LIST: {
		pShipManager->_launch_list->Update(byNumBoat, &Data, xShipLaunchList::eSupply);
	} break;
	case mission::BERTH_BOATLEVEL_LIST: {
		pShipManager->_launch_list->Update(byNumBoat, &Data, xShipLaunchList::eUpgrade);
	} break;
	}
}
...
void NetUpdateTradeAllData(const NET_TRADEINFO& TradeInfo, BYTE byCmd, DWORD dwNpcID, DWORD dwParam) {
	if (byCmd == mission::TRADE_GOODS && dwNpcID == g_stUIBourse.GetNpcId() && g_stUIBourse.IsShow()) {
		...
		g_stUIBourse.ShowBourse(TradeInfo, byCmd, dwNpcID, dwParam); // 更新全部货物
	}
}
...
void NetUpdateTradeData(DWORD dwNpcID, BYTE byPage, BYTE byIndex, USHORT sItemID, USHORT sCount, DWORD dwPrice) {
	if (dwNpcID == g_stUIBourse.GetNpcId() && g_stUIBourse.IsShow()) {
		g_stUIBourse.UpdateOneGood(byPage, byIndex, sItemID, sCount, dwPrice); // 更新单个货物
	}
}

void NetTradeResult(BYTE byCmd, BYTE byIndex, WORD wCount, USHORT sItemID, DWORD dwMoney) {
	if (byCmd == mission::TRADE_SALE) {
		g_stUINpcTrade.SaleToNpc(byIndex, wCount, sItemID, dwMoney);
	} else if (byCmd == mission::TRADE_BUY) {
		g_stUINpcTrade.BuyFromNpc(byIndex, wCount, sItemID, dwMoney);
	}
}
...
void NetTradeAddBoat(DWORD dwCharID, BYTE byOpType, USHORT sItemID, BYTE byIndex, WORD wCount, BYTE byItemIndex, const NET_CHARTRADE_BOATDATA& Data) {
	if (byOpType == mission::TRADE_DRAGTO_ITEM) { //
		g_stUITrade.DragTradeToItem(dwCharID, byIndex, byItemIndex);
	} else { // 处理船只信息
		g_stUITrade.DragItemToTrade(dwCharID, sItemID, byIndex, wCount, byItemIndex, nullptr, &Data);
	}
}

void NetTradeAddItem(DWORD dwCharID, BYTE byOpType, USHORT sItemID, BYTE byIndex, WORD wCount, BYTE byItemIndex, const NET_CHARTRADE_ITEMDATA& Data) {
	if (byOpType == mission::TRADE_DRAGTO_ITEM) {
		g_stUITrade.DragTradeToItem(dwCharID, byIndex, byItemIndex);
	} else {
		SItemGrid sGrid;
		...
		g_stUITrade.DragItemToTrade(dwCharID, sItemID, byIndex, wCount, byItemIndex, &sGrid, nullptr);
	}
}
...
void NetStallAddBoat(BYTE byGrid, USHORT sItemID, WORD wCount, DWORD dwMoney, NET_CHARTRADE_BOATDATA& Data) {
	g_stUIBooth.AddTradeBoothBoat(byGrid, sItemID, wCount, dwMoney, Data);
}

void NetStallAddItem(BYTE byGrid, USHORT sItemID, WORD wCount, DWORD dwMoney, NET_CHARTRADE_ITEMDATA& Data) {
	SItemGrid sGrid;
	memset(&sGrid, 0, sizeof(sGrid));
	sGrid.sID = sItemID;
	sGrid.sNum = wCount;
	...
	g_stUIBooth.AddTradeBoothGood(byGrid, sItemID, wCount, dwMoney, sGrid);
}

void NetStallDelGoods(DWORD dwCharID, BYTE byGrid, WORD wCount) {
	if (g_stUIBooth.IsOpen()) {
		g_stUIBooth.RemoveTradeBoothItem(dwCharID, byGrid, wCount);
	}
}
...
void NetStallName(DWORD dwCharID, const char* szStallName) {
	if (CCharacter* pCha = GetCharacter(dwCharID, "NetStallName"); pCha) {
...
void NetSynAttr(DWORD dwWorldID, char chType, short sNum, stEffect* pEffect) {
	...
	auto* pSynchro = new CAttribSynchro;
	pSynchro->SetCha(pCha);
	...
	// 乱斗白银城特殊处理
	//NOTE: Obfuscates the player look in e.g Chaos Argent
	if (g_stUIMap.IsPKSilver() && pCha->IsPlayer() && pCha->GetMainType() != enumMainPlayer && pCha->getGameAttr()) {
		SGameAttr* pAttr = pCha->getGameAttr();
		const long nJob = (long)pAttr->get(ATTR_JOB);
...
void NetFace(DWORD dwCharID, stNetFace& netface, char chType) {
	...
	case enumACTION_SKILL_POSE: {
		if (CCharacter* pCha = GetCharacter(dwCharID, "NetSkillFace"); pCha) {
			if (netface.sPose == POSE_WAITING) {
				auto st = dynamic_cast<CInsertState*>(pCha->GetActor()->GetCurState());
				st ? st->ServerEnd(0) : LG("protocol", "msgNetSkillFace Error not InsertState!\n");
			} else {
				if (!pCha->IsMainCha()) {
					...
					if (st) {
						...
						st->SetAngle(netface.sAngle);
						st->SetIsSend(false);
						pActor->SwitchState(std::unique_ptr<CInsertState>(st));
					}
				}
			}
		}
	...
	case enumACTION_FACE: {
		if (CCharacter* pCha = GetCharacter(dwCharID, "NetFace"); pCha != nullptr &&
																  pCha != CGameApp::GetCurScene()->GetMainCha()) {
			pCha->FaceTo(netface.sAngle);
			auto st = std::make_unique<CPoseState>(pCha->GetActor());
			st->SetIsSend(false);
			st->SetPose(netface.sPose);
			st->SetKeepPose(true);
			pCha->GetActor()->SwitchState(std::move(st));
		}
	} break;
	}
}
...
void NetChangeKitbag(DWORD dwChaID, stNetKitbag& SKitbag) {
	...
	// Get the affected inventory
	CGoodsGrid* grd = [&SKitbag, dwChaID] {
		switch (SKitbag.bagType) {
		case EKitBagType::temporary:
			return g_stUIStore.GetTempKitbagGrid();
		case EKitBagType::bank:
			return g_stUIBank.GetBankGoodsGrid();
		default:
			return g_stUIBoat.FindGoodsGrid(dwChaID);
		}
	}();
	if (!grd) {
		return;
	...
	const char synType = SKitbag.synType;
	// 道具栏更新前的处理
	switch (synType) {
	case enumSYN_KITBAG_INIT: {
		const int col = grd->GetCol();
		int row = SKitbag.capacity / col;
		if (SKitbag.capacity % col) {
			row++;
		}
	...
	for (int i = 0; i < count; i++) {
		...
		if (pGrid[i].SGridContent.sID > 0) {
			...
			if (item->sType == EItemType::Pet && pGrid[i].sGridID == 1 && grd == g_stUIEquip.GetGoodsGrid()) {
				g_stUIStart.RefreshPet(pObj);
			}

			if (SKitbag.bagType != EKitBagType::bank && nMarginNum > 0) { // modify by Philip.Wu  2006-06-21  拾起0个的BUG修正
				switch (synType) {
			...
			if (synType != enumSYN_KITBAG_INIT && grd == g_stUIEquip.GetGoodsGrid()) {
	...
	case enumSYN_KITBAG_INIT:
		//如果是银行，打开银行界面
		if (SKitbag.bagType == EKitBagType::bank) {
			g_stUIBank.ShowBank();
		}
		break;
...
void NetKitbagCapacity(unsigned int nID, short sKbCap) {
	if (CGoodsGrid* pGrid = g_stUIBoat.FindGoodsGrid(nID); pGrid) {
		const int col = pGrid->GetCol();
		const int row = [sKbCap, col] { int row = sKbCap / col; if(sKbCap % col){++row;} return row; }();
		pGrid->SetContent(row, col);
		pGrid->Init();
		pGrid->Refresh();
	}
}
...
void NetEspeItem(unsigned int nID, stNetEspeItem& SEspeItem) {
	if (CGoodsGrid* pGrid = g_stUIBoat.FindGoodsGrid(nID); pGrid) {
		for (size_t i = 0; i < SEspeItem.chNum; i++) {
			if (auto pItem = dynamic_cast<CItemCommand*>(pGrid->GetItem(SEspeItem.SContent[i].sPos)); pItem) {
...
void NetNpcStateChange(DWORD dwChaID, BYTE byState) {
	if (CCharacter* pCha = GetCharacter(dwChaID, "NetNpcStateChange"); pCha) {
		pCha->setNpcState(byState);
	}
}

void NetEntityStateChange(DWORD dwEntityID, BYTE byState) {
	if (CCharacter* pCha = GetCharacter(dwEntityID, "NetEntityStateChange"); pCha) {
		if (CEvent* pEvent = pCha->getEvent(); pEvent) {
...
void NetTriggerAction(stNetNpcMission& info) {
	...
	case mission::TE_KILL: {
		auto* pMission = new CMissionTrigger;
		pMission->SetData(info);

		CCharacter* pTarget = nullptr;
		if (CCharacter* pMain = CGameScene::GetMainCha(); pMain) {
			if (auto pState = dynamic_cast<CWaitAttackState*>(pMain->GetActor()->GetCurState()); pState) {
	...
	case mission::TE_GET_ITEM: {
		if (CItemRecord* pItem = GetItemRecordInfo(info.sID); pItem) {
...
void NetAreaStateBeginSee(stNetAreaState* pState) {
	...
	const int nMapWidth = pTerrain->GetWidth();
	const int x = pState->sAreaX * 200 + 50;
	const int y = pState->sAreaY * 200 + 50;
	const long lAreaID = y * nMapWidth + x;

	const int nCount = pState->chStateNum;
	if (nCount == 0) {
		CGameApp::GetCurScene()->DelAreaEff(lAreaID);
	} else {
		stAreaSkillState* p = pState->State;
		for (size_t i = 0; i < nCount; i++) {
			if (CSkillStateRecord* pInfo = GetCSkillStateRecordInfo(p[i].chID); pInfo && pInfo->sAreaEffect > 0) {
				if (CEffectObj* pEffect = CGameApp::GetCurScene()->CreateEffect(pInfo->sAreaEffect, x, y, true); pEffect) {
...
void NetAreaStateEndSee(stNetAreaState* pState) {
	...
	const int nCount = pState->chStateNum;
	const long nAreaID = pState->sAreaY * nMapWidth + pState->sAreaX;
	if (nCount == 0) {
...
void NetFailedAction(char chState) {
	if (CCharacter* pCha = CGameScene::GetMainCha(); pCha) {
...
void NetShowMessage(long lMes) {
	if (CNotifyInfo* pInfo = GetNotifyInfo(lMes); pInfo) {
		switch (pInfo->chType) { //TODO: chType is enum?
		case 0: {
		...
		default: { g_pGameApp->MsgBox(pInfo->szInfo); } break;
		}
	}
}

void NetChaTLeaderID(long lID, long lLeaderID) {
	if (CCharacter* pCha = GetCharacter(lID); pCha) {
		pCha->SetTeamLeaderID(lLeaderID);
	}
}

void NetChaEmotion(long lID, short sEmotion) {
	if (CCharacter* pCha = GetCharacter(lID, "NetChaEmotion"); pCha && pCha->IsEnabled()) {
...
void stNetPKCtrl::Exec(unsigned long ulWorldID) {
	if (CCharacter* pCha = GetCharacter(ulWorldID, "stNetPKCtrl"); pCha) {
		Exec(pCha);
	}
}

void stNetDefaultSkill::Exec() {
...
void stNetUpdateHairRes::Exec() {
	if (CCharacter* pCha = GetCharacter(ulWorldID, "stNetUpdateHairRes"); pCha) {
...
CEvent* stNetEvent::ChangeEvent() {
	if (CEvent* pEvent = CGameApp::GetCurScene()->GetEventMgr()->Search(lEntityID); pEvent) {
		pEvent->SetName(cszEventName);
		return pEvent;
	}
	return nullptr;
...
void NetChaSideInfo(long lID, stNetChaSideInfo& SNetSideInfo) {
	if (CCharacter* pCha = GetCharacter(lID, "NetChaSideInfo"); pCha) {
...
void stNetTeamFightAsk::Exec() {
	...
	if (pMain && pScene) {
		...
		if (pInfo && (_stricmp(pInfo->szDataName, "garner") == 0)) {
			const int x = pMain->GetCurX() / 100;
			const int y = pMain->GetCurY() / 100;
...
void stNetAppendLook::Exec(unsigned long ulWorldID) {
	// 目前仅考虑第一格
	if (CCharacter* pCha = GetCharacter(ulWorldID, "stNetAppendLook"); pCha) {
		Exec(pCha);
	}
}
...
void NetBeginRepairItem() {
	if (CCharacter* pCha = CGameScene::GetMainCha(); pCha) {
		auto pState = std::make_unique<CRepairState>(pCha->GetActor());
		pCha->GetActor()->SwitchState(std::move(pState));
	}
}
...
void NetItemUseSuccess(unsigned int nID, short sItemID) {
	...
	const int nEffectID = pInfo->sUseItemEffect[0];
	const int nDummy = pInfo->sUseItemEffect[1];

	if (g_stUIMap.IsPKSilver() && (200 <= nEffectID && nEffectID <= 205)) {
		return; // added by Philip.Wu  2008-01-15 乱斗白银城里不允许这些道具特效
	}

	// added by Philip.Wu  2007-12-07  烟花特效
	if ((nEffectID >= 361 && nEffectID <= 369) || (nEffectID >= 3354 && nEffectID <= 3359) || (nEffectID >= 564 && nEffectID < 600)) {
		CEffectObj* pEffect = CGameApp::GetCurScene()->GetFirstInvalidEffObj();
		if (!pEffect) {
			return;
		}
...
void NetKitbagCheckAnswer(bool bLock) {
	if (g_stUIEquip.GetIsLock() && bLock) { // 解锁失败
		g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_802));
	} else if (g_stUIEquip.GetIsLock() && !bLock) { // 解锁成功
		g_stUIDoublePwd.CloseAllForm();
	}
...
void NetChaPlayEffect(unsigned int uiWorldID, int nEffectID) {
	if (CCharacter* pCha = CGameApp::GetCurScene()->SearchByHumanID(uiWorldID); pCha) {
