...
bool CPackFile::SaveToPack(char* pszDirName, char* pszOutFile, const char* pszFilter) {
	FILE* ptempfile;
...
bool CPackFile::PackDirectory(FILE* ptf, DirectoryData* pParentDir, char* pszDir) {
	...
	_snprintf_s(t_Path, _TRUNCATE, "%s\\%s", pszDir, strFilter.c_str());
	...
	do {
		if (t_sfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
			...
			_snprintf_s(t_FilePath, _TRUNCATE, "%s\\%s", pszDir, t_sfd.cFileName);
...
bool CMiniPack::Init(char* pszMapName) {
	...
	_snprintf_s(pszPath, _TRUNCATE, R"(texture\minimap\%s\%s.pk)", pszMapName, pszMapName);
