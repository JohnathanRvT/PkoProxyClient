...
bool CS_Connect(dbc::cChar* hostname, dbc::uShort port, dbc::uLong timeout) {
	...
	g_NetIF->m_pCProCir = std::make_unique<CProCirculateCS>(g_NetIF.get());
...
void CS_Sale(DWORD dwNpcID, BYTE byIndex, WORD wCount) {
	WPacket packet = g_NetIF->GetWPacket();
	...
	packet.WriteChar(byIndex);
	packet.WriteShort(wCount);

	g_NetIF->SendPacketMessage(packet);
}

void CS_Buy(DWORD dwNpcID, BYTE byItemType, BYTE byIndex1, BYTE byIndex2, WORD wCount) {
	WPacket packet = g_NetIF->GetWPacket();
	packet.WriteCmd(CMD_CM_REQUESTTRADE);
	packet.WriteLong(dwNpcID);
	packet.WriteShort(CMD_CM_TRADEITEM);
...
void CS_SaleGoods(DWORD dwNpcID, DWORD dwBoatID, BYTE byIndex, WORD wCount) {
	WPacket packet = g_NetIF->GetWPacket();
	packet.WriteCmd(CMD_CM_REQUESTTRADE);
	packet.WriteLong(dwNpcID);
	packet.WriteShort(CMD_CM_TRADEITEM);
...
void CS_BuyGoods(DWORD dwNpcID, DWORD dwBoatID, BYTE byItemType, BYTE byIndex1, BYTE byIndex2, WORD wCount) {
	WPacket packet = g_NetIF->GetWPacket();
	packet.WriteCmd(CMD_CM_REQUESTTRADE);
	packet.WriteLong(dwNpcID);
	packet.WriteShort(CMD_CM_TRADEITEM);
...
void CS_AddItem(BYTE byType, DWORD dwCharID, BYTE byOpType, BYTE byIndex, BYTE byItemIndex, WORD wCount) {
	WPacket packet = g_NetIF->GetWPacket();
...
void CS_StallInfo(const char szName[], mission::NET_STALL_ALLDATA& Data) {
	...
	for (BYTE i = 0; i < Data.byNum; ++i) {
		...
		packet.WriteShort(Data.Info[i].wCount);
...
void CS_StallBuy(DWORD dwCharID, BYTE byIndex, WORD wCount) {
	WPacket packet = g_NetIF->GetWPacket();
...
void CP_Master_Refresh_Info(unsigned long chaid) {
	...
	wpk.WriteCmd(CMD_CP_MASTER_REFRESH_INFO);
	wpk.WriteLong(chaid);
...
void CP_Prentice_Refresh_Info(unsigned long chaid) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CP_PRENTICE_REFRESH_INFO);
	wpk.WriteLong(chaid);
	g_NetIF->SendPacketMessage(wpk);
}
...
void CS_GMSend(DWORD dwNPCID, const char* szTitle, const char* szContent) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CM_GM_SEND);
	wpk.WriteLong(dwNPCID);
	wpk.WriteString(szTitle);
	wpk.WriteString(szContent);
	g_NetIF->SendPacketMessage(wpk);
}
...
//{
//	WPacket wpk	=g_NetIF->GetWPacket();
//	wpk.WriteCmd(CMD_CM_PK_CTRL);
//	wpk.WriteChar(bCanPK? 1 : 0);
//	g_NetIF->SendPacketMessage(wpk);
//}

void CS_CheatCheck(cChar* answer) {
	WPacket wpk = g_NetIF->GetWPacket();
	wpk.WriteCmd(CMD_CM_CHEAT_CHECK);
	wpk.WriteString(answer);
	g_NetIF->SendPacketMessage(wpk);
}
...
extern void CS_LifeSkill(ELifeSkillType type, DWORD dwNPCID) {
	WPacket packet = g_NetIF->GetWPacket();
	packet.WriteCmd(CMD_CM_LIFESKILL_ASR);
	packet.WriteChar(static_cast<unsigned char>(type));
	packet.WriteLong(dwNPCID);
	g_NetIF->SendPacketMessage(packet);
}
...
void CS_Compose(DWORD dwNPCID, int* iPos, int iCount, bool asr /* = false */) {
	WPacket packet = g_NetIF->GetWPacket();
	packet.WriteCmd(asr ? CMD_CM_LIFESKILL_ASR : CMD_CM_LIFESKILL_ASK);
	packet.WriteChar(static_cast<unsigned char>(ELifeSkillType::manufacturing)); //  分支
	packet.WriteLong(dwNPCID);
...
void CS_Break(DWORD dwNPCID, int* iPos, int iCount, bool asr /* = false */) {
	WPacket packet = g_NetIF->GetWPacket();
	packet.WriteCmd(asr ? CMD_CM_LIFESKILL_ASR : CMD_CM_LIFESKILL_ASK);
	packet.WriteChar(static_cast<unsigned char>(ELifeSkillType::analyze)); //  分支
	packet.WriteLong(dwNPCID);
	for (int i = 0; i < iCount; i++) {
		packet.WriteShort((short)iPos[i]);
	}
...
void CS_Found(DWORD dwNPCID, int* iPos, int iCount, short big, bool asr /* = false */) {
	WPacket packet = g_NetIF->GetWPacket();
	packet.WriteCmd(asr ? CMD_CM_LIFESKILL_ASR : CMD_CM_LIFESKILL_ASK);
	packet.WriteChar(static_cast<unsigned char>(ELifeSkillType::crafting));
	packet.WriteLong(dwNPCID);
	for (int i = 0; i < iCount; i++) {
		packet.WriteShort((short)iPos[i]);
	}
...
void CS_Cooking(DWORD dwNPCID, int* iPos, int iCount, short percent, bool asr /* = false */) {
	WPacket packet = g_NetIF->GetWPacket();
	packet.WriteCmd(asr ? CMD_CM_LIFESKILL_ASR : CMD_CM_LIFESKILL_ASK);
	packet.WriteChar(static_cast<unsigned char>(ELifeSkillType::cooking));
	packet.WriteLong(dwNPCID);
	for (int i = 0; i < iCount; i++) {
		packet.WriteShort((short)iPos[i]);
	}
