...
#ifndef CALUA_COMPAT
#include "lua_platform.h" //CHANGED: Made it use Luajit instead.
#endif
...
BOOL SC_CHARSTR(LPRPACKET pk) {
	try {
		g_NetIF->m_chapstr = pk.ReadString();

// 网络协议是否加密
#if NET_PROTOCOL_ENCRYPT
		{
			const int nNoise = g_sKeyData * int(*(int*)(g_NetIF->m_chapstr + g_NetIF->m_chapstr.len() - 4));
...
BOOL SC_Login(LPRPACKET pk) {
	try {
		if (auto err = pk.ReadShort(); err) {
			auto errtext = pk.ReadString(); //NOTE: We do nothing with errtext
			NetLoginFailure(err);
		...
		//成功
			...
			l_chap.GetSessionClrKey((void*)l_sKey, l_keylen, (unsigned char *)l_buf, l_len);

			memset(g_NetIF->m_passwd, 0, sizeof(g_NetIF->m_passwd));
			g_NetIF->m_chapstr.clear();

			size_t l_chanum = pk.ReadChar();
			NetChaBehave l_netcha[10];
			uShort l_looklen;
			for (size_t i = 0; i < l_chanum; i++) {
				if (pk.ReadChar() == 0) {
					i--;
					l_chanum--;
					continue;
			...
			for (int i = 0; i < g_NetIF->_key_len; ++i) {
...
#else
				lua_handled_pcall(g_NetIF->g_rLvm, 2, 0, NULL);
#endif
				...
				lua_call(g_NetIF->g_sLvm, 2, 0);
#else
				lua_handled_pcall(g_NetIF->g_sLvm, 2, 0, NULL);
#endif
			}
...
BOOL SC_EnterMap(LPRPACKET pk) {
	try {
		...
		const bool bAutoLock = pk.ReadChar() ? true : false;   // 自动锁状态
		const bool bKitbagLock = pk.ReadChar() ? true : false; // 是否已锁背包
		...
		SKitbag.bagType = EKitBagType::player_or_boat;
...
BOOL SC_BeginPlay(LPRPACKET pk) {
	try {
		unsigned short l_errno = pk.ReadShort();
...
BOOL SC_ChaBeginSee(LPRPACKET pk) {
	try {
		stNetActorCreate SCreateInfo;
		const char chSeeType = pk.ReadChar();
...
BOOL SC_ChaEndSee(LPRPACKET pk) {
	try {
		...
		return TRUE;
#endif
		const char chSeeType = pk.ReadChar(); // 参见 CompCommand.h的EEntitySeenType
		const unsigned long l_id = pk.ReadLong();
...
BOOL SC_ItemCreate(LPRPACKET pk) {
	try {
		...
		if (CSceneItem* CItem = NetCreateItem(SCreateInfo); !CItem) {
			return FALSE;
		}
...
BOOL SC_ItemDestroy(LPRPACKET pk) {
	try {
		const unsigned long lID = pk.ReadLong(); //ID
...
BOOL SC_AddItemCha(LPRPACKET pk) {
	try {
		const long lMainChaID = pk.ReadLong();
		stNetActorCreate SCreateInfo;
		ReadChaBasePacket(pk, SCreateInfo);
		SCreateInfo.chSeeType = enumENTITY_SEEN_NEW;
		SCreateInfo.chMainCha = 2;
...
BOOL SC_DelItemCha(LPRPACKET pk) {
	try {
		const long lMainChaID = pk.ReadLong();
		const char chSeeType = enumENTITY_SEEN_NEW;
		const unsigned long l_id = pk.ReadLong();
		NetActorDestroy(l_id, chSeeType);
...
BOOL SC_Cha_Emotion(LPRPACKET pk) {
	try {
		unsigned long l_id = pk.ReadLong();
		unsigned short sEmotion = pk.ReadShort();
	...
	try {
		...
		// try
		{
			...
			case enumACTION_MOVE: {
				...
				Point* STurnPos;
				uShort ulTurnNum;
				STurnPos = reinterpret_cast<Point*>(const_cast<char*>(pk.ReadSequence(ulTurnNum)));
				...
				const bool isMainCha = g_LogName.IsMainCha(l_id);
				...
				if (SMoveInfo.sState & enumMSTATE_CANCEL) {
					if (CCharacter* pCMainCha = CGameScene::GetMainCha(); pCMainCha) {
						const long lDist = (pCMainCha->GetCurX() - SMoveInfo.SPos[SMoveInfo.nPointNum - 1].x) * (pCMainCha->GetCurX() - SMoveInfo.SPos[SMoveInfo.nPointNum - 1].x) + (pCMainCha->GetCurY() - SMoveInfo.SPos[SMoveInfo.nPointNum - 1].y) * (pCMainCha->GetCurY() - SMoveInfo.SPos[SMoveInfo.nPointNum - 1].y);
			...
			case enumACTION_SKILL_SRC: {
				...
				if (char chTarType = pk.ReadChar(); chTarType == 1) {
				...
				// 同步属性
				if (short lResNum = pk.ReadShort(); lResNum > 0) {
				...
				// 同步技能状态
				if (short chSStateNum = pk.ReadChar(); chSStateNum > 0) {
			...
			case enumACTION_SKILL_TAR: {
				...
				if (lResNum > 0) {
					...
					for (long i = 0; i < lResNum; i++) {
						...
						char val[32];
						char buff[234];
						_i64toa_s(SSkillInfo.SEffect[i].lVal, val, _countof(val), 10);
						_snprintf_s(buff, _TRUNCATE, "ID = %d val = %s\r\n", SSkillInfo.SEffect[i].lAttrID, val);
						::OutputDebugStr(buff);
					}
				}
				...
				if (pk.ReadChar()) {
					...
					if (pk.ReadChar() == 1) {
						// 同步技能源技能状态
						if (chSrcSStateNum = pk.ReadChar(); chSrcSStateNum > 0) {
				...
				LG(szLogName, "State: 0x%x, Double: %d, Miss: %d, Pos[%d, %d], ExecTime: %d, ResNum: %d\n",
				   SSkillInfo.sState, SSkillInfo.bDoubleAttack, SSkillInfo.bMiss, SSkillInfo.SSrcPos.x,
				   SSkillInfo.SSrcPos.y, SSkillInfo.sExecTime, SSkillInfo.SEffect.GetCount());
			...
			case enumACTION_LEAN: { // 倚靠
				stNetLeanInfo SLean;
				if (SLean.chState = pk.ReadChar(); !SLean.chState) {
					SLean.lPose = pk.ReadLong();
					SLean.lAngle = pk.ReadLong();
					SLean.lPosX = pk.ReadLong();
					SLean.lPosY = pk.ReadLong();
			...
			case enumACTION_ITEM_FAILED: {
				stNetSysInfo l_sysinfo;
				const short sFailedID = pk.ReadShort();
			...
			case enumACTION_BANK: { // 打开银行
				stNetKitbag SKitbag;
				ReadChaKitbagPacket(pk, SKitbag, szLogName);
				SKitbag.bagType = EKitBagType::bank;
				NetChangeKitbag(l_id, SKitbag);
			} break;
			...
			case enumACTION_KITBAGTMP: { // 打开临时背包
				...
				STempKitbag.bagType = EKitBagType::temporary;
...
BOOL SC_FailedAction(LPRPACKET pk) {
	try {
		...
		const char chType = pk.ReadChar();
		const char chReason = pk.ReadChar();
...
BOOL SC_SynAttribute(LPRPACKET pk) {
	try {
		...
		//char val[32];
		//char buff[245];
		//_i64toa_s(SChaAttr.SEff[0].lVal , val, _countof(val), 10);
		//_snprintf_s(buff, _TRUNCATE, "NetSynAttr %s\r\n", val);
		//OutputDebugStr(buff);
...
BOOL SC_SynTeam(LPRPACKET pk) {
	try {
		...
		for (size_t i = 0; i < std::size(STeamState.SFace.SLink); i++) {
...
BOOL SC_SynTLeaderID(LPRPACKET pk) {
	try {
		const long lID = pk.ReadLong();
		const long lLeaderID = pk.ReadLong();
...
extern bool SC_TalkInfo(LPRPACKET packet) {
	try {
		const unsigned long npcId = packet.ReadLong();
		const char byCmd = packet.ReadChar();
		if (const char* pszDesp = packet.ReadString(); pszDesp) {
			NetShowTalk(pszDesp, byCmd, npcId);
			return true;
		}
		return false;
	}
...
BOOL SC_FuncInfo(LPRPACKET packet) {
	try {
		...
		const DWORD dwNpcID = packet.ReadLong();
		const BYTE byPage = packet.ReadChar();
...
BOOL SC_CloseTalk(LPRPACKET packet) {
	try {
		const unsigned long dwNpcID = packet.ReadLong();
...
BOOL SC_TradeData(LPRPACKET packet) {
	try {
		const unsigned long dwNpcID = packet.ReadLong();
		const char byPage = packet.ReadChar();
		const char byIndex = packet.ReadChar();
		const unsigned short sItemID = packet.ReadShort();
		const unsigned short sCount = packet.ReadShort();
		const unsigned long dwPrice = packet.ReadLong();
...
BOOL SC_TradeAllData(LPRPACKET packet) {
	try {
		const unsigned long dwNpcID = packet.ReadLong();
		const char byType = packet.ReadChar();
		const unsigned long dwParam = packet.ReadLong();
		const char byCount = packet.ReadChar();

		NET_TRADEINFO Info;
		for (BYTE i = 0; i < byCount; i++) {
			switch (const char byItemType = packet.ReadChar()) {
...
BOOL SC_TradeResult(LPRPACKET packet) {
	try {
		const char byType = packet.ReadChar();
		const char byIndex = packet.ReadChar();
		const unsigned short wCount = packet.ReadShort();
		const unsigned short sItemID = packet.ReadShort();
		const unsigned long dwMoney = packet.ReadLong();
		LG("trade", RES_STRING(CL_LANGUAGE_MATCH_301), byType, byIndex, wCount, sItemID, dwMoney);
		NetTradeResult(byType, byIndex, wCount, sItemID, dwMoney);
...
BOOL SC_CharTradeInfo(LPRPACKET packet) {
	try {
		switch (USHORT usCmd = packet.ReadShort()) {
		case CMD_MC_CHARTRADE_REQUEST: {
			const BYTE byType = packet.ReadChar();
			const DWORD dwRequestID = packet.ReadLong();
			NetShowCharTradeRequest(byType, dwRequestID);
		} break;
		case CMD_MC_CHARTRADE_CANCEL: {
			const DWORD dwCharID = packet.ReadLong();
			NetCancelCharTrade(dwCharID);
		} break;
		case CMD_MC_CHARTRADE_MONEY: {
			const DWORD dwCharID = packet.ReadLong();
			const DWORD dwMoney = packet.ReadLong();
			NetTradeShowMoney(dwCharID, dwMoney);
		} break;
		case CMD_MC_CHARTRADE_ITEM: {
			const DWORD dwRequestID = packet.ReadLong();
			switch (const BYTE byOpType = packet.ReadChar()) {
			case mission::TRADE_DRAGTO_ITEM: {
				const BYTE byItemIndex = packet.ReadChar();
				const BYTE byIndex = packet.ReadChar();
				const WORD wCount = packet.ReadShort();

				// 物品实例属性
				NET_CHARTRADE_ITEMDATA Data;
				memset(&Data, 0, sizeof(NET_CHARTRADE_ITEMDATA));

				NetTradeAddItem(dwRequestID, byOpType, 0, byIndex, wCount, byItemIndex, Data);
			} break;
			case mission::TRADE_DRAGTO_TRADE: {
				const USHORT sItemID = packet.ReadShort();
				const BYTE byItemIndex = packet.ReadChar();
				const BYTE byIndex = packet.ReadChar();
				const WORD wCount = packet.ReadShort();
				const USHORT sType = packet.ReadShort();

				if (sType == EItemType::Boat) {
					...
					if (packet.ReadChar() == 0) {
						...
						NetTradeAddBoat(dwRequestID, byOpType, sItemID, byIndex, wCount, byItemIndex, Data);
					}
				} else {
					// 物品实例属性
					NET_CHARTRADE_ITEMDATA Data;
					...
					if (Data.byHasAttr) {
						for (auto& i : Data.sInstAttr) {
							i[0] = packet.ReadShort();
							i[1] = packet.ReadShort();
						}
					}

					NetTradeAddItem(dwRequestID, byOpType, sItemID, byIndex, wCount, byItemIndex, Data);
				}
			} break;
			default: {
		...
		case CMD_MC_CHARTRADE_PAGE: {
			const BYTE byType = packet.ReadChar();
			const DWORD dwAcceptID = packet.ReadLong();
			const DWORD dwRequestID = packet.ReadLong();
		...
		case CMD_MC_CHARTRADE_RESULT: {
			const BYTE byResult = packet.ReadChar();
...
BOOL SC_MisPage(LPRPACKET packet) {
	try {
		const BYTE byCmd = packet.ReadChar();
		const DWORD dwNpcID = packet.ReadLong();
		NET_MISPAGE page;
...
BOOL SC_MisLogInfo(LPRPACKET packet) {
	try {
		...
		// 任务基本信息
		const WORD wMisID = packet.ReadShort();
		const char* pszName = packet.ReadString();
		strncpy_s(page.szName, ROLE_MAXSIZE_MISNAME - 1, pszName, _TRUNCATE);
...
BOOL SC_MisLogAdd(LPRPACKET packet) {
	try {
		const WORD wMisID = packet.ReadShort();
		const BYTE byState = packet.ReadChar();
...
BOOL SC_MisLogState(LPRPACKET packet) {
	try {
		const WORD wID = packet.ReadShort();
		const BYTE byState = packet.ReadChar();
...
BOOL SC_EntityStateChange(LPRPACKET packet) {
	try {
		const DWORD dwEntityID = packet.ReadLong();
		const BYTE byState = packet.ReadChar();
...
BOOL SC_Amphitheater(LPRPACKET packet) {
	try {
		...
		if (const BYTE byState = packet.ReadChar(); byState) {
...
BOOL SC_CreateBoat(LPRPACKET packet) {
	try {
		const DWORD dwBoatID = packet.ReadLong();
		xShipBuildInfo Info;
		...
		if (Info.byIsUpdate) {
			...
			for (unsigned long& i : Info.dwMotor) {
				i = packet.ReadLong();
			}
		}
...
BOOL SC_BoatList(LPRPACKET packet) {
	try {
		const DWORD dwNpcID = packet.ReadLong();
		const BYTE byType = packet.ReadChar();
		const BYTE byNumBoat = packet.ReadChar();
...
//	for( BYTE i = 0; i < byNumBoat; i++ )
//	{
//	strncpy_s( Data.szName[i], BOAT_MAXSIZE_BOATNAME - 1 , packet.ReadString(),_TRUNCATE);
//	}
//
...
BOOL SC_StallInfo(LPRPACKET packet) {
	try {
		const DWORD dwCharID = packet.ReadLong();
		const BYTE byNum = packet.ReadChar();
		const char* pszName = packet.ReadString();
		if (!pszName) {
			return FALSE;
		}
		...
		for (BYTE i = 0; i < byNum; ++i) {
			const BYTE byGrid = packet.ReadChar();
			const USHORT sItemID = packet.ReadShort();
			const WORD wCount = packet.ReadShort();
			const DWORD dwMoney = packet.ReadLong();
			const USHORT sType = packet.ReadShort();
			...
			if (sType == EItemType::Boat) {
				...
				if (packet.ReadChar() == 0) {
					...
					Data.byCapacity = packet.ReadChar();
					Data.dwPrice = packet.ReadLong();
					NetStallAddBoat(byGrid, sItemID, wCount, dwMoney, Data);
				}
			} else {
				// 物品实例属性
				NET_CHARTRADE_ITEMDATA Data;
				...
				NetStallAddItem(byGrid, sItemID, wCount, dwMoney, Data);
			}
		}
		return TRUE;
	}
...
BOOL SC_StallDelGoods(LPRPACKET packet) {
	try {
		...
		const BYTE byGrid = packet.ReadChar();
		const WORD wCount = packet.ReadShort();
		NetStallDelGoods(dwCharID, byGrid, wCount);
		return TRUE;
	}
	T_E
}
...
BOOL SC_StartExit(LPRPACKET packet) {
	try {
		const DWORD dwExitTime = packet.ReadLong();
...
BOOL SC_ItemAmphitheaterAnswer(LPRPACKET packet) {
	try {
		...
		if (byState) {
			const int ProSize = (int)packet.ReadChar();
			for (int i = 0; i < ProSize; i++) {
				TeamProUnit Punit;
				//unit.PromID  = (int)packet.ReadChar();
				//long captain = packet.ReadLong();
			...
			const int RelSize = (int)packet.ReadChar();
			for (int i = 0; i < RelSize; i++) {
				TeamRelUnit Runit;
				strncpy_s(Runit.RelCapName, sizeof(Runit.RelCapName), packet.ReadString(), _TRUNCATE);
				Runit.Ballot = (int)packet.ReadLong();
...
BOOL SC_JewelryUpAnswer(LPRPACKET packet) {
	const unsigned char result = packet.ReadChar();
	if (result == '1') {
		const long OldJeyID = packet.ReadLong();
		const long OldJeyLev = packet.ReadLong();
		const long NewJeyID = packet.ReadLong();
		const long NewJeyLev = packet.ReadLong();
...
BOOL SC_ItemUseSuc(LPRPACKET packet) {
	try {
		const unsigned int nChaID = packet.ReadLong();
		const short sItemID = packet.ReadShort();
...
BOOL SC_KitbagCapacity(LPRPACKET packet) {
	try {
		const unsigned int nChaID = packet.ReadLong();
		const short sKbCap = packet.ReadShort();
...
BOOL SC_EspeItem(LPRPACKET packet) {
	try {
		...
		for (auto& i : SEspItem.SContent) {
			i.sPos = packet.ReadShort();
			i.sEndure = packet.ReadShort();
			i.sEnergy = packet.ReadShort();
		}
...
BOOL SC_QueryCha(LPRPACKET pk) {
	try {
		...
		const long lPosX = pk.ReadLong();
		const long lPosY = pk.ReadLong();
		const long lChaID = pk.ReadLong();
		_snprintf_s(szInfo, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_308),
					pChaName, lChaID, pMapName, lPosX, lPosY);
...
BOOL SC_QueryChaPing(LPRPACKET pk) {
	try {
		...
		const char* pChaName = pk.ReadString();
		const char* pMapName = pk.ReadString();
		const long lPing = pk.ReadLong();
		_snprintf_s(szInfo, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_309),
					pMapName, lPing);
		SShowInfo.m_sysinfo = szInfo;
		NetSysInfo(SShowInfo);
...
BOOL SC_PreMoveTime(LPRPACKET pk) {
	try {
		const unsigned long ulPreMoveTime = pk.ReadLong();
...
BOOL SC_MapMask(LPRPACKET pk) {
	try {
		const uLong l_id = pk.ReadLong();
		uShort usLen = 0;
		BYTE* pMapMask = nullptr;
		if (pk.ReadChar()) {
...
BOOL SC_StoreOpenAnswer(LPRPACKET packet) {
	try {
		const bool bValid = packet.ReadChar() ? true : false; // 商城是否开启
		if (bValid) {
			g_stUIStore.ClearStoreTreeNode();
			g_stUIStore.ClearStoreItemList();
			//g_stUIStore.SetStoreBuyButtonEnable(true);

			const long lVip = packet.ReadLong();	  // VIP
			const long lMoBean = packet.ReadLong();   // 摩豆
			const long lRplMoney = packet.ReadLong(); // 代币（水晶）
			g_stUIStore.SetStoreMoney(lMoBean, lRplMoney);
			g_stUIStore.SetStoreVip(lVip);

			const long lAfficheNum = packet.ReadLong(); // 公告数量
			int i;
			for (i = 0; i < lAfficheNum; i++) {
				const long lAfficheID = packet.ReadLong(); // 公告ID
				uShort len;
				const char* szTitle = packet.ReadSequence(len); // 公告标题
				const char* szComID = packet.ReadSequence(len); // 对应商品ID,用逗号隔开

				// 添加公告
			}
			long lFirstClass = 0;
			const long lClsNum = packet.ReadLong(); // 分类数量
			for (i = 0; i < lClsNum; i++) {
				const short lClsID = packet.ReadShort(); // 分类ID
				uShort len;
				const char* szClsName = packet.ReadSequence(len); // 分类名
				const short lParentID = packet.ReadShort();		  // 父类ID
...
BOOL SC_StoreListAnswer(LPRPACKET packet) {
	try {
		...
		const short sPageNum = packet.ReadShort(); // 页数
		const short sPageCur = packet.ReadShort(); // 当前页
		const short sComNum = packet.ReadShort();  // 商品数

		long i;
		for (i = 0; i < sComNum; i++) {
			const long lComID = packet.ReadLong(); // 商品ID
			uShort len;
			const char* szComName = packet.ReadSequence(len);	// 商品名称
			const long lPrice = packet.ReadLong();				 // 商品价格
			const char* szRemark = packet.ReadSequence(len);	 // 商品备注
			const bool isHot = packet.ReadChar() ? true : false; // 是否是热卖商品
			const long nTime = packet.ReadLong();
			const long lComNumber = packet.ReadLong(); // 商品剩余个数。-1不限制
			const long lComExpire = packet.ReadLong(); // 商品到期时间
			...
			for (j = 0; j < sItemClsNum; j++) {
				const short sItemID = packet.ReadShort();  // 道具ID
				const short sItemNum = packet.ReadShort(); // 道具数量
				const short sFlute = packet.ReadShort();   // 凹槽数量
				short pItemAttrID[5];
...
BOOL SC_StoreBuyAnswer(LPRPACKET packet) {
	try {
		const bool bSucc = packet.ReadChar() ? true : false; // 购买是否成功
		long lMoBean = 0;
		long lRplMoney = 0;
...
BOOL SC_StoreChangeAnswer(LPRPACKET packet) {
	try {
		const bool bSucc = packet.ReadChar() ? true : false; // 兑换代币是否成功
		if (bSucc) {
...
BOOL SC_StoreHistory(LPRPACKET packet) {
	try {
		const long lNum = packet.ReadLong(); // 获取的交易记录数量
		for (int i = 0; i < lNum; i++) {
			uShort len;
			const char* szTime = packet.ReadSequence(len); // 交易时间
			const long lComID = packet.ReadLong();		   // 商品ID
			const char* szName = packet.ReadSequence(len); // 商品名称
			const long lMoney = packet.ReadLong();		   // 交易金额
		}
		return TRUE;
	}
	T_E
...
BOOL SC_ActInfo(LPRPACKET packet) {
	try {
		const bool bSucc = packet.ReadChar() ? true : false; // 帐号信息查询是否成功
		if (bSucc) {
			const long lMoBean = packet.ReadLong();   // 摩豆数量
			const long lRplMoney = packet.ReadLong(); // 代币数量
		}
		return TRUE;
	}
	T_E
...
BOOL SC_StoreVIP(LPRPACKET packet) {
	try {
		const bool bSucc = packet.ReadChar() ? true : false;
		if (bSucc) {
			const short sVipID = packet.ReadShort();
			const short sMonth = packet.ReadShort();
			const long lShuijing = packet.ReadLong();
			const long lModou = packet.ReadLong();
...
BOOL SC_BlackMarketExchangeData(LPRPACKET packet) {
	try {
		...
		const short sCount = packet.ReadShort();
...
BOOL SC_BlackMarketExchangeAsr(LPRPACKET packet) {
	try {
		const bool bSucc = (packet.ReadChar() == 1) ? true : false;
		if (bSucc) {
			stBlackTrade SBlackTrade;
			memset(&SBlackTrade, 0, sizeof(stBlackTrade));
...
BOOL SC_TigerItemID(LPRPACKET packet) {
	try {
		const short sNum = packet.ReadShort(); // 次数
		short sItemID[3] = {0};
		for (short& i : sItemID) {
			i = packet.ReadShort();
		}
...
BOOL SC_VolunteerList(LPRPACKET packet) {
	try {
		const short sPageNum = packet.ReadShort(); //总页数
		const short sPage = packet.ReadShort();	//当前页数
		const short sRetNum = packet.ReadShort();  //志愿者数量
		...
		for (int i = 0; i < sRetNum; i++) {
			...
			const long level = packet.ReadLong();
			const long job = packet.ReadLong();
...
BOOL SC_VolunteerState(LPRPACKET packet) {
	try {
		const bool bState = (packet.ReadChar() == 0) ? false : true;
...
BOOL SC_SyncKitbagTemp(LPRPACKET packet) {
	...
	SKitbagTemp.bagType = EKitBagType::temporary;
...
BOOL SC_CaptainAsk(LPRPACKET packet) {
	const DWORD dwTeamID = packet.ReadLong();
...
BOOL PC_MasterRefresh(LPRPACKET packet) {
	switch (unsigned char l_type = packet.ReadChar()) {
	case MSG_MASTER_REFRESH_ONLINE: {
		const unsigned long ulChaID = packet.ReadLong();
	...
	case MSG_MASTER_REFRESH_ADD: {
		const char* l_grp = packet.ReadString();
		const unsigned long l_chaid = packet.ReadLong();
		const char* l_chaname = packet.ReadString();
		const char* l_motto = packet.ReadString();
		const unsigned short l_icon = packet.ReadShort();
	...
	case MSG_MASTER_REFRESH_START: {
		...
		unsigned short l_nfnum = 0, l_grpnum = packet.ReadShort();
		for (uShort l_grpi = 0; l_grpi < l_grpnum; l_grpi++) {
			const char* l_grp = packet.ReadString();
			unsigned short l_grpmnum = packet.ReadShort();
			for (unsigned short l_grpmi = 0; l_grpmi < l_grpmnum; l_grpmi++) {
	...
	case MSG_PRENTICE_REFRESH_START: {
		...
		stNetFrndStart l_nfs[100];
		unsigned short l_nfnum = 0, l_grpnum = packet.ReadShort();
		for (unsigned short l_grpi = 0; l_grpi < l_grpnum; l_grpi++) {
			const char* l_grp = packet.ReadString();
			const unsigned short l_grpmnum = packet.ReadShort();
			for (unsigned short l_grpmi = 0; l_grpmi < l_grpmnum; l_grpmi++) {
				unsigned short index = l_grpmi % (sizeof(l_nfs) / sizeof(l_nfs[0]));
...
BOOL PC_MasterCancel(LPRPACKET packet) {
	const unsigned char reason = packet.ReadChar();
	const unsigned long ulChaID = packet.ReadLong();
...
BOOL PC_MasterRefreshInfo(LPRPACKET packet) {
	...
	const char* l_motto = packet.ReadString();
	const unsigned short l_icon = packet.ReadShort();
	const unsigned short l_degr = packet.ReadShort();
...
BOOL SC_ChaPlayEffect(LPRPACKET packet) {
	const unsigned int uiWorldID = packet.ReadLong();
	const int nEffectID = packet.ReadLong();
...
BOOL SC_Say2Camp(LPRPACKET packet) {
	const char* szName = packet.ReadString();
	const char* szContent = packet.ReadString();
...
BOOL SC_GMMail(LPRPACKET packet) {
	const char* szTitle = packet.ReadString();
	const char* szContent = packet.ReadString();
	const long lTime = packet.ReadLong();
...
BOOL SC_CheatCheck(LPRPACKET packet) {
	const short count = packet.ReadShort(); // 图片数量
	for (int i = 0; i < count; i++) {
		const short size = packet.ReadShort(); // 图片大小
		auto picture = std::make_unique<char[]>(size);
		for (int j = 0; j < size; j++) {
			picture[j] = packet.ReadChar();
		}
		g_stUINumAnswer.SetBmp(i, reinterpret_cast<BYTE*>(picture.get()), size);
	}
...
BOOL SC_ListAuction(LPRPACKET pk) {
	const short sNum = pk.ReverseReadShort(); // 竞拍数量
	stChurchChallenge stInfo;

	for (int i = 0; i < sNum; i++) {
		const short sItemID = pk.ReadShort();	// id
		const char* szName = pk.ReadString();	// name
		const char* szChaName = pk.ReadString(); // 挑战者
		const short sCount = pk.ReadShort();	 // 数量
		const long baseprice = pk.ReadLong();	// 底价
		const long minbid = pk.ReadLong();		 // 最低出价
		const long curprice = pk.ReadLong();	 // 当前竞拍价
...
void ReadChaBasePacket(LPRPACKET pk, stNetActorCreate& SCreateInfo) {
	try {
		...
		SCreateInfo.szCommName = (char*)pk.ReadString(); //FIXED: Added (char *) to make sure it is interpreted properly.
		...
		SCreateInfo.strMottoName = static_cast<std::string>(pk.ReadString()); // 读出座右铭 //FIXED: Strings were corrupted (wrong type)
		SCreateInfo.sIcon = pk.ReadShort();									  // 角色聊天头像
		SCreateInfo.lGuildID = pk.ReadLong();
		SCreateInfo.strGuildName = static_cast<std::string>(pk.ReadString());  // 工会名 //FIXED: Strings were corrupted (wrong type)
		SCreateInfo.strGuildMotto = static_cast<std::string>(pk.ReadString()); // 工会座右铭 //FIXED: Strings were corrupted (wrong type)
		SCreateInfo.strStallName = static_cast<std::string>(pk.ReadString());  // 工会座右铭 //FIXED: Strings were corrupted (wrong type)
...
BOOL ReadChaSkillBagPacket(LPRPACKET pk, stNetSkillBag& SCurSkill, const char* szLogName) {
	try {
		...
		const short sSkillNum = pk.ReadShort();
		if (sSkillNum <= 0) {
			return TRUE;
		}
		...
		char szRange[256];
		for (size_t i = 0; i < sSkillNum; i++) {
			_snprintf_s(szRange, _TRUNCATE, "%d", pSBag[i].sRange[0]);
			if (pSBag[i].sRange[0] != enumRANGE_TYPE_NONE) {
				for (short j = 1; j < defSKILL_RANGE_PARAM_NUM; j++) {
					_snprintf_s(szRange + strlen(szRange), sizeof(szRange + strlen(szRange)), _TRUNCATE, ",%d", pSBag[i].sRange[j]);
				}
			}
...
extern void ReadChaLookPacket(LPRPACKET pk, stNetLookInfo& SLookInfo, const char* szLogName) {
	try {
		...
		if (pk.ReadChar() == 1) { // 船的外观
			...
			for (auto& item : SChaPart.SLink) {
				item.sID = pk.ReadShort();
				item.dwDBID = pk.ReadLong();
				if (item.sID == 0) {
					memset(&item, 0, sizeof(SItemGrid));
					continue;
				}
				if (SLookInfo.chSynType == enumSYN_LOOK_CHANGE) {
					item.sEndure[0] = pk.ReadShort();
					item.sEnergy[0] = pk.ReadShort();
					item.SetValid(pk.ReadChar() != 0 ? true : false);
					continue;
				} else {
					item.sNum = pk.ReadShort();
					item.sEndure[0] = pk.ReadShort();
					item.sEndure[1] = pk.ReadShort();
					item.sEnergy[0] = pk.ReadShort();
					item.sEnergy[1] = pk.ReadShort();
					item.chForgeLv = pk.ReadChar();
					item.SetValid(pk.ReadChar() != 0 ? true : false);
				}
				...
				item.SetDBParam(enumITEMDBP_FORGE, pk.ReadLong());
				item.SetDBParam(enumITEMDBP_INST_ID, pk.ReadLong());
				if (pk.ReadChar()) { // 存在实例属性
					for (auto& j : item.sInstAttr) {
						j[0] = pk.ReadShort();
						j[1] = pk.ReadShort();
					}
				}
...
extern void ReadChaLookEnergyPacket(LPRPACKET pk, stLookEnergy& SLookEnergy, const char* szLogName) {
	try {
		SLookEnergy = {};
		for (auto& energy : SLookEnergy.sEnergy) {
			energy = pk.ReadShort();
		}
	}
	T_E
...
extern void ReadChaPKPacket(LPRPACKET pk, stNetPKCtrl& SNetPKCtrl, const char* szLogName) {
	try {
		const char chPKCtrl = pk.ReadChar();
...
extern void ReadChaKitbagPacket(LPRPACKET pk, stNetKitbag& SKitbag, const char* szLogName) {
	try {
		SKitbag = {};
		SKitbag.synType = static_cast<ESynKitbagType>(pk.ReadChar());
		int nGridNum = 0;
		if (SKitbag.synType == enumSYN_KITBAG_INIT) { // 道具栏容量
			SKitbag.capacity = pk.ReadShort();
		}
		LG(szLogName, "===Recieve(Update Kitbag):\tGridNum:%d\tType:%d\tTick:[%u]\n", SKitbag.capacity, SKitbag.synType, GetTickCount());
		stNetKitbag::stGrid* Grid = SKitbag.Grid;
		SItemGrid* pItem;
		CItemRecord* pItemRec;

		while (true) {
			const short sGridID = pk.ReadShort();
			if (sGridID < 0) {
				break;
			}
			...
			if (pItem->sID > 0) { // 存在道具
				...
				if (pItemRec == nullptr) {
					char szBuf[256] = {0};
					_snprintf_s(szBuf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_315), pItem->sID);
					MessageBox(nullptr, szBuf, "Error", 0);
#ifdef USE_DSOUND
					if (g_dwCurMusicID) {
				...
				if (pItemRec->sType == EItemType::Boat) { // 船道具，写入船的WorldID，用于客户端将道具与船角色挂钩
					pItem->SetDBParam(enumITEMDBP_INST_ID, pk.ReadLong());
				}

				pItem->SetDBParam(enumITEMDBP_FORGE, pk.ReadLong());
				if (pItemRec->sType == EItemType::Boat) {
					pk.ReadLong();
				} else {
					pItem->SetDBParam(enumITEMDBP_INST_ID, pk.ReadLong());
				}
				...
				if (pk.ReadChar()) { // 存在实例属性
					for (auto& j : pItem->sInstAttr) {
						j[0] = pk.ReadShort();
						j[1] = pk.ReadShort();
						LG(szLogName, RES_STRING(CL_LANGUAGE_MATCH_317), j[0], j[1]);
					}
				}
			}
			nGridNum++;
...
BOOL SC_LeaveExpShow(LPRPACKET packet) {
	try {
		const short sSelectTime = packet.ReadShort();
		const std::string baseExp = packet.ReadString();
		const std::string addExp = packet.ReadString();
		const std::string allExp = packet.ReadString();
		const std::string addLev = packet.ReadString();
		const std::string addPercent = packet.ReadString();
		const short sNum = packet.ReadShort();
		const std::string amplitude = packet.ReadString();
		const short coin = packet.ReadShort();
...
BOOL SC_LifeSkillShow(LPRPACKET packet) {
	try {
		using type = ELifeSkillType; // mimic C++20 'using enum declaration'
		switch (static_cast<type>(packet.ReadChar())) {
...
BOOL SC_LifeSkill(LPRPACKET packet) {
	try {
		...
		const auto skillType = static_cast<type>(packet.ReadChar());
		const auto ret = packet.ReadShort();
		const std::string txt = packet.ReadString();
...
BOOL SC_LifeSkillAsr(LPRPACKET packet) {
	try {
		using type = ELifeSkillType; // mimic C++20 'using enum declaration'
		const auto t = static_cast<type>(packet.ReadChar());
		const auto tim = packet.ReadShort();
		const std::string txt = packet.ReadString();
...
BOOL SC_DropLockAsr(LPRPACKET pk) {
	try {
		//
		const char success = pk.ReadChar();
