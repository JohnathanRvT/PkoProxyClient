...
void CProCirculateCS::BeginAction(CCharacter* pCha, DWORD type, const void* param, CActionState* pState) {
	...
	try {
		...
		case enumACTION_SKILL: {
			auto pSkill = static_cast<const stNetSkillInfo*>(param);
			...
			if (pSkill->chMove == 2) {
				...
				for (size_t i = 0; i < pSkill->SMove.pos_num; i++) {
		...
		case enumACTION_STOP_STATE: {
			pk.WriteShort(*(static_cast<const short*>(param)));
			pCNetIf->SendPacketMessage(pk);
		...
		case enumACTION_LEAN: { // 倚靠
			auto pSLean = static_cast<const stNetLeanInfo*>(param);
		...
		case enumACTION_ITEM_PICK: { // 捡道具
			auto pPick = static_cast<const stNetItemPick*>(param);
		...
		case enumACTION_TOTAL_ITEM_PICK: {
			auto pPickinfo = static_cast<const std::vector<stNetItemPick>*>(param);
			pk.WriteShort(static_cast<uShort>((*pPickinfo).size()));
			for (auto& info : *pPickinfo) {
				pk.WriteLong(info.lWorldID);
				pk.WriteLong(info.lHandle);
			}
			pCNetIf->SendPacketMessage(pk);
		} break;
		case enumACTION_ITEM_THROW: { // 丢道具
			auto pThrow = static_cast<const stNetItemThrow*>(param);
		...
		case enumACTION_ITEM_USE: {
			auto pUseItem = static_cast<const stNetUseItem*>(param);
		...
		case enumACTION_ITEM_UNFIX: { // 卸装道具
			auto pUnfix = static_cast<const stNetItemUnfix*>(param);
		...
		case enumACTION_ITEM_POS: {
			auto pChangePos = static_cast<const stNetItemPos*>(param);
		...
		case enumACTION_ITEM_DELETE: {
			auto pDelItem = static_cast<const stNetDelItem*>(param);
		...
		case enumACTION_ITEM_INFO: {
			auto pItemInfo = static_cast<const stNetItemInfo*>(param);
		...
		case enumACTION_SHORTCUT: { // 更新快捷栏
			auto pShortcutChange = static_cast<const stNetShortCutChange*>(param);
		...
		case enumACTION_LOOK: { // 更新外观（如船的换装）
			auto pSChaPart = static_cast<const stNetChangeChaPart*>(param);
			pk.WriteShort(pSChaPart->sTypeID);
			for (const auto& item : pSChaPart->SLink) {
				pk.WriteShort(item.sID);
			}
			pCNetIf->SendPacketMessage(pk);
		...
		case enumACTION_TEMP: { //临时换装协议
			auto pSTempChaPart = static_cast<const stTempChangeChaPart*>(param);
		...
		case enumACTION_EVENT: { // 触发事件
			auto pEvent = static_cast<const stNetActivateEvent*>(param);
		...
		case enumACTION_FACE: {
			auto pNetFace = static_cast<const stNetFace*>(param);
		...
		case enumACTION_BANK: {
			auto pNetBank = static_cast<const stNetBank*>(param);
			pk.WriteChar(static_cast<char>(pNetBank->chSrcType));
			pk.WriteChar(static_cast<char>(pNetBank->chTarType));
		...
		case enumACTION_KITBAGTMP_DRAG: { // 拖动临时背包
			auto pNetTempKitbag = static_cast<const stNetTempKitbag*>(param);
		...
		MessageBox(nullptr, "!!!!!!!!!!!!!!!!!!!!exception: Begin Action", "error", 0);
	}
}
...
void CProCirculate::Login(const char* accounts, const char* password, const char* passport) {
	...
	char l_buf[1024] = {0};
	int l_len;
	l_chap.GetStep1Data((unsigned char *)l_buf, l_len);

	extern short g_sClientVer;
	WPacket pk = pCNetIf->GetWPacket();
	pk.WriteCmd(CMD_CM_LOGIN);
	//Add by sunny.sun 20081223
	std::string szLocale = CResourceBundleManage::Instance()->GetLocale().getName();
	...
	std::string strMac = GetMacString();
	if (strMac.empty()) {
		strMac = "Unknown";
	}
