...
HRESULT _DbgOuts(char* strFile, DWORD dwLine, HRESULT hr, char* strMsg) {
	char buffer[256];
...
bool CSMallWnd::Create(LPDIRECT3DDEVICEX pDev, RECT rcwnd, CGameScene* pScene, int bksize) {
	m_pDev = pDev;
	_rcWnd = rcwnd;
...
void CSMallMap2D::InitScene() {
...
#if (defined LW_USE_DX9)
		_dwVShaderD = ResMgr.GetMinimapVSD();
#endif
	}
	//#ifdef MGR
	//#else
	...
	//		_dwVShader = 0;
	//	}
	//	_snprintf_s(t_Path,_TRUNCATE,"shader\\minimap.vsh");
	//	if(SUCCEEDED(D3DXAssembleShaderFromFileX( t_Path, NULL, NULL, NULL, &pCode, NULL )))
	//	{
...
void CSMallMap2D::RenderScene() {
	...
	_pVB->BeginSet();
#else
	m_pDev->SetStreamSource(0, _pVB, 0, sizeof(M2D_VER));
#endif
	...
	if (_dwVShader == nullptr) {
		...
		D3DXMatrixTranspose(&vpPose, &v);

#if (defined LW_USE_DX9)
		g_Render.SetVertexShader(_dwVShader);
		g_Render.SetVertexDeclaration(_dwVShaderD);
#elif (defined LW_USE_DX8)
		g_Render.SetVertexShader(*_dwVShader);
#endif
		g_Render.SetVertexShaderConstantX(0, &matIdentity, 4);
		g_Render.SetVertexShaderConstantX(4, &vpPose, 4);
		g_Render.SetVertexShaderConstantX(8, &_dwColor, 1);
	}
	...
	for (m = 0; m < 3; m++) {
		for (int n = 0; n < 3; n++) {
			_snprintf_s(filename, _TRUNCATE, R"(texture\minimap\%s\sm%d%d.bmp)",
			...
			if (_dwVShader == nullptr) {
				...
				const D3DXVECTOR4 v[] = {
					D3DXVECTOR4(float(sx), float(sy), 0, 1),
				...
				g_Render.SetVertexShaderConstantX(9, &v[0], 1);
				g_Render.SetVertexShaderConstantX(10, &v[1], 1);

				g_Render.SetVertexShaderConstantX(11, &v[2], 1);
				g_Render.SetVertexShaderConstantX(12, &v[3], 1);
			}
#ifdef MGR
			_pVB->DrawSubset(0);
#else
	...
	if (_dwVShader != nullptr) {
#if (defined LW_USE_DX9)
		_Cha.Begin(_dwVShader, _dwVShaderD, NULL);
#elif (defined LW_USE_DX8)
...
#if (defined LW_USE_DX9)
		_Cha.Begin(nullptr, nullptr, D3DFVF_M2D);
#elif (defined LW_USE_DX8)
...
void CSMallMap2D::RenderMask() {
	...
	m_pDev->SetStreamSource(0, _pVBWnd, 0, sizeof(M2D_VER));
...
void CAniWnd::Render() {
	if (_bUpdate) {
		LPDIRECT3DSURFACEX pSaveSuf = nullptr;
		LPDIRECT3DSURFACEX dsface = nullptr;
		LPDIRECT3DSURFACEX surface = nullptr;

		m_pDev->GetRenderTargetX(NULL, &pSaveSuf);
		m_pDev->GetDepthStencilSurface(&dsface);
#ifdef MGR
		_pCurSuf->GetTex()->GetSurfaceLevel(0, &surface);
#if (defined LW_USE_DX9)
		m_pDev->SetRenderTarget(NULL, surface);
		m_pDev->SetDepthStencilSurface(_pDSSuf->GetSurface()); // ViM
#elif (defined LW_USE_DX8)
...
#if (defined LW_USE_DX9)
		m_pDev->SetRenderTarget(NULL, surface);
		m_pDev->SetDepthStencilSurface(_pDSSuf); // ViM
#elif (defined LW_USE_DX8)
		m_pDev->SetRenderTarget(surface, _pDSSuf);
#endif
#endif
		RenderScene();
#if (defined LW_USE_DX9)
		m_pDev->SetRenderTarget(NULL, pSaveSuf);
		m_pDev->SetDepthStencilSurface(dsface); // ViM
#elif (defined LW_USE_DX8)
		m_pDev->SetRenderTarget(pSaveSuf, dsface);
#endif
...
void CAniWnd::MoveWnd(int x, int y) {
	...
	(*pVers++).SetValue(v[0], 0x80ffffff, vUV[0]);
	(*pVers++).SetValue(v[1], 0x80ffffff, vUV[1]);
	(*pVers++).SetValue(v[2], 0x80ffffff, vUV[2]);
	(*pVers++).SetValue(v[3], 0x80ffffff, vUV[3]);
#ifdef MGR
	_lpSVB->Unlock();
#else
	_pVBWnd->Unlock();
...
void CAniWnd::RenderMask() {
	...
	m_pDev->SetStreamSource(0, _pVBWnd, 0, sizeof(M2D_AVER));
...
void CAniClock::Render(int x, int y) {
	...
	g_Render.SetVertexShader(nullptr);
#endif
	g_Render.SetFVFX(D3DFVF_CLOCK2);
	for (int n = 0; n < 10; ++n) {
...
void CBigMap::Create() {
	...
	for (int m = 0; m < 3; ++m) {
		for (int n = 0; n < 4; ++n) {
			_snprintf_s(filename, _TRUNCATE, "texture\\bigmap\\%d%d.bmp", m, n + 1);
...
void Ctemp::Render() {
	if (!_pTex) {
		...
		lwLoadTex(&_pTex, g_Render.GetInterfaceMgr()->res_mgr,
				  R"(texture\ui\minimap\mapmask.bmp)", nullptr, D3DFMT_A4R4G4B4);
	}
...
void CMinimap::InitScene() {
	...
	if (m_pScene) {
		...
		char filename[128];
		_snprintf_s(filename, _TRUNCATE, R"(texture\minimap\%s\%s.pk)",
...
void CMinimap::RenderScene() {
	...
	//		for(int n = 0; n < 3; n++)
	//		{
	//			_snprintf_s(filename,_TRUNCATE,"texture\\minimap\\%s\\sm%d%d.bmp",
	//				m_pScene->GetTerrainName(),_sx + n,_sy + m);
	//
	//			if(_access(filename,0)== -1)
	//			{
	...
	for (m = 0; m < VIEWRANGE; m++) {
		...
		for (int n = 0; n < VIEWRANGE; n++) {
			if (_sx + n < 0 || _sy + m < 0) {
				...
				if (!_pMiniPack) {
					_snprintf_s(filename, _TRUNCATE, R"(texture\minimap\%s\sm_%d_%d.bmp)",
								m_pScene->GetTerrainName(), _sx + n, _sy + m);
	...
	g_Render.SetTexture(0, nullptr);
	D3DXMATRIX matIden;
	auto v2 = D3DXVECTOR3(float(lenw + _rcWnd.left), float(lenw + _rcWnd.top), 0);
	const auto v3 = D3DXVECTOR3(0, 0, 1);
	GetMatrixRotation(&matIden, &v2,
					  &v3, _fCameraAngle);
	...
	g_Render.SetVertexShader(nullptr);
#endif
	g_Render.SetFVFX(D3DFVF_XYZRHW);
	g_Render.SetRenderState(D3DRS_ALPHABLENDENABLE, FALSE);
	g_Render.SetRenderState(D3DRS_TEXTUREFACTOR, 0xffffffff);
	//g_Render.SetTextureStageState( 0, D3DTSS_MAGFILTER, D3DTEXF_LINEAR);
	//g_Render.SetTextureStageState( 0, D3DTSS_MINFILTER, D3DTEXF_LINEAR);
...
void CLargerMap::RenderScene() {
	...
	if ((g_pGameApp->GetMouseKey() & MouseClickState::RDown) != MouseClickState()) {
		Update(_nInitX, _nInitY);
	} else if (g_pGameApp->IsMouseContinue(0) || ((g_pGameApp->GetMouseKey() & MouseClickState::LDown) != MouseClickState())) {
	...
	_snprintf_s(psz, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_394), _nCenterX / 100, _nCenterY / 100);
