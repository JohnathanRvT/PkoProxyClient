...
bool CAttackState::_Start() {
	...
	//add by ALLEN 2007-10-16
		...
		if (CReadBookMgr::IsCanReadBook(GetActor()->GetCha())) {
			...
			auto readingState = std::make_unique<CReadingState>(GetActor());
			GetActor()->SwitchState(std::move(readingState));
...
bool CHitAttackState::_Start() {
	...
	char szBuf[128] = {0};
	_snprintf_s(szBuf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_403), _nPoseNum, _fSpeed);
	std::string str = szBuf;
	for (int i = 0; i < _nPoseNum; i++) {
		_snprintf_s(szBuf, _TRUNCATE, " %d", _nPose[i]);
		str += szBuf;
	}
		...
		for (;;) {
			...
			if (_nCurChaID >= _nChaEnd) {
				...
				if (pCha) {
					...
					auto pState = std::make_unique<CAllPoseState>(_pSelf->GetActor());
					pState->SetChaID(_nCurChaID);
					pState->SetChaRange(_nChaStart, _nChaEnd);
					_pSelf->GetActor()->AddState(std::move(pState));
