...
void operator<<(FILE* file, const ReallyBigObjectInfo& info) {
	fprintf_s(file, "%d %f %f %f %f %f %f %f %f\n",
			  info.typeID,
			  info.position.x,
			  info.position.y,
			  info.position.z,
...
void CGameScene::_RecordRBO() {
	...
	for (auto* itr : _reallyBigObjectList) {
		itr->SetValid(false);
	}
}
...
void CGameScene::_ReadRBO() {
	...
	std::ifstream file(("map\\" + _stInit.strMapFile + ".rbo").c_str());
	struct ReallyBigObjectInfo info;
	char c1 = file.get();
	char c2 = file.get();
	if (c1 == '\\' && c2 == '\\') {
...
bool CGameScene::_InitScene() {
	...
	g_lit_mgr.Load(R"(.\scripts\txt\lit.tx)");

	if (!InitItemLit(R"(.\scripts\txt\item.lit)"))
		return false;
...
bool CGameScene::LoadMap(const char* file) {
	...
	map = strMapFile + ".obj";
	map = "map/" + map;
	g_ObjFile.Init(map.c_str(), false);
...
int CGameScene::UpdateSceneAnimLight() {
	...
	// point light
	for (auto index : _SceneObjIdx[SCENEOBJ_TYPE_POINTLIGHT]) {
		auto s = static_cast<SceneLight*>(&_pSceneObjArray[index]);
...
int CGameScene::BeginUpdateSceneObjLight(const CSceneObj* obj, BOOL is_enable) {
	...
	for (; it != it_end; ++it) {
		...
		if (s->type != SceneLight::SL_LIGHT) {
			MessageBox(nullptr, "SceneLight::SL_LIGHT error", "x", MB_OK);
		}
...
void CGameScene::PlayEnvSound(char* szFile, int nX, int nY) {
	...
	if (dis <= VOICE_DIS) {
		...
		ulong musid = AudioSDL::get_instance()->get_resID(szFile, AUDIO_SAMPLE);
		if (musid) {
		...
		if (pObj->IsValid() && !pObj->IsHide()) {
			...
			if (dis <= pInfo->nEnvSoundDis) {
				...
				if (pObj->GetMusicID() == -1) {
					...
					_snprintf_s(szSoundName, _TRUNCATE, "music/sound/%s.wav", pInfo->szEnvSound);
					...
					ulong m = GetAudioDevice()->get_resID(szSoundName, AUDIO_SAMPLE);
#else
					ulong m = GetAudioDevice()->get_resID(szSoundName, AUDIO_SAMPLE);
#endif
					continue;
...
int CGameScene::SetTextureLOD(DWORD level) {
	...
	for (size_t i = 0; i < _nSceneItemCnt; i++) {
...
void CGameScene::RefreshLevel() {
	...
	auto nMainLevel = (int)g_stUIBoat.GetHuman()->getGameAttr()->get(ATTR_LV);
...
bool CGameScene::DelAreaEff(long nAreaID, int nEffectID) {
	int nCount = 0;
	if (nEffectID == 0) {
		for (auto& areaeff : _areaeffs) {
			if (areaeff->getTag() == nAreaID) {
				areaeff->SetValid(FALSE);
				areaeff = nullptr;
				nCount++;
			}
		}
		...
		for (auto& areaeff : _areaeffs) {
			if (areaeff->getTag() == nAreaID && areaeff->getIdxID() == nEffectID) {
				areaeff->SetValid(FALSE);
				areaeff = nullptr;
				nCount++;
			}
