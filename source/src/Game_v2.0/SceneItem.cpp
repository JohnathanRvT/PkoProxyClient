...
CSceneItem::CSceneItem()
	: CSceneNode(), _pSceneHeight(std::make_unique<CSceneHeight>(this)) {}
...
BOOL CSceneItem::_Create(int nObjTypeID, int nType) {
	...
	char str[260];
	_snprintf_s(str, _TRUNCATE, "%s.lgo", _pItemInfo->chModule[nType]);
	...
	if (l) {
		...
		lwPrimitiveTexLit(p, l->str_buf[0], R"(.\texture\lit\)", l->color_op, l->anim_type);
	}
	// end
...
void CSceneItem::PlayArcAni(D3DXVECTOR3 vStart, D3DXVECTOR3 vEnd, float fVel, float fHei) {
	if (!_pArcTrack) {
		_pArcTrack = std::make_unique<CArcTrack>();
	}
