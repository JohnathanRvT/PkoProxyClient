...
void CSceneObjFile::_Serialize_RBO() {
	if (!filename_RBO.empty()) {
		...
		for (const auto& RBOInfo : RBOinfoList) {
			file << RBOInfo;
		}
	}
}
...
void CSceneObjFile::_Serialize_RBO_ToMap() {
	...
	for (auto& RBOInfo : RBOinfoList) {
		auto sectionX = (int)(RBOInfo.position.x / (100.f * terrain->GetSectionWidth()));
		auto sectionY = (int)(RBOInfo.position.y / (100.f * terrain->GetSectionHeight()));
		...
		if (ReadSectionObjInfo(sectionNumber, infoex, &sectionObjCount)) {
			...
			if (RBOIndex <= MAX_MAP_SECTION_OBJ) {
				infoex[RBOIndex].sTypeID = RBOInfo.typeID;
				infoex[RBOIndex].nX = (int)RBOInfo.position.x;
				infoex[RBOIndex].nY = (int)RBOInfo.position.y;
				infoex[RBOIndex].sYawAngle = (short)RBOInfo.orientation.w;
				infoex[RBOIndex].sHeightOff = (short)RBOInfo.position.z;
			}
...
long CSceneObjFile::Init(const char* ptcsFileName, bool bSilence) {
	...
	if (!fFile) {
		if (CreateFile(ptcsFileName) == 0) {
			if (!bSilence) {
				_snprintf_s(tcsPrint, _TRUNCATE, "%s %s %s", RES_STRING(CL_LANGUAGE_MATCH_355), ptcsFileName, RES_STRING(CL_LANGUAGE_MATCH_356));
	...
	if (m_SFileHead.lFileSize != lFileSize) {
		if (!bSilence) {
			_snprintf_s(tcsPrint, _TRUNCATE, "%s %s", ptcsFileName, RES_STRING(CL_LANGUAGE_MATCH_357));
			MessageBox(nullptr, tcsPrint, RES_STRING(CL_LANGUAGE_MATCH_25), 0);
		}
		lRet = 0;
		goto end;
	...
	if (m_SFileHead.lVersion == OBJ_FILE_VER500) //需要版本转换
	{
		...
		_tfopen_s(&fFile, ptcsFileName, "rb");
		if (fFile == nullptr) {
			if (!bSilence) {
				_snprintf_s(tcsPrint, _TRUNCATE, "%s %s", ptcsFileName, RES_STRING(CL_LANGUAGE_MATCH_358));
				MessageBox(nullptr, tcsPrint, RES_STRING(CL_LANGUAGE_MATCH_25), 0);
			}
			lRet = 0;
			goto end;
	...
	if (m_SFileHead.lVersion != OBJ_FILE_VER600) {
		if (!bSilence) {
			_snprintf_s(tcsPrint, _TRUNCATE, "%s %s", ptcsFileName, RES_STRING(CL_LANGUAGE_MATCH_340));
			MessageBox(nullptr, tcsPrint, RES_STRING(CL_LANGUAGE_MATCH_25), 0);
		}
		lRet = 0;
		goto end;
	...
	if (g_Config.m_bEditor)
		_tfopen_s(&m_fRdWr, ptcsFileName, "r+b");
	else
		_tfopen_s(&m_fRdWr, ptcsFileName, "rb");

	if (m_fRdWr == nullptr) {
		if (!bSilence) {
			_snprintf_s(tcsPrint, _TRUNCATE, "%s %s", ptcsFileName, RES_STRING(CL_LANGUAGE_MATCH_339));
			MessageBox(nullptr, tcsPrint, RES_STRING(CL_LANGUAGE_MATCH_25), 0);
		}
		lRet = 0;
		goto end;
	...
	if (g_Config.m_bEditor)
		_tfopen_s(&m_fAppend, ptcsFileName, "a+b");
	else
		_tfopen_s(&m_fAppend, ptcsFileName, "rb");

	if (m_fAppend == nullptr) {
		if (!bSilence) {
			_snprintf_s(tcsPrint, _TRUNCATE, "%s %s", ptcsFileName, RES_STRING(CL_LANGUAGE_MATCH_359));
			MessageBox(nullptr, tcsPrint, RES_STRING(CL_LANGUAGE_MATCH_25), 0);
		}
		lRet = 0;
		goto end;
...
long CSceneObjFile::CreateFile(const char* ptcsFileName,
	...
	if (g_Config.m_bEditor)
		_tfopen_s(&fFile, ptcsFileName, "wb");
	else
		_tfopen_s(&fFile, ptcsFileName, "rb");
	if (fFile == nullptr)
		return 0;

	strncpy_s(SHead.tcsTitle, "HF Object File!", _TRUNCATE);
...
long CSceneObjFile::ConvertObjFileVer(const char* ptcsFileName, bool bBackUp) // 从版本500转换到600(物件的绝对坐标转换成相对section的相对坐标)
{
	...
	strncpy_s(tcsBackUpName, ptcsFileName, _TRUNCATE);
	for (i = 0; i < _MAX_FNAME - (long)_tcslen(ptcsFileName); i += 4) {
		strncat_s(tcsBackUpName, ".bak", _TRUNCATE);
		_tfopen_s(&fFileNew, tcsBackUpName, "rb");
		if (fFileNew == nullptr)
			break;
		else {
			fclose(fFileNew);
	...
	_tfopen_s(&fFileOld, tcsBackUpName, "rb");
	if (fFileOld == nullptr) {
		lRet = -3; // 打开源文件失败
		goto end;
	}
	...
	if (g_Config.m_bEditor)
		_tfopen_s(&fFileNew, ptcsFileName, "wb");
	else
		_tfopen_s(&fFileNew, ptcsFileName, "rb");
	if (fFileNew == nullptr) {
		lRet = -1; // 建立新文件失败
		goto end;
	}
	...
	_snprintf_s(tcsPrint, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_360), ptcsFileName);
	...
	if (lRet == 2) {
		_snprintf_s(tcsPrint, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_362), ptcsFileName, tcsBackUpName);
...
long CSceneObjFile::TrimFile(char* ptcsFileName, bool bBackUp) {
	long lRet = 1;
	char tcsBackUpName[_MAX_FNAME] = "";
	long i;
	long lPos = 0;
	...
	if (_tcscmp(SHead.tcsTitle, "HF Object File!") != 0 || SHead.lVersion != OBJ_FILE_VER600) // || SHead.lFileSize != i)
	{
		lRet = -4; // 源文件已被破坏或版本错误
		goto end;
	}
...
long CSceneObjFile::TrimDirectory(const char* ptcsDirectory, bool bBackUp) {
	...
	_finddata_t c_file;
	long hFile;
	char ptcsRecordFile[] = "TrimRecord.txt";
	FILE* fRecord = nullptr;
	_timeb tTimeBuffer;
	char tcsPrint[256];

	if (bBackUp)
		_snprintf_s(tcsPrint, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_366), ptcsDirectory);
	else
		_snprintf_s(tcsPrint, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_367), ptcsDirectory);
	if (IDYES != MessageBox(nullptr, tcsPrint, RES_STRING(CL_LANGUAGE_MATCH_361), MB_YESNO))
		return 1;

	lLen = _tcslen(ptcsDirectory);
	strncpy_s(tcsPath, ptcsDirectory, _TRUNCATE);
	if (tcsPath[lLen - 1] != '\\' || tcsPath[lLen - 1] != '/') {
		tcsPath[lLen] = '/';
		tcsPath[lLen + 1] = 0;
	}

	_snprintf_s(tcsPrint, _TRUNCATE, "%s%s", tcsPath, ptcsRecordFile);
	if (g_Config.m_bEditor)
		_tfopen_s(&fRecord, tcsFileName, "w");
	else
		_tfopen_s(&fRecord, tcsFileName, "rb");
	if (fRecord == nullptr)
		return 0;
	_ftime_s(&tTimeBuffer);

	char tempBuffer[128];
	ctime_s(tempBuffer, sizeof(tempBuffer), &tTimeBuffer.time);
	fprintf_s(fRecord, RES_STRING(CL_LANGUAGE_MATCH_368), tempBuffer);

	_snprintf_s(tcsPrint, _TRUNCATE, "%s%s", tcsPath, "*.obj");
	if ((hFile = (long)_tfindfirst(tcsFileName, &c_file)) == -1L) {
		fclose(fRecord);
		return 1;
	} else {
		if (!(c_file.attrib & _A_SUBDIR)) {
			_snprintf_s(tcsPrint, _TRUNCATE, "%s%s", tcsPath, c_file.name);
