...
LW_RESULT __SceneTranspObjRenderProc(MPIPrimitive* obj, void* param) {
	...
	auto* desc = static_cast<SceneTranspObjStateDesc*>(param);
...
#define OPEN_AMBIENT_ENV
	...
	if (_bShowSceneObj) {
		...
		SceneTranspObjStateDesc* stosd = nullptr;
		...
		for (const auto& SceneObjIndex : _SceneObjIdx[SCENEOBJ_TYPE_NORMAL]) {
			CSceneObj* pObj = &_pSceneObjArray[SceneObjIndex];
			if (pObj->IsValid() && !pObj->IsHide()) {
				...
				const BOOL light_enable = info->bEnableEnvLight | info->bEnablePointLight | info->bShadeFlag;
				...
				if (transp_flag) {
					sm->SetTranspPrimitiveProc(nullptr, nullptr);
				}
		...
		//NOTE: Does more than if-condition communicates;
		if (_nShowChair) {
			// 非普通物件的渲染(虚拟物件)
