...
CSelectChaScene::CSelectChaScene(stSceneInitParam& param)
	: CGameScene(param) /*m_nCurChaIndex(-1),*/ {
...
void CSelectChaScene::_RenderUI() //Renders player name, level and class on top of a character.
{
	for (auto selectableCharacter : selectableCharacters) {
		...
		if ((selectableCharacter->iFontX == -1) || (selectableCharacter->iFontY == -1)) {
			...
			if (!g_Render.WorldToScreen(pos.x, pos.y, pos.z, &nScreenX, &nScreenY)) {
				return;
			}
		...
		char szBuf[8];
		std::string sFirst = std::string(selectableCharacter->pCha->getName());
		_itoa_s(selectableCharacter->iLevel, szBuf, sizeof(szBuf), 10);
		std::string sSecond = std::string("Lv") + std::string(szBuf) + " " + selectableCharacter->sProfession;
...
void CSelectChaScene::LoadingCall() // 在装载loading后,刷新
{
	...
	NetPC_GUILD_START_BEGIN(0, nullptr, 0);
...
bool CSelectChaScene::_InitUI() {
	if (frmSelectCha = CFormMgr::s_Mgr.Find("frmSelect", GetInitParam()->nUITemplete); !frmSelectCha) {
		return false;
	}

	if (btnDel = dynamic_cast<CTextButton*>(frmSelectCha->Find("btnDel")); !btnDel) {
		return false;
	}

	if (btnYes = dynamic_cast<CTextButton*>(frmSelectCha->Find("btnYes")); !btnYes) {
		return false;
	}

	if (btnCreate = dynamic_cast<CTextButton*>(frmSelectCha->Find("btnCreate")); !btnCreate) {
		return false;
	}

	if (btnExit = dynamic_cast<CTextButton*>(frmSelectCha->Find("btnNo")); !btnExit) {
		return false;
	}

	if (btnAlter = dynamic_cast<CTextButton*>(frmSelectCha->Find("btnAlter")); !btnAlter) {
		return false;
	}
...
void CSelectChaScene::_SelChaFrmMouseEvent(CCompent* pSender, int nMsgType,
	...
	if (strName == "btnCreate") {
		...
		auto* pkScene = dynamic_cast<CCreateChaScene*>(g_pGameApp->CreateScene(&param));
		if (!pkScene)
			return;
...
void CSelectChaScene::_evtCreateDoublePwdEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType == CForm::mrYes) {
		// 显示创建二次密码窗体
		g_stUIDoublePwd.ShowCreateForm();
	} else {
...
void CSelectChaScene::DelCurrentSelCha() {
	//在场景中删除该角色
	m_ptrCurCha->pCha->SetValid(false);

	//位置置空
	m_ptrCurCha->pCha = nullptr;
...
bool CSelectChaScene::SelectCharacters(NetChaBehave* chabehave, size_t num) {
	num = min(selectableCharacters.size(), num);
	for (int i = 0; i < num; i++) {
...
CSelectChaScene& CSelectChaScene::GetCurrScene() {
	auto* pScene = dynamic_cast<CSelectChaScene*>(g_pGameApp->GetCurScene());
...
// 欢迎界面 事件处理
void CSelectChaScene::_evtWelcomeNoticeEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string strName = pSender->GetName();
	auto* pSelectChaScene = dynamic_cast<CSelectChaScene*>(g_pGameApp->GetCurScene());
...
// 首次创建角色成功提示界面 事件处理
void CSelectChaScene::_evtCreateOKNoticeEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string strName = pSender->GetName();
	auto* pSelectChaScene = dynamic_cast<CSelectChaScene*>(g_pGameApp->GetCurScene());
...
// 首次创建角色成功提示界面 事件处理
void CSelectChaScene::_evtChaNameAlterMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string strName = pSender->GetName();
	auto* pSelectChaScene = dynamic_cast<CSelectChaScene*>(g_pGameApp->GetCurScene());
