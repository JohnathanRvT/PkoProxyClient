...
//---------------------------------------------------------------------------
static xShipFactory* __xsf = nullptr;

//---------------------------------------------------------------------------
void xShipFactory::__ButtonYesNo(CGuiData* pSender, int x, int y, MouseClickState dwKey) {
	if (__xsf)
		__xsf->Close(pSender);
}

//---------------------------------------------------------------------------
void xShipFactory::__ProcBtnItem(CGuiData* sender, int x, int y, MouseClickState key) {
	__xsf->UpdateShipPart(sender);
}

//---------------------------------------------------------------------------
void xShipFactory::__ProcBtnView3d(CGuiData* sender, int x, int y, MouseClickState key) {
...
void xShipFactory::__ButtonOpenRoom(CGuiData* pSender, int x, int y, MouseClickState dwKey) {
	if (!__xsf)
		return;
...
//---------------------------------------------------------------------------
void xShipFactory::__ButtonCloseRoom(CGuiData* pSender, int x, int y, MouseClickState dwKey) {
	if (!__xsf)
		return;
...
void xShipFactory::ShowFlipBtn(int show, const char* name) {
	...
	if (sbf.name) {
		if (name) {
			...
			sbf.name->SetIsEnabled(false);
		} else {
			//sbf.name->SetCaption("");
			sbf.name->SetIsEnabled(true);
			//sbf.name->set
		}
	} else {
		sbf.name->SetIsEnabled(true);
...
//---------------------------------------------------------------------------
BOOL xShipFactory::CreateShip(DWORD type_id, const DWORD* part_buf) {
	SAFE_DELETE(ship);
...
BOOL xShipFactory::GetCabinByID() {
	...
	if (!pBoatRoom)
		return FALSE;
	auto* pForm = dynamic_cast<CForm*>(pBoatRoom->GetParent());
...
//---------------------------------------------------------------------------
BOOL xShipFactory::SetShipModelInfo(const DWORD* dwModelInfo, size_t size) {
...
//---------------------------------------------------------------------------
BOOL xShipFactory::Update(const xShipBuildInfo* info, BOOL flag, const char* name) {
	...
	_snprintf_s(buf, _TRUNCATE, "%d/%d", info->dwMinAttack, info->dwMaxAttack);
	sbf.lbl_prop[SBF_TEXT_ATTACK]->SetCaption(buf);
	_snprintf_s(buf, _TRUNCATE, "%d/%d", info->dwCurEndure, info->dwMaxEndure);
	sbf.lbl_prop[SBF_TEXT_ENDURE]->SetCaption(buf);
	_snprintf_s(buf, _TRUNCATE, "%d/%d", info->dwCurSupply, info->dwMaxSupply);
	sbf.lbl_prop[SBF_TEXT_SUPPLY]->SetCaption(buf);
	_snprintf_s(buf, _TRUNCATE, "%d/%d", 0, info->sCapacity);
	sbf.lbl_prop[SBF_TEXT_CAPACITY]->SetCaption(buf);

	_itoa_s(info->dwDefence, buf, 10);
	sbf.lbl_prop[SBF_TEXT_DEFENCE]->SetCaption(buf);
	_itoa_s(info->dwSpeed, buf, 10);
	sbf.lbl_prop[SBF_TEXT_SPEED]->SetCaption(buf);
	_itoa_s(info->dwAttackTime, buf, 10);
	sbf.lbl_prop[SBF_TEXT_COOLDOWN]->SetCaption(buf);
	_itoa_s(info->dwConsume, buf, 10);
	sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(buf);
...
//---------------------------------------------------------------------------
BOOL xShipFactory::UpdateBoatCreate(const xShipBuildInfo* info, BOOL flag, const char* name) {
	ClearBoatAttr();
...
//---------------------------------------------------------------------------
BOOL xShipFactory::UpdateBoatInfo(const xShipBuildInfo* info, BOOL flag, const char* name) {
	//得到船舱
	if (!GetCabinByID())
		return FALSE;
...
void xShipFactory::SetBoatAttr() {
	char buf[128];
	_snprintf_s(buf, _TRUNCATE, "%d/%d", m_pkBoatInfo->dwMinAttack, m_pkBoatInfo->dwMaxAttack);
	sbf.lbl_prop[SBF_TEXT_ATTACK]->SetCaption(buf);
	_snprintf_s(buf, _TRUNCATE, "%d/%d", m_pkBoatInfo->dwCurEndure, m_pkBoatInfo->dwMaxEndure);
	sbf.lbl_prop[SBF_TEXT_ENDURE]->SetCaption(buf);
	_snprintf_s(buf, _TRUNCATE, "%d/%d", m_pkBoatInfo->dwCurSupply, m_pkBoatInfo->dwMaxSupply);
	sbf.lbl_prop[SBF_TEXT_SUPPLY]->SetCaption(buf);

	_itoa_s(m_pkBoatInfo->dwSpeed, buf, 10);
	sbf.lbl_prop[SBF_TEXT_SPEED]->SetCaption(buf);
	_itoa_s(m_pkBoatInfo->dwDistance, buf, 10);
	sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(buf);
	_itoa_s(m_pkBoatInfo->dwDefence, buf, 10);
	sbf.lbl_prop[SBF_TEXT_DEFENCE]->SetCaption(buf);
}
...
void xShipFactory::UpdateBoatAttr() {
	...
	if (sbf.chkShip->GetIsChecked()) { /*显示船只原始属性*/
		_snprintf_s(buf, _TRUNCATE, "%d/%d", (int)pkAttr->get(ATTR_BMNATK), (int)pkAttr->get(ATTR_BMXATK));
		sbf.lbl_prop[SBF_TEXT_ATTACK]->SetCaption(buf);
		_snprintf_s(buf, _TRUNCATE, "%d/%d", (int)pkAttr->get(ATTR_HP), (int)pkAttr->get(ATTR_BMXHP));
		sbf.lbl_prop[SBF_TEXT_ENDURE]->SetCaption(buf);
		_snprintf_s(buf, _TRUNCATE, "%d/%d", (int)pkAttr->get(ATTR_SP), (int)pkAttr->get(ATTR_BMXSP));
		sbf.lbl_prop[SBF_TEXT_SUPPLY]->SetCaption(buf);

		_itoa_s((int)pkAttr->get(ATTR_BMSPD), buf, 10);
		sbf.lbl_prop[SBF_TEXT_SPEED]->SetCaption(buf);
		_itoa_s((int)pkAttr->get(ATTR_BDEF), buf, 10);
		sbf.lbl_prop[SBF_TEXT_DEFENCE]->SetCaption(buf);
		_itoa_s((int)pkAttr->get(ATTR_BSREC), buf, 10);
		sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(buf);
	} else { /*显示船只属性(有人物影响)*/
		_snprintf_s(buf, _TRUNCATE, "%d/%d", (int)pkAttr->get(ATTR_MNATK), (int)pkAttr->get(ATTR_MXATK));
		sbf.lbl_prop[SBF_TEXT_ATTACK]->SetCaption(buf);
		_snprintf_s(buf, _TRUNCATE, "%d/%d", (int)pkAttr->get(ATTR_HP), (int)pkAttr->get(ATTR_MXHP));
		sbf.lbl_prop[SBF_TEXT_ENDURE]->SetCaption(buf);
		_snprintf_s(buf, _TRUNCATE, "%d/%d", (int)pkAttr->get(ATTR_SP), (int)pkAttr->get(ATTR_MXSP));
		sbf.lbl_prop[SBF_TEXT_SUPPLY]->SetCaption(buf);

		_itoa_s((int)pkAttr->get(ATTR_MSPD), buf, 10);
		sbf.lbl_prop[SBF_TEXT_SPEED]->SetCaption(buf);
		_itoa_s((int)pkAttr->get(ATTR_DEF), buf, 10);
		sbf.lbl_prop[SBF_TEXT_DEFENCE]->SetCaption(buf);
		_itoa_s((int)pkAttr->get(ATTR_SREC), buf, 10);
		sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(buf);
	}
	...
	_snprintf_s(buf, _TRUNCATE, "%d/%d", nCurCapacity, nMaxCapacity);
	sbf.lbl_prop[SBF_TEXT_CAPACITY]->SetCaption(buf); //背包容量

	// 攻击间隔
	int v = (int)pkAttr->get(ATTR_ASPD);
	if (v == 0)
		_snprintf_s(buf, _TRUNCATE, "-1");
	else
		_snprintf_s(buf, _TRUNCATE, "%d", 100000 / v);
	sbf.lbl_prop[SBF_TEXT_COOLDOWN]->SetCaption(buf);

	//船只等级和熟练度
	_itoa_s((int)pkAttr->get(ATTR_LV), buf, 10);
	sbf.lbl_ship_level->SetCaption(buf);
	_itoa_s((int)pkAttr->get(ATTR_CEXP), buf, 10);
...
//---------------------------------------------------------------------------
// xShipLaunchList
static xShipLaunchList* __xsl = nullptr;
...
//---------------------------------------------------------------------------
void xShipLaunchList::__ButtonClose(CGuiData* pSender, int x, int y, MouseClickState dwKey) {
	__xsl->wnd->Close();
}
