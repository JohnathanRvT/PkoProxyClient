...
CTeamInviteFormMgr::CTeamInviteFormMgr() {
}
...
bool CTeamInviteFormMgr::AddInviteForm(DWORD id, std::string inviterName) {
	...
	char str[256];
	_snprintf_s(str, _TRUNCATE, "frmAcceptGroup_Arcol_%d", id);
	node->pForm->SetName(str);
	m_FormLink.push_back(node);
	//node->pForm->SetPos(200,200);
	//node->pForm->Refresh();
	auto* labGroupName = dynamic_cast<CLabelEx*>(node->pForm->Find("labGroupName"));
	if (labGroupName) {
	...
	auto* labGroup = dynamic_cast<CLabelEx*>(node->pForm->Find("labGroup"));
	if (labGroup) {
...
void CTeamInviteFormMgr::_MainMousePlayerGroupEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
