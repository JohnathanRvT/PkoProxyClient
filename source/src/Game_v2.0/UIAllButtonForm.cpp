...
CCombo* AllButtonMgr::cboCion1 = nullptr;
CCombo* AllButtonMgr::cboCion2 = nullptr;
CCombo* AllButtonMgr::cboCion3 = nullptr;
...
bool AllButtonMgr::Init() {
	...
	frmElfCoin->evtClose = evtElfCoinFormClose;
...
void AllButtonMgr::_evtAllButtonMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
void AllButtonMgr::evtbtnClickDown(CGuiData* pSender, int x, int y, MouseClickState key) {
...
int AllButtonMgr::JudgeGost() {
	auto* pItemCommand = dynamic_cast<CItemCommand*>(g_stUIEquip.GetGoodsGrid()->GetItem(1));
...
void AllButtonMgr::CloseAllFormButThis(std::string frmName) {
...
void AllButtonMgr::evtElfCoinFormClose(CForm* pForm, bool& IsClose) {
