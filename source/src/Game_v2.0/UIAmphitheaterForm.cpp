...
using namespace GUI;
CAmphitheaterForm* CAmphitheaterForm::m_pStaticAmphiForm = nullptr;
/////////////////////////////////////////////
//          竞技场排行榜数据
////////////////////////////////////////////
...
//根据链表位置返回链表内容
TeamProUnit* AmphitheaterData::GetTeamPro(int index, TeamProUnit* t) {
	std::vector<TeamProUnit>::iterator ProIt = TeamProList.begin();
	for (int i = 0; i < (int)TeamProList.size(); i++) {
		if (i == index) {
			strncpy_s(t->PromCapName, sizeof(ProIt->PromCapName), ProIt->PromCapName, _TRUNCATE);
			t->PromID = ProIt->PromID;
			t->WinNum = ProIt->WinNum;
			return t;
		}
	}
	return nullptr;
...
//根据list位置获取复活信息
TeamRelUnit* AmphitheaterData::GetTeamRel(int index, TeamRelUnit* t) {
	std::vector<TeamRelUnit>::iterator RelIt = TeamRelList.begin();

	for (int i = 0; i < (int)TeamRelList.size(); i++) {
		if (i == index) {
			t->RelID = RelIt->RelID;
			strncpy_s(t->RelCapName, sizeof(RelIt->RelCapName), RelIt->RelCapName, _TRUNCATE);
			t->Ballot = RelIt->Ballot;
			return t;
		}
		RelIt++;
	}
...
CAmphitheaterList* CAmphitheaterList::g_pAmphiList = nullptr;
...
bool CAmphitheaterList::MouseRun(int x, int y, MouseClickState key) {
...
//投票按钮响应
void CAmphitheaterForm::ListMouseRun(CGuiData* pSender, int x, int y, MouseClickState key) {
	int ID = 6068;
	...
	if (IdNum <= 0) {
		...
		for (; i < 20; i++) {
			if (pSender == (CGuiData*)(m_pStaticAmphiForm->m_pSecReliveList->_button[i])) {
				...
				int ReID = 0;
				TeamRelUnit data2;
				m_pStaticAmphiForm->m_pAmphitheaterData.GetTeamRel(index, &data2);
				ReID = data2.RelID;
	...
	case 0: {
		if (m_pSecPromotionList) {
			for (; ProIt != Prolist.end(); ProIt++) {
				...
				_snprintf_s(id, _TRUNCATE, "%d", ProIt->PromID);
				strName += id;
	...
	case 1: {
		...
		if (m_pSecReliveList) {
			for (; RelIt != Rellist.end(); RelIt++) {
				...
				_snprintf_s(ballot, _TRUNCATE, "%d", RelIt->Ballot);
				strName += ballot;
