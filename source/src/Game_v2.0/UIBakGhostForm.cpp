...
bool CBakGhostMgr::Init() {
	...
	auto* btnHelp = static_cast<CTextButton*>(frmBakGhost->Find("btnHelp"));
	if (!btnHelp)
		return false;
...
void CBakGhostMgr::evtCreateSpirite(CGuiData* pSender, int x, int y, MouseClickState key) {
	char buf[256] = {0};
...
void CBakGhostMgr::evtShowHelpInfo(CGuiData* pSender, int x, int y, MouseClickState key) {
