...
void CBankMgr::ShowBank() // 显示物品
{
	...
	char szBuf[32];
	_snprintf_s(szBuf, _TRUNCATE, "%s%s", g_stUIBoat.GetHuman()->getName(), RES_STRING(CL_LANGUAGE_MATCH_440)); //显示人物名及专用
...
#define BANK_TYPE 1
	...
	// 设置发送拖动物品的服务器信息
	m_kNetBank.chSrcType = EKitBagType::player_or_boat;
	m_kNetBank.sSrcID = rkDrag.GetDragIndex();
	//m_kNetBank.sSrcNum = ; 数量在回调函数中设置
	m_kNetBank.chTarType = EKitBagType::bank;
	...
	//NOTE: This is only implemented on client-side as of my current knowing. 
	// If making certain items un-bankable is of importance, this SHOULD also be verified on server-side.
	// if(pkItemRecord->lID == 2520 || pkItemRecord->lID == 2521)
	if (pkItemRecord->lID == 2520 || pkItemRecord->lID == 2521 || pkItemRecord->lID == 6341 ||
		pkItemRecord->lID == 6343 || pkItemRecord->lID == 6347 || pkItemRecord->lID == 6359 ||
		pkItemRecord->lID == 6370 || pkItemRecord->lID == 6371 || pkItemRecord->lID == 6373 ||
	...
	if (pkItemCmd->GetItemInfo()->GetIsPile() && pkItemCmd->GetTotalNum() > 1) { /*存放多个物品*/
		...
		return true;
		//char buf[256] = { 0 };
		//_snprintf_s(buf, _TRUNCATE, "您确认放入银行\n[%s]?", pkItemCmd->GetName());
...
bool CBankMgr::PopFromBank(CGoodsGrid& rkDrag, CGoodsGrid& rkSelf, int nGridID, CCommandObj& rkItem) {
	// 设置发送拖动物品的服务器信息
	m_kNetBank.chSrcType = EKitBagType::bank;
	m_kNetBank.sSrcID = rkDrag.GetDragIndex();
	//m_kNetBank.sSrcNum = ; 数量在回掉函数中设置
	m_kNetBank.chTarType = EKitBagType::player_or_boat;
	m_kNetBank.sTarID = nGridID;
	...
	if (pkItemCmd->GetItemInfo()->GetIsPile() && pkItemCmd->GetTotalNum() > 1) { /*取出多个物品*/
		...
		//char buf[256] = { 0 };
		//_snprintf_s(buf, _TRUNCATE, "您确认取出\n[%s]?", pkItemCmd->GetName());
		//g_stUIBox.ShowSelectBox(_MoveAItemEvent, buf, true);
		//return true;
	}
}
...
//-------------------------------------------------------------------------
void CBankMgr::_MoveItemsEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) // 多个物品移动
{
...
//-------------------------------------------------------------------------
void CBankMgr::_MoveAItemEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) // 单个道具移动
{
...
void CBankMgr::_evtBankToBank(CGuiData* pSender, int nFirst, int nSecond, bool& isSwap) // 用于用户银行表单中道具互换
{
	...
	g_stUIBank.m_kNetBank.chSrcType = EKitBagType::bank;
	g_stUIBank.m_kNetBank.sSrcID = nSecond;
	g_stUIBank.m_kNetBank.sSrcNum = 0;
	g_stUIBank.m_kNetBank.chTarType = EKitBagType::bank;
