...
struct CBoothMgr::SBoothItem {
	...
	SBoothItem()
		: lId(0), iPrice(0), iNum(0), iEquipIndex(0),
		  iBoothIndex(0), pkEquipGrid(nullptr), pkBoothGrid(nullptr) {
	}
};

//~ ------------------------------------------------------------------
CBoothMgr::CBoothMgr() : m_pkCurrSetupBooth(nullptr), m_iBoothItemMaxNum(0),
						 m_bSetupedBooth(false), m_NumBox(nullptr), m_TradeBox(nullptr), m_SelectBox(nullptr) {
}
...
bool CBoothMgr::Init() // 摆摊信息初始化
{
	...
	grdBoothItem;
	grdBoothItem->SetShowStyle(CGoodsGrid::eShowStyle::Sale);
...
void CBoothMgr::AddTradeBoothBoat(int iGrid, DWORD dwItemID, int iCount, DWORD dwMoney, NET_CHARTRADE_BOATDATA& Data) {
	...
	auto* pItemCommand = dynamic_cast<CItemCommand*>(grdBoothItem->GetItem(iGrid));
	if (!pItemCommand)
		return;
...
void CBoothMgr::AddBoothItem(SBoothItem* pBoothItem) {
	...
	if (pBoothItem->pkEquipGrid) {
		auto* pOldItem = dynamic_cast<CItemCommand*>(pBoothItem->pkEquipGrid->GetItem(pBoothItem->iEquipIndex));
		if (pOldItem) {
			auto* pNewItem = new CItemCommand(*pOldItem);
	...
	if (name == "btnNo" || name == "btnClose") { ///关闭表单
		...
		for (int i(0); i < g_stUIBooth.m_iBoothItemMaxNum; i++) {
			if (g_stUIBooth.m_kBoothItems[i]) {
				...
				netCreateBoothData.Info[iNum].wCount = WORD(g_stUIBooth.m_kBoothItems[i]->iNum);
...
//~ ------------------------------------------------------------------
void CBoothMgr::_InquireSetupPushItemNumEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
//~ ------------------------------------------------------------------
void CBoothMgr::_InquireSetupPushItemPriceEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes) {
		SAFE_DELETE(g_stUIBooth.m_pkCurrSetupBooth);
		return;
	}
	...
	g_stUIBooth.m_pkCurrSetupBooth = nullptr;
}

//~ ------------------------------------------------------------------
void CBoothMgr::_InquireSetupPopItemNumEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes) {
		g_stUIBooth.m_pkCurrSetupBooth = nullptr;
		return;
	}
...
//~ ------------------------------------------------------------------
void CBoothMgr::_InquireTradeItemNumEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes) {
		return;
	}
	stNumBox* kItemNumBox = (stNumBox*)pSender->GetForm()->GetPointer();
...
//~ ------------------------------------------------------------------
void CBoothMgr::_BuyGoodsEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes) {
		g_stUIBooth.m_pkCurrSetupBooth = nullptr;
		return;
	}
...
//~ ------------------------------------------------------------------
void CBoothMgr::_BuyAGoodEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes) {
		g_stUIBooth.m_pkCurrSetupBooth = nullptr;
		return;
	}
...
bool CBoothMgr::PushToBoothSetup(CGoodsGrid& rkDrag, CGoodsGrid& rkSelf, int nGridID, CItemCommand& rkItemCmd) {
	...
	if (rkItemCmd.GetIsPile() && rkItemCmd.GetTotalNum() > 1) { /// 先询问数量,回调函数询问单价
...
bool CBoothMgr::PopFromBoothTrade(CGoodsGrid& rkDrag, CGoodsGrid& rkSelf, int nGridID, CItemCommand& rkItemCmd) {
	...
	if (rkItemCmd.GetIsPile() && rkItemCmd.GetTotalNum() > 1) { /// 询问数量，然后在执行交易的操作
		...
		_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_455),
...
CCharacter* CReadBookMgr::_pCha = nullptr;
...
void CReadBookMgr::_evtSelectBox(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes) {
		if (_pCha) {
...
void CReadBookMgr::_evtMsgBox(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
