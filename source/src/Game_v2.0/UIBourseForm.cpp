...
bool CBourseMgr::Init() {
	...
	/*货舱交易所界面和控件*/
	{
		...
		grdItemBuy->SetShowStyle(CGoodsGrid::eShowStyle::Sale);
		...
		grdItemSale->SetShowStyle(CGoodsGrid::eShowStyle::Sale);
...
void CBourseMgr::BuyGoods(CItemCommand& rkBuy, int nFreeCnt) {
	...
	if (rkBuy.GetPrice() > 0 && CGameScene::GetMainCha()) { /* 钱不够 */
		nMax = (int)(CGameScene::GetMainCha()->getGameAttr()->get(ATTR_GD) / rkBuy.GetPrice());
	...
	if (rkBuy.GetIsPile()) { // 买多个物品
		...
		char buf[256] = {0};
		_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_460), rkBuy.GetName());
...
bool CBourseMgr::SaleGoods(CItemCommand& rkSaleCmd, int iGridIndex) {
	...
	{
		char buf[256] = {0};
		_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_461),
...
BYTE CBourseMgr::GetTradeLevel() {
	...
	for (int i(0); i < iGoodMaxNum; i++) {
		...
		if (pItem && EItemType::Trade == pItem->GetItemInfo()->sType) {
...
void CBourseMgr::__gui_event_sale_drag_before(CGuiData* pSender, CCommandObj* pItem,
	...
	auto* pSelf = dynamic_cast<CGoodsGrid*>(pSender);
	if (!pSelf)
		return;

	auto* pDrag = dynamic_cast<CGoodsGrid*>(CDrag::GetParent());
	if (!pDrag)
		return;

	auto* pkSaleCmd = dynamic_cast<CItemCommand*>(pItem);
	if (!pkSaleCmd)
		return;
...
void CBourseMgr::BuyItem(CGoodsGrid& rkToGoodsGrid, CCommandObj& rkItem,
						 int nGridID) {
	auto* pkBuyCmd = dynamic_cast<CItemCommand*>(&rkItem);
	if (!pkBuyCmd)
		return;
...
void CBourseMgr::__gui_event_drag_before_com(CGuiData* pSender, CCommandObj* pItem,
	...
	auto* pSelf = dynamic_cast<COneCommand*>(pSender);
	if (!pSelf)
		return;
...
bool CBlackTradeMgr::Init() {
	...
	grdItemSale->SetShowStyle(CGoodsGrid::eShowStyle::OwnDef);
	grdItemSale->SetIsHint(false);

	grdItemBuy->SetShowStyle(CGoodsGrid::eShowStyle::Small);
	grdItemBuy->SetIsHint(false);
...
bool CBlackTradeMgr::SailToBuy(CGoodsGrid& rkDrag, CGoodsGrid& rkSelf, int nGridID, CCommandObj& rkItem) {
	...
	auto* pItem = dynamic_cast<CItemCommand*>(rkDrag.GetItem(m_nDragIndex));
	if (!pItem)
		return false;
...
void CBlackTradeMgr::SetItem(stBlackTrade* pBlackTrade) {
	...
	if (pInfo) {
		char szBuffer[128] = {0};
		_snprintf_s(szBuffer, _TRUNCATE, "%d x %s", pBlackTrade->sSrcNum, pInfo->szName);
...
void CBlackTradeMgr::ExchangeAnswerProc(bool bSuccess, stBlackTrade* pBlackTrade) {
	if (bSuccess) {
		...
		if (nCurNum < g_stUIBlackTrade.grdItemBuy->GetMaxNum()) {
			...
			if (pInfo) {
				char szBuffer[128] = {0};
				_snprintf_s(szBuffer, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_843), pBlackTrade->sSrcNum, pInfo->szName); // x个纳卡
				pItem->SetOwnDefText(szBuffer);
			}
...
// 兑换确认
void CBlackTradeMgr::_TradeExchangeEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes) {
		return;
	}
