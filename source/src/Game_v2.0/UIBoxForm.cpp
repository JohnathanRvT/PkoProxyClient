...
bool CBoxMgr::Init() {
	...
	auto* edtNumber = dynamic_cast<CEdit*>(frmTrade->Find("edtNumber"));
	if (!edtNumber)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmTrade->GetName(), "edtNumber");
	edtNumber->SetIsDigit(true);

	auto* labPrice = dynamic_cast<CLabel*>(frmTrade->Find("labPrice"));
	if (!labPrice)
	...
	auto* edtPassword = dynamic_cast<CEdit*>(frmPassword->Find("edtPassword"));
	if (!edtPassword)
...
stTradeBox* CBoxMgr::ShowTradeBox(FormMouseEvent evtForm, float fUnitMoney, int nTotalNum, const char* szName) {
	...
	if (nTotalNum > 0) {
		char buf[32] = {0};
		_snprintf_s(buf, _TRUNCATE, "%d", nTotalNum);
...
stMsgBox* CBoxMgr::ShowMsgBox(FormMouseEvent evtForm, const char* szTitle, bool bModal) {
	...
	auto* t = static_cast<stMsgBox*>(frm->GetPointer());
	if (!t) {
		t = new stMsgBox;
		t->frmDialog = frm;
	...
	if (_cStart) {
		if (_cTick == 0) {
			...
			if (dwTime - _cTick > _cSeconds * 1000) {
				if (_cfrmError) {
					auto* labInfo = dynamic_cast<CLabel*>(_cfrmError->Find("labError"));
					//labInfo->SetCaption(
					_cfrmError->evtEntrustMouseEvent(labInfo, 0, 0, 0, MouseClickState());
					_cStart = false;
					_cTick = 0;
				}
			}
...
stTradeBox::stTradeBox()
	: labPrice(nullptr), edtNumber(nullptr), frmDialog(nullptr), labName(nullptr), fUnitMoney(0.0), nTotalNum(0) {
}
...
void stTradeBox::Refresh() {
	...
	if (nTotalNum > 0) {
		if (n > nTotalNum) {
			...
			_snprintf_s(szBuf, _TRUNCATE, "%d", n);
	...
	_snprintf_s(szBuf, _TRUNCATE, "%s", StringSplitNum((int)((float)n * fUnitMoney)));
...
stSelectBox::stSelectBox()
	: frmDialog(nullptr), labInfo(nullptr), dwTag(0), dwParam(0), pointer(nullptr) {
}
