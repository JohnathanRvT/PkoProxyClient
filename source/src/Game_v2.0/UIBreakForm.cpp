		...
		if (NO_USE != iBreakPos[i]) {
			...
			if (pItemCommand) {
				if (3 == i) {
					...
					case EItemType::Sword:	 //  剑
					case EItemType::Glave:	 //  巨剑
					case EItemType::Bow:	   //  弓
					case EItemType::Harquebus: //  火枪
					case EItemType::Stylet:	//  匕首
					case EItemType::Cosh:	  //  法杖
					case EItemType::Shield:	//  盾牌
					case EItemType::Hair:	  //  帽子
					case EItemType::Clothing:  //  衣服
					case EItemType::Glove:	 //  手套
					case EItemType::Boot:	  //  鞋子
					case EItemType::Tattoo:	//  纹身
					{
						PushItem(i, *pItemCommand, true);
					} break;
					default: {
...
void CBreakMgr::PopItem(int iIndex, bool bRetry /* = false*/) {
	...
	auto* pItemCommand = dynamic_cast<CItemCommand*>(cmdBreak[iIndex]->GetCommand());
	if (!pItemCommand)
		return;
	...
	if (bRnd) {
		...
		if (frmBreak && frmBreak->GetIsShow()) {
			...
			if (dwCurrTickCount - m_dwLastTickCount > ERNIE_SPEED) {
				...
				int rnd = rng.uniform(10000, 10000 + 90000 - 1);
				_snprintf_s(buff, _TRUNCATE, "%i", rnd);
...
void CBreakMgr::_evtbtnForgeYes(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
