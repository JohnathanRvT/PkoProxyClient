...
using namespace GUI;
CForm* CChaQeryInfo::m_pFrmSInfo = nullptr;
CForm* CChaQeryInfo::m_pFrmQInfo = nullptr;
CForm* CChaQeryInfo::m_pFrmQRInfo = nullptr;
...
void CChaQeryInfo::_evSFrm(CGuiData* pSender, int x, int y, MouseClickState key) {
...
void CChaQeryInfo::_evQFrm(CGuiData* pSender, int x, int y, MouseClickState key) {
...
bool CChaQeryInfo::SetSearchFrmData(stPersonInfo* _info, CMember* _m) {
	...
	if (m_pFrmSInfo) {
		...
		_snprintf_s(buff, _TRUNCATE, "%d", _m->GetLevel());
		m_pChaLv->SetCaption(buff);
		m_pChaJob->SetCaption(_m->GetJobName());
		// 声望不知道是什么
		m_pReputation->SetCaption("");
		//
...
bool CChaQeryInfo::SetQueryRFrmData(stQueryResoultPersonInfo* _info, int num) {
	...
	if (m_pFrmQRInfo) {
		...
		for (int i = 0; i < num; i++) {
			...
			if ((*_info).nMinAge == -1)
				continue;
			_snprintf_s(resoult, _TRUNCATE, "%s  %d  %s  %s  %s ", (*_info).sChaName,
