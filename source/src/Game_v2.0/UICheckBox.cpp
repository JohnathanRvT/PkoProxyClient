...
bool CCheckBox::MouseRun(int x, int y, MouseClickState key) {
	if (!IsNormal())
		return false;

	if (IsNoDrag(x, y, key)) {
		if (_isDown && ((key & MouseClickState::LUp) != MouseClickState())) {
			_isDown = false;
...
void CCheckGroup::_SetSelf() {
	...
	for (auto* item : _items) {
		static_cast<CCheckBox*>(item)->evtCheckChange = _CheckChange;
		static_cast<CCheckBox*>(item)->SetIsChecked(false);
	}
