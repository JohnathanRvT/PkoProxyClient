...
bool CCombo::MouseRun(int x, int y, MouseClickState key) {
	if (!IsNormal())
		return false;
	...
	if (_pList->GetIsShow() && ((key & MouseClickState::LDown) != MouseClickState())) {
		_pList->SetIsShow(false);
	}
...
void CCombo::_OnButtonClick(int x, int y, MouseClickState key) {
	_pList->SetIsShow(!_pList->GetIsShow());
	if (evtMouseClick)
		evtMouseClick(this, x, y, key);
}
