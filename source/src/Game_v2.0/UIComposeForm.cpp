...
namespace GUI {
constexpr const char* g_prompt[] = {"Thankyou",
									"Abysmal",
									"Terrible",
									"Bad",
									"Poor",
...
void CComposeMgr::ShowComposeForm(bool bShow /* = true*/) {
	...
	if (bShow) {
		...
		// Show inventory relative to Compose Form
		auto pInventoryForm = g_stUIEquip.GetItemForm();
		pInventoryForm->SetPos(frmCompose->GetX2(), frmCompose->GetY());
...
void CComposeMgr::CheckResult(short sRet, const char* txt) {
	...
	if ((sRet > 0) && (lTimes > 0)) {
		char buff[16] = {0};
		_snprintf_s(buff, _TRUNCATE, "%li", lTimes);
...
void CComposeMgr::PopItem(int iIndex, bool bRetry /* = false*/) {
	...
	auto* pItemCommand = dynamic_cast<CItemCommand*>(cmdCompose[iIndex]->GetCommand());
	if (!pItemCommand)
		return;
...
void CComposeMgr::_evtbtnForgeYes(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	...
	if (szName == "btnForgeYes") {
		...
		if (g_stUICompose.lTimes > 1) {
			...
			_itoa_s(g_stUICompose.lTimes, buff, 10);
