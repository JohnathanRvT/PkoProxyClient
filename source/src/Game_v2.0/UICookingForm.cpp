...
void CCookingMgr::ShowCookingForm(bool bShow /* = true*/) {
	...
	if (bShow) {
		...
		// Show Inventory relative to Cooking Form
		auto pInventoryForm = g_stUIEquip.GetItemForm();
		pInventoryForm->SetPos(frmCooking->GetX2(), frmCooking->GetY());
...
void CCookingMgr::PopItem(int iIndex, bool bRetry /* = false*/) {
	...
	auto* pItemCommand = dynamic_cast<CItemCommand*>(cmdCooking[iIndex]->GetCommand());
	if (!pItemCommand)
		return;
...
void CCookingMgr::_evtbtnForgeYes(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
