...
void CDoublePwdMgr::SendDeleteCharactor() {
	...
	std::string szMD5 = md5string(g_stUIDoublePwd.edtDoublePwd->GetCaption());

	pScene->SendDelChaToServer(szMD5.c_str());
	CGameApp::Waiting();
}
...
void CDoublePwdMgr::RandomInputButton() {
	...
	CTextButton* btnNum[10] = {};

	char szName[32] = {0};
	for (int i = 0; i < 10; ++i) {
		_snprintf_s(szName, _TRUNCATE, "btnNum%d", i);
	...
	for (int i = 0; i < nRandomCount;) {
		nNum1 = rng.uniform(0, 10 - 1);
		nNum2 = rng.uniform(0, 10 - 1);
...
// 创建二次密码表单按钮事件
void CDoublePwdMgr::_evtCreateFromMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	...
	if (strName == "btnYes") {
		...
		if (6 > nPwdLen || 12 < nPwdLen) //TODO: Give an option to provite 0 digits to make it skip all dialogs.
		{
			// 密码长度不在 6 ~ 12 之间
		...
		std::string szMD5 = md5string(g_stUIDoublePwd.edtDoublePwdCreate->GetCaption());

		CS_CreatePassword2(szMD5.c_str());
...
// 修改二次密码表单按钮事件
void CDoublePwdMgr::_evtAlterFromMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string strName = pSender->GetName();
	if (strName.empty())
		return;
	...
	if (strName == "btnYes") {
		...
		std::string szOldMD5 = md5string(g_stUIDoublePwd.edtDoublePwdAlterOld->GetCaption());
		std::string szNewMD5 = md5string(g_stUIDoublePwd.edtDoublePwdAlterNew->GetCaption());

		CS_UpdatePassword2(szOldMD5.c_str(), szNewMD5.c_str());
	} else {
...
// 输入二次密码表单按钮事件
void CDoublePwdMgr::_evtDoublePwdFromMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string strName = pSender->GetName();
	if (strName.empty())
		return;
...
// 二次密码输入软键盘表单按钮事件
void CDoublePwdMgr::_evtInputFromMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
