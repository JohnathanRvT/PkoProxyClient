...
void CEdit::_Copy(const CEdit& rhs) {
	...
	_nCursorHeight = rhs._nCursorHeight;
	...
	default:
		if (GetIsMulti()) {
			...
			else if ((int) _str.length() >= _nMaxNumVisible  ) 
			{
				...
				++ _nCursorCol ; 
				_strVisible = _str;
				if ( _nCursorCol  > _nMaxNumVisible )  // 私聊的最大字符数目
				{	
					_nCursorCol = _nMaxNumVisible ;
					_strVisible = _str.substr( _str.length() -_nMaxNumVisible ,_nMaxNumVisible );
				}
...
void CEdit::_Copy() {
	...
	// 拷贝字符串
strncpy_s( pData, len , _str.c_str(),_TRUNCATE);
	pData[ len ] = 0;
...
void CEdit::Render() {
	...
	if (!GetIsPassWord()) {
		const auto& Text = _str.length() > _nMaxNumVisible ? _strVisible : _str;
		if (_bParseText) {
			g_TextParse.Render(Text, GetX() - _nOffset, GetY(), _color);
		} else {
			CGuiFont::s_Font.Render(Text.c_str(), GetX() - _nOffset, GetY(), _color);
		}
	} else {
		auto len = _str.length() < _nMaxNum ? _str.length() : _nMaxNum;
		const std::string password(len, '*');
		CGuiFont::s_Font.Render(password.c_str(), GetX() - _nOffset, GetY(), _color);
...
void CEdit::SetCaption(const char* str) {
	if (GetActive() == this) {
		if ((int)strlen(str) <= this->GetMaxNum()) // Add by lark.li 20080820
		{
...
void CEdit::OnActive() {
	...
	::SetWindowPos(g_InputBox.GetEditWindow(), nullptr, GetX(), GetY(), 10000, 50, SWP_NOOWNERZORDER | SWP_NOREDRAW | SWP_HIDEWINDOW);
