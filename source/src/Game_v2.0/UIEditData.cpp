...
void CEditArticle::Render() {
	...
	char szBuf[10];
	_snprintf_s(szBuf, _TRUNCATE, "%d, %d, %d", _nCursor, _nSelStart, _nSelEnd);
...
bool CEditArticle::_AddObj(CEditObj* pObj) {
	...
	if (_nCursor >= _memorys.size()) {
		...
		auto it = std::find(_memorys.begin(), _memorys.end(), r);
...
void CEditArticle::DelSelect() {
	if (_eRunType == enumSelect) {
		if (_nSelEnd <= _memorys.size() && _nSelStart <= _memorys.size()) {
			...
			auto it1 = std::find(_memorys.begin(), _memorys.end(), r1);
			auto it2 = std::find(_memorys.begin(), _memorys.end(), r2);
