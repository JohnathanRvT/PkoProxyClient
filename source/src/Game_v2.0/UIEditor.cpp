...
bool CEditor::Init() {
	...
	auto* edtShotMiniMap = dynamic_cast<CEdit*>(frmEditor->Find("edtShotMiniMap"));
	if (edtShotMiniMap)
	...
	char buf[10] = {0};
	_itoa_s(g_Editor.m_nBrushHeight, buf, 10);
...
void CEditor::_TreeSelectLost(CGuiData* pSender) {
	auto* tree = dynamic_cast<CTreeView*>(pSender);
	if (!tree)
		return;
...
void CEditor::_TreeMouseDown(CGuiData* pSender, int x, int y, MouseClickState key) {
	auto* tree = dynamic_cast<CTreeView*>(pSender);
	if (!tree)
		return;
...
void CEditor::_TreeEvent(CTreeView* tree, bool press) {
	...
	auto* select = dynamic_cast<CTreeGridNode*>(tree->GetSelectNode());
	if (!select)
		return;

	auto* item = dynamic_cast<CItem*>(select->GetItem());
	if (!item)
		return;

	if (_stricmp(item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_528)) == 0) {
		auto* p = dynamic_cast<CGraph*>(tree->GetSelect()->GetItem());
		if (p)
	...
	if (_stricmp(item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_535)) == 0) {
		...
		if (p) {
			...
			case 0: {
				...
				break;
			case 1:
				auto st = std::make_unique<CReadyDieState>(pCha->GetActor());
				st->SetState(enumDied);
				pCha->GetActor()->InsertState(std::move(st));
	...
	auto* parent = dynamic_cast<CTreeNode*>(select->GetParent());
	if (!parent)
		return;
...
void CEditor::_ColorMouseDown(CGuiData* pSender, int x, int y, MouseClickState key) {
...
void CEditor::_HideTreeCheckChange(CGuiData* pSender) {
	auto* g = dynamic_cast<CCheckBox*>(pSender);
	if (!g)
		return;
...
bool CEditor::_evtShotMiniMap(CGuiData* pSender, char& key) {
	...
	if (key == ',' || isdigit(key) || key == VK_RETURN || key == VK_BACK || key == '-') {
		if (key == VK_RETURN) {
			auto* pEdit = dynamic_cast<CEdit*>(pSender);
			if (!pEdit)
				return false;
	...
	if (isdigit(key) || key == VK_RETURN || key == VK_BACK || key == '-') {
		if (key == VK_RETURN) {
			...
			if (str.empty()) {
				...
				auto* pEffect = dynamic_cast<CEffectObj*>(g_Editor.GetSelectSceneObj());
				if (!pEffect) {
...
void CEditor::_EraseAttribCheckChange(CGuiData* pSender) {
	auto* cb = dynamic_cast<CCheckBox*>(pSender);
