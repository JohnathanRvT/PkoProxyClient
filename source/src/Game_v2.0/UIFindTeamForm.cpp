...
bool CFindTeamMgr::Init() {
	...
	for (int i = 0; i < FINDTEAM_PAGE_SIZE; ++i) {
		...
		_snprintf_s(szName, _TRUNCATE, "labLevel_%d", i);
		...
		_snprintf_s(szName, _TRUNCATE, "labJob_%d", i);
		...
		_snprintf_s(szName, _TRUNCATE, "labPlace_%d", i);
		...
		_snprintf_s(szName, _TRUNCATE, "btnSubmit_%d", i);
...
void CFindTeamMgr::SetFindTeamPage(int nCurPage, int nPageNum) {
	...
	char szPage[32] = {0};
	_snprintf_s(szPage, _TRUNCATE, "%d/%d", nCurPage, nPageNum);
...
void CFindTeamMgr::AddFindTeamInfo(int nSeq, const char* szName, long nLevel, long nJob, const char* szPlace) {
	if (0 <= nSeq && nSeq < FINDTEAM_PAGE_SIZE) {
		...
		char szLevel[32] = {0};
		_snprintf_s(szLevel, _TRUNCATE, "%d", nLevel);
...
void CFindTeamMgr::FindTeamAsk(const char* szName) {
	...
	char szBuffer[256] = {0};
	_snprintf_s(szBuffer, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_863), szName);
	CBoxMgr::ShowSelectBox(_evtFindTeamCheckEvent, szBuffer, true);
}

// 寻找组队界面按钮消息处理
void CFindTeamMgr::_evtFindTeamMouseButton(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
// 组队申请回调
void CFindTeamMgr::_evtFindTeamCheckEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
