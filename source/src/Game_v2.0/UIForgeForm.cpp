...
bool CForgeMgr::Init() {
	...
	auto* btnHelp = static_cast<CTextButton*>(frmNPCforge->Find("btnHelp"));
	if (!btnHelp)
		return false;
	...
	for (int i(0); i < ITEM_NUM; i++) {
		_snprintf_s(szBuf, _TRUNCATE, "cmdForgeItem%d", i);
...
// Callback Function
//---------------------------------------------------------------------------
void CForgeMgr::_MainMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
//---------------------------------------------------------------------------
void CForgeMgr::_evtConfirmEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
void CForgeMgr::_DragBeforeEvt(CGuiData* pSender, int x, int y, MouseClickState key) {
	if (g_stUIForge.IsRunning())
		if (pSender->GetDrag())
...
bool CForgeMgr::IsEquip(CItemCommand& rItem) {
	...
	if (pItemRecord) {
		...
		if (sType < EQUIP_TYPE &&
			sType != EItemType::Arrow &&
			sType != EItemType::Ammo &&
			sType != EItemType::LifeSkillRepairHammer &&
			sType != EItemType::LifeSkillAxe &&
			sType != EItemType::LifeSkillPickAxe /*&& sType != 20*/ &&
			sType != EItemType::Face)
			return true;
	}
...
bool CForgeMgr::IsGenStone(CItemCommand& rItem) {
	...
	if (pItemRecord)
		return pItemRecord->sType == EItemType::GemStone;
...
bool CForgeMgr::IsForgStone(CItemCommand& rItem) {
	...
	if (pItemRecord)
		return pItemRecord->sType == EItemType::RefiningStone;
...
void CForgeMgr::SetForgeUI() {
	...
	*/
	...
	if (cmdForgeItem[EQUIP]->GetCommand()) {
		...
		if (pItem) {
			std::string sEquipState = RES_STRING(CL_LANGUAGE_MATCH_841);
	...
	if (!m_isMilling) {
		if (cmdForgeItem[GEN_STONE]->GetCommand()) {
			...
			if (pItem) {
				std::string sEquipState = RES_STRING(CL_LANGUAGE_MATCH_842) + pItemCommand->GetStoneHint(1);
				pItem->GetBegin()->SetString(sEquipState.c_str());
			}
		} else {
			CItemRow* pItem;
...
void CForgeMgr::evtShowHelpInfo(CGuiData* pSender, int x, int y, MouseClickState key) {
