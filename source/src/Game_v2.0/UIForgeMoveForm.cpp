...
bool CForgeMoveMgr::Init() {
	...
	for (int i = 0; i < REQUIRED_ITEM_COUNT; i++) {
		if (cmdRequirement[i]) {
			...
			cmdRequirement[i]->SetActivePic(nullptr);
		}
	}
	...
	auto* btnHelp = static_cast<CTextButton*>(frmforgeMove->Find("btnHelp"));
	if (!btnHelp)
		return false;
...
void CForgeMoveMgr::PushItem(int Index, CItemCommand& Item) {
	auto* pItemCommand = static_cast<CItemCommand*>(cmdRequirement[Index]->GetCommand());
	if (pItemCommand)
		PopItem(Index);
...
void CForgeMoveMgr::evtRequirementEvent(CGuiData* pSender, CCommandObj* pItem, bool& isAccept) {
	...
	if (pCmd->nTag != 999) {
		...
		if (pCmd == cmdRequirement[0]) {
			if (pItemRecord->sType < 5 || pItemRecord->sType == EItemType::Mitten || pItemRecord->sType == EItemType::Cosh ||
				pItemRecord->sType == EItemType::Shield || pItemRecord->sType == EItemType::Hair || pItemRecord->sType == EItemType::Tattoo ||
			...
			if (pItemRecord->sType < 5 || pItemRecord->sType == EItemType::Stylet || pItemRecord->sType == EItemType::Cosh ||
				pItemRecord->sType == 11 || pItemRecord->sType == EItemType::Hair || pItemRecord->sType == EItemType::Tattoo ||
				(pItemRecord->sType > 21 && pItemRecord->sType < 25)) {
...
void CForgeMoveMgr::evtForgeMove(CGuiData* pSender, int x, int y, MouseClickState key) {
	...
	for (int i = 0; i < REQUIRED_ITEM_COUNT; ++i) {
		kNetItemForgeAsk.SGroup[i].sCellNum = RequirementCounts[i] ? 1 : 0; // ΚΌΦΥΚΗ1
...
void CForgeMoveMgr::evtThrowDialogEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
void CForgeMoveMgr::evtShowHelpInfo(CGuiData* pSender, int x, int y, MouseClickState key) {
