...
bool CForm::_AddFrameMove(CCompent* c, bool isCheck) {
	// Prevent duplicates to be appended if desired
	if (isCheck && std::find(_frames.begin(), _frames.end(), c) != _frames.end()) {
		return false;
	}
...
void CForm::_CopyCompent(const CForm* rhs) {
	for (auto const& it : rhs->_allCompents) {
		if (auto* p = dynamic_cast<CCompent*>(it->Clone())) {
			p->SetForm(this);
			p->SetParent(this);
			p->_AddForm();
		}
...
void CForm::Init() {
	if (_pParent) {
		auto* rhs = dynamic_cast<CForm*>(_pParent);
		_pParent = nullptr;
...
void CForm::Render() {
	...
	for (auto const& component : _compents) {
		if (component->GetIsShow()) {
			component->Render();
		}
	}
...
void CForm::Refresh() {
	...
	for (auto const& it : _compents) {
		it->Refresh();
	}

	for (auto const& it : _childs) {
		it->Refresh();
	}
}

void CForm::SetActiveCompent(CCompent* pActive) {
	auto nCount = (int)_actives.size();
...
bool CForm::MenuMouseRun(int x, int y, MouseClickState key) {
...
bool CForm::MouseRun(int x, int y, MouseClickState key) {
	if (!IsNormal())
		return false;
	...
	for (auto rit = _mouse.rbegin(); rit != _mouse.rend(); rit++) {
		if ((*rit)->MouseRun(x, y, key)) {
			if (!CCompent::_pLastMouseCompent) {
				CCompent::_pLastMouseCompent = *rit;
			}
			if (IsNormal() && ((key & MouseClickState::LDown) != MouseClickState())) {
				_ActiveForm(key);
			}
			return true;
		}
	...
	if (InRect(x, y) && ((key & MouseClickState::LDown) != MouseClickState()) && IsNormal()) {
		_ActiveForm(key);
	}

	if (!GetParent() && _pDrag && static_cast<bool>(_pDrag->BeginMouseRun(this, _IsMouseIn, x, y, key))) {
		return true;
	}
	return _IsMouseIn;
}
...
CCompent* CForm::GetHitCommand(int x, int y) {
	for (auto const& it : _mouse) {
		if (CCompent* pCommand = it->GetHitCommand(x, y)) {
			return pCommand;
		}
	}
	return nullptr;
...
CGuiData* CForm::GetHintGui(int x, int y) {
	for (auto const& it : _compents) {
		if (CCompent* pHint = it->GetHintCompent(x, y)) {
			return pHint;
		}
	}
...
CCompent* CForm::Find(const char* str) {
	for (auto const& it : _allCompents) {
...
void CForm::_DragMouseEvent(CGuiData* pSender, int x, int y, MouseClickState key) {
	auto* frm = dynamic_cast<CForm*>(pSender);
...
void CForm::_DragMouseEventBegin(CGuiData* pSender, int x, int y, MouseClickState key) {
...
void CForm::PopMenu(CMenu* pMenu, int x, int y) {
	...
	if (_pPopMenu) {
		if (_pPopMenu->GetParent() && _pPopMenu->GetParent() != this) {
			auto* old_form = static_cast<CForm*>(_pPopMenu->GetParent());
...
void CForm::SetParent(CGuiData* p) {
	...
	if (!p || dynamic_cast<CForm*>(p)) {
		auto* frm = dynamic_cast<CForm*>(_pParent);
		if (frm) {
