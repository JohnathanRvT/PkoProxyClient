...
void CFoundMgr::ShowFoundForm(bool bShow /* = true*/) {
	...
	if (bShow) {
		...
		// Show Inventory relative to Crafting Form
		auto pInventoryForm = g_stUIEquip.GetItemForm();
		pInventoryForm->SetPos(frmFound->GetX2(), frmFound->GetY());
		pInventoryForm->Refresh();
		pInventoryForm->SetIsShow(true);
	} else {
		if (!bLock) {
			frmFound->Hide();
		}
...
bool CFoundMgr::Init() {
	...
	char buff[32] = {0};
	for (int i = 0; i < FOUND_COUNT; i++) {
		_snprintf_s(buff, _TRUNCATE, "cmdItemBase%i", (i + 1));
	...
	for (int i = 0; i < 6; i++) {
		_snprintf_s(buff, _TRUNCATE, "imgShaiZi1_%i", (i + 1));
		Dice1[i] = dynamic_cast<CImage*>(frmFound->Find(buff));
		if (!Dice1[i]) {
			LG("gui", "frmFound::%s not found.\n", buff);
			return false;
		}
		_snprintf_s(buff, _TRUNCATE, "imgShaiZi2_%i", (i + 1));
		Dice2[i] = dynamic_cast<CImage*>(frmFound->Find(buff));
		if (!Dice2[i]) {
			LG("gui", "frmFound::%s not found.\n", buff);
			return false;
		}
		_snprintf_s(buff, _TRUNCATE, "imgShaiZi3_%i", (i + 1));
...
void CFoundMgr::PopItem(int iIndex, bool bRetry /* = false*/) {
	...
	auto* pItemCommand = dynamic_cast<CItemCommand*>(cmdFound[iIndex]->GetCommand());
	if (!pItemCommand)
		return;
...
void CFoundMgr::_evtbtnForgeYes(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
void CFoundMgr::_ProTimeArriveEvt(CGuiData* pSender) {
	...
	CS_Found(g_stUINpcTalk.GetNpcId(), g_stUIFound.iFoundPos, (FOUND_COUNT - 1), (rng.uniform((short)0, (short)(2 - 1))));
}
...
void CFoundMgr::_evtDragItemBase1(CGuiData* pSender, CCommandObj* pItem, bool& isAccept) {
	...
	if (!pItemCommand || !pItemCommand->GetIsValid())
		return;
	if (pItemCommand->GetItemInfo()->sType != EItemType::Pet) {
		return;
	}
