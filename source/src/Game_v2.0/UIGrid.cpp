...
void CGrid::_Copy(const CGrid& rhs) {
	...
	for (auto* p : rhs._memory) {
		_memory.push_back(p->Clone());
	}
...
bool CGrid::MouseRun(int x, int y, MouseClickState key) {
	if (!IsNormal())
		return false;
	...
	if (IsNoDrag(x, y, key)) {
		...
		if (_IsSelect) {
			...
			if (((key & MouseClickState::LDown) != MouseClickState()) && evtSelectChange)
