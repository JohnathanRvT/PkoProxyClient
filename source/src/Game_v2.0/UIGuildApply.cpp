...
void CUIGuildApply::OnConfirm(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
void CUIGuildApply::OnShowForm(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (m_eType == CGuildData::navy) {
		m_pGuildNameEdit->SetCaption(RES_STRING(CL_LANGUAGE_MATCH_579));
		m_pGuildNameEdit->SetIsEnabled(false);
		m_pGuildNameInputForm->ShowModal();
