...
bool CGuildChallengeMgr::Init() {
	...
	char szBuf[32];
	for (int i(0); i < NUM; i++) {
		_snprintf_s(szBuf, _TRUNCATE, "labGuildName%d", i);
		...
		_snprintf_s(szBuf, _TRUNCATE, "labChallenger%d", i);
		...
		_snprintf_s(szBuf, _TRUNCATE, "labMoney%d", i);
		...
		_snprintf_s(szBuf, _TRUNCATE, "btnCharge%d", i);
...
//-------------------------------------------------------------------------
void CGuildChallengeMgr::_FirstChallengeButtonDown(CGuiData* pSender, int x, int y, MouseClickState key) {
	g_stGuildChallenge.ChargeMoney(CGuildChallengeMgr::FIRST);
}
//-------------------------------------------------------------------------
void CGuildChallengeMgr::_SecondChallengeButtonDown(CGuiData* pSender, int x, int y, MouseClickState key) {
	g_stGuildChallenge.ChargeMoney(CGuildChallengeMgr::SECOND);
}
//-------------------------------------------------------------------------
void CGuildChallengeMgr::_ThirdChallengeButtonDown(CGuiData* pSender, int x, int y, MouseClickState key) {
	g_stGuildChallenge.ChargeMoney(CGuildChallengeMgr::THIRD);
}
//-------------------------------------------------------------------------
void CGuildChallengeMgr::_enterChargeMoney(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	...
	char buf[256] = {0};
	_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_583), StringSplitNum(iChargeMoney));
	g_stUIBox.ShowSelectBox(_ChargeEvent, buf, true);
}
//-------------------------------------------------------------------------
void CGuildChallengeMgr::_ChargeEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
void CGuildChallengeMgr::ChargeMoney(int iIndex) {
	...
	_snprintf_s(szBuf, _TRUNCATE, "%d", iNextCharge);
