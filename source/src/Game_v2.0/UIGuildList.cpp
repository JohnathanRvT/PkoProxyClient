...
void CUIGuildList::ShowGuildList() {
	...
	for (DWORD i = 0; i < CGuildListMgr::GetTotalGuilds(); i++) {
		...
		_itoa_s(pListData->GetMembers(), buf, 10);
		auto* pGuildMemberCountItem = new CItem(buf, COLOR_BLACK);
		_i64toa_s(pListData->GetExperence(), buf, sizeof(buf), 10);
		auto* pGuildExperienceItem = new CItem(buf, COLOR_BLACK);
...
void CUIGuildList::OnClickSortByGuildName(CGuiData* pSender, int x, int y, MouseClickState key) {
	m_bSortIncByName = !m_bSortIncByName;
	CGuildListMgr::SortGuildsByName(m_bSortIncByName);
	ShowGuildList();
}

void CUIGuildList::OnClickSortByGuildMemberCount(CGuiData* pSender, int x, int y, MouseClickState key) {
	m_bSortIncByMemberCount = !m_bSortIncByMemberCount;
	CGuildListMgr::SortGuildsByMemberCount(m_bSortIncByMemberCount);
	ShowGuildList();
}

void CUIGuildList::OnClickSortByGuildExperience(CGuiData* pSender, int x, int y, MouseClickState key) {
...
void CUIGuildList::OnClickApply(CGuiData* pSender, int x, int y, MouseClickState key) {
	CItemRow* pItemRow = m_pGuildList->GetList()->GetSelectItem();
	if (!pItemRow)
		return;
	auto* pListData = static_cast<CGuildListData*>(pItemRow->GetPointer());
	if (!pListData)
		return;
...
void CUIGuildList::OnMsgReplaceApply(std::string strOldGuildName) {
	std::string str = RES_STRING(CL_LANGUAGE_MATCH_593) + strOldGuildName;
	CBoxMgr::ShowSelectBox(OnMsgReplaceApplySelectClick, str.c_str(), true);
}

void CUIGuildList::OnMsgReplaceApplySelectClick(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	auto* pButton = dynamic_cast<CTextButton*>(pSender);
