...
CTextButton*	CUIGuildMgr::btnyqrh = nullptr;
CEdit*	CUIGuildMgr::edtyqname = nullptr;

CUIGuildMgr::CUIGuildMgr() {
}
...
void CUIGuildMgr::_OnClickEditMottoName(CGuiData* pSender, int x, int y, MouseClickState key) {
...
void CUIGuildMgr::_OnClickDismiss(CGuiData* pSender, int x, int y, MouseClickState key) {
	CGuildMemberData* pSelfData = CGuildMembersMgr::GetSelfData();
	if (pSelfData && pSelfData->IsManager()) {
...
void CUIGuildMgr::_OnClickRecruit(CGuiData* pSender, int x, int y, MouseClickState key) {
	CGuildMemberData* pSelfData = CGuildMembersMgr::GetSelfData();
	if (pSelfData && pSelfData->IsManager()) {
		...
		auto* pMemberData = static_cast<CRecruitMemberData*>(pRow->GetPointer());
...
void CUIGuildMgr::_OnClickRefuse(CGuiData* pSender, int x, int y, MouseClickState key) {
	CGuildMemberData* pSelfData = CGuildMembersMgr::GetSelfData();
	if (pSelfData && pSelfData->IsManager()) {
		CItemRow* pRow = m_plstRecruitMember->GetList()->GetSelectItem();
		if (!pRow) {
...
void CUIGuildMgr::_OnClickKick(CGuiData* pSender, int x, int y, MouseClickState key) {
	CGuildMemberData* pSelfData = CGuildMembersMgr::GetSelfData();
	if (pSelfData && pSelfData->IsManager()) {
		...
		auto* pMemberData = static_cast<CGuildMemberData*>(pRow->GetPointer());
		std::string str = RES_STRING(CL_LANGUAGE_MATCH_601) + pMemberData->GetName() + RES_STRING(CL_LANGUAGE_MATCH_602);
...
void CUIGuildMgr::_OnClickMottoFormOK(CGuiData* pSender, int x, int y, MouseClickState key) {
...
void CUIGuildMgr::RefreshAttribute() {
	...
	_snprintf_s(buf, _TRUNCATE, "%d/%d", CGuildData::GetMemberCount(), CGuildData::GetMaxMembers());
	...
	if (state == CGuildData::normal) {
		...
		if (remain > 1440) {
			_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_611), remain / 1440);
		} else if (remain > 60) {
			_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_612), remain / 60);
		} else {
			_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_613), remain);
		}
...
void CUIGuildMgr::RefreshList() {
	...
	for (DWORD i = 0; i < CGuildMembersMgr::GetTotalGuildMembers(); i++) {
		...
		auto* pMemberLevelItem = new CItem(buf, COLOR_BLACK);
...
void CUIGuildMgr::_OnPassDismiss(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes)
		return;
	auto* pBox = (stPasswordBox*)pSender->GetForm()->GetPointer();
	if (!pBox)
		return;
...
void CUIGuildMgr::_OnClickLeave(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType == CForm::mrYes) {
		CM_GUILD_LEAVE();
	}
}

void CUIGuildMgr::_OnPassKick(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes)
		return;
	CItemRow* pRow = m_plstGuildMember->GetList()->GetSelectItem();
	if (!pRow) {
...
void CUIGuildMgr::SendInviteName(CGuiData *pSender, int x, int y, MouseClickState key)
{
	CGuildMemberData* pSelfData=CGuildMembersMgr::GetSelfData();
	if ( pSelfData && pSelfData->IsManager())
	{
		std::string name = edtyqname->GetCaption();
		if (!name.empty())
		{
