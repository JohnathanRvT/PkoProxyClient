...
bool CItemReMgr::Init() {
	...
	auto* btnHelp = static_cast<CTextButton*>(frmItemRe->Find("btnHelp"));
	if (!btnHelp)
		return false;
...
void CItemReMgr::evtRequirementEvent(CGuiData* pSender, CCommandObj* pItem, bool& isAccept) {
	...
	if (pCmd->nTag != 999) {
		...
		if (pCmd == cmdRequirement[0]) {
			if (pItemRecord->sType == EItemType::Pet)
			...
			if (pItemRecord->sType == EItemType::Tattoo || pItemRecord->sType <= 24)
...
void CItemReMgr::evtItemReset(CGuiData* pSender, int x, int y, MouseClickState key) {
	char buf[256] = {0};
...
void CItemReMgr::evtShowHelpInfo(CGuiData* pSender, int x, int y, MouseClickState key) {
