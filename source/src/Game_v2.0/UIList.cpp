...
bool CListItems::Clone(CListItems* rhs) {
	auto* p = dynamic_cast<CListItems*>(rhs);
	if (!p)
		return false;
	*this = *p;
	return true;
...
bool CListItems::MouseRun(int x, int y, MouseClickState key) {
	if (x >= _nX1 && x <= _nX2 && y >= _nY1 && y <= _nY2) {
		if (_IsMouseFollow || ((key & MouseClickState::LDown) != MouseClickState()) || ((key & MouseClickState::LDB) != MouseClickState()) || ((key & MouseClickState::RDown) != MouseClickState())) {
...
bool CListItemsEx::Clone(CListItems* rhs) {
	auto* p = dynamic_cast<CListItemsEx*>(rhs);
	if (!p)
		return false;
	*this = *p;
	return true;
...
bool CList::MouseRun(int x, int y, MouseClickState key) {
	if (!IsNormal())
		return false;
