...
bool CLotteryMgr::Init() {
	...
	for (int i(0); i < ITEM_NUM; i++) {
		_snprintf_s(szBuf, _TRUNCATE, "cmdNoItem%d", i);
...
// Callback Function
//---------------------------------------------------------------------------
void CLotteryMgr::_MainMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
//---------------------------------------------------------------------------
void CLotteryMgr::_evtConfirmEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
void CLotteryMgr::_DragBeforeEvt(CGuiData* pSender, int x, int y, MouseClickState key) {
	if (pSender->GetDrag())
...
void CLotteryMgr::PopItem(int iIndex) {
	...
	auto* pItemCommand = dynamic_cast<CItemCommand*>(cmdNoItem[iIndex]->GetCommand());
...
void CLotteryMgr::PopLottery() {
	// 删除Cmd中的Item，该Item会在PushItem()中由new生成
	auto* pItemCommand = dynamic_cast<CItemCommand*>(cmdLotteryItem->GetCommand());
...
void CLotteryMgr::SetLotteryUI() {
	...
	//		sEquipState += "  +";
	//      _itoa_s(rItemForgeInfo.nLevel,szBuf, _countof(szBuf),10);
	//		sEquipState += szBuf;
	...
	//			else
	//			{
	//              _itoa_s(rItemForgeInfo.nStoneLevel[0], szBuf, 10);
	//				sEquipState += szBuf;
	...
	//			else
	//			{
	//              _itoa_s(rItemForgeInfo.nStoneLevel[1], szBuf, 10);
	//				sEquipState += szBuf;
	//				sEquipState += RES_STRING(CL_LANGUAGE_MATCH_837);
	...
	//			else
	//			{
	//              _itoa_s(rItemForgeInfo.nStoneLevel[2], szBuf, 10);
	//				sEquipState += szBuf;
	//				sEquipState += RES_STRING(CL_LANGUAGE_MATCH_837);
