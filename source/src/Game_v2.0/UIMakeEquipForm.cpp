...
bool CMakeEquipMgr::Init() {
	...
	auto* btnHelp = static_cast<CTextButton*>(frmMakeEquip->Find("btnHelp"));
	if (!btnHelp)
		return false;
	...
	for (int i(0); i < ITEM_NUM; i++) {
		_snprintf_s(szBuf, _TRUNCATE, "cmdItem%d", i);
...
void CMakeEquipMgr::ShowConfirmDialog(long lMoney) {
	char szBuf[255] = {0};
	_snprintf_s(szBuf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_568), lMoney);
...
void CMakeEquipMgr::PushEquipFusionItem(int iIndex, CItemCommand& rItem) {
	if (iIndex == 0) {
		...
		if ((pItemCommand->GetItemInfo()->sType == EItemType::Tattoo && rItem.GetItemInfo()->sType == EItemType::Clothing) ||
...
void CMakeEquipMgr::PushRouleau(CItemCommand& rItem) {
	...
	// 将创建的Command放入
	auto* pItemCmd = new CItemCommand(rItem);
	cmdRouleau->AddCommand(pItemCmd);
	pItemCmd->SetIsValid(true);

	// 更新相应配方的COneCommand的数据,这里分成两个流程，道具精炼和宝石合成
	if (rItem.GetItemInfo()->sType == EItemType::GemCombineScroll) { //宝石合成
		PushNewGems();
	} else { //道具精炼
...
bool CMakeEquipMgr::CanPushStone(int iIndex, CItemCommand& rItem) {
	...
	//  不是宝石和精炼石直接返回false
	if (pItemRecord->sType != EItemType::GemStone && pItemRecord->sType != EItemType::RefiningStone) {
...
bool CMakeEquipMgr::IsEquipMakeRouleau(CItemCommand& rItem) {
	...
	if (pItemRecord && pItemRecord->sType == EItemType::GemCombineScroll) {
		return true;
	}
	return false;
}
...
bool CMakeEquipMgr::IsEquipFusionRouleau(CItemCommand& rItem) {
	...
	if (pItemRecord && pItemRecord->sType == EItemType::FusionScroll) {
		return true;
	}
	return false;
}
...
bool CMakeEquipMgr::IsEquipUpgradeRouleau(CItemCommand& rItem) {
	...
	if (pItemRecord && pItemRecord->sType == EItemType::StrengthenScroll) {
		return true;
	}
	return false;
}
...
bool CMakeEquipMgr::IsEquipFusionCatalyzer(CItemCommand& rItem) {
	...
	if (pItemRecord && pItemRecord->sType == EItemType::FusionCatalyst) {
		return true;
	}
	return false;
}
...
// 回调函数
//-----------------------------------------------------------------------------
void CMakeEquipMgr::_MainMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
//-----------------------------------------------------------------------------
void CMakeEquipMgr::_evtConfirmEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
//---------------------------------------------------------------------
void CMakeEquipMgr::_evtFusionNoCatalyzerConfirmEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
bool CMakeEquipMgr::IsMakeGem() {
	auto* pItemCommand =
		dynamic_cast<CItemCommand*>(cmdRouleau->GetCommand());
	if (pItemCommand && pItemCommand->GetItemInfo()->sType == GemCombineScroll) {
		return true;
	}
...
bool CMakeEquipMgr::IsElfShiftItem(CItemCommand& rItem) {
	CItemRecord* pItemRecord = rItem.GetItemInfo();
	if (pItemRecord && pItemRecord->sType == EItemType::Pet) {
		return true;
	}
...
void CMakeEquipMgr::PushElfShiftItem(int iIndex, CItemCommand& rItem) {
	...
	int nLevel = sItemHint.sInstAttr[ITEMATTR_VAL_STR] +
				 ...
				 sItemHint.sInstAttr[ITEMATTR_VAL_LUK];
...
void CMakeEquipMgr::evtShowHelpInfo(CGuiData* pSender, int x, int y, MouseClickState key) {
