...
void CMenu::_Copy(const CMenu& rhs) {
	...
	_MenusClear();
	for (auto* menu : rhs._menus)
		_menus.push_back(menu->Clone());
}
...
bool CMenu::MouseRun(int x, int y, MouseClickState key) {
	if (!IsNormal())
		return false;
	...
	if (InRect(x, y)) {
		...
		for (menus::iterator it = _menus.begin(); it != _menus.end(); it++) {
			...
			if (y >= y0 && y <= y1) {
				...
				if (((key & MouseClickState::LDown) != MouseClickState()) && _pSelectMenu->GetIsEnabled()) {
					if (evtListMouseDown)
...
void CMenu::Render() {
	...
	for (auto* menu : _menus) {
		if (!menu->GetIsHide()) {
			if (_pSelectMenu == menu) {
				_pSelect->Render(x, y0);
			}

			menu->Render(x, y0);
			y0 += menu->GetHeight() + _nRowSpace;
		}
	}
}
...
bool CMenu::IsAllDisabled() {
	for (auto* menu : _menus) {
		if (menu->GetIsEnabled() && !menu->GetIsHide())
			return false;
	}
	return true;
}
