...
CForm* CMiniMapMgr::frmRadar = nullptr;
CEdit* CMiniMapMgr::edtX = nullptr;
CEdit* CMiniMapMgr::edtY = nullptr;
CCombo* CMiniMapMgr::cboAddr = nullptr;
...
bool CMiniMapMgr::Init() {
	...
	for (int i = 0; i < MAP_POS_MAX; i++) {
		_snprintf_s(szName, _TRUNCATE, "labMapPos%d", i);
...
void CMiniMapMgr::_evtShowNPCList(CGuiData* pSender, int x, int y, MouseClickState key) {
	CForm* f = CFormMgr::s_Mgr.Find("frmNpcShow");
	bool b = !f->GetIsShow();
	g_stUIStart.ShowCurrentHelper(g_stUIStart.GetCurrMapName(), b);
}

void CMiniMapMgr::_evtShowQueen(CGuiData* pSender, int x, int y, MouseClickState key) {
	g_stUIStart.ShowQueenButtonForm(true);
}

void CMiniMapMgr::_evtShowQQ(CGuiData* pSender, int x, int y, MouseClickState key) {
...
void CMiniMapMgr::_evtSearchQueen(CGuiData* pSender, int x, int y, MouseClickState key) {
...
void CMiniMapMgr::_evtShowbigmap(CGuiData* pSender) {
	auto* pWorldScene = dynamic_cast<CWorldScene*>(CGameApp::GetCurScene());
	if (!pWorldScene)
		return;
...
void CMiniMapMgr::RefreshChaPos(int x, int y) {
	...
	_snprintf_s(buf, _TRUNCATE, "%d,%d", x / 100, y / 100);
...
void CMiniMapMgr::_MiniFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
void CMiniMapMgr::_RadarFormEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string name = pSender->GetName();
...
void CMiniMapMgr::ShowRadar(const char* szX, const char* szY) {
	...
	if ((strcmp(curMap, _strName) == 0) && (!pScene->GetMainCha()->IsBoat())) {
		if (g_stUIStart.chkID->GetIsChecked()) {
			...
			g_cFindPathEx.SetDestDirection(pScene->GetMainCha()->GetPos().x, pScene->GetMainCha()->GetPos().y, x, y);
			g_cFindPathEx.SetTarget(pScene->GetMainCha()->GetPos().x, pScene->GetMainCha()->GetPos().y, x, y);
		}
	} else {
...
//Begin
void CMiniMapMgr::_RenderBigMapHint() {
	...
	if (rcBigMap.left < ptMouse.x && ptMouse.x < rcBigMap.right &&
		...
		if (pInfo) {
			...
			// 获得该区域怪物信息
			CChaRecord* pMonsterInfo[MONSTER_LIST_MAX] = {nullptr};
			...
			for (int i = 0; i < MONSTER_LIST_MAX; ++i) {
				...
				char szBuf[256] = {0};
				_snprintf_s(szBuf, _TRUNCATE, " %d. %s   LV:%2d ", nMonCount, pMonsterInfo[i]->szName, pMonsterInfo[i]->lLv);
			...
			if (nMonCount > 0) {
				auto nLevelCha = static_cast<int>(g_pGameApp->GetCurScene()->GetMainCha()->getGameAttr()->get(ATTR_LV));
		...
		//        // 绘制鼠标指向的坐标
		char szBuf[256] = {0};
		_snprintf_s(szBuf, _TRUNCATE, " Point: %d, %d ", ptCurMouse.x, ptCurMouse.y);
