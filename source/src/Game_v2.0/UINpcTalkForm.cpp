	...
	if (byCount > 0) //读取选项数据
	{
		...
		for (int i = 0; i < byCount; i++) {
			if (FuncArray.FuncItem[i].szFunc[0] == '@') {
				std::string item = &FuncArray.FuncItem[i].szFunc[1];
				size_t pos = item.find("http");
				if (pos == std::string::npos) {
					memCtrl->AddItemRowContent(i, item.c_str());
				} else {
					std::string http = item.substr(pos, item.length());
	...
	if (byMisNum > 0) {
		...
		for (int i = 0; i < byMisNum; i++) {
			if (FuncArray.MisItem[i].byState == ROLE_MIS_ACCEPT) {
				auto* p = new CGraph("texture/ui/SystemBotton3.tga", 7, 15, 170, 119, 1);
				memCtrl->AddIcon(i, p);
			} else if (FuncArray.MisItem[i].byState == ROLE_MIS_DELIVERY) {
				auto* p = new CGraph("texture/ui/SystemBotton3.tga", 7, 15, 177, 119, 1);
				memCtrl->AddIcon(i, p);
			} else if (FuncArray.MisItem[i].byState == ROLE_MIS_PENDING) {
				auto* p = new CGraph("texture/ui/SystemBotton3.tga", 7, 15, 184, 119, 1);
				memCtrl->AddIcon(i, p);
			}
...
void CNpcTalkMgr::_MainMouseNPCEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
void CNpcTalkMgr::_evtMemSelectChange(CGuiData* pSender) {
	auto* memo = dynamic_cast<CMemo*>(pSender);
	if (!memo)
		return;
