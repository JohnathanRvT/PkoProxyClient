...
void CNpcTradeMgr::ShowTradePage(const NET_TRADEINFO& TradeInfo, BYTE byCmd, DWORD dwNpcID) {
	...
	auto SetPageShownOnTradeOpen = [&]() {
		...
		auto* pgeNPCtrade = dynamic_cast<CPage*>(frmNPCtrade->Find("pgeNPCtrade"));
		if (pgeNPCtrade) {
...
void CNpcTradeMgr::_NpcItemRefresh(CItemCommand* pItem) {
	...
	if (pInfo->sType >= 1 && pInfo->sType <= 10) // 武器
	{
		...
		pItem->SetData(data);
	} else if (pInfo->sType == EItemType::Clothing || pInfo->sType == EItemType::Shield || pInfo->sType == EItemType::Tattoo) {
		// 防御力
		int i = 0;
		data.sInstAttr[i][0] = ITEMATTR_VAL_DEF;
		data.sInstAttr[i][1] = pInfo->sDefValu[0];
		...
		pItem->SetData(data);
	} else if (pInfo->sType == EItemType::Necklace) // 项链
	{
		int i = 0;
		data.sInstAttr[i][0] = ITEMATTR_VAL_MXHP;
		data.sInstAttr[i][1] = pInfo->sMxHpValu[0];
		...
		pItem->SetData(data);
	} else if (pInfo->sType == EItemType::Ring) // 戒指
	{
		int i = 0;
		data.sInstAttr[i][0] = ITEMATTR_VAL_MXATK;
		data.sInstAttr[i][1] = pInfo->sMxAtkValu[0];
		...
		pItem->SetData(data);
	} else if (pInfo->sType == EItemType::Glove) //手套
	{
		// 防御力
		int i = 0;
		data.sInstAttr[i][0] = ITEMATTR_VAL_DEF;
		...
		pItem->SetData(data);
	} else if (pInfo->sType == EItemType::Boot) // 鞋子
	{
		// 防御力
		int i = 0;
		data.sInstAttr[i][0] = ITEMATTR_VAL_DEF;
		...
		pItem->SetData(data);
	} else if (pInfo->sType == EItemType::Hair) // 帽子
	{
		// 防御力
		int i = 0;
		data.sInstAttr[i][0] = ITEMATTR_VAL_DEF;
		...
		pItem->SetData(data);
	} else if (pInfo->sType == EItemType::Conch) // 贝壳
	{
...
void CNpcTradeMgr::LocalBuyFromNpc(CGoodsGrid* pNpcGrid, CGoodsGrid* pSelfGrid, int nGridID, CCommandObj* pItem) {
	auto* pBuy = dynamic_cast<CItemCommand*>(pItem);
	if (!pBuy)
		return;
	...
	if (pBuy->GetItemInfo()->GetIsPile()) {
		...
		for (int gridIndex = 0; gridIndex < count; gridIndex++) {
			auto* pInfo = dynamic_cast<CItemCommand*>(pSelfGrid->GetItem(gridIndex));
...
void CNpcTradeMgr::LocalSaleToNpc(CGoodsGrid* pNpcGrid, CGoodsGrid* pSelfGrid, int nGridID, CCommandObj* pItem) {
	auto* pSaleItem = dynamic_cast<CItemCommand*>(pItem);
	if (!pSaleItem)
		return;
	...
	if (pSaleItem->GetIsPile() && pSaleItem->GetTotalNum() > 1) {
		...
		char buf[256] = {0};
		if (pSaleItem->GetItemInfo()->sType == EItemType::Boat) {
...
void CNpcTradeMgr::_BuyTradeEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
void CNpcTradeMgr::_BuyEquipYesNoTradeEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes)
		return;
...
void CNpcTradeMgr::_SaleTradeEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes)
		return;
...
void CNpcTradeMgr::_SaleEquipYesNoTradeEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes)
		return;
