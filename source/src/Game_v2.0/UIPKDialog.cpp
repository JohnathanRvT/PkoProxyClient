...
bool CPkDialog::Init() {
	...
	{ // pk 开始前的对话框
		...
		for (int i(0); i < TEAM_NUM; i++) {
			_snprintf_s(szBuf, _TRUNCATE, "lstTeam%d", i);
...

	{
		...
		//for (int i(0); i<TEAM_NUM; i++)
		//{
		//	_snprintf_s(szBuf, _TRUNCATE, "lvEndTeam%d", i);
...
void CPkDialog::SetStartDialogContent(const stNetTeamFightAsk& sNetTeamFightAsk) {
	...
	auto iLeftNum = static_cast<int>(sNetTeamFightAsk.chSideNum1);
	auto iRightNum = static_cast<int>(sNetTeamFightAsk.chSideNum2);
	...
	for (int i(0); i < iLeftNum; i++) {
		...
		_snprintf_s(szBuf, _TRUNCATE, "%d", static_cast<int>(sNetTeamFightAsk.Info[i].chLv));
		lvStartTeams[TEAM_LEFT]->GetList()->GetItems()->GetItem(i)->GetIndex(2)->SetString(szBuf);
		_snprintf_s(szBuf, _TRUNCATE, "%d", static_cast<int>(sNetTeamFightAsk.Info[i].usVictoryNum));
		lvStartTeams[TEAM_LEFT]->GetList()->GetItems()->GetItem(i)->GetIndex(3)->SetString(szBuf);
		_snprintf_s(szBuf, _TRUNCATE, "%d", static_cast<int>(sNetTeamFightAsk.Info[i].usFightNum));
	...
	for (int i(0); i < iRightNum; i++) {
		...
		_snprintf_s(szBuf, _TRUNCATE, "%d", static_cast<int>(sNetTeamFightAsk.Info[i + iLeftNum].chLv));
		lvStartTeams[TEAM_RIGHT]->GetList()->GetItems()->GetItem(i)->GetIndex(2)->SetString(szBuf);
		_snprintf_s(szBuf, _TRUNCATE, "%d", static_cast<int>(sNetTeamFightAsk.Info[i + iLeftNum].usVictoryNum));
		lvStartTeams[TEAM_RIGHT]->GetList()->GetItems()->GetItem(i)->GetIndex(3)->SetString(szBuf);
		_snprintf_s(szBuf, _TRUNCATE, "%d", static_cast<int>(sNetTeamFightAsk.Info[i + iLeftNum].usFightNum));
...
//-----------------------------------------------------------------------------
void CPkDialog::_MainMousePkStartEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
//-----------------------------------------------------------------------------
void CPkDialog::_MainMousePkEndEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string name = pSender->GetName();
	if (_stricmp("frmTeamPk", pSender->GetForm()->GetName()) == 0) {
