...
bool CPKSilverMgr::AddFormAttribute(int idx, const std::string& szName, long sLevel, const std::string& szJob, long lPkval) {
	...
	_snprintf_s(buff, _TRUNCATE, "%li", sLevel);
	labLevel[idx]->SetCaption(buff);

	labJob[idx]->SetCaption(szJob.c_str());

	_snprintf_s(buff, _TRUNCATE, "%li", lPkval);
...
bool CPKSilverMgr::Init() {
	...
	for (int i = 0; i < MAX_PKSILVER_PLAYER; i++) {
		_snprintf_s(szTitle, _TRUNCATE, "labName_%i", i);
		...
		_snprintf_s(szTitle, _TRUNCATE, "labLevel_%i", i);
		...
		_snprintf_s(szTitle, _TRUNCATE, "labJob_%i", i);
		...
		_snprintf_s(szTitle, _TRUNCATE, "labData_%i", i);
...
// void CPKSilverMgr::_evtPKSilverSortBtn(CCompent *pSender, int nMsgType, int x, int y, MouseClickState dwKey)
// {
