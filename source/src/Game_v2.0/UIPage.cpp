...
bool CPageItem::MouseRun(int x, int y, MouseClickState key) {
...
bool CPage::MouseRun(int x, int y, MouseClickState key) {
	if (!IsNormal()) {
		return false;
	}

	for (auto* page : _pages) {
		if (page->MouseRun(x, y, key)) {
			return true;
		}
	}
...
CPageItem* CPage::NewPage() {
	auto* tmp = new CPageItem(*_frmOwn);
	tmp->SetParent(this);
