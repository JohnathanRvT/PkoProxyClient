...
void CProgressBar::RenderHint(int x, int y) {
	...
	if (_eHintStyle == hsHintNum) {
		_snprintf_s(buf, _TRUNCATE, "%s:%d/%d", _strHint.c_str(), (int)_step.GetPosition(), (int)_step.GetRange());
	} else {
		_snprintf_s(buf, _TRUNCATE, "%s:%.02f%%", _strHint.c_str(), _step.GetPosition() / _step.GetRange() * 100.0f);
	}
	_RenderHint(buf, x, y);
}

bool CProgressBar::MouseRun(int x, int y, MouseClickState key) {
	if (!_bActiveMouse)
		return false;
	if (!IsNormal())
		return false;
