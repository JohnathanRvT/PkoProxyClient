...
bool CPurifyMgr::Init() {
	...
	for (int i = 0; i < TYPE_COUNT; ++i) {
		_snprintf_s(szName, _TRUNCATE, "labHintleft%d", i + 1);
		...
		_snprintf_s(szName, _TRUNCATE, "labHintright%d", i + 1);
		...
		_snprintf_s(szName, _TRUNCATE, "labTitle%d", i + 1);
...
void CPurifyMgr::SetPurifyUI() {
	auto* pItem1 = dynamic_cast<CItemCommand*>(cmdEquipPurify[PURIFY_ONE]->GetCommand());
	auto* pItem2 = dynamic_cast<CItemCommand*>(cmdEquipPurify[PURIFY_TWO]->GetCommand());
...
void CPurifyMgr::_evtDragPurifyOne(CGuiData* pSender, CCommandObj* pItem, bool& isAccept) {
	...
	// 贝壳
		...
		if (pItemCommand->GetItemInfo()->sType != EItemType::Conch) {
...
void CPurifyMgr::_evtDragPurifyTwo(CGuiData* pSender, CCommandObj* pItem, bool& isAccept) {
	...
	auto* pMainItem = dynamic_cast<CItemCommand*>(g_stUIPurify.cmdEquipPurify[PURIFY_ONE]->GetCommand());
...
// 鼠标点击按钮事件
void CPurifyMgr::_evtMainMouseButton(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
