...
static CList* GetList(int list_id) {
	if (CGuiData* base = CGuiData::GetGui(list_id); base) {
		if (auto* derived = dynamic_cast<CList*>(base); derived) {
			return derived;
		}
		if (auto* derived = dynamic_cast<CListView*>(base); derived) {
			return derived->GetList();
		}
	}
	return nullptr;
}
...
int UI_SetFormInMianForm(int formId) {
	auto* f = dynamic_cast<CForm*>(CGuiData::GetGui(formId));
	if (!f)
		return R_FAIL;
...
int UI_AddAllFormTemplete(int form_id) {
	auto* p = dynamic_cast<CForm*>(CGuiData::GetGui(form_id));
	if (!p)
		return R_FAIL;
...
int UI_AddFormToTemplete(int formid, int nTempleteNo) {
	auto* p = dynamic_cast<CForm*>(CGuiData::GetGui(formid));
	if (!p)
		return R_FAIL;
...
int UI_FormSetIsEscClose(int nFormID, int IsEscClose) {
	auto* p = dynamic_cast<CForm*>(CGuiData::GetGui(nFormID));
	if (!p)
		return R_FAIL;
...
int UI_FormSetEnterButton(int nFormID, int nButtonID) {
	...
	auto* b = dynamic_cast<CTextButton*>(CGuiData::GetGui(nButtonID));
	if (!b)
		return R_FAIL;
...
int UI_FormSetHotKey(int id, int control_key, int key) {
	auto* p = dynamic_cast<CForm*>(CGuiData::GetGui(id));
	if (!p)
		return R_FAIL;
...
int UI_LoadFrameImage(int id, const char* client, int cw, int ch, int tx, int ty, const char* file, int w, int h) {
	...
	auto* f = dynamic_cast<CFrameImage*>(p);
	if (!f)
		return R_FAIL;
...
int UI_CreateListView(int formId, char* pszName, int w, int h, int x, int y, int col, int style) {
	...
	auto* g = new CListView(*f, col, (CListView::eStyle)style);
	g->SetName(pszName);
	g->SetPos(x, y);
	g->SetSize(w, h);
...
int UI_ListViewSetTitle(int listviewid, int index, int width, const char* titleimage, int w, int h, int sx, int sy) {
	auto* f = dynamic_cast<CListView*>(CGuiData::GetGui(listviewid));
	if (!f)
		return R_FAIL;

	f->GetTitle()->SetColumnWidth(index, width);
	auto* p = dynamic_cast<CImage*>(f->GetColumnImage(index));
	if (p)
...
int UI_LoadListFixSelect(int id, const char* imagefile, int w, int h, int sx, int sy) {
	auto* f = dynamic_cast<CFixList*>(CGuiData::GetGui(id));
	if (!f)
		return R_FAIL;
...
int UI_LoadCheckFixListCheck(int id, const char* checkimage, int cw, int ch, int csx, int csy, const char* uncheckimage, int uw, int uh, int usx, int usy) {
	auto* f = dynamic_cast<CCheckFixList*>(CGuiData::GetGui(id));
	if (!f)
		return R_FAIL;
...
int UI_GetScroll(int id) {
	...
	auto* list = dynamic_cast<CList*>(p);
	if (list)
		return list->GetScroll()->GetID();

	auto* view = dynamic_cast<CListView*>(p);
	if (view)
	...
	auto* combo = dynamic_cast<CCombo*>(p);
	if (combo)
		return combo->GetList()->GetScroll()->GetID();

	auto* tree = dynamic_cast<CTreeView*>(p);
	if (tree)
		return tree->GetScroll()->GetID();

	auto* grid = dynamic_cast<CGoodsGrid*>(p);
	if (grid)
		return grid->GetScroll()->GetID();

	auto* memo = dynamic_cast<CMemo*>(p);
	if (memo)
		return memo->GetScroll()->GetID();

	auto* memoex = dynamic_cast<CMemoEx*>(p);
	if (memoex)
		return memoex->GetScroll()->GetID();

	auto* richmemo = dynamic_cast<CRichMemo*>(p);
	if (richmemo)
		return richmemo->GetScroll()->GetID();

	auto* skill = dynamic_cast<CSkillList*>(p);
	if (skill)
		return skill->GetScroll()->GetID();
	//Add by sunny.sun20080721
	auto* lpList = dynamic_cast<CAmphitheaterList*>(p);
	if (lpList)
...
int UI_GetList(int id) {
	...
	auto* v = dynamic_cast<CListView*>(p);
	if (v)
...
int UI_GetScrollObj(int id, int scrolltype) {
	auto* p = dynamic_cast<CScroll*>(CGuiData::GetGui(id));
	if (!p)
		return R_FAIL;
...
int UI_GetEXListObj(int id, int type) {
	auto* p = dynamic_cast<CAmphitheaterList*>(CGuiData::GetGui(id));
	if (!p)
		return R_FAIL;
	switch (type) {
	case EXLIST_BUTTON:
...
int UI_ComboSetStyle(int id, int style) {
	auto* combo = dynamic_cast<CCombo*>(CGuiData::GetGui(id));
	if (!combo)
		return R_FAIL;
...
int UI_LoadButtonImage(int id, const char* file, int w, int h, int sx, int sy, int isHorizontal) {
	...
	auto* b = dynamic_cast<CTextButton*>(p);
	if (!b)
		return R_FAIL;
...
int UI_LoadFlashScaleImage(int id, int flash, char* file, int frame, int w, int h, int tx, int ty, float scalex, float scaley) {
	...
	auto* pro = dynamic_cast<CProgressBar*>(p);
	if (!pro)
		return R_FAIL;
...
int UI_SetAlign(int id, int align) {
	auto* s = dynamic_cast<CCompent*>(CGuiData::GetGui(id));
	if (!s)
		return R_FAIL;
...
int UI_SetButtonModalResult(int id, int modal) {
	auto* g = dynamic_cast<CTextButton*>(CGuiData::GetGui(id));
	if (!g)
		return R_FAIL;
...
{
	auto* g = dynamic_cast<CTitle*>(CGuiData::GetGui(id));
	if (!g)
		return R_FAIL;
...
int UI_ButtonSetHint(int id, char* hint) {
	auto* b = dynamic_cast<CTextButton*>(CGuiData::GetGui(id));
	if (!b)
		return R_FAIL;
...
int UI_SetProgressStyle(int id, int style) {
	...
	auto* s = dynamic_cast<CProgressBar*>(t);
	if (!s)
		return R_FAIL;
...
int UI_AddGroupBox(int id, int checkbox) {
	...
	auto* box = dynamic_cast<CCheckBox*>(b);
	if (!box)
		return R_FAIL;

	auto* s = dynamic_cast<CCheckGroup*>(t);
	if (!s)
		return R_FAIL;
...
int UI_SetEditEnterButton(int nEditID, int nButtonID) {
	auto* t = dynamic_cast<CEdit*>(CGuiData::GetGui(nEditID));
	if (!t)
		return R_FAIL;
...
int UI_SetEditMaxNum(int id, int num) {
	auto* t = dynamic_cast<CEdit*>(CGuiData::GetGui(id));
	if (!t)
		return R_FAIL;
...
int UI_SetListRowHeight(int id, int height) {
	...
	auto* lst = dynamic_cast<CList*>(p);
	if (lst) {
		lst->SetRowHeight(height);
		return R_OK;
	}

	auto* lsv = dynamic_cast<CListView*>(p);
	if (lsv) {
		lsv->GetList()->SetRowHeight(height);
		return R_OK;
	}

	auto* fls = dynamic_cast<CFixList*>(p);
	if (fls) {
...
int UI_ListLoadSelectImage(int id, char* file, int w, int h, int sx, int sy) {
	...
	auto* pView = dynamic_cast<CListView*>(p);
	CList* lst = nullptr;
	if (pView) {
		lst = pView->GetList();
	}
...
int UI_LoadSkillListButtonImage(int id, char* file, int w, int h, int sx, int sy, int item_w, int item_h) {
	auto* p = dynamic_cast<CSkillList*>(CGuiData::GetGui(id));
	if (!p)
		return R_FAIL;
...
int UI_FixListSetRowSpace(int id, int height) {
	auto* fls = dynamic_cast<CFixList*>(CGuiData::GetGui(id));
	if (fls) {
...
int UI_CheckFixListSetCheckMargin(int id, int left, int top) {
	auto* fls = dynamic_cast<CCheckFixList*>(CGuiData::GetGui(id));
	if (fls) {
...
int UI_GridLoadSelectImage(int id, char* file, int w, int h, int tx, int ty) {
	auto* g = dynamic_cast<CGrid*>(CGuiData::GetGui(id));
	if (!g)
		return R_FAIL;
...
int UI_SetGridSpace(int id, int x, int y) {
	...
	auto* d = dynamic_cast<CGoodsGrid*>(p);
	if (d) {
		d->SetSpace(x, y);
		return R_OK;
	}

	auto* g = dynamic_cast<CGrid*>(p);
	if (g) {
		g->SetSpace(x, y);
		return R_OK;
	}
...
int UI_GoodGridLoadUnitImage(int id, char* file, int w, int h, int tx, int ty) {
	auto* g = dynamic_cast<CGoodsGrid*>(CGuiData::GetGui(id));
	if (!g)
		return R_FAIL;
...
int UI_AddFaceToGrid(int id, char* file, int w, int h, int sx, int sy, int frame, int nTag) {
	...
	auto* p = new CGraph(file, w, h, sx, sy, frame);
...
int UI_FixListSetMaxNum(int id, int num) {
	auto* g = dynamic_cast<CFixList*>(CGuiData::GetGui(id));
	if (!g)
		return R_FAIL;
...
int UI_TreeLoadImage(int nTreeID, int nType, const char* imagefile, int w, int h, int sx, int sy, int itemw, int itemh) {
	auto* f = dynamic_cast<CTreeView*>(CGuiData::GetGui(nTreeID));
	if (!f)
		return R_FAIL;
...
int UI_CreateNoteGraphItem(char* file, int w, int h, int sx, int sy, int frame, const char* text, int TextX, int TextY) {
	auto* item = new CNoteGraph(frame);
...
int UI_CreateSingleNode(int treeid, int itemid, int nodeid_parent) {
	auto* g = dynamic_cast<CTreeView*>(CGuiData::GetGui(treeid));
	if (!g)
		return R_FAIL;
...
int UI_CreateGridNode(int treeid, int itemid, int maxcol, int uw, int uh, int nodeid_parent) {
	...
	auto* obj = new CTreeGridNode(g, item);
...
int UI_GridNodeAddItem(int nodeid, int itemid) {
	auto* obj = dynamic_cast<CTreeGridNode*>(CTreeNodeObj::GetNode(nodeid));
	if (!obj)
		return R_FAIL;
	...
	// 初始化提示
	auto* pHint = dynamic_cast<CHintGraph*>(item);
	if (pHint) {
		auto* pParent = dynamic_cast<CTreeNode*>(obj->GetParent());
...
int UI_SetChatPrefixColor(DWORD world, DWORD road, DWORD team, DWORD guild, DWORD gm, DWORD system, DWORD trade, DWORD person) //FIXED: Color not working (changed int to DWORD)
{
	// 世界,路人,队聊,公会,GM,系统,交易,私聊
	//g_stUICoze.chatColor.road = road;
	//g_stUICoze.chatColor.person = person;
	...
	CCharMsg::SetChannelPrefixColor(CCharMsg::CHANNEL_SIGHT, road);
	CCharMsg::SetChannelPrefixColor(CCharMsg::CHANNEL_PRIVATE, person);
	CCharMsg::SetChannelPrefixColor(CCharMsg::CHANNEL_TEAM, team);
	CCharMsg::SetChannelPrefixColor(CCharMsg::CHANNEL_GUILD, guild);
	CCharMsg::SetChannelPrefixColor(CCharMsg::CHANNEL_WORLD, world);
	CCharMsg::SetChannelPrefixColor(CCharMsg::CHANNEL_SYSTEM, system);
	CCharMsg::SetChannelPrefixColor(CCharMsg::CHANNEL_TRADE, trade);
	CCharMsg::SetChannelPrefixColor(CCharMsg::CHANNEL_PUBLISH, gm);
	return R_OK;
}

int UI_CreatePageItem(int page_id) {
	auto* g = dynamic_cast<CPage*>(CGuiData::GetGui(page_id));
	if (!g)
		return R_FAIL;
...
int UI_GetPageItemObj(int page_item_id, int type) {
	auto* g = dynamic_cast<CPageItem*>(CGuiData::GetGui(page_item_id));
	if (!g)
		return R_FAIL;
...
int UI_AddCompent(int container_id, int compent_id) {
	auto* g = dynamic_cast<CContainer*>(CGuiData::GetGui(container_id));
	if (!g)
		return R_FAIL;

	auto* p = dynamic_cast<CCompent*>(CGuiData::GetGui(compent_id));
	if (!p)
		return R_FAIL;
...
{
	auto* g = dynamic_cast<CLabelEx*>(CGuiData::GetGui(label_id));
	if (!g)
		return R_FAIL;
...
int UI_SetProgressHintStyle(int id, int style) {
	auto* g = dynamic_cast<CProgressBar*>(CGuiData::GetGui(id));
	if (!g)
		return R_FAIL;
...
int UI_RichSetMaxLine(int id, int line) {
	auto* s = dynamic_cast<CRichMemo*>(CGuiData::GetGui(id));
	if (!s)
		return R_FAIL;
...
int UI_MenuLoadSelect(int id, const char* imagefile, int w, int h, int sx, int sy) {
	auto* f = dynamic_cast<CMenu*>(CGuiData::GetGui(id));
	if (!f)
		return R_FAIL;
...
int UI_SetFastKeybind(int id, unsigned short keybind) {
	auto* f = dynamic_cast<CFastCommand*>(CGuiData::GetGui(id));
	if (!f)
		return R_FAIL;

	f->SetKeybind(keybind);

	return R_OK;
}

int UI_GetFastKeybind(int id) {
	auto* f = dynamic_cast<CFastCommand*>(CGuiData::GetGui(id));
	if (!f)
		return R_FAIL;
...
void MPInitLua_Gui() {
	...
	CLU_RegisterFunction("UI_SetChatPrefixColor", "int", "int, int, int, int, int, int, int, int", CLU_CDECL, CLU_CAST(UI_SetChatPrefixColor));
	...
	// Add by lark.li
	CLU_RegisterFunction("UI_ResString", "char*", "char*", CLU_CDECL, CLU_CAST(UI_ResString));
	CLU_RegisterFunction("UI_SetCaptionEx", "int", "int, char*, int", CLU_CDECL, CLU_CAST(UI_SetCaptionEx));

	CLU_RegisterFunction("UI_SetFastKeybind", "int", "int", CLU_CDECL, CLU_CAST(UI_SetFastKeybind));
	CLU_RegisterFunction("UI_GetFastKeybind", "int", "", CLU_CDECL, CLU_CAST(UI_GetFastKeybind));
...
LUA_FUNC_ARITY8(UI_SetChatPrefixColor,
				number, number, number, number, number, number, number, number, number,
				DWORD, DWORD, DWORD, DWORD, DWORD, DWORD, DWORD, DWORD, DWORD); //FIXED: Color not working (changed int to DWORD)
...
// Added by deguix
LUA_FUNC_ARITY2(UI_SetFastKeybind,
				number, number, number,
				int, int, int);

LUA_FUNC_ARITY1(UI_GetFastKeybind,
				number, number,
				int, int);
...
void MPInitLua_Gui(lua_State* L) {
	...
	LUA_FUNC_REG(UI_SetChatPrefixColor);
	...
	// Added by deguix
	LUA_FUNC_REG(UI_SetFastKeybind);
	LUA_FUNC_REG(UI_GetFastKeybind);
