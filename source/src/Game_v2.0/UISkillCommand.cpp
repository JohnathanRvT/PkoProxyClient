...
CSkillCommand::CSkillCommand(CSkillRecord* pSkill)
	: CCommandObj(), _pSkill(pSkill), _pImage(new CGuiPic()) {
	if (!_pSkill) {
...
bool CSkillCommand::UseCommand() {
	...
	if (_pSkill->IsReadingSkill()) {
		auto reading = std::make_unique<CReadingState>(pCha->GetActor());
		return pCha->GetActor()->SwitchState(std::move(reading));
	}
...
bool CSkillCommand::IsAllowUse() const {
	if (_pSkill->GetIsActive())
		return true;
...
void CSkillCommand::AddHint(int x, int y) {
	...
	_snprintf_s(szBuf, _TRUNCATE, "%s", _pSkill->szName);
...
bool CSkillCommand::_WriteNeed(int nType, int nValue, const char* szStr) {
	...
	if (nType == enumSKILL_ITEM_NEED_TYPE) {
		...
		if (pInfo) {
			_snprintf_s(szBuf, _TRUNCATE, "%s %s", szStr, pInfo->szDataName);
...
const char* CSkillCommand::GetName() const {
	return _pSkill->szName;
}

const char* CSkillCommand::GetSkillName() const {
	if (_pSkill->chType == 1) {
		_snprintf_s(szBuf, _TRUNCATE, "%s\nLV:%d, SP:%d", _pSkill->szName, _pSkill->GetLevel(), _pSkill->GetSPExpend());
	} else {
