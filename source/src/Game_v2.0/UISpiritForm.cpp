...
bool CSpiritMgr::Init() {
	...
	auto* btnHelp = static_cast<CTextButton*>(frmSpiritMarry->Find("btnHelp"));
	if (!btnHelp)
		return false;
	...
	for (int i = 0; i < 9; ++i) {
		...
		for (int j = 0; j < ERNIE_IMAGE_COUNT; ++j) {
			_snprintf_s(szName, _TRUNCATE, "imgLine%d_%d", i + 1, j + 1);
	...
	for (int i = 0; i < 3; ++i) {
		...
		for (int j = 0; j < 3; ++j) {
			int nSeq = i * 3 + j;
			_snprintf_s(szName, _TRUNCATE, "cmdItemTiger%d", nSeq + 1);
		...
		// 下注 Check
		_snprintf_s(szName, _TRUNCATE, "chkSetmoney%d", i + 1);
		...
		// 下注硬币个数显示
		_snprintf_s(szName, _TRUNCATE, "labUsemoney%d", i + 1);
		...
		// 停止转动按钮
		_snprintf_s(szName, _TRUNCATE, "btnStop%d", i + 1);
...
bool CSpiritMgr::IsValidSpirit(CItemCommand& rItem) {
	...
	if (pInfo && pInfo->sType == EItemType::Pet && nLevel >= 20) {
		return true;
	}
...
void CSpiritMgr::SetSpiritUI() {
	...
	char szBuffer[64] = {0};
	_snprintf_s(szBuffer, _TRUNCATE, "%d", nMoney);
...
void CSpiritMgr::_evtDragMarryOne(CGuiData* pSender, CCommandObj* pItem, bool& isAccept) {
	...
	auto* pStoneItem = dynamic_cast<CItemCommand*>(g_stUISpirit.cmdSpiritMarry[SPIRIT_MARRY_ITEM]->GetCommand());
...
// 鼠标点击按钮事件
void CSpiritMgr::_evtMainMouseButton(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
void CSpiritMgr::FrameMove(DWORD dwTime) {
	if (frmSpiritErnie && frmSpiritErnie->GetIsShow()) {
		...
		//
		char szBuffer[32] = {0};
		_snprintf_s(szBuffer, _TRUNCATE, "%d", ERNIE_COIN_COUNT);
		for (int i = 0; i < 3; ++i) {
...
// 精灵老虎机，鼠标点击事件
void CSpiritMgr::_evtErnieMouseButton(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string strName = pSender->GetName();
...
void CSpiritMgr::ShowErnieHighLight() {
	int nID[9] = {0};
	for (int i = 0; i < 9; ++i) {
		auto* pItem = dynamic_cast<CItemCommand*>(cmdItem[i]->GetCommand());
...
void CSpiritMgr::ErnieHightLight(int nNum, bool b) {
	if (0 <= nNum && nNum < 9) {
		auto* pItem = dynamic_cast<CItemCommand*>(cmdItem[nNum]->GetCommand());
		if (pItem) {
			pItem->SetIsValid(b);
			return;
		}
...
void CSpiritMgr::evtShowHelpInfo(CGuiData* pSender, int x, int y, MouseClickState key) {
