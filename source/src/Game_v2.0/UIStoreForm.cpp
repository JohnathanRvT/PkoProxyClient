...
bool CStoreMgr::Init() {
	...
	for (int i = 0; i < STORE_PAGE_SIZE; ++i) {
		_snprintf_s(szName, _TRUNCATE, "labName_%d", i);
		...
		_snprintf_s(szName, _TRUNCATE, "labPrice_%d", i);
		...
		_snprintf_s(szName, _TRUNCATE, "labLeftTime_%d", i);
		...
		_snprintf_s(szName, _TRUNCATE, "labLeftNum_%d", i);
		...
		_snprintf_s(szName, _TRUNCATE, "labRemark_%d", i);
		...
		_snprintf_s(szName, _TRUNCATE, "labRightClickView_%d", i);
		...
		_snprintf_s(szName, _TRUNCATE, "cmdStore_%d", i);
		...
		_snprintf_s(szName, _TRUNCATE, "imgSquare_%d", i);
		...
		_snprintf_s(szName, _TRUNCATE, "btnBlue_%d", i);
		...
		_snprintf_s(szName, _TRUNCATE, "imgCutLine_%d", i);
		...
		_snprintf_s(szName, _TRUNCATE, "imgHot_%d", i);
		...
		_snprintf_s(szName, _TRUNCATE, "imgNew_%d", i);
		...
		_snprintf_s(szName, _TRUNCATE, "imgBlue_%d", i);
	...
	for (int i = 0; i < STORE_ITEM_COUNT; ++i) {
		_snprintf_s(szName, _TRUNCATE, "imgSquareViewAll_%d", i);
		...
		_snprintf_s(szName, _TRUNCATE, "cmdSquareIcon_%d", i);
	...
	auto* btnClearAll = dynamic_cast<CTextButton*>(frmTryon->Find("btnClearAll"));
	if (btnClearAll) {
		btnClearAll->evtMouseClick = _evtChaEquipClearAll;
	}

	auto* btnTurnleft3d = dynamic_cast<CTextButton*>(frmTryon->Find("btnTurnleft3d"));
	if (btnTurnleft3d) {
		btnTurnleft3d->evtMouseClick = _evtChaLeftRotate;
		btnTurnleft3d->evtMouseDownContinue = _evtChaLeftContinueRotate;
	}

	auto* btnTurnright3d = dynamic_cast<CTextButton*>(frmTryon->Find("btnTurnright3d"));
	if (btnTurnright3d) {
...
void CStoreMgr::ShowViewAllForm(bool bShow) {
	...
	for (int i = 0; i < STORE_ITEM_COUNT; ++i) {
		...
		if (m_stStoreInfo[m_nCurSel].comItemInfo[i].itemID > 0) {
			...
			auto* pItem = new CItemCommand(pInfo);
...
void CStoreMgr::ShowStoreWebPage() {
	...
	if (g_cooperate.code == COP_CGA) // 浩方特殊处理
	{
		...
		ShellExecute(nullptr, "open", strURL.c_str(),
...
bool CStoreMgr::PopFromTempKitbag(CGoodsGrid& rkDrag, CGoodsGrid& rkSelf, int nGridID, CCommandObj& rkItem) {
	...
	// 判断物品是否是可重叠的物品
	auto* pkItemCmd = dynamic_cast<CItemCommand*>(&rkItem);
	if (!pkItemCmd)
		return false;
...
void CStoreMgr::_evtDragItemsEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
void CStoreMgr::AddStoreTreeNode(long nParentID, long nID, const char* szCaption) {
	...
	if (0 == nParentID) {
		...
		if (iterNode != m_mapNode.end()) {
			auto* pGridNode = dynamic_cast<GUI::CTreeGridNode*>(trvStore->GetRootNode()->FindNode(iterNode->second.c_str()));
			if (pGridNode) {
...
void CStoreMgr::AddStoreItemInfo(long nSeq, long nID, const char* szName, long nPrice, const char* szRemark, bool isHot, long nTime, long nRemainNum, long nRemainTime) {
	...
	char szTemp[128] = {0};
	//_snprintf_s(szTemp, _TRUNCATE, "%s:%s", RES_STRING(CL_LANGUAGE_MATCH_845), szName);
	m_stStoreGui[nSeq].labName->SetCaption(szName); // 商品名
	m_stStoreGui[nSeq].labName->SetIsShow(true);

	//_snprintf_s(szTemp, _TRUNCATE, "%s:%s", RES_STRING(CL_LANGUAGE_MATCH_846), StringSplitNum(nPrice, 3, ','));
	m_stStoreGui[nSeq].labPrice->SetCaption(StringSplitNum(nPrice, 3, ',')); // 商品价格
	m_stStoreGui[nSeq].labPrice->SetIsShow(true);

	_snprintf_s(szTemp, _TRUNCATE, "%s:%s", RES_STRING(CL_LANGUAGE_MATCH_847), StringLimit(szRemark, 25).c_str());
	m_stStoreGui[nSeq].labRemark->SetCaption(strlen(szRemark) > 0 ? szTemp : szRemark); // 商品描述
	m_stStoreGui[nSeq].labRemark->SetIsShow(true);

	_snprintf_s(szTemp, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_910), nRemainNum); // "%d个"
	...
	_snprintf_s(szTemp, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_911), nRemainTime); // "%d小时"
	...
	strncpy_s(m_stStoreInfo[nSeq].comName, szName, _TRUNCATE);	 // 商品名
	strncpy_s(m_stStoreInfo[nSeq].comRemark, szRemark, _TRUNCATE); // 商品备注
}
...
// 添加用户设置结点
void CStoreMgr::AddStoreUserTreeNode() {
...
void CStoreMgr::SetStoreItemPage(long nCurPage, long nMaxPage) {
	...
	if (m_nMaxPage > 0) {
		char szBuffer[32] = {0};
		_snprintf_s(szBuffer, _TRUNCATE, "%d/%d", nCurPage, nMaxPage);
...
void CStoreMgr::SetStoreMoney(long nMoBean, long nRplMoney) {
	...
	if (nMoBean >= 0) {
		_snprintf_s(szBuffer, _TRUNCATE, "%ld", nMoBean);
		labBeanLeft->SetCaption(szBuffer);
	}

	if (nRplMoney >= 0) {
		_snprintf_s(szBuffer, _TRUNCATE, "%ld", nRplMoney);
...
void CStoreMgr::_SetIsShowCozeForm(bool bShow) {
	...
	auto* lstOnSay = dynamic_cast<CList*>(frmMainChat->Find("lstOnSay"));
	if (lstOnSay)
		lstOnSay->SetIsShow(bShow);

	auto* lstOnSaySystem = dynamic_cast<CList*>(frmMainChat->Find("lstOnSaySystem"));
	if (lstOnSaySystem)
		lstOnSaySystem->SetIsShow(bShow);

	auto* drpTitle = dynamic_cast<CDragTitle*>(frmMainChat->Find("drpTitle"));
	if (drpTitle)
		drpTitle->SetIsShow(bShow);

	auto* drpTitleSystem = dynamic_cast<CDragTitle*>(frmMainChat->Find("drpTitleSystem"));
	if (drpTitleSystem)
...
void CStoreMgr::_ShowTradeSelectBox() {
	if (0 <= m_nCurSel && m_nCurSel < STORE_PAGE_SIZE &&
		...
		char szTitle[256] = {0};
		_snprintf_s(szTitle, _TRUNCATE, "%s%s: %s\n%s: %s",
...
bool CStoreMgr::_IsCurSelVipNode() {
...
// 商城树的鼠标点击事件
void CStoreMgr::_evtStoreTreeMouseClick(CGuiData* pSender, int x, int y, MouseClickState key) {
...
// 商城按钮点击事件
void CStoreMgr::_evtStoreFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	...
	else if (strName == "btnReceiveMoDou") // 兑换摩豆
	{
		...
		ShellExecute(nullptr, "open",
					 strURL.c_str(),
					 nullptr, nullptr, SW_SHOW);
	} else if (!strName.empty() && strName.substr(0, 8) == "btnBlue_" &&
...
void CStoreMgr::_evtStoreListMouseRClick(CGuiData* pSender, int x, int y, MouseClickState key) {
	std::string strName = pSender->GetName();
	if (!strName.empty() && strName.substr(0, 8) == "btnBlue_" &&
		'0' <= strName[strName.size() - 1] && strName[strName.size() - 1] <= '0' + CStoreMgr::STORE_PAGE_SIZE) // 选中商品
	{
		g_stUIStore.m_nCurSel = strName[strName.size() - 1] - '0';
		g_stUIStore._RefreshStoreListHighLight();
...
void CStoreMgr::_evtStoreListMouseDBClick(CGuiData* pSender, int x, int y, MouseClickState key) {
	std::string strName = pSender->GetName();
	if (!strName.empty() && strName.substr(0, 8) == "btnBlue_" &&
		'0' <= strName[strName.size() - 1] && strName[strName.size() - 1] <= '0' + CStoreMgr::STORE_PAGE_SIZE) // 选中商品
	{
...
void CStoreMgr::_evtStoreOpenCheckEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes) {
		return;
	}
...
// 成为VIP 确认
void CStoreMgr::_evtStoreToVipEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	//if(nMsgType != CForm::mrYes)
	//{
	//	return;
	//}
...
// 代币兑换事件处理
void CStoreMgr::_evtExchangeEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes) {
		return;
	}
	...
	if (num > 0) {
		...
		_snprintf_s(szBuffer, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_905), g_stUIStore.m_nExchangeNum); // 确认兑换代币: %d
		CBoxMgr::ShowSelectBox(_evtExchangeCheckEvent, szBuffer, true);
	}
}

// 代币兑换确认
void CStoreMgr::_evtExchangeCheckEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes) {
		return;
	}
...
// 确认购买处理
void CStoreMgr::_evtTradeCheckEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes) {
		return;
	}
...
void CStoreMgr::_evtStoreViewAllMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string strName = pSender->GetName();
...
void CStoreMgr::_evtChaEquipClearAll(CGuiData* sender, int x, int y, MouseClickState key) {
	g_stUIStore.ChaEquipClearAll();
}

void CStoreMgr::_evtChaLeftRotate(CGuiData* sender, int x, int y, MouseClickState key) {
	g_stUIStore.ChaLeftRotate();
}

void CStoreMgr::_evtChaRightRotate(CGuiData* sender, int x, int y, MouseClickState key) {
