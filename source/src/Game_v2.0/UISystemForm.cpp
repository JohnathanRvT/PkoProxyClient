...
void CSystemMgr::LoadCustomProp() {
	...
	if (!m_isLoad) {
		if (m_sysProp.Load("user\\system.ini")) {
			...
			m_sysProp.m_videoProp.bFullScreen = g_pGameApp->IsFullScreen();
			m_sysProp.m_videoProp.bPixel1024 = g_Render.GetScrWidth() > 1000 ? true : false;
...
bool CSystemMgr::Init() {
	...
	*/
	...
	if (!cbxSize)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmVideo->GetName(), "cbxSize");
...
void CSystemMgr::_evtVideoChangeChange(CGuiData* pSender) {
	auto* g = dynamic_cast<CCheckGroup*>(pSender);
	if (!g)
		return;
...
void CSystemMgr::_evtVideoFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
void CSystemMgr::_evtSystemFromMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string name = pSender->GetName();
...
void CSystemMgr::_evtAudioFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string name = pSender->GetName();
...
void CSystemMgr::_evtMainMusicMouseDown(CGuiData* pSender, int x, int y, MouseClickState key) {
...
void CSystemMgr::_evtAskReloginFormMouseDown(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string name = pSender->GetName();
	pSender->GetForm()->SetIsShow(false);
...
void CSystemMgr::_evtAskExitFormMouseDown(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string name = pSender->GetName();
	pSender->GetForm()->SetIsShow(false);
...
void CSystemMgr::_evtAskChangeFormMouseDown(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string name = pSender->GetName();
	pSender->GetForm()->SetIsShow(false);
...
void CSystemMgr::_evtGameOptionFormMouseDown(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string name = pSender->GetName();
	if (name != "btnYes")
		return;
