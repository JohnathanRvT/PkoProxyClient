...
bool CTextButton::MouseRun(int x, int y, MouseClickState key) {
	if (!IsNormal())
		return false;
	...
	if (IsNoDrag(x, y, key)) {
		...
		if ((key & MouseClickState::RUp) != MouseClickState()) {
			DoClick(MouseClickState::RUp);
			return true;
		}
...
void CTextButton::DoClick(MouseClickState state) {
	g_pGameApp->PlaySound(26);

	// modify by Philip.Wu  2007/01/23  扩展按钮功能，支持右键和双击
	switch (state) {
	case MouseClickState::LUp : {
		if (evtMouseClick)
			evtMouseClick(this, GetX(), GetY(), MouseClickState::LUp);
		else if (_frmOwn->evtEntrustMouseEvent)
			_frmOwn->evtEntrustMouseEvent(this, _eFormModal, GetX(), GetY(), MouseClickState::LUp);
	} break;

	case MouseClickState::RUp: {
		if (evtMouseRClick)
			evtMouseRClick(this, GetX(), GetY(), MouseClickState::RUp);
	} break;

	case MouseClickState::LDB : {
		if (evtMouseDBClick)
			evtMouseDBClick(this, GetX(), GetY(), MouseClickState::LDB);
	} break;
	}
