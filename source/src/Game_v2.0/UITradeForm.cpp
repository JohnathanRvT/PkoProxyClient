...
void CTradeMgr::ShowCharTradeRequest(BYTE byType, DWORD dwRequestID) {
	...
	if (byType == mission::TRADE_CHAR)
		_snprintf_s(szBuf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_779), pCha->getHumanName());
	else
		_snprintf_s(szBuf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_780), pCha->getHumanName());
...
void CTradeMgr::_evtSelectYesNoEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
void CTradeMgr::_MainMousePlayerTradeEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	std::string name = pSender->GetName();
...
void CTradeMgr::_evtGoldFormEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
...
void CTradeMgr::ShowCharTrade(BYTE byType, DWORD dwAcceptID, DWORD dwRequestID) {
	...
	std::string strPlayTradeLabName(""); //labPlayertradeName控件Caption
	...
	auto* labPlayertradeName = dynamic_cast<CLabelEx*>(frmPlayertrade->Find("labPlayertradeName"));
...
void CTradeMgr::DragItemToTrade(DWORD dwCharID, USHORT sItemID, BYTE byIndex, WORD wCount, BYTE byItemIndex, SItemGrid* pGrid, const NET_CHARTRADE_BOATDATA* const pBoat) {
	LG("Trade", "DragItemToTrade( CharID:%u, ItemID:%d, Index:%d, Count:%d, ItemIndex:%d ) - MainID:%d\n", dwCharID, sItemID, byIndex, wCount, byItemIndex, g_stUITrade.m_dwMainID);
	...
	if (pObj) {
		...
		auto* pObj = new CItemCommand(pItem);
		if (pGrid)
			pObj->SetData(*pGrid);
...
void CTradeMgr::_evtLocalSaleEvent(CCompent* pSender, int nMsgType, int x, int y, MouseClickState dwKey) {
	if (nMsgType != CForm::mrYes)
		return;
...
void CTradeMgr::LocalSaleItem(CGoodsGrid* pSaleGrid, CGoodsGrid* pSelfGrid, int nGridID, CCommandObj* pItem) {
	...
	if (auto* pBoat = dynamic_cast<CItemCommand*>(pItem)) {
		if (pBoat && pBoat->GetItemInfo()->sType == EItemType::Boat) {
