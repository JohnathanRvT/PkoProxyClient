...
int CTreeNodeObj::GetTreeHeight(int& height, int rowheight) {
	...
	if (HasChild() && GetIsExpand()) {
		for (auto* child : GetChild()) {
			child->GetTreeHeight(height, rowheight);
		}
	}
	return height;
}
...
void CTreeNodeObj::Refresh(int x, int& y, int colwidth, int rowheight) {
	...
	if (HasChild() && GetIsExpand()) {
		...
		for (auto* child : GetChild()) {
			child->Refresh(x, y, colwidth, rowheight);
			//y += ((*it)->GetHeight() + rowheight);
		}
	}
}

bool CTreeNodeObj::MouseRun(int x, int y, MouseClickState key) {
	if (InRect(x, y)) {
...
CItemObj* CTreeNodeObj::GetHitItem(int x, int y) {
	if (HasChild() && GetIsExpand()) {
		...
		for (auto* child : GetChild()) {
			if (child->InRect(x, y) && (pObj = child->_GetSelfHitItem(x, y))) {
				return pObj;
			} else if (pObj = child->GetHitItem(x, y)) {
				return pObj;
			}
		}
	}
...
CTreeNodeObj* CTreeNodeObj::FindNode(const char* str) {
	...
	for (auto* child : GetChild()) {
		if (strcmp(str, child->GetCaption()) == 0) {
			return child;
		}
	}
	return nullptr;
...
bool CTreeNodeRoot::RootMouseRun(int x, int y, MouseClickState key) {
	if (HasChild()) {
		for (auto* child : GetChild()) {
			if (child->MouseRun(x, y, key))
				return true;
...
void CTreeNode::MouseRunSelf(int x, int y, MouseClickState key) {
...
void CTreeGridNode::MouseRunSelf(int x, int y, MouseClickState key) {
	if (y >= _nY && y <= _nY + _pItem->GetHeight()) {
		_IsShowFocus = false;
		if (!_pOwn->GetSelect()->SetItem(this, _pItem) && _pOwn->GetIsAutoExpand()) {
			_IsExpand = !_IsExpand;
...
bool CTreeGridNode::AddItem(CItemObj* item) {
	if (auto it = std::find(_items.begin(), _items.end(), item);
		it == _items.end()) {
...
CTreeView::CTreeView(CForm& frmOwn)
	: CCompent(frmOwn), _nColSpace(4), _nRowSpace(5), _pRoot(nullptr), _IsAutoExpand(true), evtSelectChange(nullptr), evtMouseDown(nullptr), evtSelectLost(nullptr), evtItemLost(nullptr), evtItemChange(nullptr), evtMouseDragEnd(nullptr), _pDragItem(nullptr), _pDragNode(nullptr), evtMouseDB(nullptr), _dwSelectColor(COLOR_BLACK), evtMouseDragMove(nullptr) {
	// _IsFocus = true;
...
bool CTreeView::MouseRun(int x, int y, MouseClickState key) {
	if (!IsNormal())
		return false;
	...
	if (InRect(x, y)) {
		...
		if (_pDrag && _pDrag->BeginMouseRun(this, _IsMouseIn, x, y, key) == CDrag::eState::Drag) {
			// Add by lark.li 20080805 begin
			auto* pSelectItem = dynamic_cast<CTextGraph*>(_pRoot->GetHitItem(x, y));
			if (!pSelectItem) {
				_pDrag->Reset();
				return true;
			}
		...
		if (((key & MouseClickState::Down) != MouseClickState()) && _pRoot->RootMouseRun(x, y, key)) {
			if (evtMouseDown)
...
void CTreeView::_DragEnd(int x, int y, MouseClickState key) {
...
// Add by lark.li 20080805 begin
void CTreeView::_DragMove(int x, int y, MouseClickState key) {
	//if( key & Mouse_LDown)
	{
		if (x >= _nX1 && x <= _nX2) {
			if (y > _nY2) {
	...
	if (_pDragNode && _pDragItem) {
		...
		if (form && evtMouseDragMove) {
			evtMouseDragMove(form, this, _pDragNode, _pDragMouseMoveNode, nullptr, x, y, key);
		}
	}
}
// End
