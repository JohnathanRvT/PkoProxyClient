...
// class CWorldScene
//---------------------------------------------------------------------------
int CWorldScene::nSelectChaType = 0; // ±£´æÑ¡ÖÐ½ÇÉ«µÄÐÅÏ¢
int CWorldScene::nSelectChaPart[enumEQUIP_PART_NUM] = {0};
...
CWorldScene::CWorldScene(stSceneInitParam& param)
	...
	_pShipMgr = std::make_unique<xShipMgr>();
	_pShipMgr->Init(this);
...
bool CWorldScene::_Init() {
	...
	if (g_Config.m_bEditor) {
		...
		for (int i = 0; i < g_Config.m_nChaCnt; i++) {
			...
			if (pCha) {
				...
				pCha->setYaw(rng.uniform(45, -45 - 1));
				if (CWorldScene::nSelectChaType == 0) {
					if (i == 0)
	...
	_LoadAnimLight(R"(.\scripts\txt\aaa.tx)");
...
void CWorldScene::_FrameMove(DWORD dwTimeParam) {
	...
	if ((dwLastTime < dwTimeParam) && GetMainCha()) {
		dwLastTime = rng.uniform(dwTimeParam + 200, dwTimeParam + 200 + 300 - 1);
	...
	case GUI::eMouseAction::Gui:
		break;
	case GUI::eMouseAction::Skill:
		...
		break;
	case GUI::eMouseAction::Drill:
		if (!CGameApp::IsMouseInScene())
			break;

	case GUI::eMouseAction::None:
		if (GetUseLevel().IsTrue(LEVEL_CHA_RUN)) {
			static bool IsMouseWalk = false;
			MouseClickState key = g_pGameApp->GetMouseKey();
			if (IsMouseWalk) {
				if ((key & MouseClickState::Down) != MouseClickState()) {
					if (GetMainCha()) {
						if ((key & MouseClickState::LDown) != MouseClickState()) {
							...
							auto* pMove = dynamic_cast<CMoveState*>(GetMainCha()->GetActor()->GetCurState());
							if (pMove) {
				...
				IsMouseWalk = true;
				CCursor::I()->Restore();
			} else if (key == MouseClickState()) {
				_SceneCursor();
			} else if ((key & MouseClickState::LDown) != MouseClickState()) {
	...
	static MouseClickState key = MouseClickState();
	...
	if (((key & MouseClickState::RUp) != MouseClickState()) && (CGameApp::GetCurTick() <= RDownTime)) {
		...
		if (CFormMgr::GetMouseAction() == GUI::eMouseAction::None)
			_cMouseDown.Reset();
	}
...
void CWorldScene::_KeyDownEvent(int key) {
	...
	if (g_Editor.IsEnable()) {
		...
		if (key == VK_F12) {
			...
			fscanf_s( fp, "%d %d %d %d %d %d", &id, &partry {uf[ 0 ], &partry {uf[ 1 ], &partry {uf[ 2 ], &partry {uf[ 3 ],
				&partry {uf[ 4 ], &partry {uf[ 5 ] );
			...
			fprintf_s( fp, "%d\n%010d %010d %010d %010d %010d", id > 4 ? 0 : id+1,
				partry {uf[ 0 ], partry {uf[ 1 ], partry {uf[ 2 ], partry {uf[ 3 ], partry {uf[ 4 ] );

			fclose( fp );

			this->GetMainCha()->LoadPart( id, partry {uf[ id ] );
	...
	__end:
			...
			char file[260];
			_snprintf_s(file, _TRUNCATE, R"(.\model\character\%010d.lgo)", part);
	...
	if (key == VK_INSERT && !GetMainCha()->IsBoat()) {
		auto* seat = dynamic_cast<CInsertState*>(GetMainCha()->GetActor()->GetCurState());
		if (seat) {
			seat->Cancel();
			return;
		}
		...
		GetMainCha()->GetActor()->SwitchState(std::unique_ptr<CInsertState>(seat));
		return;
	}
...
bool CWorldScene::_MouseButtonDown(int nButton) {
	...
	switch (nButton) {
		case 0: {
			g_stUIEquip.ExecFastKey(VK_LBUTTON);
		...
		case 1: {
			...
			*/
			g_stUIEquip.ExecFastKey(VK_RBUTTON);
			break;				//optional
		}
		case 2: {
			g_stUIEquip.ExecFastKey(VK_MBUTTON);
			break;				//optional
		}
		case 3: {
			g_stUIEquip.ExecFastKey(VK_XBUTTON1);
			break;							//optional
		}
		case 4: {
			g_stUIEquip.ExecFastKey(VK_XBUTTON2);
			break;							//optional
		}
	}
	return true;
