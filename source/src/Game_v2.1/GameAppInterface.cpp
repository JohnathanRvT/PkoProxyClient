...
void CGameApp::ResetCaption() {
	...
	char szCaption[350];
	_snprintf_s(szCaption, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_77), szSpace); // , curr_ver.c_str());
...
void LoadResModelBuf(MPIResourceMgr* res_mgr) {
	...
	for (DWORD i = 0; i < model_num; i++) {
		...
		//      }
		handle = obj_info->nID;
		_snprintf_s(path, _TRUNCATE, "%s%s", model_path, obj_info->szDataName);
