...
int APIENTRY _tWinMain(HINSTANCE hInstance,
	...
	T_B
		string const strParam = lpCmdLine;

	if (strParam.find("editor") != std::string::npos) // 锟斤拷锟斤拷锟较凤拷嗉拷锟?
	{
	...
	//  ??loading??
	//GameLoading::Init(strParam)->Create(); //CHANGE: Scrap game loading movie (GameLoading.cpp and .h not used)
	...
	if (strParam.find("-objtrim") != std::string::npos) {
		bool const backup = strParam.find("backup") != std::string::npos ? true : false;
		g_ObjFile.TrimDirectory("map", backup);
		return 0;
	}
	...
	if (strParam.find("pack") != std::string::npos) {
		...
		char pszpath[128];
		char pszFile[128];
		_snprintf_s(pszpath, _TRUNCATE, "texture\\minimap\\%s", pszPos);
		_snprintf_s(pszFile, _TRUNCATE, "texture\\minimap\\%s\\%s.pk", pszPos, pszPos);
	...
	if (strParam.find("startgame") == std::string::npos) {
	...
	if (strParam.find("table_bin") != std::string::npos) { //锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟絫able_bin 锟斤拷锟斤拷锟缴讹拷锟斤拷锟狡憋拷
	...
	int nScreenMode[2][2] = // 锟斤拷幕锟斤拷示锟斤拷式
		{
			800, 600,
			1024, 768};
...
void MakeBinTable() {
	...
	//CHANGED: Removed MPResourceSet from here, because InitAllTable does that already
