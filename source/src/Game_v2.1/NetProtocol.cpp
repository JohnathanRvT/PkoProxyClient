...
#if (IS_NEW_LOGIN == 1) //CHANGED: Unknown
	CCreateChaScene& rkScene = CCreateChaScene::GetCurrScene();

	//pScene->ShowChaFoundForm();

	if (Errno == 0) {			// Success create new character.
		rkScene.CreateNewCha(); //CHANGED: uncommented this (which fixes char not showing up on screen).

		//CWorldScene& rkScene = CWorldScene::GetCurrScene();
		//rkScene.CreateCha();
	} else {
		CCreateChaScene* pScene = dynamic_cast<CCreateChaScene*>(CGameApp::GetCurScene());
		pScene->NewChaError(Errno, "NetNewCha");
	}
#else
	CLoginScene* pScene = dynamic_cast<CLoginScene*>(CGameApp::GetCurScene());
	if (!pScene) {
...
void NetDelCha(unsigned short Errno) //从NetRetCode.h文件中查询errno意义
{
...
#if (IS_NEW_LOGIN == 1)
	//CWorldScene* pScene = dynamic_cast<CWorldScene*>(CGameApp::GetCurScene()); //CHANGED: Put proper function here
	...
	if (Errno == 0) {
		...
		pScene->DelCurrentSelCha(); //CHANGED: Put proper function here
		CGameApp::Waiting(false);
...
void NetChangeChaLookEnergy(unsigned int nID, stLookEnergy& SLookEnergy) {
	...
	for (int i = 0; i < enumEQUIP_NUM; i++) {
		...
		if (pItem) {
			pItem->GetData().sEnergy[0] = SLookEnergy.sEnergy[i]; //CHANGED: Uncommented this
		}
	}
}
...
void NetTradeAddBoat(DWORD dwCharID, BYTE byOpType, USHORT sItemID, BYTE byIndex,
					 WORD wCount, BYTE byItemIndex, const NET_CHARTRADE_BOATDATA& Data) {
	if (byOpType == mission::TRADE_DRAGTO_ITEM) {
		//
		g_stUITrade.DragTradeToItem(dwCharID, byIndex, byItemIndex);
	} else {
		// 处理船只信息
		g_stUITrade.DragItemToTrade(dwCharID, sItemID, byIndex, wCount, byItemIndex, NULL, &Data);
	}
}

void NetTradeAddItem(DWORD dwCharID, BYTE byOpType, USHORT sItemID, BYTE byIndex,
					 WORD wCount, BYTE byItemIndex, const NET_CHARTRADE_ITEMDATA& Data) {
	if (byOpType == mission::TRADE_DRAGTO_ITEM) {
		g_stUITrade.DragTradeToItem(dwCharID, byIndex, byItemIndex);
	} else {
		SItemGrid sGrid;
		...
		sGrid.sEnergy[0] = Data.sEnergy[0]; //CHANGED: Uncommented this.
		sGrid.sEnergy[1] = Data.sEnergy[1]; //CHANGED: Uncommented this.
		...
		g_stUITrade.DragItemToTrade(dwCharID, sItemID, byIndex, wCount, byItemIndex, &sGrid, NULL);
	}
}
		...
		if (pGrid[i].SGridContent.sID > 0) {
			...
			if (SKitbag.chBagType != 1 && nMarginNum > 0) // modify by Philip.Wu  2006-06-21  拾起0个的BUG修正
			{
				...
				case enumSYN_KITBAG_PICK:
					...
					break;
				//case enumSYN_KITBAG_FROM_NPC:	g_pGameApp->SysInfo("NPC?[%s נ%d]!", item->szName, nMarginNum );	break;
