...
BOOL SC_QueryCha(LPRPACKET pk) {
	...
	_snprintf_s(szInfo, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_308), pChaName, lChaID, pMapName, lPosX, lPosY);
...
BOOL SC_QueryChaPing(LPRPACKET pk) {
	...
	_snprintf_s(szInfo, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_309), pMapName, lPing);
	SShowInfo.m_sysinfo = szInfo;
	NetSysInfo(SShowInfo);
...
void ReadChaBasePacket(LPRPACKET pk, stNetActorCreate& SCreateInfo) {
	...
	SCreateInfo.strMottoName = (string)pk.ReadString(); // 读出座右铭 //FIXED: Strings were corrupted (wrong type)
	SCreateInfo.sIcon = pk.ReadShort();					// 角色聊天头像
	SCreateInfo.lGuildID = pk.ReadLong();
	SCreateInfo.strGuildName = (string)pk.ReadString();  // 工会名 //FIXED: Strings were corrupted (wrong type)
	SCreateInfo.strGuildMotto = (string)pk.ReadString(); // 工会座右铭 //FIXED: Strings were corrupted (wrong type)
	SCreateInfo.strStallName = (string)pk.ReadString();  // 工会座右铭 //FIXED: Strings were corrupted (wrong type)
	...
	if (pk.ReadChar() == 1) // 船的外观
	{
		...
		for (int i = 0; i < enumEQUIP_NUM; i++) {
			...
			if (SLookInfo.chSynType == enumSYN_LOOK_CHANGE) {
				...
				pItem->sEnergy[0] = pk.ReadShort(); //CHANGED: Uncommented this
				...
				pItem->sEnergy[0] = pk.ReadShort(); //CHANGED: Uncommented this
				pItem->sEnergy[1] = pk.ReadShort(); //CHANGED: Uncommented this
...
void ReadChaKitbagPacket(LPRPACKET pk, stNetKitbag& SKitbag) {
	...
	while (1) {
		...
		pItem->sID = pk.ReadShort();
		LG("item", RES_STRING(CL_LANGUAGE_MATCH_313), Grid[nGridNum].sGridID, pItem->sID);
		if (pItem->sID > 0) // 存在道具
		{
			pItem->dwDBID = pk.ReadLong();
			pItem->sNum = pk.ReadShort();
...
BOOL SC_LeaveTimeShow(LPRPACKET packet) {
	T_B long distime = packet.ReadLong();
	if (distime >= 1) {
