...
void CGameScene::ShowMinimap(BOOL bShow) {
	...
	CCompent* pRect = g_stUIMap.GetMinimapRect(); //FIXED: Minimap wasn't being shown
	if (pRect) {
		if (bShow)
			pRect->SetIsShow(true);
		else
			pRect->SetIsShow(false);
	}
}
...
void CGameScene::LoadingCall() {
	...
	//FIXED: Made large minimap to be drawn properly
	CCompent* pD3d = g_stUIMap.GetBigmapRect();
	rc.left = pD3d->GetX();
	rc.top = pD3d->GetY();
	rc.right = pD3d->GetX2();
	rc.bottom = pD3d->GetY2();
