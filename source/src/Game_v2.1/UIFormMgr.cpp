...
void CFormMgr::FrameMove(int x, int y, DWORD dwMouseKey, DWORD dwTime) {
	...
	if (dwMouseKey) {
		...
		_nMouseHover = 0;
	} else if (_eMouseAction != eMouseAction::None) {
		_nMouseHover++;
...
void CFormMgr::ShowDebugInfo() {
	...
	for (i = 0; i < (int)_forms->size(); i++) {
		...
		_snprintf_s(buf, _TRUNCATE, "%-6d %-20s %s   %s     %s     %s     %3s    %3d    %2s     %4s", i, pInfo[i].formName.c_str(), pInfo[i].coordinate.c_str(), pInfo[i].isShow.c_str(), pInfo[i].isEnable.c_str(), pInfo[i].hasPopMenu.c_str(), (pInfo[i].layer >= 0) ? temp : "N/A",
