...
CGoodsGrid::~CGoodsGrid() {
	...
	SAFE_DELETE(_pScroll); // UI????
	SAFE_DELETE(_pUnit);   // UI????
}
...
void CGoodsGrid::Render() {
	...
	if (_eShowStyle == eShowStyle::Small) {
		for (i = _nFirst; i < _nLast; i++) {
			if (_pItems[i])
	...
	} else if (_eShowStyle == eShowStyle::Sale) {
		for (i = _nFirst; i < _nLast; i++) {
			if (_pItems[i])
	...
	} else if (_eShowStyle == eShowStyle::OwnDef) {
		for (i = _nFirst; i < _nLast; i++) {
			if (_pItems[i])
...
bool CGoodsGrid::SetContent(int nRow, int nCol) {
	...
	if (pOldItems) {
		...
		SAFE_DELETE_ARRAY(pOldItems); //?????? by Waiting 2009-06-18
	}
	Init();
	return true;
}
