...
CGuiData::CGuiData()
	: _dwID(0), _nLeft(0), _nTop(0), _nWidth(0), _nHeight(0), _nX1(0), _nX2(0), _nY1(0), _nY2(0), _bShow(true), _bEnabled(true), _pParent(NULL), _pDrag(nullptr), _IsMouseIn(false), nTag(0), _pVoid(NULL) {
...
CGuiData::CGuiData(const CGuiData& rhs)
	...
	if (!rhs._pDrag) {
		...
		_pDrag.reset(new CDrag(*rhs._pDrag));
	}
}
...
CDrag::CDrag()
	: evtMouseDragBegin(NULL), evtMouseDragMove(NULL), evtMouseDragEnd(NULL), _eState(eState::None), _nYareLen(2), _IsMove(true), _IsUseGrid(true), _crDragIn(CCursor::stEnd), _crDrag(CCursor::stDrag) {
}
