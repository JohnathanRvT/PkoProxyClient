...
void CItemCommand::Render(int x, int y) {
	...
	if (_ItemData.dwDBID) {
		_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_961));
		static int w, h;
		CGuiFont::s_Font.GetSize(buf, w, h);
...
void CItemCommand::AddHint(int x, int y, int index) {
	...
	if (_pItem->sType >= 1 && _pItem->sType <= 10) // 武器:剑,巨剑,弓,火绳枪,刀,拳套,匕首,钱袋,短棒,锤子
	{
		...
		if (CItemRecord::IsVaildFusionID(_pItem) && _ItemData.GetFusionItemID() > 0) // modify by ning.yan  20080821
		{
			_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_627), _ItemData.GetItemLevel() * 2 + 80); // 发挥度定义 0级-80% 1级-82% ...
			PushHint(buf, GENERIC_COLOR, 5, 1, index);
		}
	...
	} else if (_pItem->sType == 22 || _pItem->sType == 11 || _pItem->sType == 27) // 衣服,盾,图腾
	{
		...
		_PushValue(RES_STRING(CL_LANGUAGE_MATCH_630), ITEMATTR_VAL_PDEF, item, COLOR_WHITE, index); //FIXED: Uncommented this
	...
	} else if (_pItem->sType == 25) // 项链
	{
		...
		if (!isStore) // 商城内不显示耐久
		{
			//FIXED: Commented this (no durability on necklaces)
			//_snprintf_s( buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_626), item.sEndure[0], item.sEndure[1] );
			//PushHint( buf, COLOR_WHITE, 5, 1, index );
		}
		//	End.
	...
	} else if (_pItem->sType == 26) // 戒指
	{
		...
		if (!isStore) // 商城内不显示耐久
		{
			//FIXED: Commented this (no durability on rings)
			//_snprintf_s( buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_626), item.sEndure[0], item.sEndure[1] );
			//PushHint( buf, COLOR_WHITE, 5, 1, index );
		}
		//	End.
		...
		{
			_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_862), _ItemData.sEndure[0] * 10 - 1000, _ItemData.sEndure[1] * 10 - 1000); // 具体计算方式询问策划
			PushHint(buf, COLOR_WHITE, 5, 1, index);
	...
	} else if (_pItem->sType == 43) // 船长证明
	{
		...
		if (_pBoatHint) {
			...
			_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_UIITEMCOMMAND_CPP_00014), _pBoatHint->dwExp);
			PushHint(buf, COLOR_WHITE, 5, 1, index);
			...
			if (pBoat) {
				...
				_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_643), StringSplitNum((long)pAttr->get(ATTR_BOAT_PRICE) / 2));
				PushHint(buf, COLOR_WHITE, 5, 1, index);
			}
		}
	...
	} else if (_pItem->sType == 29) // 贝壳
	{
		...
		_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_644), _ItemData.sEnergy[0], _ItemData.sEnergy[1]); //CHANGED: Uncommented this
		PushHint(buf, COLOR_WHITE, 5, 1, index);
	...
	} else if (_pItem->sType == 45) // 贸易证明
	{
		...
		PUSH_HINT(RES_STRING(CL_LANGUAGE_MATCH_634), _ItemData.sEnergy[0], COLOR_WHITE, index); //CHANGED: Uncommented this.

		float fB320 = (float)_ItemData.sEnergy[1]; //CHANGED: Uncommented this block (up to PushHint)
		float fRate = 0.0f;
	...
	} else if (_pItem->sType == 46) // 勇者之证
	{
		...
		//FIXED: Uncommented the whole section as follows
		...
		//_snprintf_s( buf, _TRUNCATE, "%s:%d", RES_STRING(CL_LANGUAGE_MATCH_848), _ItemData.sEnergy[1] );	// 乱斗点数
		PushHint(buf, GENERIC_COLOR, 5, 1, index);
	...
	{
		//FIXED: Fairy not showing proper level //ADDED: Luck affects fairy level
		int nLevel = item.sInstAttr[ITEMATTR_VAL_STR] + item.sInstAttr[ITEMATTR_VAL_AGI] + item.sInstAttr[ITEMATTR_VAL_DEX] + item.sInstAttr[ITEMATTR_VAL_CON] + item.sInstAttr[ITEMATTR_VAL_STA] + item.sInstAttr[ITEMATTR_VAL_LUK];
		...
		PUSH_HINT(RES_STRING(CL_LANGUAGE_MATCH_660), item.sInstAttr[ITEMATTR_VAL_DEX], COLOR_WHITE, index); //FIXED: Uncommented this
		PUSH_HINT(RES_STRING(CL_LANGUAGE_MATCH_661), item.sInstAttr[ITEMATTR_VAL_STA], COLOR_WHITE, index);
		PUSH_HINT(RES_STRING(CL_LANGUAGE_MATCH_1085), item.sInstAttr[ITEMATTR_VAL_LUK], COLOR_WHITE, index);

		//FIXED: Uncommented this section //ADDED: Luck
		...
		if (!isStore) // 商城内不显示精灵的体力和成长度
		{
			...
			_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_663), _ItemData.sEnergy[0], _ItemData.sEnergy[1]); //CHANGED: Uncommented this
			PushHint(buf, COLOR_WHITE, 5, 1, index);																	//CHANGED: Uncommented this
		}
	...
	} else if (_pItem->sType == 65) // 黑龙祭坛  add by Philip.Wu  2006-06-19
	{
		...
		//	Add by alfred.shi 20081126	begin
			...
			if (index != -1)
				PushHint(RES_STRING(CL_LANGUAGE_MATCH_1041), COLOR_WHITE, 5, 1, index);
			_snprintf_s(buf, _TRUNCATE, "%s:%d", RES_STRING(CL_UIITEMCOMMAND_CPP_00002), (item.sInstAttr[ITEMATTR_VAL_STR] * 32768 + item.sInstAttr[ITEMATTR_VAL_CON]));
			PushHint(buf, GENERIC_COLOR, 5, 1, index);
			_snprintf_s(buf, _TRUNCATE, "%s:%d", RES_STRING(CL_UIITEMCOMMAND_CPP_00003), item.sInstAttr[ITEMATTR_VAL_AGI]);
			PushHint(buf, GENERIC_COLOR, 5, 1, index);
			_snprintf_s(buf, _TRUNCATE, "%s:%d", RES_STRING(CL_UIITEMCOMMAND_CPP_00004), item.sInstAttr[ITEMATTR_VAL_DEX]); //FIXED: Uncommented this (round number to arena pass)
			PushHint(buf, GENERIC_COLOR, 5, 1, index);
			return;
		}
		//	End
		...
		} else if (_pItem->lID == 2954) // 特殊道具处理（死亡证明）
		{
			...
			_snprintf_s(buf, _TRUNCATE, "%s: %d", RES_STRING(CL_LANGUAGE_MATCH_933), nCount); // "死亡次数"
			PushHint(buf, COLOR_WHITE, 5, 1, index);
		...
		//Begin
			...
			if (_pItem->nID == 6373) {
				...
				_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_UIITEMCOMMAND_CPP_00010), nCount);
			}
		...
		_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_666), 5 - item.sInstAttr[ITEMATTR_VAL_DEX]); //FIXED: Uncommented this
		PushHint(buf, COLOR_WHITE, 5, 1, index);
	...
	} else if (_pItem->sType == 69) // XXX图纸
	{
		...
		_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_871), _ItemData.sEnergy[1] - 100); //CHANGED: Uncommented this
		PushHint(buf, GENERIC_COLOR, 5, 1, index);													//CHANGED: Uncommented this
		...
		lForge %= 1000;
		iItem = item.sInstAttr[ITEMATTR_VAL_DEX]; //FIXED: Uncommented this
		if (iItem) {
			pCItemRec = GetItemRecordInfo(iItem);
			if (pCItemRec) {
		...
		if (_nPrice != 0) {
			...
			//FIXED: Added string from item description below price removed
			//TODO: Detect language string so that this only gets called when Chinese is used
		...
		{
			_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_909), item.sInstAttr[ITEMATTR_VAL_STR]); // "情缘编码：%d"
			PushHint(buf, GENERIC_COLOR, 5, 1, index);
		...
		if (_pItem->lID != 2236) {
			...
			//FIXED: Uncommented this
			_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_897), _ItemData.sEnergy[0]); // "工具经验：%i"
			PushHint(buf, GENERIC_COLOR, 5, 1, index);
		}
	...
	} else if (_pItem->sType == 71 && _pItem->lID == 3289) //学生证处理
	{
		...
		if (0 <= nLevel && nLevel <= 4) {
			_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_949), arShowName[nLevel]); // "学历:%s"
			PushHint(buf, COLOR_WHITE, 5, 1, index);
		}

		_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_950), item.sEndure[0], item.sEndure[1]); // "学分(%d/%d)"
		PushHint(buf, COLOR_WHITE, 5, 1, index);

		_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_951), item.sEnergy[0] * 1000, item.sEnergy[1] * 1000); // "经验(%d/%d)"
		PushHint(buf, COLOR_WHITE, 5, 1, index);
	...
	//	End.
		...
		} else if (_pItem->lID == 2967) // 马嫂要求特殊处理，红酒
		{
			...
			_snprintf_s(buf, _TRUNCATE, "%s:%d", RES_STRING(CL_UIITEMCOMMAND_CPP_00003), item.sInstAttr[ITEMATTR_VAL_AGI]);
			PushHint(buf, GENERIC_COLOR, 5, 1, index);
			_snprintf_s(buf, _TRUNCATE, "%s:%d", RES_STRING(CL_UIITEMCOMMAND_CPP_00004), item.sInstAttr[ITEMATTR_VAL_DEX]); //FIXED: Uncommented this
			PushHint(buf, GENERIC_COLOR, 5, 1, index);
			return;
		}
		...
		// end
			...
			if (index != -1)
				PushHint(RES_STRING(CL_LANGUAGE_MATCH_1041), COLOR_WHITE, 5, 1, index);
			_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_UIITEMCOMMAND_CPP_00014), item.sInstAttr[ITEMATTR_VAL_STR]);
			PushHint(buf, GENERIC_COLOR, 5, 1, index);
			...
			_snprintf_s(buf, _TRUNCATE, "%s:%d", RES_STRING(CL_LANGUAGE_MATCH_1028), value);
			PushHint(buf, GENERIC_COLOR, 5, 1, index);
	...
	} else // 一般道具
	{
		...
		if (_pItem->sType == 77) {
			_snprintf_s(buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_980), item.sEndure[0], item.sEndure[1]);
			PushHint(buf, COLOR_WHITE, 5, 1, index);
		}
	...
	if (_pItem->sType == 80) {
		//FIXED: Uncommented this whole section
		...
		if (isZero != 0) {
			_snprintf_s(buf, _TRUNCATE, "装备：%s", sNameInfo->szName);
	...
	if (_hints.GetCount() > 0 && _pItem->sType <= 27 && _pItem->sType != 18 && _pItem->sType != 19 && _pItem->sType != 21) {
		...
		char szBuf[16] = {0};
		_snprintf_s(szBuf, _TRUNCATE, "%09d", _ItemData.sEnergy[1] / 10); // 除十是因为服务器的数值为4位数,等待以后改为三位 //CHANGED: Uncommented this.
	...
	if (_hints.GetCount() > 0 && Forge.IsForge) {
		...
		if (Forge.nLevel > 0) {
			if (index == -1) {
				...
				if (_ItemData.dwDBID) {
					_snprintf_s(buf, _TRUNCATE, "%s +%d", _hints.GetHintRelated(index, 2)->hint.c_str(), Forge.nLevel);
					_hints.GetHintRelated(index, 2)->hint = buf;
				} else {
					_snprintf_s(buf, _TRUNCATE, "%s +%d", _hints.GetHintRelated(index, 0)->hint.c_str(), Forge.nLevel);
					_hints.GetHintRelated(index, 0)->hint = buf;
				}
			}
		}
	...
	if (track_related) {
		...
		for (int i = 0; i < enumEQUIP_NUM; ++i) {
			...
			if (pItem) {
				...
				if( CItemRecord::IsVaildFusionID( pItemRecord ) && pItem->_ItemData.GetFusionItemID() > 0 )
					_snprintf_s( buf, _TRUNCATE, "Lv%d %s", pItem->_ItemData.GetItemLevel(), pItem->GetName() );
				else
					_snprintf_s( buf, _TRUNCATE, "%s", pItem->GetName() );
				...
				if( pItemRecord->sType == 26 )
				{
					...
					{
						_snprintf_s( buf, _TRUNCATE, RES_STRING( CL_LANGUAGE_MATCH_862 ),
				...
				if( pItemRecord->sType >= 1 && pItemRecord->sType <= 10 )  
				{
					...
					{
						_snprintf_s( buf, _TRUNCATE, RES_STRING( CL_LANGUAGE_MATCH_625 ),
					...
					pItemRecord->sType == 23 || pItemRecord->sType == 24 || pItemRecord->sType == 27  )
				{
					if ( CItemRecord::IsVaildFusionID( pItemRecord ) && pItem->_ItemData.GetFusionItemID() > 0 )
					{
						_snprintf_s( buf, _TRUNCATE, RES_STRING( CL_LANGUAGE_MATCH_627 ), 
				...
				if( pItem->_hints.GetCount() > 0 && Forge.IsForge )
				{
					...
					for( int i= 0; i < Forge.nStoneNum && i < Forge.nHoleNum; i++ )
					{
						_snprintf_s( buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_656 ), 
				...
				if( pItem->_nPrice != 0 )
				{
					...
					_snprintf_s( buf, _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_674 ), StringSplitNum( pItem->_nPrice / 2 ) );
					PushHint( buf, COLOR_WHITE, 5, 0, index );

					//FIXED: Added string from item description below price removed
					//TODO: Detect language string so that this only gets called when Chinese is used
					//_snprintf_s( buf, _TRUNCATE, "%s", ConvertNumToChinese( pItem->_nPrice / 2 ).c_str() );
					//PushHint( buf, COLOR_WHITE, 5, 0, index );
				}
...
string CItemCommand::GetStoneHint(int nLevel) {
	...
	if (_pItem->sType == 49) {
		...
		for (int i = 0; i < nCount; i++) {
			...
			if (nLevel < 0) //CHANGED: Uncommented this block.
...
void CItemCommand::_PushItemAttr(int attr, SItemHint& item, DWORD color, int index) {
	...
	if (attr <= ITEMATTR_COE_PDEF) //FIXED: Uncommented this
	{
...
void CItemCommand::RenderEnergy(int x, int y) {
	if (_pItem->sType == 29 && _ItemData.sEnergy[1] != 0) //CHANGED: Uncommented this.
	{
...
// class SItemHint
//---------------------------------------------------------------------------
void SItemHint::Convert(SItemGrid& ItemGrid, CItemRecord* pInfo) //CHANGED: Uncommented this whole function contents
{
	sID = ItemGrid.sID;
	sNum = ItemGrid.sNum;
