...
void CSystemMgr::LoadCustomProp() {
	...
	if (!m_isLoad) {
		if (m_sysProp.Load("user\\system.ini")) {
			...
			m_sysProp.m_videoProp.bFullScreen = g_pGameApp->IsFullScreen() ? true : false;
...
bool CSystemMgr::Init() {
	...
	*/
	...
	if (!cbxEquipCmp)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmGameOption->GetName(), "comequip");
...
void CSystemMgr::_evtVideoFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, DWORD dwKey) {
	...
	if (name == "btnYes") //直接关闭表单
	{
		...
		g_pGameApp->ChangeVideoStyle(width, height, format, bWindowed); //TODO: Models disappear on video options change.
