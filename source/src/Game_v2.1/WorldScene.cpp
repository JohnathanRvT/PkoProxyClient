...
// class CWorldScene
//---------------------------------------------------------------------------
int CWorldScene::nSelectChaType = 0; // ???摡֐?ǉ??Đŏ?
...
void CWorldScene::_FrameMove(DWORD dwTimeParam) {
	...
	if ((dwLastTime < dwTimeParam) && GetMainCha()) {
		...
		if (_cSceenSign.IsShowMove() && GetMainCha()->GetIsArrive() && GetMainCha()->GetActor()->IsEmpty()) {
			_cSceenSign.Hide();
			//TODO: Crash on GetEffect(536) - returns nullptr
			// Call Non-existent in 1.x source
	...
	if (g_pGameApp->_CanInput()) {
		...
		case GUI::eMouseAction::Drill:
			if (!CGameApp::IsMouseInScene()) //TODO: Intentional fallthrough?
				break;
...
void CWorldScene::_KeyDownEvent(int key) {
	...
	__end:
			...
			char file[260];
			_snprintf_s(file, _TRUNCATE, ".\\model\\character\\%010d.lgo", part);
