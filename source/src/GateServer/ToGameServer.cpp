...
void ToGameServer::OnDisconnect(DataSocket* datasock, int reason) // reason值:0-本地程序正常退出；-3-网络被对方关闭；-1-Socket错误;-5-包长度超过限制。
{
	...
	try {
		...
		for (size_t i = 0; i < player_count; ++i) {
			auto ply_addr = ToPointer<Player>(retpk.ReadAddress());
			...
			try {
				g_GateServer->toClient->post_mapcrash_msg(ply_addr);
			} catch (...) {
...
void ToGameServer::SendAllGameServer(WPacket& in_para) {
	...
	for (const auto& [name, pGameServer] : _map_game) {
		if (pGameServer) {
			if (auto it = std::find(informedGameServers.begin(), informedGameServers.end(), pGameServer);
				it == informedGameServers.end()) {
				if (auto sock = pGameServer->m_datasock; sock) {
					this->SendData(sock, in_para);
				}
				informedGameServers.push_back(pGameServer);
			}
		}
	}
}
...
void ToGameServer::OnProcessData(DataSocket* datasock, RPacket& recvbuf) {
	...
	try {
		...
		case CMD_MT_SWITCHMAP: {
			...
			recvbuf.DiscardLast(sizeof(uChar) + sizeof(uintptr_t) * 2 * l_aimnum + sizeof(uShort));
			...
			LogLine l_line(g_gatelog);
			GameServer* inner_scope_game = g_GateServer->toGameServer->find(l_map);
			if (inner_scope_game) {
			...
			} else if (!l_return) //目标地图不可达，重新进入源地图
			{
				...
				wpk.WriteString((std::string("[") + l_map + "] can't reach, please retry later!").c_str());
				...
				g_GateServer->toClient->Disconnect(player->m_datasock, 0, -24);
			}
		...
		case CMD_MC_ENTERMAP: {
			RPacket rpk = recvbuf;
			const auto l_aimnum = rpk.ReverseReadShort(); //l_aimnum永远等于1
			...
			const auto l_dbid = rpk.ReverseReadLong();
			LogLine l_line(g_gatelog);
			if (player->m_dbid != l_dbid) { //chaid
				l_line << newln << "receive from [" << datasock->GetPeerIP() << "] EnterMap command ,can't match DBID:locale [" << player->m_dbid << "],far[" << l_dbid << "]"
					   << std::uppercase << std::hex << ",Gate address:" << ToAddress(player) << endln;
				return;
			}

			if (const auto ret_code = rpk.ReadShort(); ret_code == ERR_SUCCESS) {
				player->game = outer_scope_game;
				player->gm_addr = rpk.ReverseReadAddress();
				outer_scope_game->m_plynum = rpk.ReverseReadLong();
				const auto l_isSwitch = rpk.ReverseReadChar();

				player->m_switch = 0;

				l_line << newln << "client: " << player->m_datasock->GetPeerIP() << ":" << player->m_datasock->GetPeerPort()
					   << "	receive Gate  from [" << datasock->GetPeerIP() << "]success EnterMap command,Game address:"
					   << std::uppercase << std::hex << player->gm_addr << ",Gate address:" << ToAddress(player) << std::dec << std::nouppercase << endln;

				recvbuf.DiscardLast(sizeof(uShort) + (sizeof(uintptr_t) + sizeof(uLong)) * l_aimnum + sizeof(uintptr_t) + sizeof(uLong) + sizeof(uChar));
				...
				recvbuf.DiscardLast(sizeof(uShort) + (sizeof(uintptr_t) + sizeof(uLong)) * l_aimnum);
				//g_gtsvr->cli_conn->SendData(player->m_datasock,recvbuf);
				g_GateServer->toClient->Disconnect(player->m_datasock, 10, -33);
			}
		} break;
		case CMD_MT_KICKUSER: {
			...
			for (uShort i = 0; i < l_aimnum; i++) {
				...
				if (player && player->m_dbid == recvbuf.ReverseReadLong()) {
					g_GateServer->toClient->Disconnect(player->m_datasock, 0, -23); //－23错误码表示GameServer要求踢掉某人
				}
			}
			break;
		}
		case CMD_MT_MAPENTRY: {
			...
			if (l_game) {
				...
				g_GateServer->toGameServer->SendData(datasock, wpk);
			}
		} break;
		case CMD_MT_STATE: {
			RPacket rpk = recvbuf;
		...
		case CMD_MT_MAP_ADMIN: {
			...
			if (l_game) {
				...
				WPacket wpk = datasock->GetWPacket();
				wpk.WriteCmd(CMD_MC_SYSINFO);
				wpk.WriteString((std::string("[") + l_map + "] can't find!").c_str());
				player->m_datasock->SendData(wpk);
			}
		} break;
		default: // 缺省转发
		{
			if (cmd / 500 == CMD_MC_BASE / 500) {
				...
				recvbuf.DiscardLast((sizeof(uintptr_t) + sizeof(uLong)) * l_aimnum + sizeof(uShort));
				for (uShort i = 0; i < l_aimnum; i++) {
					auto player = ToPointer<Player>(rpk.ReverseReadAddress());
...
void ToGameServer::MT_LOGIN(DataSocket* datasock, RPacket& rpk) {
	...
	const char* gms_name = rpk.ReadString();
	const char* map_list = rpk.ReadString();
	//==================================
...
GameServer* ToGameServer::find(cChar* mapname) {
	auto it = _map_game.find(mapname);
	if (it == _map_game.end()) {
		LogLine l_line(g_gatelog);
...
void GameServer::EnterMap(Player* ply, uLong actid, uLong dbid, uLong worldid, cChar* map, Long lMapCpyNO, uLong x, uLong y, char entertype, short swiner) {
	...
	wpk.WriteAddress(ToAddress(ply)); //第一次附加上自己的地址
	wpk.WriteShort(swiner);
	g_GateServer->toGameServer->SendData(m_datasock, wpk);
}
