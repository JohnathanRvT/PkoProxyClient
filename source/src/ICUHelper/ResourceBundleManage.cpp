...
CResourceBundleManage::CResourceBundleManage(const char* configFileName)
	: m_ResDir(nullptr), m_ResLocale(nullptr), m_pResourceBundle(nullptr), m_pConverter(nullptr), m_MaxSize(0) /*, m_LogFile(NULL), m_LogFlag(0)*/
{
...
CResourceBundleManage::~CResourceBundleManage() {
	Release();
}

// 取得资源个数
int CResourceBundleManage::GetSize() {
...
// 释放资源
void CResourceBundleManage::Release() {
	delete[] m_ResDir;
	//cdelete(m_ResDir);
	delete[] m_ResLocale;
	//cdelete(m_ResLocale);
...
int32_t CBreakLine::WrapParagraph(const icu::UnicodeString& s, const icu::Locale& locale, int32_t maxCharsPreLine, int32_t lineStarts[], int32_t lineEnds[], int32_t maxLines) {
	...
	while (p < s.length()) {
		...
		if (q < s.length()) {
			c = s[q];
			while (q < s.length() && (u_isspace(c) || u_charType(c) == U_CONTROL_CHAR || u_charType(c) == U_NON_SPACING_MARK)) {
				++q;
				c = s[q];
			}
		}
