...
using NetSndList = _AnyQueue<pNetMessage>;
...
int PeekNetMessage(pNetMessage* nm, unsigned char** buf, long& bufsize, void** tmpbuf, long& tmpsize) {
	__BEGIN_TRY
	if ((nullptr == buf) || (nullptr == (*buf)) || (nullptr == tmpbuf)) {
		return PNM_EXCEPTION;
	}
...
unsigned InfoThread::Run() {
	...
	for (;;) {
		...
		{
			chkTime = clock();
			LG("InfoNet", "Ping: Timestamp: %li..." __LN, time(nullptr));
			if (!inb->PostInfoSend(INFO_CHECK_LIVE, 0, 0, 0, nullptr, 0)) {
...
InfoNetBase::InfoNetBase()
	: m_state(INFO_INIT), m_client(nullptr), m_list(nullptr) {
}
...
bool InfoNetBase::PostInfoSend(pNetMessage msg, bool resnd /* = true*/) {
	...
	if (SOCKET_ERROR == send(__cli->sock, buf, size, 0)) {
		...
		if (resnd) {
			...
			auto newNM = new NetMessage; //  新建一份copy放入发送队列，等待重发
