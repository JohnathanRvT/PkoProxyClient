...
bool TcpCommApp::AddSocket(DataSocket* datasock) {
	if (datasock) {
		datasock->m_sendtime = atomic_exchange(&datasock->m_recvtime, m_TickCount);
...
void TcpCommApp::Disconnect(DataSocket* datasock, uLong remain, int reason) {
	if (datasock) {
		...
		atomic_exchange(&datasock->m_deltime, datasock->__tca->m_TickCount);
	}
};
//--------------------------------------------------------------------
...
_DBC_BEGIN
struct selparm {
	fd_set m_readfds{0, 0}, m_writefds{0, 0}, m_errfds{0, 0};
	bool m_readflag{false}, m_writeflag{false}, m_errflag{false};
	int m_selretval{0};
	uLong m_tick{0};
};
_DBC_END
...
void TcpCommApp::BeforeSel(selparm& p) {
	...
	while (l_datasock = __socklist.GetNextItem()) {
		if (l_flag) {
			uLong l_ulong = atomic_exchange(&l_datasock->m_sbts, 0);
			if (l_ulong) {
				l_sendbytes += l_ulong;
			...
			l_ulong = atomic_exchange(&l_datasock->m_spks, 0);
			if (l_ulong) {
				l_sendpkts += l_ulong;
			...
			l_ulong = atomic_exchange(&l_datasock->m_rbts, 0);
			if (l_ulong) {
				l_recvbytes += l_ulong;
			...
			l_ulong = atomic_exchange(&l_datasock->m_rpks, 0);
			if (l_ulong) {
				l_recvpkts += l_ulong;
		...
		if (m_keepalive) {
			if (l_tick - uLong(l_datasock->m_recvtime) > m_keepalive) {
				...
				wpk.m_wpos = __len_offset + __len_size - wpk.GetPktLen();
				_SendData(l_datasock, wpk);
...
void TcpClientApp::TaskDispatcher(Task* l_dt) {
	...
	while (!l_dt->GetExitFlag() && !m_selexit) {
		...
		l_p.m_selretval = select(0, l_p.m_readflag ? &(l_p.m_readfds) : nullptr, l_p.m_writeflag ? &(l_p.m_writefds) : nullptr, l_p.m_errflag ? &(l_p.m_errfds) : nullptr, &l_timeout);
...
std::string TcpCommApp::GetDisconnectErrText(int reason) const {
	...
	default:
		return "未知Disconnect码：" + std::to_string(reason);
	}
}
