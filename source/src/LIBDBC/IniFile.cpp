...
IniFile::IniFile(const char* filename)
	: m_rw(false), m_update(false), m_sectcount(0), m_sectsize(0), m_sect(nullptr) {
...
IniSection& IniFile::operator[](const char* sectname) const {
	...
	if (i < m_sectcount) {
		...
		THROW_EXCP(excpIniF, std::string("Section:") + sectname + " 没有找到.");
	}
}
...
IniItem& IniSection::operator[](int i) {
	...
	if (i >= 0 && i < m_itemcount)
		...
		THROW_EXCP(excpArr, std::string("Section:") + m_sectname + " index " + std::to_string(i) + " out of range");
	}
	return *l_it;
}
dstring& IniSection::operator[](const char* name) const {
	...
	if (i < m_itemcount) {
		...
		THROW_EXCP(excpIniF, std::string("Section:") + m_sectname + " Name:" + name + " can't find.");
	}
}
