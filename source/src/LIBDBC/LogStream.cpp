...
static RunBiDirectChain<LogStream>* s_logmgr = nullptr;
...
LogStream::LogStream(const char* LogFileName, const char* DirName)
	: m_line(false), m_logline(nullptr) {
...
std::ofstream& endln(LogLine& line) {
	if (line.m_line) {
		line.m_line = false;
		line.m_logstream.m_logline = nullptr;
...
std::ostream& endln(std::ostream& of) {
	try {
		auto* l_ofs = static_cast<std::ofstream*>(&of);
		auto* l_ls = reinterpret_cast<LogStream*>(CONTAINING_RECORD(l_ofs, LogStream, m_ofs));
		if (l_ls->m_logline) {
