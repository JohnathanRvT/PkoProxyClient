...
//================NTSvcObject=================================================================
NTSvcObject::NTSvcObject() {
	m_semSignal1.Create(0, 1, nullptr);
	m_semSignal2.Create(0, 1, nullptr);
...
void NTSvcObject::Trace_Event(uChar Severity, cChar* msg) const {
	if (HasAppEvent() || HasSysEvent()) {
		ReportEvent(HasSysEvent() ? m_hSysEvtSrc : m_hAppEvtSrc,
					Severity == TE_SUCC ? EVENTLOG_SUCCESS : Severity == TE_INFO ? EVENTLOG_INFORMATION_TYPE : Severity == TE_WARN ? EVENTLOG_WARNING_TYPE : EVENTLOG_ERROR_TYPE,
					m_dwCurrentState == SERVICE_START_PENDING ? CAT_STARTING : m_dwCurrentState == SERVICE_STOP_PENDING ? CAT_STOPPING : CAT_RUNNING,
					Severity == TE_SUCC ? EVT_SUCCESS : Severity == TE_INFO ? EVT_INFO : Severity == TE_WARN ? EVT_WARNING : EVT_ERROR,
...
void NTServiceSet::UnRegEventLog(NTSvcObject* svc) const {
	dstring l_dstr;
	if (svc->HasAppEvent()) {
		l_dstr = R"(SYSTEM\CurrentControlSet\Services\Eventlog\Application\)";
		l_dstr << svc->SetDispName();
		SHDeleteKey(HKEY_LOCAL_MACHINE, l_dstr);
	}
	if (svc->HasSecEvent()) {
		l_dstr = R"(SYSTEM\CurrentControlSet\Services\Eventlog\Security\)";
		l_dstr << svc->SetDispName() << ".SEC";
		SHDeleteKey(HKEY_LOCAL_MACHINE, l_dstr);
	}
	if (svc->HasSysEvent()) {
		l_dstr = R"(SYSTEM\CurrentControlSet\Services\Eventlog\System\)";
...
int NTServiceSet::RegEventLog(NTSvcObject* svc) const {
	...
	if (svc->HasAppEvent()) {
		...
		l_dstr << svc->SetDispName();
		SHDeleteKey(HKEY_LOCAL_MACHINE, l_dstr);
		l_retval = l_retval ? l_retval : RegCreateKeyEx(HKEY_LOCAL_MACHINE, l_dstr, 0, nullptr, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, nullptr, &l_hkey, nullptr);
...
int NTServiceSet::RegSvc() const {
	...
	SC_HANDLE sc = OpenSCManager(nullptr, nullptr, GENERIC_WRITE);
	if (!sc) {
		...
		for (PNTSVC l_svc = m_svchead; l_svc; l_svc = l_svc->m_nextservice) {
			...
			sv = CreateService(sc, l_svc->SetSvcName(),
							   ...
							   SERVICE_AUTO_START,
							   SERVICE_ERROR_NORMAL, m_argv[0], nullptr, nullptr,
							   l_svc->SetDependencies(), nullptr, nullptr);
			if (sv)
				CloseServiceHandle(sv);
			else
				l_errcount++;
...
#ifdef _DEBUG
	MessageBox(0, l_msg.c_str(), l_caption.c_str(), MB_OK);
#else
...
int NTServiceSet::Run() {
	...
	for (int i = 0; i < m_argc; i++) {
		...
		if (!i) {
			...
			GetFullPathName(m_argv[i] + 1, 1024, m_argv[i] + 1, nullptr);
	...
	//分析参数
	cChar *l_s0 = nullptr, *l_s1 = nullptr, *l_sd = nullptr;
	for (long i = 1; i < m_argc; i++) {
		l_dstr = m_argv[i];
		//strlwr(l_dstr.GetBuffer());			//转换成小写
		_strlwr_s(l_dstr.GetBuffer(), l_dstr.Size()); //转换成小写
		l_s0 = (!strncmp(l_dstr, "-reg", 4) || !strncmp(l_dstr, "-inst", 5)) ? l_dstr.c_str() : nullptr;
		l_s1 = (!strncmp(l_dstr, "-dereg", 6) || !strncmp(l_dstr, "-uninst", 7)) ? l_dstr.c_str() : nullptr;
		l_sd = !strncmp(l_dstr, "-dbg", 4) ? l_dstr.c_str() : nullptr; //是否有调试参数
		if (l_s0 || l_s1 || l_sd) {
			break;
		}
	}
	...
	if (l_s0) {		   //建立服务
		...
		if (l_svc && !i) {
			try {
				...
				MessageBox(0, (std::string("启动过程出现异常,异常包括如下信息:") + e.what()).c_str(), (std::string("服务:") + l_svc->SetDispName() + " 调试模式").c_str(), MB_OK);
			} catch (...) {
				MessageBox(0, "启动过程出现未知异常", (std::string("服务:") + l_svc->SetDispName() + " 调试模式").c_str(), MB_OK);
			}
			while (IDYES == MessageBox(0, "继续调试服务吗?(提示:选择是将运行[服务处理]函数)", (std::string("服务:") + l_svc->SetDispName() + " 调试模式").c_str(), MB_YESNO)) {
				try {
					l_svc->ServiceProcess();
				} catch (excp& e) {
					MessageBox(0, (std::string("处理过程出现异常,异常包括如下信息:") + e.what()).c_str(), (std::string("服务:") + l_svc->SetDispName() + " 调试模式").c_str(), MB_OK);
				} catch (...) {
					MessageBox(0, "处理过程出现未知异常", (std::string("服务:") + l_svc->SetDispName() + " 调试模式").c_str(), MB_OK);
				}
			}
			try {
				l_svc->ServiceStop();
			} catch (excp& e) {
				MessageBox(0, (std::string("停止过程出现异常,异常包括如下信息:") + e.what()).c_str(), (std::string("服务:") + l_svc->SetDispName() + " 调试模式").c_str(), MB_OK);
			} catch (...) {
				MessageBox(0, "停止过程出现未知异常", (std::string("服务:") + l_svc->SetDispName() + " 调试模式").c_str(), MB_OK);
			}
		}
		return 0;
	}
	...
	DispatchTable[k].lpServiceName = nullptr;
	DispatchTable[k].lpServiceProc = nullptr;
