...
//--------------------------------------------------------------------
bool WPacket::WriteLongLong(ULONG64 ll) {
	if (!bool(*this))
		return false;
	if (m_wpos >= SIGN32)
		return false;
	if (m_offset + GetPktLen() + sizeof(ULONG64) > Size()) {
		uLong l_newlen = (GetPktLen() + sizeof(ULONG64)) + (Size() - m_offset) + (__bufheap.GetUnitSize() - 1);
		l_newlen /= __bufheap.GetUnitSize();
	...
	ll = htonll(ll);
	MemCpy(const_cast<char*>(GetDataAddr()) + GetDataLen(), (cChar*)&ll, sizeof(ULONG64));
	m_wpos += sizeof(ULONG64);
	return true;
}
//--------------------------------------------------------------------
bool WPacket::WriteAddress(uintptr_t loll) {
#ifdef _WIN64
	return WriteLongLong(loll);
#elif _WIN32
	return WriteLong(loll);
#endif
}
//--------------------------------------------------------------------
...
//--------------------------------------------------------------------
ULONG64 RPacket::ReadLongLong() {
	ULONG64 l_retval = 0;
	if (RemainData() >= sizeof(ULONG64)) {
		MemCpy((char*)&l_retval, GetDataAddr() + em_cmdsize + m_rpos, sizeof(ULONG64));
		l_retval = ntohll(l_retval);
		m_rpos += sizeof(ULONG64);
	}
	return l_retval;
}
//--------------------------------------------------------------------
#if (defined WIN64)
uintptr_t RPacket::ReadAddress() {
	return ReadLongLong();
}
#elif (defined WIN32)
uintptr_t RPacket::ReadAddress() {
...
ULONG64 RPacket::ReverseReadLongLong() {
	ULONG64 l_retval = 0;
	if (GetDataLen() >= m_revrpos + sizeof(ULONG64)) {
		m_revrpos += sizeof(ULONG64);
		MemCpy((char*)&l_retval, GetDataAddr() + GetDataLen() - m_revrpos, sizeof(ULONG64));
		l_retval = ntohll(l_retval);
	}
	return l_retval;
}
#ifdef _WIN64
uintptr_t RPacket::ReverseReadAddress() {
	return ReverseReadLongLong();
}
#elif _WIN32
uintptr_t RPacket::ReverseReadAddress() {
	return ReverseReadLong();
}
#endif
