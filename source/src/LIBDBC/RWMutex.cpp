...
public:
	AccThread(size_t size) : PreAllocStru(size) {};
	~AccThread() {
		if (nextthread)
			delete nextthread;
	}
...
private:
	...
	void Finally() override { nextthread = nullptr; }

	friend class RWMutex;
	RWMutex* m_rwsync{nullptr};
	volatile int m_RDCount{0}; //读计数
	volatile int m_WRCount{0}; //写计数
	AccThread* nextthread{nullptr}; //链式结构的下一个项目
};

using PAccThread = AccThread*;
