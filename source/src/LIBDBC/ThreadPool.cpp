...
inline Task* Task::TaskExec(Thread* ThisThread) {
	...
	__ThisThread = nullptr;
	Task* l_nextTask = nullptr;
	try {
		l_nextTask = Lastly();
	} catch (...) {
	}
...
{
	auto* l_myself = reinterpret_cast<Thread*>(lpParameter);
...
Thread::Thread(ThreadPool* pool) : m_pool(pool) {
	...
	if (!m_handle)
		THROW_EXCP(excpThrd, "Create thread failed!");
