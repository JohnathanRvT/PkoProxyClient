...
void CAniFile::CompressData(const void* pSrc, void* pDst, const DWORD& srcSize, DWORD& dstSize) const {
	...
	while (totalCompress < len) {
		if (!(*(pRawData + repCount - 1) & 0xff000000) && !(*(pRawData + repCount) & 0xff000000)) { // 通道为0x00
			while (!(*(pRawData + repCount - 1) & 0xff000000) && !(*(pRawData + repCount) & 0xff000000) && ((totalCompress + repCount) < len)) {
				++repCount;
			}
			...
			*pComData++ = blockHead;
			...
			dstSize += 1;
		} else if (*pRawData == *(pRawData + repCount)) { // 颜色相同
			while (*pRawData == *(pRawData + repCount) && ((totalCompress + repCount) < len)) {
				++repCount;
			}
			...
			*pComData++ = *pRawData;
			...
			// 颜色不同
			while (*(pRawData + repCount - 1) != *(pRawData + repCount) && ((totalCompress + repCount) < len)) {
