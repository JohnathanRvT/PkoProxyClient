...
bool cfl_db::_connect(std::string& errinfo) {
	bool ret = true;
	SQLRETURN sqlret;
	char outbuf[2048];
...
bool cfl_db::connect(char* source, char* userid, char* passwd, std::string& err_info) {
	if (_connected) {
...
bool cfl_db::connect(const char* servername, const char* database, const char* userid, const char* passwd, std::string& err_info) {
	if (_connected) {
		err_info = "Already connected\n";
		return false;
	}
	...
	_openDatabase = std::string("use ") + std::string(database) + std::string(";");
	// End
...
bool cfl_rs::_get_row(std::string field_text[], int field_max_cnt, const char* param,
...
bool cfl_rs::_get_row3(std::string field_text[], int field_max_cnt, char* param,
					   char* filter, int* affect_rows /* = NULL */) {
	bool ret = false;
	char sql[SQL_MAXLEN];
	int i = 0;
...
bool cfl_rs::_get_rowOderby(std::string field_text[], int field_max_cnt, char* param,
							char* filter, int* affect_rows /* = NULL*/) {
	bool ret = false;
	char sql[SQL_MAXLEN];
	int i = 0;
...
bool cfl_rs::getalldata(const char* sql, std::vector<std::vector<std::string>>& data, unsigned short timeout) {
	bool ret = false;
		...
		do {
			...
			for (int i = 0; ((sqlret = SQLFetch(hstmt)) == SQL_SUCCESS) || (sqlret == SQL_SUCCESS_WITH_INFO); ++i) {
				...
				for (int j = 0; j < col_num; j++) {
					rowV.emplace_back((char const*)_buf[j]);
				}
...
bool cfl_rs::get_page_data(char* tablename, char* param, int pagesize, int pageindex, char* filter, char* sort, int sorttype, int& totalpage, int& totalrecord,
						   std::vector<std::vector<std::string>>& data, unsigned short timeout) {
	bool ret;
	...
	try {
		do {
			...
			if ((SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_VARCHAR, (SQLUINTEGER)strlen(tablename), 0, tablename, 0, &sql_nts) != SQL_SUCCESS) || (SQLBindParameter(hstmt, 2, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_VARCHAR, (SQLUINTEGER)strlen(param), 0, param, 0, &sql_nts) != SQL_SUCCESS) || (SQLBindParameter(hstmt, 3, SQL_PARAM_INPUT, SQL_C_SLONG, SQL_INTEGER, (SQLUINTEGER)sizeof(pagesize), 0, &pagesize, 0, &sql_nts) != SQL_SUCCESS) || (SQLBindParameter(hstmt, 4, SQL_PARAM_INPUT, SQL_C_SLONG, SQL_INTEGER, (SQLUINTEGER)sizeof(pageindex), 0, &pageindex, 0, &sql_nts) != SQL_SUCCESS) || (SQLBindParameter(hstmt, 5, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_VARCHAR, (SQLUINTEGER)strlen(filter), 0, filter, 0, &sql_nts) != SQL_SUCCESS) || (SQLBindParameter(hstmt, 6, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_VARCHAR, (SQLUINTEGER)strlen(sort), 0, sort, 0, &sql_nts) != SQL_SUCCESS) || (SQLBindParameter(hstmt, 7, SQL_PARAM_INPUT, SQL_C_SLONG, SQL_INTEGER, (SQLUINTEGER)sizeof(sorttype), 0, &sorttype, 0, &sql_nts) != SQL_SUCCESS)
...
bool friend_tbl::get_friend_dat(friend_dat* farray, int& array_num, unsigned int cha_id, bool* drop /* = NULL */) {
	if (farray == nullptr || array_num <= 0 || cha_id == 0)
		return false;
