...
void TDt::Format(char* ach, int size, int a_nDisplayFormat) {
	std::string s = Format(a_nDisplayFormat);
...
std::string TDt::Format(int a_nDisplayFormat) {
	...
	case TDt::FULL: // Monday, January 2nd, 1998
		if (!IsNull()) {
			std::string csSuffix = GetNumberSuffix(Day());
			std::string csDayName = DayName[DOW()];
			std::string csMonthName = MonthName[Month() - 1];
			s = csDayName;
			s += ", ";
			s += csMonthName;
			s += " ";
...
int TDt::DifferenceInMonths(long d1, long d2) {
	TDt dt1 = d1;
	TDt dt2 = d2;
	double dbl1 = (((double)dt1.Year() * 12) + dt1.Month());
	double dbl2 = (((double)dt2.Year() * 12) + dt2.Month());
...
std::string TDt::GetNumberSuffix(int day) {
	static std::string s;
...
bool CSQLBase::ConvertQuotes(std::string& s) {
	if (!m_bConvertQuotes)
		return false;
	...
	while (true) {
		...
		if (index = s.find("''", index) == std::string::npos) {
			break;
		}
...
std::string& CSQLDatabase::dbmsName() {
	if (!_dbmsName.empty())
		return _dbmsName;
...
SQLLEN CSQLDatabase::GetAffectedRows() {
	SQLLEN nRows = 0;
...
void CSQLDatabase::ThrowError(SQLSMALLINT handleType) {
	...
	auto* pException = new CSQLException();
