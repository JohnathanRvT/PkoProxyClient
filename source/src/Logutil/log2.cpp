...
void CLog2::Log(char const* szFormat, ...) {
	...
	if (bMsgBox) {
		...
		_vsnprintf_s(szBuf, _TRUNCATE, szFormat + 3, list);
	} else {
		//vsprintf(szBuf, szFormat, list);
		_vsnprintf_s(szBuf, _TRUNCATE, szFormat, list);
	}
	va_end(list);
...
void MakeFileNameValid(char* name) {
	if (nullptr == name || !name[0])
		return;
	auto* l_name = reinterpret_cast<unsigned char*>(name);
...
CLog2* CLog2Mgr::Add(char const* szName, char const* szExt /* = "log" */) {
	...
	try {
		...
		auto it = _LogMap.find(szFileName);
		if (it != _LogMap.end()) {
			pLog = (*it).second;
		} else {
			try {
