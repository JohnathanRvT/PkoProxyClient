...
#else
void I_Effect::Init(LPDIRECT3DDEVICEX pDev, EFFECT_TYPE eType, WPARAM wParam, LPARAM lParam)
#endif
{
	m_pDev = pDev;
	_eEffectType = eType;
...
int I_Effect::BoundingRes(CMPResManger* m_CResMagr, const char* pszParentName) {
	...
	if (_eEffectType == EFFECT_FRAMETEX) {
		for (WORD n = 0; n < m_CTexFrame.m_wTexCount; ++n) {
			...
			if (!m_CTexFrame.m_vecTexs[n]) {
				char pszMsg[64];
				_snprintf_s(pszMsg, _TRUNCATE, "特效文件[%s]加载帖图[%s]时出错",
	...
	if (!_bBillBoard && IsTobMesh(m_strModelName)) {
		if (!m_pCModel) {
			...
			if (!m_pCModel->CreateTob(m_strModelName, m_nSegments, m_rHeight, m_rRadius, m_rBotRadius)) {
				char pszMsg[64];
				_snprintf_s(pszMsg, _TRUNCATE, "特效文件[%s]缺少可变模型[%s]",
		...
		if (!m_pCModel) {
			char pszMsg[64];
			_snprintf_s(pszMsg, _TRUNCATE, "特效文件[%s]加载模型[%s]时出错",
						pszParentName, m_CTextruelist.m_vecTexName.c_str());
			LG("ERROR", "msg%s", pszMsg);
			return 2;
		}
	...
	//{
	//		//char pszn[128];
	//		//_snprintf_s(pszn, _TRUNCATE, "texture\\effect\\%s",m_CTextruelist.m_vecTexName.c_str());
...
bool I_Effect::SaveToFile(FILE* pFile) {
	...
	for (int n = 0; n < m_CTexCoordlist.m_wCoordCount; n++) {
	...
	for (int n = 0; n < m_CTextruelist.m_wTexCount; n++) {
	...
	for (int n = 0; n < m_CTexFrame.m_wTexCount; ++n) {
...
bool I_Effect::LoadFromFile(FILE* pFile, DWORD dwVersion) {
	...
	if ((strstr(pszDataName, ".dds") == nullptr) && strstr(pszDataName, ".tga") == nullptr) {
...
#else
CEffectModel::CEffectModel(LPDIRECT3DDEVICEX pDev, lwIResourceMgr* pRes)
#endif
{
	m_pDev = pDev;
	m_strName = "";
...
#else
void CEffectModel::InitDevice(LPDIRECT3DDEVICEX pDev, lwIResourceMgr* pRes)
#endif
{
	m_pDev = pDev;
	m_pRes = pRes;
...
#else
LPDIRECT3DDEVICEX CEffectModel::GetDev()
#endif
{
	return m_pDev;
}
...
//NOTE: Function never returns false! Is this expected?
...
void CEffectModel::RenderModel() {
	...
	if (!_lpVB)
		...
		if (LW_FAILED(_lwMesh->DrawSubset(0)))
			LG("error", "CEffectModel %s", (char*)m_strName.c_str());
#else

		m_pDev->SetStreamSource(0, _lpVB, 0, sizeof(SEFFECT_VERTEX));

		if (m_strName == MESH_CONE) {
			//m_pDev->SetIndices(_lpIB,0);
			//m_pDev->DrawIndexedPrimitive( D3DPT_TRIANGLELIST, 0, 0, _dwVerCount, 0,  _dwFaceCount);
		...
		m_pDev->SetIndices(_lpIB, 0);
		m_pDev->DrawIndexedPrimitive(D3DPT_TRIANGLEFAN, 0, 0, _dwVerCount, 0, _dwFaceCount);
#endif
	}
}
...
#endif
	m_pDev->SetFVFX(EFFECT_VER_FVF);
	m_pDev->GetDevice()
		->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, _dwFaceCount, m_vEffVer, sizeof(SEFFECT_VERTEX));
}
...
void CTexList::SetTextureName(const std::string& pszName) {
...
void CTexFrame::AddTexture(const std::string& pszName) {
...
#else
bool CEffectFont::CreateEffectFont(LPDIRECT3DDEVICEX pDev,
								   CMPResManger* pCResMagr, int iTexID, D3DXCOLOR dwColor, bool bUseBack, bool bmain)
#endif
{
	//if(!pDev)
	...
	char psName[64];
	if (!bmain) {
		_snprintf_s(psName, _TRUNCATE, "%s", str[_iTextureID].c_str());
	} else {
		//if(_iTextureID == 6)
		//	_snprintf_s(psName,_TRUNCATE,"%s.tga",str[_iTextureID].c_str());
		//else
		_snprintf_s(psName, _TRUNCATE, "%s2", str[_iTextureID].c_str());
	}
...
void CEffectFont::RenderEffectFontBack(D3DXMATRIX* pmat) {
	...
	m_pDev->SetTexture(0, _lpBackTex);
		...
		//m_pDev->SetStreamSource(0,_lpBackVB, 0, sizeof(SEFFECT_VERTEX));
		//D3DXMATRIX	mat;
...
CScriptFile::CScriptFile() {
	...
	InitAction("scripts/txt/CharacterAction.tx");
}
CScriptFile::~CScriptFile() {
	if (m_fp != nullptr) {
		CloseFile();
...
bool CScriptFile::OpenFileRead(const char* filename) {
	...
	if (m_fp == nullptr) {
		_snprintf_s(msg, _TRUNCATE, "CIniFile::OpenFileRead() Error (0) : Could not open file for read : <%s>\n", filename);
...
bool CScriptFile::ReadSection(char* section) {
		...
		while (bFound == false) {
			...
			if (rval == ']') {
				...
				if (_stricmp(section, "") == 0) {
					strncpy_s(section, sizeof(section), Section, _TRUNCATE);
					bFound = true;
				} else {
	...
	while (bFound == false) {
		if ((rval = (char)fgetc(m_fp)) == EOF) {
			_snprintf_s(msg, _TRUNCATE, "CIniFile::ReadSection() Error(0) : Reach the end of the file in reading section <%s>\n", section);
			OutputDebugString(msg);
			return false;
		}
...
bool CScriptFile::ReadLine(const char* name, char* value, int nSize) {
	...
	while (bFound == false) {
		if ((rval = fgetc(m_fp)) == EOF) {
			_snprintf_s(msg, _TRUNCATE, "CIniFile::ReadLine() Error(0) : Reach the end of the file in reading named <%s>\n", name);
			OutputDebugString(msg);
			return false;
		}
...
bool CScriptFile::InitAction(const char* ptcsFileName) {
	...
	while (!feof(fFile)) {
		...
		if (tcsLine[lIndex] == 0x0a || tcsLine[lIndex] == 0x0d) //回车, 换行
			continue;
		if (tcsLine[lIndex] == '/' && tcsLine[lIndex + 1] == '/') // 注释
			continue;
		if (tcsLine[lIndex] == '\t') // tab键 (在该文本中用于"角色类型的一个动作"行的开始符号)
			continue;

		m_iActualCharacterType++;

		StringGetT(tcsTemp, 1023, tcsLine, &lIndex, " ,\x09\x0a\x0d", 5);
		if (m_iMaxCharacterType < atoi(tcsTemp))
			m_iMaxCharacterType = atoi(tcsTemp);
	}
	//
	...
	while (!feof(fFile)) {
		...
		if (tcsLine[lIndex] == '/' && tcsLine[lIndex + 1] == '/') // 注释
			continue;
		if (tcsLine[lIndex] == '\t') // tab键 (在该文本中用于"角色类型的一个动作行的开始符号)
			continue;
		...
		while (!feof(fFile)) {
			...
			if (tcsLine[lIndex] == '\t') // tab键
			{
				...
				// 临时代码， 用于提示更新文件版本。
				if (_ttoi(tcsTemp) < 1) {
					MessageBox(nullptr, "请更新 .\\scripts\\CharacterAction.tx", "错误", 0);
					goto end;
				}
				//
...
void StringSkipCompartmentT(const char* in, long* in_from, const char* skip_list, const long skip_len) {
	long i; // temp variable
...
std::string& I_Effect::getEffectModelName() {
