...
MPConsole::MPConsole()
	: _pfnCmd(nullptr), _dwCursorTick(0), _bVisible(FALSE) {
...
BOOL MPConsole::OnKeyDownEvent(int nKeyCode) {
	...
	case VK_UP: {
		...
		if (_bShowCursor)
			_snprintf_s(_szInputTmp, _TRUNCATE, "]%s_", _szInput);
		else
			_snprintf_s(_szInputTmp, _TRUNCATE, "]%s", _szInput);
		break;
	}
	}
	return TRUE;
...
BOOL MPConsole::OnCharEvent(char iChar, DWORD dwParam) {
#ifdef DEBUG
	if (iChar == '`') {
		Show(!IsVisible());
		return TRUE;
...
void MPConsole::_AddText(const char* pszText, bool bCmd) {
	if (strlen(pszText) == 0)
		return;
	std::list<std::string>* pList = &_TextList;
	if (bCmd) {
		pList = &_CmdList;
	}
...
void MPConsole::FrameMove() // ¹â±êÉÁ¶¯
{
