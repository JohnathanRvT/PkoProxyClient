...
//-----------------------------------------------------------------------------
String MPDataStream::getAsString() {
	// 读取整个缓冲
...
//-----------------------------------------------------------------------------
size_t MPMemoryDataStream::tell() const {
	return m_Pos - m_Data;
}
//-----------------------------------------------------------------------------
bool MPMemoryDataStream::eof() const {
	return m_Pos >= m_End;
}
//-----------------------------------------------------------------------------
void MPMemoryDataStream::close() {
...
size_t MPFileStreamDataStream::readLine(char* buf, size_t maxCount, const String& delim) {
	...
	size_t ret = (size_t)m_pStream->gcount();
	// 3种情况
	// 1) 遇到了eof
	// 2) 超过缓存的空间
...
//-----------------------------------------------------------------------------
size_t MPFileStreamDataStream::tell() const {
	m_pStream->clear(); //万一eof已经设置，则清除
	return (size_t)m_pStream->tellg();
}
//-----------------------------------------------------------------------------
bool MPFileStreamDataStream::eof() const {
	return m_pStream->eof();
}
//-----------------------------------------------------------------------------
void MPFileStreamDataStream::close() {
	if (m_pStream) {
		m_pStream->close();
		if (m_FreeOnClose) {
			delete m_pStream;
...
//-----------------------------------------------------------------------------
size_t MPFileHandleDataStream::tell() const {
	return ftell(m_FileHandle);
}
//-----------------------------------------------------------------------------
bool MPFileHandleDataStream::eof() const {
	return feof(m_FileHandle) != 0;
}
//-----------------------------------------------------------------------------
void MPFileHandleDataStream::close() {
	fclose(m_FileHandle);
