...
bool CEffectScript::LoadParamFromName(EFF_Param* pParam, const std::string& strName) {
	...
	for (int n = 0; n < pParam->m_iModelNum; n++) {
		_snprintf_s(pszN, _TRUNCATE, "模型%d", n + 1);
	...
	for (n = 0; n < pParam->m_iPartNum; n++) {
		_snprintf_s(pszN, _TRUNCATE, "par%d", n + 1);
		if (!m_cScript.ReadLine(pszN, pszModel, 64))
			return false;
		//char* s  = pszModel;
		...
		_snprintf_s(pszN, _TRUNCATE, "dummy%d", n + 1);
...
bool CEffectScript::LoadParamFromName(Group_Param* pParam, const std::string& strName) {
	if (!m_cScript.ReadSection((char*)(LPCSTR)strName.c_str()))
		return false;
	char pszN[64];
	...
	for (int n = 0; n < pParam->nClass; n++) {
		_snprintf_s(pszN, _TRUNCATE, "元素%d", n + 1);
...
#else
CEff_ParamSet* CEff_ParamSet::_Instance = nullptr;
CGroup_ParamSet* CGroup_ParamSet::_Instance = nullptr;
//CEff_ParamSet* CEff_ParamSet::I() { return _Instance; }
#endif
/************************************************************************/
/* class CMPEffectCtrl*/
/************************************************************************/
CMPEffectCtrl::CMPEffectCtrl() {
}
CMPEffectCtrl::~CMPEffectCtrl() {
}
...
bool CMagicCtrl::Create(int iID, CMPResManger* pCResMagr) {
	...
	EFF_Param pParam;
	char pszID[10];
	_snprintf_s(pszID, _TRUNCATE, "%d", iID);
...
void CMagicCtrl::CalculateEmission(const D3DXVECTOR3* vStart, const D3DXVECTOR3* vTarget) {
	if (!vTarget || !vStart)
		return;
	_vOldPos = *vStart;
	_vOldTarget = *vTarget;
...
void CMagicCtrl::Emission(const D3DXVECTOR3* vStart, const D3DXVECTOR3* vTarget) {
	if (!vTarget || !vStart)
		return;
