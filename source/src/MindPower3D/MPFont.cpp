...
ui::UIClip* ui::UIClip::m_pClip = nullptr;
...
#else
bool CMPFont::CreateFont(LPDIRECT3DDEVICEX pd3dDevice, char szFontName[], int nSize, int nLevel, DWORD dwFlag)
#endif
{
	...
	if (!_hDc) {
		MessageBox(nullptr, "CreateCompatibleDC", "ERROR", 0);
		return false;
	}
	...
	if (nullptr == _hFont) {
		DeleteDC(_hDc);
		MessageBox(nullptr, "CreateFontIndirect", "ERROR", 0);
		return false;
	}
	...
	_hBmp = CreateDIBSection(_hDc, &bmi, DIB_RGB_COLORS,
							 (void**)&_pBits, nullptr, 0);
	if (nullptr == _hBmp || nullptr == _pBits) {
		DeleteObject(_hFont);
		DeleteDC(_hDc);
		MessageBox(nullptr, "CreateDIBSection", "ERROR", 0);
		return false;
	}
	...
	if (FAILED(lwLoadTex(&_pTex, res_mgr, &info))) {
		...
		MessageBox(nullptr, "CreateTexture", "ERROR", 0);
...
void CMPFont::BindingRes(CMPResManger* pResMagr) {
	...
	INT iw = 0;
	for (iw = 0; iw < RECTNUM; iw++) {
		*_vecValidID[iw] = iw;
	}
	FillTextToTex("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ……！？?~!@#$%^&*()_+<>1234567890和的有人我一地在要工上是中同不为这了发以经说职业验等级血日没个哈小大干操敏捷力量幸运体质多少装备看你去死杀攻击防御命回避技能点爆老公婆队长药卖买好垃圾极品滚求哪里市场出售收购几打哥姐兄弟会船港口战医生剑士狙手海客强弱航商师武器精炼合成宠物召唤岛声名什么吗啊呵傻就练男女妖红卵任务军舰她他得啦很都爱砍手脚上下身头怪妈组入盗团新带给可让晕低高也用十百千");
}
int CMPFont::GetHzLength(float fscale) const {
	float w = (float)_TextSize * fscale;
	w += ASSIZE;
	return (int)w;
}
int CMPFont::GetAscLength(float fscale) const {
	float w = (float)_TextSize * fscale;
	w += HLSIZE;
	return (int)w;
}

SIZE* CMPFont::GetTextSize(const char* szText, SIZE* pSize, float fScale) const {
	if (!szText)
		return pSize;
...
bool CMPFont::TextToTexture(char c1, char c2, float& tX, float& tY) {
	...
	auto it = std::find(_vecBuf.begin(), _vecBuf.end(), w);
	if (it == _vecBuf.end()) {
		it = std::find(_vecBuf.begin(), _vecBuf.end(), 0);
		if (it == _vecBuf.end()) {
			std::vector<WORD>::pointer pt = &_vecBuf[0];
		...
		_pTex->GetTex()->LockRect(0, &d3dlr, nullptr, 0);
...
void CMPFont::DrawTextClipOnce(char* szText, int nLen, LPRECT psrc, LPRECT pclip, D3DXCOLOR color) {
	...
	_pDev->SetVertexShader(nullptr);
#endif
	_pDev->SetFVFX(D3DFVF_FONT);

	bool bExit = false;
	sx = float(x);
	sy = float(y);
	char ch[3] = {0, 0, 0};
	FONT_VER* pVertices = nullptr;
...
bool CMPFont::DrawTextClip(char* szText, int nLen, LPRECT psrc, LPRECT pclip, D3DXCOLOR color) {
	...
	w = h = (float)(_TextSize);
	{
		...
		_pDev->SetVertexShaderConstantX(8, &color, 1);
	}
	...
	for (int n = 0; n < nLen; n++) {
...

		{
			...
			if (!GetIntersectRect(&dest, &rc, pclip)) {
				...
				const D3DXVECTOR4 v[] = {
					D3DXVECTOR4(sx, sy, 0.9f, 1),
					D3DXVECTOR4(tx1, ty2, 0, 1),
					D3DXVECTOR4(sx, sy + h, 0.9f, 1),
					D3DXVECTOR4(tx1, ty1, 0, 1),
					D3DXVECTOR4(sx + w, sy + h, 0.9f, 1),
					D3DXVECTOR4(tx2, ty1, 0, 1),
					D3DXVECTOR4(sx + w, sy, 0.9f, 1),
					D3DXVECTOR4(tx2, ty2, 0, 1)};
				_pDev->SetVertexShaderConstantX(9, &v[0], 1);
				_pDev->SetVertexShaderConstantX(10, &v[1], 1);
				_pDev->SetVertexShaderConstantX(11, &v[2], 1);
				_pDev->SetVertexShaderConstantX(12, &v[3], 1);
				_pDev->SetVertexShaderConstantX(13, &v[4], 1);
				_pDev->SetVertexShaderConstantX(14, &v[5], 1);
				_pDev->SetVertexShaderConstantX(15, &v[6], 1);
				_pDev->SetVertexShaderConstantX(16, &v[7], 1);
				//如果这个字全部在裁切区，显示
			} else {
				//裁切
				tx = (float)dest.left;
				...
				const D3DXVECTOR4 v[] = {
					D3DXVECTOR4(tx, ty, 0.9f, 1),
					D3DXVECTOR4(tu1, tv2, 0, 1),
					D3DXVECTOR4(tx, (ty + th), 0.9f, 1),
					D3DXVECTOR4(tu1, tv1, 0, 1),
					D3DXVECTOR4(tx + tw, (ty + th), 0.9f, 1),
					D3DXVECTOR4(tu2, tv1, 0, 1),
					D3DXVECTOR4(tx + tw, ty, 0.9f, 1),
					D3DXVECTOR4(tu2, tv2, 0, 1)};
				_pDev->SetVertexShaderConstantX(9, &v[0], 1);
				_pDev->SetVertexShaderConstantX(10, &v[1], 1);
				_pDev->SetVertexShaderConstantX(11, &v[2], 1);
				_pDev->SetVertexShaderConstantX(12, &v[3], 1);
...
bool CMPFont::DrawText(int iNumber, int x, int y, D3DXCOLOR color, float fScale) {
	char psz[64];
	_snprintf_s(psz, _TRUNCATE, "%d", iNumber);
...
bool CMPFont::Draw3DText(const char* szText, D3DXVECTOR3& vPos, D3DXCOLOR color, float fScale) {
	if (!szText)
		return false;
	if (!_pTex || !_pTex->IsLoadingOK())
		return false;
	...
	_pDev->SetStreamSource(0, _pVB, 0, sizeof(FONT_VER));
#endif
	...
	if (!_bUseSoft) {
		...
		_pDev->SetVertexShaderConstantX(0, &matIdentity, 4);
		_pDev->SetVertexShaderConstantX(4, _pmatViewProj, 4);
		_pDev->SetVertexShaderConstantX(8, &color, 1);
	} else {
#if (defined LW_USE_DX9)
		_pDev->SetVertexShader(nullptr);
#endif
		_pDev->SetFVFX(D3DFVF_3DFONT);
#ifdef USE_RENDER
	...
	for (int n = 0; n < nLen; n++) {
		...
		if (!_bUseSoft) {
			const D3DXVECTOR4 v[] = {
				D3DXVECTOR4(sx, sy, 0, 1),
				D3DXVECTOR4(tx1, ty2, 0, 1),
				D3DXVECTOR4(sx, sy + h, 0, 1),
				D3DXVECTOR4(tx1, ty1, 0, 1),
				D3DXVECTOR4(sx + w, sy, 0, 1),
				D3DXVECTOR4(tx2, ty2, 0, 1),
				D3DXVECTOR4(sx + w, sy + h, 0, 1),
				D3DXVECTOR4(tx2, ty1, 0, 1),
				D3DXVECTOR4(sx + w, sy, 0, 1),
				D3DXVECTOR4(tx2, ty2, 0, 1),
				D3DXVECTOR4(sx, sy + h, 0, 1),
			...
			_pDev->SetVertexShaderConstantX(15, &v[6], 1);
			_pDev->SetVertexShaderConstantX(16, &v[7], 1);
			_pDev->SetVertexShaderConstantX(17, &v[8], 1);
			_pDev->SetVertexShaderConstantX(18, &v[9], 1);
			_pDev->SetVertexShaderConstantX(19, &v[10], 1);
			_pDev->SetVertexShaderConstantX(20, &v[11], 1);
		} else {
#ifdef USE_RENDER
...
void CMPFont::FillTextToTex(const char* szText) {
skip:
	...
	for (int n = 0; n < nLen; n++) {
		...
		auto it = std::find(_vecBufFast.begin(), _vecBufFast.end(), w);
		if (it == _vecBufFast.end()) {
			it = std::find(_vecBufFast.begin(), _vecBufFast.end(), 0);
			if (it == _vecBufFast.end()) {
				return;
			}
			...
			D3DLOCKED_RECT d3dlr;
#ifdef USE_RENDER
			_pTexFast->GetTex()->LockRect(0, &d3dlr, nullptr, D3DLOCK_NOSYSLOCK);
#else
