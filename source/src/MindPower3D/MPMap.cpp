...
void MPMap::Render() {
	...
	if (_bUseLandVB) {
		if (_pLandVB == nullptr) {
			...
			HRESULT hr = g_Render.GetDevice()->CreateVertexBufferX(_dwLandVBSize,
	...
	if (_bUseVB) {
		...
		if (_pVB == nullptr) {
			...
			HRESULT hr = g_Render.GetDevice()->CreateVertexBufferX(sizeof(MPSeaTileVertex) * nVertexCnt,
	...
	if (_bShowCenterPoint) {
		_RenderFocusRect();
		const D3DXVECTOR3 v[] = {D3DXVECTOR3(_fShowCenterX, _fShowCenterY, 0.0f), D3DXVECTOR3(_fShowCenterX, _fShowCenterY, 4.0f)};
	...
	//g_Render.SetSamplerState( 0, D3DTSS_MAGFILTER, D3DTEXF_ANISOTROPIC);
	//g_Render.SetSamplerState( 0, D3DTSS_MINFILTER, D3DTEXF_ANISOTROPIC);
	//g_Render.SetSamplerState( 1, D3DTSS_MAGFILTER, D3DTEXF_ANISOTROPIC);
	//g_Render.SetSamplerState( 1, D3DTSS_MINFILTER, D3DTEXF_ANISOTROPIC);
	//g_Render.SetSamplerState(0, D3DTSS_MAXANISOTROPY, 16);
	//g_Render.SetSamplerState(1, D3DTSS_MAXANISOTROPY, 16);
...
#if (defined LW_USE_DX9)
	g_Render.SetSamplerState(1, D3DSAMP_MAGFILTER, D3DTEXF_POINT); // tile texture
	g_Render.SetSamplerState(1, D3DSAMP_MINFILTER, D3DTEXF_POINT);
	g_Render.SetSamplerState(1, D3DSAMP_MIPFILTER, D3DTEXF_POINT);

	g_Render.SetSamplerState(0, D3DSAMP_MAGFILTER, D3DTEXF_POINT); // tile texture
...
#if (defined LW_USE_DX9)
	g_Render.SetSamplerState(1, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR); // tile texture
	g_Render.SetSamplerState(1, D3DSAMP_MINFILTER, D3DTEXF_LINEAR);
	g_Render.SetSamplerState(1, D3DSAMP_MIPFILTER, D3DTEXF_LINEAR);

	g_Render.SetSamplerState(0, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR); // tile texture
	g_Render.SetSamplerState(0, D3DSAMP_MINFILTER, D3DTEXF_LINEAR);
	g_Render.SetSamplerState(0, D3DSAMP_MIPFILTER, D3DTEXF_LINEAR);
#elif (defined LW_USE_DX8)
	g_Render.SetTextureStageState(0, D3DTSS_MAGFILTER, D3DTEXF_LINEAR);
...
void MPMap::RenderSea() {
	...
	MPSeaTileVertex* pCurVertex = nullptr;
	...
	if (_bUseVB) {
		...
		//
		//#else
		HRESULT hr = _pVB->LockX(0, nVertexCnt * sizeof(MPSeaTileVertex), &pCurVertex, D3DLOCK_NOOVERWRITE);
		if (FAILED(hr))
			return;
		for (y = 0; y < nSeaCntY; y++) {
			for (x = 0; x < nSeaCntX; x++) {
...
BOOL MPMap::GetPickPos(int nPosX, int nPosY, D3DXVECTOR3& vPickPos) {
	...
	for( i = 0; i < 4; i++)
		...
		float u, v;
		if (D3DXIntersectTri(&v0, &v1, &v2, &vPickRayOrig, &vPickRayDir, &u, &v, nullptr) == TRUE) {
			vPickPos = v0 + u * (v1 - v0) + v * (v2 - v0);
			return TRUE;
		}

		if (D3DXIntersectTri(&v2, &v1, &v3, &vPickRayOrig, &vPickRayDir, &u, &v, nullptr) == TRUE) {
...
void MPMap::_CalTileNormal(int nX, int nY) {
	...
	float f1 = rng.uniform(0.0f, 0.19f);
	float f2 = rng.uniform(0.0f, 0.19f);
	float f3 = rng.uniform(0.0f, 0.09f);
...
void MPMap::_GenerateTerrainGroup(int nTileStartX, int nTileStartY) {
...

	{
		...
		for (y = 0; y < m_iRange; y++) {
			short x = 0;
			for (x = 0; x < m_iRange; x++) {
...
void MPMap::_FillVB() {
	...
	MPTileVertex* pCurVertex = nullptr;
	HRESULT hr = _pLandVB->LockX(0, nLockVertex * sizeof(MPTileVertex), &pCurVertex, 0);
	if (FAILED(hr)) {
...
void MPMap::_RenderVB(BOOL bWireframe) {
	...
	g_Render.GetDevice()->SetStreamSourceX(0, _pLandVB, 0, sizeof(MPTileVertex));
	//g_Render.SetTexture(0, NULL);
...
void MPMap::CreateSkyDoom(D3DXVECTOR3 center, float radius, char* txPath, bool hemisphere) {
	...
	if (UseShader()) {
		...
		if (m_SkyDoomVertexShaderHandle == NULL) {
			_snprintf_s(Path, _TRUNCATE, "shader\\vs_skydoom.vsh");
			if (SUCCEEDED(D3DXAssembleShaderFromFileX(Path, nullptr, nullptr, 0, &pCode, &pError))) {
				if (FAILED(dev->CreateVertexShaderX(dwDecl, (DWORD*)pCode->GetBufferPointer(), &m_SkyDoomVertexShaderHandle, 0))) {
					m_SkyDoomVertexShaderHandle = 0;
					MessageBox(nullptr, "Failed To Create Vertex Shader", "D3DXAssembleShaderFromFile For Vertex Shader Failed", 0);
				}
			} else {
				m_SkyDoomVertexShaderHandle = 0;
				if (pError != nullptr) {
					const char* str = (const char*)pError->GetBufferPointer();
					MessageBox(nullptr, str, "D3DXAssembleShaderFromFile For Vertex Shader Failed", 0);
				} else {
					MessageBox(nullptr, "File Of Vertex Shader Not Exist", "D3DXAssembleShaderFromFile For Vertex Shader Failed", 0);
				}
			}
			if (pError) {
				pError->Release();
		...
		if (m_SkyDoomPixelShaderHandle == NULL) {
			_snprintf_s(Path, _TRUNCATE, "shader\\ps_skydoom.psh");
			if (SUCCEEDED(D3DXAssembleShaderFromFileX(Path, nullptr, nullptr, 0, &pCode, &pError))) {
				if (FAILED(dev->CreatePixelShaderX((DWORD*)pCode->GetBufferPointer(), &m_SkyDoomPixelShaderHandle))) {
					m_SkyDoomPixelShaderHandle = 0;
					MessageBox(nullptr, "Failed To Create Pixel Shader", "D3DXAssembleShaderFromFile For Pixel Shader Failed", 0);
				}
			} else {
				m_SkyDoomPixelShaderHandle = 0;
				if (pError != nullptr) {
					const char* str = (const char*)pError->GetBufferPointer();
					MessageBox(nullptr, str, "D3DXAssembleShaderFromFile For Pixel Shader Failed", 0);
				} else {
					MessageBox(nullptr, "File Of Pixel Shader Not Exist", "D3DXAssembleShaderFromFile For Pixel Shader Failed", 0);
				}
			}
			if (pError) {
				pError->Release();
		...
		dev->SetVertexShaderConstantX(0, &mat, 4);
		const auto v = D3DXVECTOR4(0, (m_txMoveSpeed -= 1.0f) / 10000 * SKYDOOM_TEXTURE_MOVESPEED, 0, 0);
		dev->SetVertexShaderConstantX(4, &v, 1); //add by jze pass current frame number to vertex shader
