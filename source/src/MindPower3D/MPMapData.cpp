...
void MPMap::SetSectionTileData(int nX, int nY, BYTE btTexNo) {
	...
	for (int y = 0; y < _nSectionHeight; y++) {
		for (int x = 0; x < _nSectionWidth; x++) {
			...
			pTile->fHeight = rng.uniform(0.006f, 0.296f);
...
MPActiveMapSection* MPMap::LoadSectionData(int nSectionX, int nSectionY) {
	auto* pSection = new MPActiveMapSection;
	pSection->Init();
...
void MPMap::ClearSectionData(int nSectionX, int nSectionY) {
	...
	if (pSection) // 此Section还未读入内存
	{
		...
		_ActiveSectionArray[nSectionY][nSectionX] = nullptr;
...
void MPMap::DynamicLoading(DWORD dwTimeParam) {
	...
	int nCenterSectionX = (int)(_fShowCenterX / _nSectionWidth);
	int nCenterSectionY = (int)(_fShowCenterY / _nSectionHeight);
	...
	std::list<MPActiveMapSection*> _NewList;
	...
	if ((int)(_ActiveSectionList.size()) >= _nSectionBufferSize) {
		...
		int n = 0;
		BOOL bDelFail = FALSE;
		for (std::list<MPActiveMapSection*>::iterator it = _ActiveSectionList.begin(); it != _ActiveSectionList.end(); it++) {
		...
		for (int i = 0; i < n; i++) // 清除不需要的Section Tile Data
		{
			...
			_ActiveSectionArray[DelSectionList[i]->nY][DelSectionList[i]->nX] = nullptr;
	...
	// 统一执行MapNotice
	for (std::list<MPActiveMapSection*>::iterator it = _NewList.begin(); it != _NewList.end(); it++) {
	...
	m_pBlock->GetBlockByRange((int)_fShowCenterX, (int)_fShowCenterY, m_iRange);
}
...
void MPMap::ClearAllSection(BOOL bSaveFlag) {
	for (std::list<MPActiveMapSection*>::iterator it = _ActiveSectionList.begin(); it != _ActiveSectionList.end(); it++) {
		...
		_ActiveSectionArray[pSection->nY][pSection->nX] = nullptr;
		SAFE_DELETE(pSection->pTileData);
		SAFE_DELETE(pSection);
	}
