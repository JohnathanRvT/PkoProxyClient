...
void CEffectCortrol::FillModelUV(CEffectModel* pCModel) {
	WORD i = 0;
	for (i = 0; i < pCModel->GetVerCount(); i++) {
		pCModel->GetDev()->SetVertexShaderConstantX(9 + i, m_vecCurCoord[i], 1);
	}
}
void CEffectCortrol::FillTextureUV(CEffectModel* pCModel) {
	WORD i = 0;
	for (i = 0; i < pCModel->GetVerCount(); i++) {
		pCModel->GetDev()->SetVertexShaderConstantX(9 + i, m_lpCurTex[i], 1);
	}
}
void CEffectCortrol::FillDefaultUV(CEffectModel* pCModel, TEXCOORD& coord) {
	WORD i = 0;
	for (i = 0; i < pCModel->GetVerCount(); i++) {
		pCModel->GetDev()->SetVertexShaderConstantX(9 + i, coord[i], 1);
	}
}
...
void CMPModelEff::ReleaseAll() {
	...
	m_vecEffect.clear();
	m_vecCortrol.clear();
	m_pCurCortrol = nullptr;

	m_iTimes = 0;
	m_pfDailTime = nullptr;
	...
	m_iIdxTech = 0;
	m_pCEffectFile = nullptr;
	m_pMatViewProj = nullptr;
...
void CMPModelEff::Begin() {
	...
	m_vecEffect[0]->m_pDev->SetVertexShaderConstantX(4, m_pMatViewProj, 4);
}
...
void CMPModelEff::RenderAccel(float& fTime) {
	...
	for (n = 0; n < m_iEffNum; n++) {
		...
		m_pCEffect->m_pDev->SetVertexShaderConstantX(0, &m_pCurCortrol->m_SMatResult, 4);
		m_pCEffect->m_pDev->SetVertexShaderConstantX(8, &m_pCurCortrol->m_dwCurColor, 1);
	...
	for (n = 0; n < m_iEffNum; n++) {
		...
		if (m_pCEffect->IsItem()) {
			...
			if (m_pCEffect->m_pCModel) {
				...
				//char szData[128];
				//_snprintf_s( szData, _TRUNCATE, "No model file. name = %s.", m_pCEffect->m_strModelName.c_str() );
		...
		m_pCEffect->m_pDev->SetVertexShaderConstantX(0, &m_pCurCortrol->m_SMatResult, 4);
		m_pCEffect->m_pDev->SetVertexShaderConstantX(8, &m_pCurCortrol->m_dwCurColor, 1);
		m_pCEffect->m_pDev->SetVertexShaderConstantX(4, m_pMatViewProj, 4);
...
#endif
		m_pCEffect->m_pDev->SetFVFX(EFFECT_VER_FVF);
		if (m_pCEffect->IsBillBoard()) {
			if (!m_bUseZ)
				m_pCEffect->m_pDev->SetRenderState(D3DRS_ZENABLE, FALSE);
...
void CMPModelEff::ShowCurFrame(int iCurSubEff, int iCurFrame) {
	...
	//for( n = 0; n < m_iEffNum; n++)
	{
		...
		//	WORD i = 0;
		//for( i = 0; i < m_pCEffect->m_pCModel->GetVerCount(); ++i)
		//	{
...
void CMPModelEff::ShowTempFrame(int iCurSubEff,
								...
								D3DXCOLOR& pColor, TEXCOORD& vecCoord, LPDIRECT3DTEXTUREX lpTex) {
	m_pCEffect = m_vecEffect[iCurSubEff];
...
bool CMPStrip::LoadFromFile(FILE* t_pFile, DWORD dwVersion) {
	...
	if ((strstr(pszName, ".dds") == nullptr) && strstr(pszName, ".tga") == nullptr) {
		_strTexName = pszName;
	} else {
