...
CChaModel* CMPPartCtrl::NewModel(const std::string& strID, CMPResManger* pResMagr) {
	m_iModelNum++;
	if (m_iModelNum > 1) {
		m_iModelNum = 1;
		return nullptr;
...
CMPStrip* CMPPartCtrl::NewStrip(const std::string& strTex, CMPResManger* pResMagr) {
	SAFE_DELETE_ARRAY(m_pcStrip);
...
void CMPPartCtrl::CopyPartCtrl(CMPPartCtrl* pPart) {
	...
	for (int n = 0; n < m_iModelNum; n++) {
		...
		_snprintf_s(psID, _TRUNCATE, "%d", pModel->GetID());
...
bool CMPPartCtrl::LoadFromFile(const char* pszName) {
	FILE* t_pFile;
	...
	if (m_iStripNum > 0) {
		for (n = 0; n < m_iStripNum; ++n) {
			...
			if (id >= 0) {
				...
				if (it == vecTex.end()) {
					vecTex.push_back(id);
					_snprintf_s(pszPath, _TRUNCATE, "texture/effect/%s", strName.c_str());
					_snprintf_s(pszNewPath, _TRUNCATE, "effect/new/texture/%s", strName.c_str());
...
void CMPPartCtrl::GetHitRes(CMPResManger* pResMagr, std::vector<std::string>& vecPar) {
	if (m_iStripNum > 0) {
		return;
	} else if (m_iPartNum > 0) {
		std::string strname;
...
bool CChaModel::LoadScript(const std::string& strModel) {
	...
	short i = 0;
	for (i = 0; i < 5; i++) {
		_snprintf_s(str, _TRUNCATE, "皮肤%d", i + 1);
...
void CChaModel::LoadFromFile(FILE* file) {
	...
	char psID[32];
	_snprintf_s(psID, _TRUNCATE, "%d", _iID);
	LoadScript(psID);
...
bool CMPLink::Create(MPCharacter* pChaMain, int iDummy1, MPCharacter* pChaTag, int iDummy2,
	...
	for (n = 0; n < iTexNum; n++) {
		_pTex[n] = nullptr;
		_snprintf_s(pszName, _TRUNCATE, "%s%d", pszTex, n);
		strName = pszName;
