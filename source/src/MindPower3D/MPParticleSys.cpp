			...
			if (_fCurPlayTime >= _fDelayTime) {
				...
				if (!IsCreatePart()) {
					...
					case PARTTICLE_BLAST:
						_CreateBlast(this, nullptr);
						break;
					case PARTTICLE_BLAST2:
						_CreateBlast2(this, nullptr);
						break;
					case PARTTICLE_BLAST3:
						_CreateBlast3(this, nullptr);
						break;
					}
				}
				if (_fCurPlayTime >= _fPlayTime) {
...
bool CMPPartSys::Create(int iType, const std::string& strPartName, int iNumPart,
						const std::string& strModelName, const std::string& strTexName,
	...
	if (id < 0) {
		MessageBox(nullptr, _strTexName.c_str(), "ERROR", 0);
		return false;
	}
	...
	if (id < 0) {
		...
		if (id < 0) {
			MessageBox(nullptr, _strModelName.c_str(), "ERROR", 0);
			return false;
		} else {
			//#ifdef MUD_PART
			//			if(IsPartArray())
...
void CMPPartSys::BindingRes(CMPResManger* pCResMagr) {
	...
	if (id < 0) {
		id = pCResMagr->GetEffectID(_strModelName);
		if (id < 0) {
			MessageBox(nullptr, _strModelName.c_str(), "找不到粒子模型ERROR", 0);
			return;
		} else {
			if (_bMediay) {
	...
	if (_pCModel) {
		...
		if (_pCModel->IsItem()) {
			//char pszn[128];
			//_snprintf_s(pszn, _TRUNCATE, "texture\\effect\\%s",_strTexName.c_str());
...
void CMPPartSys::SetTextureName(const std::string& strTexName, CMPResManger* pCResMagr) {
	_strTexName = strTexName;
	int id;
	id = pCResMagr->GetTextureID(_strTexName);
	if (id < 0) {
...
void CMPPartSys::SetModelName(const std::string& strModelName, CMPResManger* pCResMagr) {
	_strModelName = strModelName;
	//
	//if(_bMediay)
	{
...
void CMPPartSys::RenderSoft() {
	...
	if (_bFontCom) {
		...
		_pCEffectFile->GetDev()->SetVertexShader(nullptr);
#endif
		_pCEffectFile->GetDev()->SetFVFX(EFFECT_VER_FVF);
		...
		{
			WORD n = 0;
			for (n = 0; n < (WORD)_vecText.size(); ++n) {
				if (_vecParticle[n]->m_bLive) {
...
void CMPPartSys::Reset(bool bLife) {
	...
	for (n = 0; n < _iParNum; ++n) {
		...
		_vecParticle[n]->Reset(v); //v can be reused here (it's const after all)
	}
	if (_CPPart) {
		if (_bMediay)
			for (int t = 0; t < _iParNum; t++)
...
void _FrameMoveWind(CMPPartSys* pPart, DWORD dwDailTime) {
	...
	////在每一帧
	//int m = 0;
	//for( m = 0; m < pPart->_wFrameCount - 1; ++m)
	//{
	//	//计算当前帧和下一帧的size 的差
		...
		if (!pPart->m_strHitEff.empty()) {
			if (pPart->m_pMap) {
				...
				if (pPart->_vPos.z <= 0.1f || pPart->_vPos.z > 50.0f) {
					pPart->Stop();
					pPart->m_pCResMagr->SendResMessage(pPart->m_strHitEff, *pPart->_pcPath->GetEnd(), nullptr);
				}
			}
		}
	}
...
bool CMPPartSys::LoadFromFile(FILE* t_pFile, DWORD dwVersion) {
	...
	std::string sFileName = t_pszName;
...
void CopyPartSys(CMPPartSys* part1, CMPPartSys* part2) {
	...
	part1->_lpCurTex = nullptr; //part2->_lpCurTex;

	part1->_strModelName = part2->_strModelName;
	part1->_pCModel = nullptr; //part2->_pCModel;
	...
	int n = 0;
	for (n = 0; n < part1->_wFrameCount; n++) {
