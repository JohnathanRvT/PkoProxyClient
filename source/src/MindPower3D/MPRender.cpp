...
BOOL MPRender::Init(HWND hWnd, int nScrWidth, int nScrHeight, int nColorBit, BOOL bFullScreen) {
	...
	*/
...
#if (defined LW_USE_DX8) //Not defined on DX9 and above
	...
	if (LW_FAILED(ret = lwInitMeshLibSystem(&sys, &sys_graphics, &d3dcp, &_d3dCPAdjustInfo))) {
		const auto GetErrorMessage = [ret]() {
			switch (ret) {
			case INIT_ERR_CREATE_D3D:
				return "Create DirectX error";
			case INIT_ERR_CREATE_DEVICE:
				return "Create DirectX Device error";
			case INIT_ERR_DX_VERSION:
				return "Invalid Installed DirectX Version";
			default:
				return "Unknown Internal Error";
			}
		};

		LG("init", "msg%s", GetErrorMessage());
		if (ret == INIT_ERR_CREATE_DEVICE) {
			LG("init", RES_STRING(CL_MPRENDER_CPP_00001));
		}
	...
	{ //NOTE: ViM related
		...
		char Result[256] = "";
		GetModuleFileNameA(nullptr, Result, 256);
...
void MPRender::GetRay(int nScreenX, int nScreenY, D3DXVECTOR3& vRayStart, D3DXVECTOR3& vRayEnd) {
	...
	D3DXMatrixInverse(&matInverse, nullptr, &_matViewProj);
...
BOOL MPRender::BeginRender(bool clear) //vim
{
	...
	if (clear) //vim
	{
		if (FAILED(_pD3DDevice->Clear(0L, nullptr, _dwClearFlag, _dwBackgroundColor, 1.0f, 0L))) {
...
void MPRender::EndRender(bool present) //vim
{
	...
	if (present) {
		...
		if (_bEnableCaptureAVI) // 连续截屏
		{
			char szFileName[64];
			Util_MakeDir("screenshot/");
			_snprintf_s(szFileName, _TRUNCATE, "screenshot/avi%06d.bmp", g_nAviCnt);
...
void MPRender::RenderDebugInfo() {
	for (int i = 0; i < MAX_INFO_TYPE; i++) {
		if (_PrintInfoMask[i]) {
			for (const auto& [index, str_info] : _InfoIdx[i]) {
				const int nPosY = index / 2000;
				const int nPosX = index % 2000;
				_pFont->DrawText((char*)str_info.c_str(), nPosX, nPosY, 0xffffffff);
			}
		}
	}
}
...
void MPRender::Print(int nInfoType, int x, int y, const char* szFormat, ...) {
	...
	vsnprintf_s(_szInfo, _TRUNCATE, szFormat, list);
	va_end(list);
...
void MPRender::RenderTextureRect(int nX, int nY, MPTexRect* pRect) {
	...
	RECT* pTexRect = nullptr;
	RECT TexRect = {pRect->nTexSX, pRect->nTexSY, pRect->nTexSX + pRect->nTexW, pRect->nTexSY + pRect->nTexH};
	if (pRect->nTexW != 0)
		pTexRect = &TexRect;
	// _p2DSprite->Draw(nullptr, nullptr, nullptr , nullptr, 0, &vecDest, pRect->dwColor);
#if (defined LW_USE_DX9)
	_p2DSprite->Begin(D3DXSPRITE_ALPHABLEND);
	D3DXMATRIX m;
	D3DXMatrixTransformation2D(&m, nullptr, 0, &vecScale, nullptr, 0, &vecDest);
	_p2DSprite->SetTransform(&m);
	switch (_p2DSprite->Draw(pTexture, pTexRect, nullptr, nullptr, pRect->dwColor)) {
...
#elif (defined LW_USE_DX8)
	// _p2DSprite->Draw(nullptr, nullptr, nullptr , nullptr, 0, &vecDest, pRect->dwColor);
	_p2DSprite->Draw(pTexture, pTexRect, &vecScale, nullptr, 0, &vecDest, pRect->dwColor);
#endif
}
...
#endif
	SetFVFX(D3DFVF_XYZ | D3DFVF_DIFFUSE);
...
///////////////////////////////////////////////
bool SurfaceToBMP(LPDIRECT3DSURFACEX pSurface, const char* strName, int nFlag) {
	...
	case D3DFMT_X8R8G8B8:
		...
		//UNKNOWN: Commented this to allow x64 compilation, yet nothing from game uses this?
...
//		pbtTmp -= sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER);
	...
	case D3DFMT_X1R5G5B5:
		if (bSaveDirect) {
			...
			FILE* fp;
			strncat_s((char*)strName, sizeof(strName), ".t16", _TRUNCATE);
...
void MPRender::CaptureScreen(char* strFilename) {
	...
	_pD3DDevice->GetBackBufferX(NULL, 0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer);
	...
	LPDIRECT3DSURFACEX pCapSurface;
	_pD3DDevice->CreateOffscreenPlainSurfaceX(sDesc.Width, sDesc.Height, sDesc.Format, D3DPOOL_SCRATCH, &pCapSurface, nullptr);
	_pD3DDevice->UpdateSurfaceX(pBackBuffer, nullptr, 0, pCapSurface, &p);
