...
#else
bool CMPResManger::InitRes(LPDIRECT3DDEVICEX pDev, D3DXMATRIX* pmat, D3DXMATRIX* pMatviewproj)
#endif
{
	m_pDev = pDev;

	LPDIRECT3DSURFACEX pBackBuffer;
#ifdef USE_RENDER
	m_pDev->GetDevice()->GetBackBufferX(NULL, 0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer);
#else
	...
	//_iFontBkHeight= m_pDev->GetScrHeight()/2;//m_d3dBackBuffer.Height/2;
	RECT rc_client;
	m_pDev->GetInterfaceMgr()->dev_obj->GetWindowRect(nullptr, &rc_client);
	...
	lwGetActiveISystem()->GetInterface((LW_VOID**)&path_info, LW_GUID_PATHINFO);
	if (!_CEffectFile.LoadEffectFromFile((path_info->GetPath(PATH_TYPE_SHADER) + "\\eff.fx").c_str())) {
		MessageBox(nullptr, (path_info->GetPath(PATH_TYPE_SHADER) + "\\eff.fx").c_str(), "ERROR", 0);
		return false;
	}
...
int CMPResManger::GetTextureID(const std::string& sName) {
#if RESOURCE_SCRIPT == 1
	// 资源脚本定义时，自动生成文件用
	...
	TEXTURE_MAP::const_iterator it = _mapTexture.find(sName);
	if (it != _mapTexture.cend()) {
		return (*it).second;
	}
...
//-----------------------------------------------------------------------------
LPDIRECT3DTEXTUREX CMPResManger::GetTextureByID(int iID) {
	auto pTex = _vecTexList[iID]; //NOTE: Could it be out of range? Crashes if it is
	return pTex ? pTex->GetTex() : nullptr;
}
//-----------------------------------------------------------------------------
lwITex* CMPResManger::GetTextureByIDlw(int iID) {
	...
	if (pTex) {
		...
		std::string t_pszFile = path_info->GetPath(PATH_TYPE_TEXTURE_EFFECT) + _vecTexName[iID];

#ifdef USE_DDS_FILE_EFFECT
		t_pszFile.append(".dds");
#else
		t_pszFile.append(".tga");
#endif
		lwITex* tex;
		if (LW_FAILED(lwLoadTex(&tex, m_pSysGraphics->GetResourceMgr(), t_pszFile.c_str(), nullptr, D3DFMT_A4R4G4B4))) {
			char szMsg[64];
			_snprintf_s(szMsg, _TRUNCATE, "Error loading effect texture [id=%d]", iID);
			LG("ERROR", "msg%s", szMsg);
			return nullptr;
		}
		//#endif
...
//-----------------------------------------------------------------------------
lwITex* CMPResManger::GetTextureByNamelw(const std::string& sName) {
	const int iTexID = GetTextureID(sName);
	return iTexID == -1 ? nullptr : GetTextureByIDlw(iTexID);
}
//-----------------------------------------------------------------------------
int CMPResManger::GetMeshID(const std::string& sName) {
#if RESOURCE_SCRIPT == 1
	...
	MESH_MAP::const_iterator it = _mapMesh.find(sName);
	if (it != _mapMesh.cend()) {
		return (*it).second;
	}
	...
	// Failure
	char szMsg[256];
	_snprintf_s(szMsg, _TRUNCATE, "Missing effect model [%s] (file does not exist or effect resource configuration file error)",
				sName.c_str());
	LG("ERROR", "msg%s", szMsg);
	return -1;
}
...
CEffectModel* CMPResManger::GetMeshByID(int iID) {
	...
	CEffectModel* pRetMesh(nullptr);

	//NOTE: interval [ _vecMeshList[0], _vecMeshList[7] ] is default meshes
	// See #define MESH_TRI "Triangle" in I_Effect.h
	if (iID >= 7) {
		if (!_vecMeshList[iID]) { // 第一次需要模型对象，创建
			...
			m_pSys->GetInterface((LW_VOID**)&path_info, LW_GUID_PATHINFO);
			char szOldPath[260];
			strncpy_s(szOldPath, path_info->GetPath(PATH_TYPE_MODEL_ITEM).c_str(), _TRUNCATE);
			path_info->SetPath(PATH_TYPE_MODEL_ITEM, "model\\effect\\");
			if (!_vecMeshList[iID]->LoadModel((char*)_vecMeshName[iID].c_str())) {
				SAFE_DELETE(_vecMeshList[iID]);
				path_info->SetPath(PATH_TYPE_MODEL_ITEM, szOldPath);

				char szMsg[64];
				_snprintf_s(szMsg, _TRUNCATE, "加载特效模型[id=%d]失败", iID);
				LG("ERROR", "msg%s", szMsg);
				return nullptr;
			}
				...
				for (; n < MAXMESH_COUNT; ++n) {
					...
					if (_vecMeshList[n]->m_iID != iID) { // 如果这模型与需要的模型不同则拷贝一份
						if (!_vecMeshList[n]->Copy(*_vecMeshList[iID])) {
							...
							char szMsg[64];
							_snprintf_s(szMsg, _TRUNCATE, "缺少特效模型[id=%d]拷贝错误", iID);
							LG("ERROR", "msg%s", szMsg);
							return nullptr;
						}
					}
...
//-----------------------------------------------------------------------------
CEffectModel* CMPResManger::GetMeshByName(const std::string& sName) {
	const int iMeshID = GetMeshID(sName);
	return iMeshID == -1 ? nullptr : GetMeshByID(iMeshID);
}
//-----------------------------------------------------------------------------
...
int CMPResManger::GetEffectID(const std::string& pszName) {
#if RESOURCE_SCRIPT == 1
	...
	EFFECT_MAP::const_iterator pos = _mapEffect.find(pszName);
	if (pos != _mapEffect.cend()) {
		return (*pos).second;
	}
...
std::vector<I_Effect>& CMPResManger::GetEffectByID(int iID) {
	...
	if (n <= 0) {
		lwIPathInfo* path_info;
		m_pSys->GetInterface((LW_VOID**)&path_info, LW_GUID_PATHINFO);
		std::string t_pszFile = path_info->GetPath(PATH_TYPE_EFFECT) + _vecEffectName[iID];

		//__asm int 3;
		if (!LoadEffectFromFile(iID, t_pszFile.c_str()))
...
bool CMPResManger::LoadTotalTexture() {
#if RESOURCE_SCRIPT == 0 || RESOURCE_SCRIPT == 1
	{
		...
		t_Path = path_info->GetPath(PATH_TYPE_TEXTURE_EFFECT);
#ifdef USE_DDS_FILE_EFFECT
		t_Path.append("*.dds");
#else
		t_Path.append("*.tga");
		...
		//LPDIRECT3DTEXTUREX  t_lptex = NULL;
...
void CMPResManger::LoadTotalData() {
	...
	// 装载动作
	t_Path = path_info->GetPath(PATH_TYPE_ANIMATION) + "*.lab";
...
void CMPResManger::LoadTotalRes() {
	...
	// 角色模型
	t_Path = path_info->GetPath(PATH_TYPE_MODEL_CHARACTER) + "*.lgo";
...
bool CMPResManger::LoadTotalMesh() {
...
#if USE_RESOURCE_SCRIPT == 0 || USE_RESOURCE_SCRIPT == 1
	{
		...
		t_Path = path_info->GetPath(PATH_TYPE_MODEL_EFFECT) + "*.lgo";

		if ((t_hFind = FindFirstFile(t_Path.c_str(), &t_sfd)) == INVALID_HANDLE_VALUE)
			return true;
		std::string sFileName;

		std::string szOldPath = path_info->GetPath(PATH_TYPE_MODEL_ITEM);
		path_info->SetPath(PATH_TYPE_MODEL_ITEM, "model\\effect\\");
		...
		// 道具
		t_Path = path_info->GetPath(PATH_TYPE_MODEL_ITEM) + "*.lgo";
...
I_Effect* CMPResManger::AddEffectToMgr(const std::string& strName) {
	_iEffectNum++;
...
bool CMPResManger::LoadTotalEffect() {
#if RESOURCE_SCRIPT == 0 || RESOURCE_SCRIPT == 1
	{
		...
		t_Path = path_info->GetPath(PATH_TYPE_EFFECT) + "*.eff";
		...
		do {
			if (!(t_sfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
				...
				_snprintf_s(t_pszFile, _TRUNCATE, "%s\\%s", path_info->GetPath(PATH_TYPE_EFFECT).c_str(), t_sfd.cFileName);
				...
				if (!LoadEffectFromFile(_iEffectNum, t_pszFile)) {
					char szData[1024];
					_snprintf_s(szData, _TRUNCATE, "装载特效文件(%s)失败！", t_pszFile);
					MessageBox(nullptr, szData, "Error", MB_OK);
					//return false;
				}
...
bool CMPResManger::LoadTotalVShader(lwISysGraphics* sys_graphics) {
	...
	std::string path = path_info->GetPath(PATH_TYPE_SHADER);
...
#if (defined LW_USE_DX9)
	D3DVERTEXELEMENT9 ve_dec_effect[] =
		{
	...
	int decl_num = sizeof(shader_type) / sizeof(shader_type[0]);
	...
	for (int i = 0; i < decl_num; i++) {
		...
		if (LW_FAILED(shader_mgr->RegisterVertexDeclaration(shader_type[i], &decl_tab[i][1])))
			goto __ret;
...
#endif
			MessageBox(nullptr, shader_file[i].c_str(), "ERROR", 0);
...
#if (defined LW_USE_DX9)
			_vecVShaderDeclaration.resize(_iVShaderNum);
#endif
			_vecVShader[_iVShaderNum - 1] = 0;
#if (defined LW_USE_DX9)
			_vecVShaderDeclaration[_iVShaderNum - 1] = 0;
#endif

#if (defined LW_USE_DX9)
			shader_mgr->QueryVertexShader(&_vecVShader[_iVShaderNum - 1], shader_type[i]);
			shader_mgr->QueryVertexDeclaration(&_vecVShaderDeclaration[_iVShaderNum - 1], shader_type[i]);
		} else {
			if (i == 4) {
				shader_mgr->QueryVertexShader(&_dwFontVS, shader_type[i]);
				shader_mgr->QueryVertexDeclaration(&_dwFontVSD, shader_type[i]);
			}
			if (i == 5) {
				shader_mgr->QueryVertexShader(&_dwShadeMapVS, shader_type[i]);
				shader_mgr->QueryVertexDeclaration(&_dwShadeMapVSD, shader_type[i]);
			}
			if (i == 6) {
				shader_mgr->QueryVertexShader(&_dwMinimapVS, shader_type[i]);
				shader_mgr->QueryVertexDeclaration(&_dwMinimapVSD, shader_type[i]);
			}
		}
#elif (defined LW_USE_DX8)
			shader_mgr->QueryVertexShader(&_vecVShader[_iVShaderNum - 1], shader_type[i]);
...
int CMPResManger::GetEffPathID(const std::string& pszName) {
#if RESOURCE_SCRIPT == 1
...
bool CMPResManger::LoadTotalPath() {
#if RESOURCE_SCRIPT == 0 || RESOURCE_SCRIPT == 1
	{
		...
		t_Path = path_info->GetPath(PATH_TYPE_EFFECT) + "*.csf";

		std::string t_pszFile;

		if ((t_hFind = FindFirstFile(t_Path.c_str(), &t_sfd)) == INVALID_HANDLE_VALUE)
			return false;
		do {
			if (!(t_sfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
				t_pszFile = path_info->GetPath(PATH_TYPE_EFFECT) + t_sfd.cFileName;
				_vecPathName.emplace_back(t_sfd.cFileName);

				_iPathNum++;
				_vecPath.resize(_iPathNum);
				_vecPath[_iPathNum - 1].LoadPathFromFile(t_pszFile.c_str());
			}
...
int CMPResManger::GetPartCtrlID(const std::string& pszName) {
...
CMPPartCtrl* CMPResManger::GetPartCtrlByID(int iID) {
	...
	if ((*_vecPartCtrl[iID]) == NULL) {
		lwIPathInfo* path_info;
		m_pSys->GetInterface((LW_VOID**)&path_info, LW_GUID_PATHINFO);
		std::string t_Path = path_info->GetPath(PATH_TYPE_EFFECT) + _vecPartName[iID];

		(*_vecPartCtrl[iID]) = new CMPPartCtrl;
		if (!(*_vecPartCtrl[iID])->LoadFromFile(t_Path.c_str())) {
...
void CMPResManger::LoadTotalPartCtrl() {
	...
	//搜索文件目录加载
	{
		...
		t_Path = path_info->GetPath(PATH_TYPE_EFFECT) + "*.par";
...
#endif
				t_FilePath = path_info->GetPath(PATH_TYPE_EFFECT) + _vecPartName[_iPartCtrlNum - 1];

				(*_vecPartCtrl[_iPartCtrlNum - 1]) = new CMPPartCtrl;
				if (!(*_vecPartCtrl[_iPartCtrlNum - 1])->LoadFromFile(t_FilePath.c_str())) {
...
CMPPartCtrl* CMPResManger::NewPartCtrl(const std::string& strName) {
	//#ifndef USE_GAME
	_iPartCtrlNum++;
...
bool CMPResManger::DeleteTobMesh(CEffectModel& rEffectModel) {
	auto iter = std::find(_lstTobMeshs.begin(), _lstTobMeshs.end(), &rEffectModel);
...
void CMPResManger::RestoreEffect() {
	...
	m_pDev->GetInterfaceMgr()->dev_obj->SetTextureForced(0, nullptr);
	m_pDev->GetInterfaceMgr()->dev_obj->SetTextureForced(1, nullptr);
...
void CMPResManger::SetErrorReportFunc(ERR_FUNC pFunc) {
	::SetErrorReportFunc(pFunc);
}

void CMPResManger::SetErrorLogFunc(ERR_FUNC pFunc) {
