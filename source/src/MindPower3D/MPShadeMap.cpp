...
void CMPShadeMap::setTextureName(std::string& strName) {
	if ((strstr(strName.c_str(), ".dds") == nullptr) && strstr(strName.c_str(), ".tga") == nullptr) {
		_strTexName = strName;
	} else {
...
void CMPShadeMap::BoundingRes(CMPResManger* m_CResMagr) {
	...
	if (t_iID == -1) {
		MessageBox(nullptr, _strTexName.c_str(), "shade纹理不存在", MB_OK);
		_lpCurTex = nullptr;
	} else {
#ifdef USE_MGR
	...
	BOL = _pModel->GetDev()->GetSoftwareVertexProcessing();
#endif
	if (BOL == 1) {
		m_bUseSoft = true;
		//LG("debug","msgUsesoft");
...
bool CMPShadeMap::SetGridNum(int iNum) {
	...
	int nIndex = 0;
	int nY = 0;
	for (nY = 0; nY < _iGridCrossNum; nY++) {
		int nX = 0;
		for (nX = 0; nX < _iGridCrossNum; nX++) {
...
void CMPShadeMap::setFrameTexture(std::string& strTexName, CMPResManger* pCResMagr) {
	_strTexName = strTexName;
	int t_iID;

	t_iID = pCResMagr->GetTextureID(_strTexName);
	if (t_iID == -1) {
		MessageBox(nullptr, _strTexName.c_str(), "纹理不存在", MB_OK);
		_lpCurTex = nullptr;
	} else {
#ifdef USE_MGR
		//LPDIRECT3DTEXTUREX tex=  pCResMagr->GetTextureByID(t_iID);
...
void CMPShadeMap::MoveTo(D3DXVECTOR3 SVerPos, MPMap* pMap, float fAngle) {
	...
	//MPTile *TileList;
	int y = 0;
	for (y = 0; y < _iGridCrossNum + 1; y++) {
		int x = 0;
		for (x = 0; x < _iGridCrossNum + 1; x++) {
...
void CMPShadeMap::RenderVS() {
	...
	_pModel->m_pDev->SetVertexShaderConstantX(0, &t_mat, 4);
	_pModel->m_pDev->SetVertexShaderConstantX(4, _pMatViewProj, 4);
	_pModel->m_pDev->SetVertexShaderConstantX(8, &_dwColor, 1);
	...
	for (n = 0; n < _iVerNum; n++) {
		tv.x = _SShadeUV[n].x;
		tv.y = _SShadeUV[n].y;
		_pModel->m_pDev->SetVertexShaderConstantX(nIndex++, &/*_SShadeUV[n]*/ tv, 1);
		tv.x = _SShadePos[n].x;
		tv.y = _SShadePos[n].y;
		tv.z = _SShadePos[n].z;
		tv.w = 1;

		_pModel->m_pDev->SetVertexShaderConstantX(nIndex++, &/*D3DXVECTOR4(_SShadePos[n].x
	...
	_pModel->RenderModel();
#else
	_pModel->m_pDev->SetStreamSource(0, _pModel->GetVertexBuffer(), 0, sizeof(SEFFECT_SHADE_VERTEX));
	_pModel->m_pDev->SetIndices(_lpIB, 0);
	_pModel->m_pDev->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, _iVerNum, 0, _iFaceCount);
#endif
...
void CMPShadeMap::FillVertex() {
	SEFFECT_SHADE_VERTEX* pVertex(nullptr);
	if (!_pModel)
		return;
	...
	if (pVertex == nullptr) {
		MessageBox(nullptr, "msgLockFailed lock error msglock error CMPShadeMap::FillVertex() line 552", "error", 0);
...
void CMPShadeMap::RenderSoft() {
...
#endif
	_pModel->m_pDev->SetFVFX(EFFECT_SHADE_FVF);
#else
	_pCEffectFile->SetTechnique(_iIdxTech);
	_pCEffectFile->Begin();
	_pCEffectFile->Pass(0);
	...
	_pModel->GetDev()->SetStreamSource(0, _pModel->GetVertexBuffer(), 0, sizeof(SEFFECT_SHADE_VERTEX));
	_pModel->GetDev()->SetIndices(_lpIB, 0);
	_pModel->GetDev()->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, _iVerNum, 0, _iFaceCount);
#endif
	_pCEffectFile->End();
	//_pModel->GetDev()->SetRenderStateForced(D3DRS_ALPHATESTENABLE, FALSE);
...
CMPShadeEX::CMPShadeEX(int iFrameCount) {
	...
	for (n = 0; n < _iNumTex; n++) {
		_vecTexOffset[n] = D3DXVECTOR2(0, 0);
	}
	_vecTexSave.clear();
}

CMPShadeEX::~CMPShadeEX() {
}
...
bool CMPShadeCtrl::Create(std::string& strTexName, CMPResManger* pCResMagr, float fSize,
...
/////////////////////
void CMPShadeCtrl::setFrameTexture(std::string& strTexName, CMPResManger* pCResMagr) {
