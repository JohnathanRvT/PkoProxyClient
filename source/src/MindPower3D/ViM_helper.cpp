...
VIMD3D g_vimd3d = nullptr;												  // ViM
LPDIRECT3DTEXTUREX* g_pRenderTextures;								  // ViM
LPDIRECT3DSURFACEX* g_pRenderTargets;								  // ViM
...
DWORD ViM_Helper::_init() {
	g_pRenderTextures = nullptr; // ViM
	g_pRenderTargets = nullptr;  // ViM
	g_dwNumViews = 0;		  // ViM
...
void ViM_Helper::_create() {
	...
	IDirect3DDeviceX* g_pd3dDevice = g_Render.GetDevice();

	if (FAILED(ViMD3D_Create(g_pd3dDevice, &g_vimd3d))) // ViM
		return;											// ViM

	D3DSURFACE_DESC desc;							  // ViM
	LPDIRECT3DSURFACEX surf = nullptr;					  // ViM
	if (FAILED(g_pd3dDevice->GetRenderTargetX(NULL, &surf))) // ViM
		return;										  // ViM
	...
	g_pRenderTextures = new LPDIRECT3DTEXTUREX[g_dwNumViews];				  // ViM
	ZeroMemory(g_pRenderTextures, g_dwNumViews * sizeof(LPDIRECT3DTEXTUREX)); // ViM
	g_pRenderTargets = new LPDIRECT3DSURFACEX[g_dwNumViews];				  // ViM
	ZeroMemory(g_pRenderTargets, g_dwNumViews * sizeof(LPDIRECT3DSURFACEX));  // ViM

	for (DWORD i = 0; i < g_dwNumViews; i++) // ViM

	{																								// ViM
		if (FAILED(g_pd3dDevice->CreateTextureX(desc.Width /*/2*/, desc.Height /*/2*/, 1,			// ViM
											   D3DUSAGE_RENDERTARGET, desc.Format, D3DPOOL_DEFAULT, // ViM
											   &g_pRenderTextures[i], NULL)))								// ViM
			return;																					// ViM
...
void ViM_Helper::Load(const char* pszFileName) {
