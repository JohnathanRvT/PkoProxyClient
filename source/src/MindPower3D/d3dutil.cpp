...
//-----------------------------------------------------------------------------
HRESULT D3DUtil_CreateTexture(LPDIRECT3DDEVICEX pd3dDevice, char* strTexture,
							  LPDIRECT3DTEXTUREX* ppTexture, D3DFORMAT d3dFormat) {
	...
	return D3DXCreateTextureFromFileEx(pd3dDevice, strPath,
									   ...
									   D3DX_FILTER_TRIANGLE | D3DX_FILTER_MIRROR, 0, nullptr, nullptr, ppTexture);
}
...
//-----------------------------------------------------------------------------
HRESULT D3DUtil_SetColorKey(LPDIRECT3DTEXTUREX pTexture, DWORD dwColorKey) {
...
HRESULT D3DUtil_CreateVertexShader(LPDIRECT3DDEVICEX pd3dDevice,
								   char* strFilename, DWORD* pdwVertexDecl,
								   IDirect3DVertexShaderX* pdwVertexShader) {
...
HRESULT D3DUtil_CreateVertexShader(LPDIRECT3DDEVICEX pd3dDevice,
	...
	if (FAILED(hr = D3DXAssembleShaderFromFileX(strPath, nullptr, nullptr, 0, &pCode, nullptr)))
		return hr;

	// Create the vertex shader
#if (defined LW_USE_DX9)
	hr = pd3dDevice->CreateVertexShaderX(pdwVertexDecl,
...
//-----------------------------------------------------------------------------
HRESULT D3DUtil_SetDeviceCursor(LPDIRECT3DDEVICEX pd3dDevice, HCURSOR hCursor) {
	HRESULT hr = E_FAIL;
	ICONINFO iconinfo;
	BOOL bBWCursor;
	LPDIRECT3DSURFACEX pCursorBitmap = nullptr;
	...
	if (FAILED(hr = pd3dDevice->CreateOffscreenPlainSurfaceX(dwWidth, dwHeightDest,
	...
	if (!bBWCursor) {
		...
		hdcColor = CreateCompatibleDC(GetDC(nullptr));
		if (hdcColor == nullptr) {
			hr = E_FAIL;
			goto End;
		}
	...
	D3DLOCKED_RECT lr;
	pCursorBitmap->LockRect(&lr, nullptr, 0);
...
VOID CD3DCamera::SetViewParams(D3DXVECTOR3& vEyePt, D3DXVECTOR3& vLookatPt,
	...
	m_vUpVec = vUpVec;
	auto pdif = m_vLookatPt - m_vEyePt;
