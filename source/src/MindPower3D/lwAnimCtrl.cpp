...
lwAnimCtrlBone::lwAnimCtrlBone(lwIResourceMgr* res_mgr)
	: lwAnimCtrl(res_mgr),
	  _bone_rtmat_seq(nullptr), _dummy_rtmat_seq(nullptr), _rtbuf_seq(nullptr), _rtmat_ptr(nullptr),
	  _bone_rtmat_blend_seq(nullptr) {
...
LW_RESULT lwAnimCtrlBone::_UpdateFrameDataBone(lwMatrix44** o_mat_ptr, lwMatrix44* mat_buf, float frame, DWORD start_frame, DWORD end_frame, BOOL loop_flag) {
	...
	DWORD min_f, max_f;
	float t;
	lwMatrix44* this_mat_ptr = nullptr;
	lwRTBD* rtbd_f;
	lwRTBD* rtbd_min;
	lwRTBD* rtbd_max;
	...
	if (_rtbuf_seq) {
		...
		if (lwFloatZero(t, 1e-3f)) {
			...
			// buf_min
			if (rtbd_min->data == nullptr) {
				rtbd_min->data = LW_NEW(lwMatrix44[_data._bone_num]);

				if (LW_FAILED(_BuildRunTimeBoneMatrix(rtbd_min->data, (float)min_f, start_frame, end_frame))) {
					LW_SAFE_DELETE_A(rtbd_min->data);
					goto __ret;
				}
			}
			// buf_max
			if (rtbd_max->data == nullptr) {
				rtbd_max->data = LW_NEW(lwMatrix44[_data._bone_num]);

				if (LW_FAILED(_BuildRunTimeBoneMatrix(rtbd_max->data, (float)max_f, start_frame, end_frame))) {
...
LW_RESULT lwAnimCtrlBone::UpdateAnimData(const lwPlayPoseInfo* info) {
	...
	if (info->blend_info.op_state) {
		lwMatrix44* rtmat_blend_ptr = nullptr;

		if (_bone_rtmat_blend_seq == nullptr) {
...
lwAnimCtrlTexUV::lwAnimCtrlTexUV(lwIResourceMgr* res_mgr)
	: lwAnimCtrl(res_mgr), _keyset_prs(nullptr) {
}
...
lwAnimCtrlTexImg::lwAnimCtrlTexImg(lwIResourceMgr* res_mgr)
	: lwAnimCtrl(res_mgr),
	  _tex_seq(nullptr), _tex_num(0), _rttex(LW_INVALID_INDEX) {
}
...
lwAnimCtrlMtlOpacity::lwAnimCtrlMtlOpacity(lwIResourceMgr* res_mgr)
	: lwAnimCtrl(res_mgr), _data(nullptr), _rt_opacity(0.0f) {
}
...
LW_RESULT lwAnimCtrlMtlOpacity::ExtractAnimData(lwIAnimDataMtlOpacity* out_data) {
	...
	if (this_aksf == nullptr || LW_FAILED(this_aksf->Clone(&aksf)))
		goto __ret;
