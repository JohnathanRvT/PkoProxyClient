...
LW_RESULT lwConsole::Create(DWORD desired_access, DWORD share_mode, const SECURITY_ATTRIBUTES* security_attr) {
	_handle = ::CreateConsoleScreenBuffer(desired_access, share_mode, security_attr, CONSOLE_TEXTMODE_BUFFER, nullptr);
...
LW_RESULT lwConsole::Write(const char* str, ...) {
	...
	va_list args;
	va_start(args, str);
	_vsntprintf_s(buf, _countof(buf), _TRUNCATE, str, args);
	va_end(args);

	DWORD str_size = (DWORD)strlen(buf);

	BOOL b = ::WriteConsole(_handle, buf, str_size, &str_size, nullptr);
