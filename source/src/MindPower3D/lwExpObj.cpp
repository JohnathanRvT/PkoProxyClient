...
{
    LW_fread(&mtl, sizeof(mtl), 1, fp);
    LW_fread(&rs_set, sizeof(rs_set), 1, fp);
    LW_fread(&tex_seq[0], sizeof(tex_seq), 1, fp);
...
LW_RESULT lwMtlTexInfo_Load(lwMtlTexInfo* info, FILE* fp, DWORD version) {
	if (version >= EXP_OBJ_VERSION_1_0_0_0) {
		LW_fread(&info->opacity, sizeof(info->opacity), 1, fp);
		LW_fread(&info->transp_type, sizeof(info->transp_type), 1, fp);
		LW_fread(&info->mtl, sizeof(lwMaterial), 1, fp);
		LW_fread(&info->rs_set[0], sizeof(info->rs_set), 1, fp);

		//x64: Prevent loading a void* type from struct here (which is 8 bytes in x64)
		lwTexInfo_nopointer tex_seq_no_pointer[LW_MAX_TEXTURESTAGE_NUM];
		LW_fread(&tex_seq_no_pointer[0], sizeof(tex_seq_no_pointer), 1, fp);
		lwTexInfo* t;
		lwTexInfo_nopointer* p;
		for (int i = 0; i < LW_MAX_TEXTURESTAGE_NUM; i++) {
			p = &tex_seq_no_pointer[i];
			...
			strncpy_s(t->file_name, p->file_name, _TRUNCATE);
			t->data = nullptr;
			for (int j = 0; j < LW_TEX_TSS_NUM; j++) {
				t->tss_set[j] = p->tss_set[j];
			}
		}
		...
		LW_fread(&info->transp_type, sizeof(info->transp_type), 1, fp);
		LW_fread(&info->mtl, sizeof(lwMaterial), 1, fp);
		LW_fread(&rsm, sizeof(rsm), 1, fp);
		LW_fread(&tex_info[0], sizeof(tex_info), 1, fp);

		lwRenderStateValue* rsv;
		DWORD i = 0;
		for (i = 0; i < rsm.SEQUENCE_SIZE; i++) {
			rsv = &rsm.rsv_seq[0][i];
		...
		for (i = 0; i < LW_MAX_TEXTURESTAGE_NUM; i++) {
			...
			lwRenderStateValue* rsv;
			DWORD j = 0;
			for (j = 0; j < p->tss_set.SEQUENCE_SIZE; j++) {
		...
		LW_fread(&info->mtl, sizeof(lwMaterial), 1, fp);
		LW_fread(&rsm, sizeof(lwRenderStateSetMtl2), 1, fp);
		...
		MessageBox(nullptr, "invalid file version", "error", MB_OK);
...
LW_RESULT lwLoadMtlTexInfo(lwMtlTexInfo** out_buf, DWORD* out_num, FILE* fp, DWORD version) {
	lwMtlTexInfo* buf = nullptr;
	DWORD num = 0;

	if (version == EXP_OBJ_VERSION_0_0_0_0) {
		DWORD old_version;
		LW_fread(&old_version, sizeof(old_version), 1, fp);
		version = old_version;
	}

	LW_fread(&num, sizeof(num), 1, fp);
...
size_t lwMtlTexInfo_GetDataSize(lwMtlTexInfo* info) {
...
LW_RESULT lwAnimDataTexUV::Load(FILE* fp, DWORD version) {
	LW_fread(&_frame_num, sizeof(DWORD), 1, fp);

	_mat_seq = LW_NEW(lwMatrix44[_frame_num]);
	LW_fread(&_mat_seq[0], sizeof(lwMatrix44), _frame_num, fp);
...
LW_RESULT lwAnimDataTexImg::Load(FILE* fp, DWORD version) {
	if (version == EXP_OBJ_VERSION_0_0_0_0) {
		char buf[256];
		_snprintf_s(buf, _TRUNCATE, "old version file, need re-export it");
		MessageBox(nullptr, buf, "warning", MB_OK);
		return LW_RET_FAILED;
	} else {
		LW_fread(&_data_num, sizeof(_data_num), 1, fp);
		_data_seq = LW_NEW(lwTexInfo[_data_num]);
		LW_fread(_data_seq, sizeof(lwTexInfo), _data_num, fp);
	}
...
LW_RESULT lwAnimDataMtlOpacity::Load(FILE* fp, DWORD version) {
	...
	LW_fread(seq, sizeof(lwKeyFloat), num, fp);
...
lwAnimDataBone::lwAnimDataBone()
	: _key_type(BONE_KEY_TYPE_MAT43), _base_seq(nullptr), _invmat_seq(nullptr), _key_seq(nullptr), _dummy_seq(nullptr),
...
LW_RESULT lwAnimDataBone::Load(FILE* fp, DWORD version) {
	...
	LW_fread(&_header, sizeof(lwBoneInfoHeader), 1, fp);
	...
	LW_fread(_base_seq, sizeof(lwBoneBaseInfo), _bone_num, fp);
	LW_fread(_invmat_seq, sizeof(lwMatrix44), _bone_num, fp);
	LW_fread(_dummy_seq, sizeof(lwBoneDummyInfo), _dummy_num, fp);
	...
	case BONE_KEY_TYPE_MAT43:
		for (i = 0; i < _bone_num; i++) {
			...
			LW_fread(&key->mat43_seq[0], sizeof(lwMatrix43), _frame_num, fp);
		}
		break;
	case BONE_KEY_TYPE_MAT44:
		for (i = 0; i < _bone_num; i++) {
			...
			LW_fread(&key->mat44_seq[0], sizeof(lwMatrix44), _frame_num, fp);
		}
		break;
	case BONE_KEY_TYPE_QUAT:
		if (version >= EXP_OBJ_VERSION_1_0_0_3) {
			for (i = 0; i < _bone_num; i++) {
				...
				LW_fread(&key->pos_seq[0], sizeof(lwVector3), _frame_num, fp);

				key->quat_seq = LW_NEW(lwQuaternion[_frame_num]);
				LW_fread(&key->quat_seq[0], sizeof(lwQuaternion), _frame_num, fp);
			}
		} else // old version < EXP_OBJ_VERSION_1_0_0_3
		{
			for (i = 0; i < _bone_num; i++) {
				...
				key->pos_seq = LW_NEW(lwVector3[_frame_num]);
				LW_fread(&key->pos_seq[0], sizeof(lwVector3), pos_num, fp);
...
LW_RESULT lwAnimDataBone::Load(const char* file) {
	...
	DWORD version;
	LW_fread(&version, sizeof(version), 1, fp);

	if (version < EXP_OBJ_VERSION_1_0_0_0) {
		goto __ret;
		version = EXP_OBJ_VERSION_0_0_0_0;
		char buf[256];
		_snprintf_s(buf, _TRUNCATE, "old animation file: %s, need re-export it", file);
		MessageBox(nullptr, buf, "warning", MB_OK);
	}
...

	{
        ...
        //mat->_43 = vv.z;
			...
			case BONE_KEY_TYPE_QUAT:
				...
				//lwQuaternionToMatrix44(&mat_1, &key->quat_seq[max_f]);
				//*(lwVector3*)&mat_1._41 = key->pos_seq[max_f];
				lwMatrix44Slerp(&key->pos_seq[min_f], &key->pos_seq[max_f], nullptr, nullptr, &key->quat_seq[min_f], &key->quat_seq[max_f], t, mat);
				break;
			default:
				assert(0);
				goto __ret;
...
lwAnimDataMatrix::lwAnimDataMatrix()
	: _mat_seq(nullptr), _frame_num(0) /*, _subset_type(LW_INVALID_INDEX)*/
{
}
lwAnimDataMatrix::~lwAnimDataMatrix() {
...
LW_RESULT lwAnimDataMatrix::Load(FILE* fp, DWORD version) {
	...
	LW_fread(&_mat_seq[0], sizeof(lwMatrix43), _frame_num, fp);
...
LW_RESULT lwAnimKeySetPRS::GetValue(lwMatrix44* mat, float frame) {
	...
	const auto t2 = lwMatrix44Translate(pos);
	lwMatrix44Multiply(mat, mat, &t2);
...
LW_RESULT lwMeshInfo_Load(lwMeshInfo* info, FILE* fp, DWORD version) {
	...
	if (version >= EXP_OBJ_VERSION_1_0_0_4) {
		LW_fread(&info->header, sizeof(info->header), 1, fp);
	} else if (version >= EXP_OBJ_VERSION_1_0_0_3) {
		lwMeshInfo_0003::lwMeshInfoHeader header;
		LW_fread(&header, sizeof(header), 1, fp);
		info->fvf = header.fvf;
		...
		lwRenderStateValue* rsv;
		DWORD j = 0;
		for (j = 0; j < header.rs_set.SEQUENCE_SIZE; j++) {
		...
		MessageBox(nullptr, "invalid version", "error", MB_OK);
	}

	if (version >= EXP_OBJ_VERSION_1_0_0_4) {
		if (info->vertex_element_num > 0) {
			info->vertex_element_seq = LW_NEW(D3DVERTEXELEMENTX[info->vertex_element_num]);
			LW_fread(&info->vertex_element_seq[0], sizeof(D3DVERTEXELEMENTX), info->vertex_element_num, fp);
		}

		if (info->vertex_num > 0) {
			info->vertex_seq = LW_NEW(lwVector3[info->vertex_num]);
			LW_fread(&info->vertex_seq[0], sizeof(lwVector3), info->vertex_num, fp);
		}

		if (info->fvf & D3DFVF_NORMAL) {
			info->normal_seq = LW_NEW(lwVector3[info->vertex_num]);
			LW_fread(&info->normal_seq[0], sizeof(lwVector3), info->vertex_num, fp);
		}

		if (info->fvf & D3DFVF_TEX1) {
			info->texcoord0_seq = LW_NEW(lwVector2[info->vertex_num]);
			LW_fread(&info->texcoord0_seq[0], sizeof(lwVector2), info->vertex_num, fp);
			...
			info->texcoord1_seq = LW_NEW(lwVector2[info->vertex_num]);
			LW_fread(&info->texcoord0_seq[0], sizeof(lwVector2), info->vertex_num, fp);
			LW_fread(&info->texcoord1_seq[0], sizeof(lwVector2), info->vertex_num, fp);
			...
			LW_fread(&info->texcoord0_seq[0], sizeof(lwVector2), info->vertex_num, fp);
			LW_fread(&info->texcoord1_seq[0], sizeof(lwVector2), info->vertex_num, fp);
			LW_fread(&info->texcoord2_seq[0], sizeof(lwVector2), info->vertex_num, fp);
			...
			LW_fread(&info->texcoord1_seq[0], sizeof(lwVector2), info->vertex_num, fp);
			LW_fread(&info->texcoord2_seq[0], sizeof(lwVector2), info->vertex_num, fp);
			LW_fread(&info->texcoord3_seq[0], sizeof(lwVector2), info->vertex_num, fp);
		}

		if (info->fvf & D3DFVF_DIFFUSE) {
			info->vercol_seq = LW_NEW(DWORD[info->vertex_num]);
			LW_fread(&info->vercol_seq[0], sizeof(DWORD), info->vertex_num, fp);
		}
		...
		if (info->bone_index_num > 0) {
			...
			LW_fread(&info->blend_seq[0], sizeof(lwBlendInfo), info->vertex_num, fp);
			LW_fread(&info->bone_index_seq[0], sizeof(DWORD), info->bone_index_num, fp);
		}

		if (info->index_num > 0) {
			info->index_seq = LW_NEW(DWORD[info->index_num]);
			LW_fread(&info->index_seq[0], sizeof(DWORD), info->index_num, fp);
		}

		if (info->subset_num > 0) {
			info->subset_seq = LW_NEW(lwSubsetInfo[info->subset_num]);
			LW_fread(&info->subset_seq[0], sizeof(lwSubsetInfo), info->subset_num, fp);
		}
	} else {
		info->subset_seq = LW_NEW(lwSubsetInfo[info->subset_num]);
		LW_fread(&info->subset_seq[0], sizeof(lwSubsetInfo), info->subset_num, fp);
		...
		if (info->fvf & D3DFVF_LASTBETA_UBYTE4) {
			...
			LW_fread(&info->blend_seq[0], sizeof(lwBlendInfo), info->vertex_num, fp);
			LW_fread(&byte_index_seq[0], sizeof(BYTE), info->bone_index_num, fp);
...
LW_RESULT lwAnimDataInfo::Load(FILE* fp, DWORD version) {
	...
	LW_fread(&data_bone_size, sizeof(DWORD), 1, fp);
	LW_fread(&data_mat_size, sizeof(DWORD), 1, fp);

	if (version >= EXP_OBJ_VERSION_1_0_0_5) {
		LW_fread(&data_mtlopac_size, sizeof(data_mtlopac_size), 1, fp);
	}

	LW_fread(&data_texuv_size, sizeof(data_texuv_size), 1, fp);
	LW_fread(&data_teximg_size, sizeof(data_teximg_size), 1, fp);
...
// get size function
size_t lwGetMtlTexInfoSize(const lwMtlTexInfo* info_seq, DWORD num) {
	size_t size = 0;
...
LW_RESULT lwLoadAnimKeySetPRS(lwAnimKeySetPRS* info, FILE* fp) {
	LW_fread(&info->frame_num, sizeof(DWORD), 1, fp);

	LW_fread(&info->pos_num, sizeof(DWORD), 1, fp);
	LW_fread(&info->rot_num, sizeof(DWORD), 1, fp);
	LW_fread(&info->sca_num, sizeof(DWORD), 1, fp);

	if (info->pos_num > 0) {
		info->pos_seq = LW_NEW(lwKeyDataVector3[info->pos_num]);
		LW_fread(&info->pos_seq[0], sizeof(lwKeyDataVector3), info->pos_num, fp);
	}

	if (info->rot_num > 0) {
		info->rot_seq = LW_NEW(lwKeyDataQuaternion[info->rot_num]);
		LW_fread(&info->rot_seq[0], sizeof(lwKeyDataQuaternion), info->rot_num, fp);
	}

	if (info->sca_num > 0) {
		info->sca_seq = LW_NEW(lwKeyDataVector3[info->sca_num]);
		LW_fread(&info->sca_seq[0], sizeof(lwKeyDataVector3), info->sca_num, fp);
	}
			...
			if (p1->vertex[i] == p2->vertex[j]) {
				if (r1 != -1) {
					if (p1->vertex[i] == p1->vertex[r1]) {
						MessageBox(nullptr, "the same vertices in one triangle", "warning", MB_OK);
...
LW_RESULT lwHelperInfo::Load(FILE* fp, DWORD version) {
	...
	LW_fread(&type, sizeof(type), 1, fp);
...
LW_RESULT lwHelperInfo::_LoadHelperDummyInfo(FILE* fp, DWORD version) {
	...
	if (version >= EXP_OBJ_VERSION_1_0_0_1) {
		LW_fread(&dummy_num, sizeof(dummy_num), 1, fp);
		dummy_seq = LW_NEW(lwHelperDummyInfo[dummy_num]);
		LW_fread(&dummy_seq[0], sizeof(lwHelperDummyInfo), dummy_num, fp);
	} else if (version <= EXP_OBJ_VERSION_1_0_0_0) {
		LW_fread(&dummy_num, sizeof(dummy_num), 1, fp);
		lwHelperDummyInfo_1000* old_s = LW_NEW(lwHelperDummyInfo_1000[dummy_num]);
		LW_fread(&old_s[0], sizeof(lwHelperDummyInfo_1000), dummy_num, fp);
...
LW_RESULT lwHelperInfo::_LoadHelperBoxInfo(FILE* fp, DWORD version) {
	LW_fread(&box_num, sizeof(box_num), 1, fp);

	box_seq = LW_NEW(lwHelperBoxInfo[box_num]);
	LW_fread(&box_seq[0], sizeof(lwHelperBoxInfo), box_num, fp);
...
LW_RESULT lwHelperInfo::_LoadHelperMeshInfo(FILE* fp, DWORD version) {
	LW_fread(&mesh_num, sizeof(mesh_num), 1, fp);
	...
	for (i = 0; i < mesh_num; i++) {
		...
		LW_fread(&info->id, sizeof(info->id), 1, fp);
		LW_fread(&info->type, sizeof(info->type), 1, fp);
		LW_fread(&info->sub_type, sizeof(info->sub_type), 1, fp);
		LW_fread(&info->name[0], sizeof(info->name), 1, fp);
		LW_fread(&info->state, sizeof(info->state), 1, fp);
		LW_fread(&info->mat, sizeof(info->mat), 1, fp);
		LW_fread(&info->box, sizeof(info->box), 1, fp);
		LW_fread(&info->vertex_num, sizeof(info->vertex_num), 1, fp);
		LW_fread(&info->face_num, sizeof(info->face_num), 1, fp);

		info->vertex_seq = LW_NEW(lwVector3[info->vertex_num]);
		info->face_seq = LW_NEW(lwHelperMeshFaceInfo[info->face_num]);

		LW_fread(&info->vertex_seq[0], sizeof(lwVector3), info->vertex_num, fp);
		LW_fread(&info->face_seq[0], sizeof(lwHelperMeshFaceInfo), info->face_num, fp);
	}
...
LW_RESULT lwHelperInfo::_LoadBoundingBoxInfo(FILE* fp, DWORD version) {
	LW_fread(&bbox_num, sizeof(DWORD), 1, fp);

	bbox_seq = LW_NEW(lwBoundingBoxInfo[bbox_num]);

	LW_fread(&bbox_seq[0], sizeof(lwBoundingBoxInfo), bbox_num, fp);
...
LW_RESULT lwHelperInfo::_LoadBoundingSphereInfo(FILE* fp, DWORD version) {
	LW_fread(&bsphere_num, sizeof(DWORD), 1, fp);

	bsphere_seq = LW_NEW(lwBoundingSphereInfo[bsphere_num]);

	LW_fread(&bsphere_seq[0], sizeof(lwBoundingSphereInfo), bsphere_num, fp);
...
LW_RESULT lwGeomObjInfo::Load(FILE* fp, DWORD version) {
	LW_fread((lwGeomObjInfoHeader*)&id, sizeof(lwGeomObjInfoHeader), 1, fp);
...
LW_RESULT lwModelObjInfo::Load(const char* file) {
	...
	DWORD obj_num;
	LW_fread(&obj_num, sizeof(DWORD), 1, fp);

	lwModelObjInfoHeader header[LW_MAX_MODEL_OBJ_NUM];
	LW_fread(&header[0], sizeof(lwModelObjInfoHeader), obj_num, fp);
...
LW_RESULT lwModelObjInfo::GetHeader(lwModelObjInfoHeader* header_seq, DWORD* header_num, const char* file) {
	...
	DWORD version;
	LW_fread(&version, sizeof(DWORD), 1, fp);

	LW_fread(header_num, sizeof(DWORD), 1, fp);

	LW_fread(&header_seq[0], sizeof(lwModelObjInfoHeader), *header_num, fp);
...
LW_RESULT lwHelperDummyObjInfo::Load(FILE* fp, DWORD version) {
	...
	LW_fread(&_id, sizeof(_id), 1, fp);
	LW_fread(&_mat, sizeof(_mat), 1, fp);

	LW_SAFE_RELEASE(_anim_data);

	DWORD anim_data_flag = 0;
	LW_fread(&anim_data_flag, sizeof(anim_data_flag), 1, fp);
	if (anim_data_flag == 1) {
...
LW_RESULT lwModelNodeInfo::Load(FILE* fp, DWORD version) {
	...
	LW_fread(&_head, sizeof(_head), 1, fp);
...
void lwReleaseTreeNodeList_(lwITreeNode* node) {
	node->EnumTree(__tree_node_release_proc, nullptr, TREENODE_PROC_POSTORDER);
}

LW_RESULT lwModelInfo::Destroy() {
	if (_obj_tree) {
		_obj_tree->EnumTree(__tree_proc_modlinfo_destroy, nullptr, TREENODE_PROC_PREORDER);
...
LW_RESULT lwModelInfo::Load(const char* file) {
	...
	DWORD obj_num;
	LW_fread(&obj_num, sizeof(obj_num), 1, fp);

	if (obj_num == 0)
		goto __addr_ret_ok;

	{
		lwITreeNode* tree_node = nullptr;
		lwModelNodeInfo* node_info = nullptr;
		...
		for (i = 0; i < obj_num; i++) {
			...
			if (_obj_tree == nullptr) {
				...
				if (LW_FAILED(param.node->InsertChild(nullptr, tree_node)))
					goto __ret;
			}
		}
...
LW_RESULT lwModelInfo::Save(const char* file) {
		...
		goto __ret;
	{
		...
		_tcsncpy_s(_head.decriptor, "lwModelInfo", _TRUNCATE);
...
LW_RESULT lwModelNodeSortChild(lwITreeNode* node) {
	...
	child_seq[child_num - 1]->SetSibling(nullptr);
...
LW_RESULT lwModelInfo::SortChildWithID() {
	DWORD ret = _obj_tree->EnumTree(__tree_proc_sort_id, nullptr, TREENODE_PROC_PREORDER);
