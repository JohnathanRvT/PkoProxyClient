		...
		if ((ptr = _tcsrchr(path, '\\'))) {
			*ptr = '\0';
			...
			if (LW_FAILED(_CheckDirectory(path))) {
				if (LW_FAILED(CreateDirectory(path, nullptr)))
					goto __ret;
			}
		}
	}

	_handle = ::CreateFile(file, access_flag, share_mode, secu_attr, creation_flag, attributes_flag, nullptr);
...
LW_RESULT lwFile::LoadFileBuffer(const char* file, lwIBuffer* buf) {
	...
	if (LW_FAILED(CreateFile(file, access_flag, FILE_SHARE_READ, nullptr, creation_flag, attributes_flag)))
		goto __ret;
...
LW_RESULT lwFile::Read(void* buf, DWORD in_size, DWORD* out_size) {
	...
	if (ReadFile(_handle, buf, in_size, out_size, nullptr) == 0)
		goto __ret;
	if (in_size != *out_size)
		goto __ret;
...
LW_RESULT lwFile::Write(const void* buf, DWORD in_size, DWORD* out_size) {
	...
	if (WriteFile(_handle, buf, in_size, out_size, nullptr) == 0)
		goto __ret;
	if (in_size != *out_size)
		goto __ret;
...
LW_RESULT lwFile::Seek(long offset, DWORD flag) {
	...
	if (SetFilePointer(_handle, offset, nullptr, flag) == INVALID_SET_FILE_POINTER)
		goto __ret;
...
LW_RESULT lwFile::GetCreationTime(SYSTEMTIME* st) {
	...
	if (GetFileTime(_handle, nullptr, nullptr, &ft) == 0)
		goto __ret;
