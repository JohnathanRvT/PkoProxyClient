...
LW_RESULT lwFileStream::Open(const char* file, const lwFileStreamOpenInfo* info) {
	...
	if (info->adapter_type == FS_ADAPTER_FILE) {
		...
		if (LW_FAILED(_adapter_file->CreateFile(file, info->access_flag, NULL, nullptr, info->create_flag, info->attributes_flag)))
			goto __ret;
	}
