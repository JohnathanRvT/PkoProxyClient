...
static lwISystem* g_system = nullptr;
static lwISysGraphics* g_sys_graphics = nullptr;
...
LW_RESULT lwInitMeshLibSystem(lwISystem** ret_sys, lwISysGraphics** ret_sys_graphics, lwD3DCreateParam* param, lwD3DCreateParamAdjustInfo* param_info) {
...

	{
		...
		path_info->SetPath(PATH_TYPE_WORKINGDIRECTORY, R"(.\)");

		path_info->SetPath(PATH_TYPE_MODEL, R"(.\model\)");
		path_info->SetPath(PATH_TYPE_MODEL_CHARACTER, R"(.\model\character\)");
		path_info->SetPath(PATH_TYPE_MODEL_EFFECT, R"(.\model\effect\)");
		path_info->SetPath(PATH_TYPE_MODEL_SCENE, R"(.\model\scene\)");
		path_info->SetPath(PATH_TYPE_MODEL_ITEM, R"(.\model\item\)");

		path_info->SetPath(PATH_TYPE_TEXTURE, R"(.\texture\)");
		path_info->SetPath(PATH_TYPE_TEXTURE_CHARACTER, R"(.\texture\character\)");
		path_info->SetPath(PATH_TYPE_TEXTURE_EFFECT, R"(.\texture\effect\)");
		path_info->SetPath(PATH_TYPE_TEXTURE_SCENE, R"(.\texture\scene\)");
		path_info->SetPath(PATH_TYPE_TEXTURE_ITEM, R"(.\texture\item\)");

		path_info->SetPath(PATH_TYPE_ANIMATION, R"(.\animation\)");
#if (defined LW_USE_DX8)
		path_info->SetPath(PATH_TYPE_SHADER, R"(.\shader\dx8\)");
#endif
#if (defined LW_USE_DX9)
		path_info->SetPath(PATH_TYPE_SHADER, R"(.\shader\dx9\)");
#endif
		path_info->SetPath(PATH_TYPE_EFFECT, R"(.\effect\)");
...
LW_RESULT lwReleaseMeshLibSystem() {
	...
	if ((sys = lwGetActiveISystem()) == nullptr)
		goto __ret;

	if ((sys_graphics = lwGetActiveIGraphicsSystem()) == nullptr)
		goto __ret;
...
LW_RESULT LoadResBuf(lwIResourceMgr* res_mgr, const char* file) {
	...
	for (DWORD i = 0; i < model_num; i++) {
		fscanf_s(fp, "%s\n", model_file, LW_MAX_PATH);
		_snprintf_s(path, _TRUNCATE, "%s%s", path_info->GetPath(PATH_TYPE_MODEL_SCENE).c_str(), model_file);
		...
		for (DWORD j = 0; j < model_info->geom_obj_num; j++) {
			...
			for (DWORD k = 0; k < geom_info->mtl_num; k++) {
				...
				_snprintf_s(smti.file_name, _TRUNCATE, "%s%s", path_info->GetPath(PATH_TYPE_TEXTURE_SCENE).c_str(), tex_info->file_name);
