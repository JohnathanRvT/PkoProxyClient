...
LW_RESULT lwLoadTex(lwITex** out, lwIResourceMgr* res_mgr, const lwTexInfo* info) {
	...
	if (LW_FAILED(tex->LoadTexInfo(info, nullptr)))
		goto __ret;
...
void lwPhysiqueSetMaterial(lwIPhysique* phy, const lwMaterial* mtl) {
	...
	for (i = 0; i < LW_MAX_SUBSKIN_NUM; i++) {
		if ((p = phy->GetPrimitive(i)) == nullptr)
			continue;
...
// ======= anim proc
using lwTexUVAnimProc = LW_RESULT (*)(lwIAnimCtrlTexUV* ctrl_texuv);
...
LW_RESULT lwPrimitiveTexLit(lwIPrimitive* p, const char* file, const char* tex_path, DWORD color_op, DWORD anim_type) {
	...
	//_tcsncpy_s( tex_info.file_name, "boxlight2.tga" , _TRUNCATE);
	//_tcsncpy_s( tex_info.file_name, "shlight.tga" , _TRUNCATE);
	//_tcsncpy_s( tex_info.file_name, "multi009.tga" , _TRUNCATE);
	//@@_tcsncpy_s( tex_info.file_name, "ocean_h.08.bmp" , _TRUNCATE);
	//_tcsncpy_s( tex_info.file_name, "water_gsc012.BMP" , _TRUNCATE);
	_tcsncpy_s(tex_info.file_name, file, _TRUNCATE);
...
LW_RESULT lwPrimitiveTexLitA(lwIPrimitive* p, const char* alpha_file, const char* tex_file, const char* tex_path, DWORD anim_type) {
	...
	lwITex* tex;
	lwITex* ret_tex = nullptr;
	lwIMtlTexAgent* mtltex_agent = nullptr;
	lwIResourceMgr* res_mgr = p->GetResourceMgr();
	lwTexInfo tex_info;
