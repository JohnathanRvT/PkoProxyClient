...
lwItem::lwItem(lwIResourceMgr* res_mgr)
	: _id(LW_INVALID_INDEX), _res_mgr(res_mgr), _obj(nullptr), _link_ctrl(nullptr) {
	_opacity = 1.0f;
...
LW_RESULT lwItem::Load(lwIGeomObjInfo* info) {
		...
		goto __ret;
	{
		...
		if (LW_FAILED(imp->Load(info, path_info->GetPath(PATH_TYPE_TEXTURE_ITEM).c_str(), nullptr)))
			goto __ret;
...
LW_RESULT lwItem::Load(const char* file, int arbitrary_flag) {
	...
	__a:
		std::string path;
		path = path_info->GetPath(PATH_TYPE_MODEL_ITEM) + file;
		...
		if (LW_FAILED(imp->Load(&info, path_info->GetPath(PATH_TYPE_TEXTURE_ITEM).c_str(), &res)))
			return LW_RET_FAILED;
...
LW_RESULT lwItem::Destroy() {
	...
	_res_mgr->UnregisterObject(nullptr, _id, OBJ_TYPE_ITEM);
...
LW_RESULT lwItem::Copy(lwIItem* src_obj) {
	...
	if (!o->_obj)							 //WORKAROUND FIX
		_res_mgr->CreatePrimitive(&o->_obj); //WORKAROUND FIX
	o->_obj->Clone(&_obj);
...
LW_RESULT lwItem::Clone(lwIItem** ret_obj) {
	...
	*ret_obj = o;
	//_tcsncpy_s( o->_file_name, _file_name , _TRUNCATE);
	//
...
LW_RESULT lwItem::SetLinkCtrl(lwLinkCtrl* ctrl, DWORD link_parent_id, DWORD link_item_id) {
	if (LW_FAILED(GetDummyMatrix(nullptr, link_item_id)))
		return LW_RET_FAILED;
