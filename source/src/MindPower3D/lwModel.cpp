...
lwModel::lwModel(lwIResourceMgr* res_mgr)
	: _id(LW_INVALID_INDEX), _res_mgr(res_mgr), _scene_mgr(nullptr), _helper_object(nullptr) {
...
LW_RESULT lwModel::Load(lwIModelObjInfo* info) {
...

	{
		...
		for (DWORD i = 0; i < m_info->geom_obj_num; i++) {
			...
			imp->Load((lwIGeomObjInfo*)m_info->geom_obj_seq[i], path_info->GetPath(PATH_TYPE_TEXTURE_SCENE).c_str(), nullptr);
...
LW_RESULT lwModel::Load(const char* file, DWORD model_id) {
	...
	if (obj_model) {
		...
		std::string path = path_info->GetPath(PATH_TYPE_MODEL_SCENE) + file;
		...
		if (model_id == LW_INVALID_INDEX) {
			if (LW_FAILED(res_buf_mgr->QueryModelObjInfo((lwIModelObjInfo**)&model_info_ptr, path.c_str()))) {
				LW_HANDLE handle;
				if (LW_FAILED(res_buf_mgr->RegisterModelObjInfo(&handle, path.c_str())))
					return LW_RET_FAILED;
		...
		// USE_MODEL_ID_QUERY
			...
			if (LW_FAILED(res_buf_mgr->GetModelObjInfo((lwIModelObjInfo**)&model_info_ptr, model_id))) {
				if (LW_FAILED(res_buf_mgr->RegisterModelObjInfo(model_id, path.c_str())))
					return LW_RET_FAILED;
		...
		if (model_id == LW_INVALID_INDEX) {
			if (LW_SUCCEEDED(res_buf_mgr->QueryModelObjInfo((lwIModelObjInfo**)&model_info_ptr, path.c_str())))
				goto __load_model;
		} else {
			if (LW_FAILED(res_buf_mgr->GetModelObjInfo((lwIModelObjInfo**)&model_info_ptr, model_id)))
				return LW_RET_FAILED;
		...
		if (LW_FAILED(model_info.Load(path.c_str()))) {
			lwMessageBox("invalid file name: %s when called lwModel::Load", path.c_str());
			return LW_RET_FAILED;
		}
	...
	__load_model:
		...
		for (DWORD i = 0; i < model_info_ptr->geom_obj_num; i++) {
			...
			strncpy_s(res.file_name, path.c_str(), _TRUNCATE);

			lwIPrimitive* imp;
			_res_mgr->CreatePrimitive(&imp);

			imp->Load((lwIGeomObjInfo*)model_info_ptr->geom_obj_seq[i], path_info->GetPath(PATH_TYPE_TEXTURE_SCENE).c_str(), &res);
		...
		if (model_info_ptr->helper_data.type != HELPER_TYPE_INVALID) {
			if (_helper_object == nullptr) {
				...
				if (LW_FAILED(_helper_object->LoadHelperInfo(&model_info_ptr->helper_data, create_helper_primitive))) {
					lwMessageBox("load helper object error with file:%s", path.c_str());
					return LW_RET_FAILED;
				}
			}
		}
...
LW_RESULT lwModel::Destroy() {
	...
	_res_mgr->UnregisterObject(nullptr, _id, OBJ_TYPE_MODEL);
...
LW_RESULT lwModel::Copy(lwIModel* src_obj) {
	...
	strncpy_s(_file_name, o->_file_name, _TRUNCATE);
	_id = o->_id;
	_obj_num = o->_obj_num;
...
LW_RESULT lwModel::HitTestPrimitiveHelperMesh(lwPickInfo* info, const lwVector3* org, const lwVector3* ray, const char* type_name) {
	...
	for (DWORD i = 0; i < _obj_num; i++) {
		...
		if (obj == nullptr || obj->IsValidObject() == 0)
			continue;
