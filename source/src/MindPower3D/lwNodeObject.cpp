...
static DWORD __tree_proc_loadmodel(lwITreeNode* node, void* param) {
	...
	case NODE_PRIMITIVE:
		if (LW_FAILED(((lwINodePrimitive*)model_node)->Load((lwIGeomObjInfo*)node_info->_data, tex_path, nullptr)))
			goto __ret;
		break;
	case NODE_BONECTRL:
...

	{
		...
		if (node_parent) {
			...
			parent_obj_node->InsertChild(nullptr, model_tree);
...
lwNodePrimitive::lwNodePrimitive(lwIResourceMgr* res_mgr)
	...
	_mesh_agent = nullptr;
	_mtltex_agent_seq = nullptr;
	_anim_agent = nullptr;
	_render_agent = nullptr;
	_helper_object = nullptr;
	//_vertex_blend_ctrl = 0;
	_ref_ctrl_obj_bone = nullptr;
...
LW_RESULT lwNodePrimitive::_UpdateBoundingObject(lwIAnimCtrlObjBone* ctrl_obj) {
	...
	if ((b = _helper_object->GetBoundingSphere()) == nullptr)
		goto __addr_ret_ok;
...
LW_RESULT lwNodePrimitive::_UpdateTransparentState() {
	...
	for (i = 0; i < _mtltex_agent_seqsize; i++) {
		if ((mtltex_agent = _mtltex_agent_seq[i]) == nullptr)
			break;
...
LW_RESULT lwNodePrimitive::LoadAnimData(lwIAnimDataInfo* data_info, const char* tex_path, const lwResFile* res) {
	...
	if (info->anim_mat) {
		lwIAnimCtrlMatrix* ctrl = nullptr;
		lwIAnimCtrlObjMat* ctrl_obj = nullptr;
	...
	if (info->anim_bone) {
		lwIAnimCtrlBone* ctrl = nullptr;
		lwIAnimCtrlObjBone* ctrl_obj = nullptr;
	...
	for (i = 0; i < _mtltex_agent_seqsize; i++) {
		if (_mtltex_agent_seq[i] == nullptr)
			continue;
		...
		if (info->anim_mtlopac[i]) {
			...
			lwIAnimCtrlMtlOpacity* ctrl = nullptr;
			lwIAnimCtrlObjMtlOpacity* ctrl_obj = nullptr;
		...
		for (j = 0; j < LW_MAX_TEXTURESTAGE_NUM; j++) {
			if (info->anim_tex[i][j]) {
				lwIAnimCtrlTexUV* ctrl = nullptr;
				lwIAnimCtrlObjTexUV* ctrl_obj = nullptr;
			...
			if (info->anim_img[i][j]) {
				lwIAnimCtrlTexImg* ctrl = nullptr;
				lwIAnimCtrlObjTexImg* ctrl_obj = nullptr;
...
LW_RESULT lwNodePrimitive::LoadRenderCtrl(const lwRenderCtrlCreateInfo* rcci) {
	...
	lwIRenderCtrlAgent* render_agent = nullptr;
...
LW_RESULT lwNodePrimitive::Copy(lwINodePrimitive* src_obj) {
	...
	}
	{
		...
		DWORD i = 0;
		for (i = 0; i < src_mtltex_agent_seqsize; i++) {
			if ((src_mtltex_agent = src_obj->GetMtlTexAgent(i)) == nullptr)
				continue;
...
LW_RESULT lwNodePrimitive::RenderSubset(DWORD subset) {
	...
	if (_mtltex_agent_seq[subset] == nullptr)
		goto __ret;
...
LW_RESULT lwNodePrimitive::HitTest(lwPickInfo* info, const lwVector3* org, const lwVector3* ray) {
	...
	{
		lwIBoundingSphere* bs = nullptr;
		lwIBoundingBox* bb = nullptr;

		lwMatrix44* mat = _render_agent->GetGlobalMatrix();

		if ((bs = _helper_object->GetBoundingSphere()) != nullptr) {
			if (LW_SUCCEEDED(bs->HitTest(info, org, ray, mat)))
				goto __addr_ret_ok;
		}

		if ((bb = _helper_object->GetBoundingBox()) != nullptr) {
...
LW_RESULT lwNodePrimitive::SetParent(lwINode* parent) {
	...
	if (LW_FAILED(SetParent(nullptr)))
		goto __ret;
...
LW_RESULT lwNodePrimitive::GetSubsetNum(DWORD* subset_num) {
	...
	if (subset_num == nullptr)
		goto __ret;

	// use mesh subset?
	if (_mesh_agent == nullptr)
		goto __ret;
	{
...
lwNodeBoneCtrl::lwNodeBoneCtrl(lwIResourceMgr* res_mgr)
	: _res_mgr(res_mgr), _ctrl_obj(nullptr) {
	_type = NODE_BONECTRL;
}
...
LW_RESULT lwNodeBoneCtrl::Update() {
	...
	if (LW_FAILED(_ctrl_obj->UpdateObject(_ctrl_obj, nullptr)))
		goto __ret;
...
LW_RESULT lwNodeBoneCtrl::Load(lwIAnimDataBone* data) {
	...
	lwIAnimCtrlObjBone* ctrl_obj_bone = nullptr;
...
lwNodeDummy::lwNodeDummy(lwIResourceMgr* res_mgr)
	: _res_mgr(res_mgr), _ctrl_obj(nullptr), _ass_obj(nullptr) {
	_type = NODE_DUMMY;
}
...
LW_RESULT lwDestroyNodeObject(lwITreeNode* node) {
	...
	if (node->EnumTree(__tree_enum_destroy, nullptr, TREENODE_PROC_INORDER) == TREENODE_PROC_RET_ABORT)
		goto __ret;
...
lwNodeObject::lwNodeObject(lwIResourceMgr* res_mgr)
	: _res_mgr(res_mgr), _obj_root(nullptr) {
	_name[0] = '\0';

	lwGUIDCreateObject((LW_VOID**)&_obj_root, LW_GUID_TREENODE);
	lwINode* node = nullptr;
...
LW_RESULT lwNodeObject::Update() {
	...
	if (_obj_root->EnumTree(__tree_enum_update, nullptr, TREENODE_PROC_PREORDER) == TREENODE_PROC_RET_ABORT)
		goto __ret;
...
LW_RESULT lwNodeObject::Render() {
	...
	if (_obj_root->EnumTree(__tree_enum_render, nullptr, TREENODE_PROC_PREORDER) == TREENODE_PROC_RET_ABORT)
		goto __ret;
...
LW_RESULT lwNodeObject::RemoveTreeNode(lwITreeNode* node) {
		...
		goto __ret;
	{
		...
		if (LW_FAILED(model->SetParent(nullptr)))
			goto __ret;
	}
	ret = LW_RET_OK;
__ret:
...
LW_RESULT lwNodeObject::Load(const char* file, DWORD flag, lwITreeNode* parent_node) {
	...
	if (flag == MODELOBJECT_LOAD_RESET || flag == MODELOBJECT_LOAD_MERGE) {
		...
		while (child_node) {
			...
			if (LW_FAILED(InsertTreeNode(parent_node, nullptr, child_node)))
				goto __ret;

			child_node = sibling_node;
		}

		tree_node->SetChild(nullptr);
		// release root dummy node
		//LW_RELEASE((lwINodeDummy*)tree_node->GetData());
	} else if (flag == MODELOBJECT_LOAD_MERGE2) {
		if (LW_FAILED(InsertTreeNode(parent_node, nullptr, tree_node)))
			goto __ret;
	}
