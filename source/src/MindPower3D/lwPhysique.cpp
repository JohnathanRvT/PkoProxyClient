...
lwGeomManager::~lwGeomManager() {
	for (auto& [str, pInfo] : m_GeomobjMap) {
		delete pInfo;
	}
	m_GeomobjMap.clear();

	for (auto& [str, pData] : m_AnimDataMap) {
		pData->Release();
	}
	m_AnimDataMap.clear();
}
...
lwGeomObjInfo* lwGeomManager::GetGeomObjInfo(const char file[]) {
	auto it = m_GeomobjMap.find(file);
	return it != m_GeomobjMap.end() ? it->second : nullptr;
}

bool lwGeomManager::LoadGeomobj(const char file[]) {
	static char path[LW_MAX_PATH];
	_snprintf_s(path, _TRUNCATE, "%s%s", "model\\character\\", file);
	auto* pInfo = new lwGeomObjInfo;
...
lwIAnimDataBone* lwGeomManager::GetBoneData(const char file[]) {
	auto it = m_AnimDataMap.find(file);
	return it != m_AnimDataMap.end() ? it->second : nullptr;
}

bool lwGeomManager::LoadBoneData(const char file[]) {
	static char path[LW_MAX_PATH];
	_snprintf_s(path, _TRUNCATE, "%s%s", "animation\\", file);
...
LW_RESULT lwPhysique::Destroy() {
	...
	DWORD i = 0;
	for (i = 0; i < LW_MAX_SUBSKIN_NUM; i++) {
...

{
	...
	path = path_info->GetPath(PATH_TYPE_ANIMATION) + file;
	...
	_tcsncpy_s(res.file_name, path.c_str(), _TRUNCATE);
	...
	if (LW_SUCCEEDED(_res_mgr->QueryAnimCtrl(&ret_id, &res))) {
		...
		if (LW_FAILED(tp->RegisterTask(__load_bone, (void*)bi))) {
			char szData[128];
			_snprintf_s(szData, _TRUNCATE, "Load bone file error!%s.", path);
		...
		if (i_data == nullptr) {
			lwCreateObjectGUID((LW_VOID**)&i_data, LW_GUID_ANIMDATABONE);
			if (LW_FAILED(i_data->Load(path.c_str())))
				goto __addr_1;
		}
		if (LW_FAILED(_res_mgr->CreateAnimCtrl((lwIAnimCtrl**)&ctrl_bone, ANIM_CTRL_TYPE_BONE)))
			goto __addr_1;
...
LW_RESULT lwPhysique::LoadPrimitive(DWORD part_id, lwIGeomObjInfo* geom_info) {
	...
	std::string tex_path = path_info->GetPath(PATH_TYPE_TEXTURE_CHARACTER);
	...
	for (j = 0; j < info->mtl_num; j++) {
		...
		for (i = 0; i < LW_MAX_TEXTURESTAGE_NUM; i++) {
			...
			mtltex_agent->LoadTextureStage(tex_info, tex_path.c_str());
		}
	...
	if (info->anim_size > 0) {
		imp->LoadAnimData(&info->anim_data, tex_path.c_str(), nullptr);
	}
...
LW_RESULT lwPhysique::LoadPrimitive(DWORD part_id, const char* file) {
	...
	std::string path = path_info->GetPath(PATH_TYPE_MODEL_CHARACTER) + file;
	...
	//lwGeomObjInfo info;
	//if( LW_FAILED( info.Load( path.c_str() ) ) )
	//{
	//    return LW_RET_FAILED;
	//}
	...
	_tcsncpy_s(rfm.file_name, path.c_str(), _TRUNCATE);
	...
	for (j = 0; j < pInfo->mtl_num; j++) {
		...
		for (i = 0; i < LW_MAX_TEXTURESTAGE_NUM; i++) {
			...
			path = tex_path + tex_info->file_name;

			if (LW_SUCCEEDED(_res_mgr->QueryTex(&tex_id, path.c_str()))) {
	...
	if (pInfo->anim_size > 0) {
		imp->LoadAnimData(&pInfo->anim_data, tex_path.c_str(), &res);
	}
...
LW_RESULT lwPhysique::Update() {
	...
	if (_anim_agent) {
			...
			goto __ret;
		{
			...
			if (LW_FAILED(ctrl_obj->UpdateObject(ctrl_obj, nullptr)))
				goto __ret;
			{
				lwIPrimitive* pri;
