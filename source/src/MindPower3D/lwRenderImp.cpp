...
lwRenderCtrlAgent::lwRenderCtrlAgent(lwIResourceMgr* res_mgr)
	: _res_mgr(res_mgr), _mesh_agent(nullptr), _mtltex_agent(nullptr), _anim_agent(nullptr),
	  _render_ctrl(nullptr), _decl_type(VDT_INVALID), _vs_type(VST_INVALID) {
...
LW_RESULT lwRenderCtrlAgent::BeginSet() {
	...
	if (_mesh_agent == nullptr || _render_ctrl == nullptr)
		goto __ret;
...
LW_RESULT lwRenderCtrlAgent::BeginSetSubset(DWORD subset) {
	...
	if (_mtltex_agent == nullptr || _render_ctrl == nullptr)
		goto __ret;
