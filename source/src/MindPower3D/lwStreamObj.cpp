...
lwStreamObjVB::lwStreamObjVB(lwIDeviceObject* dev_obj)
	: _dev_obj(dev_obj), _buf(nullptr), _fvf(0) {
}
...
lwStreamObjIB::lwStreamObjIB(lwIDeviceObject* dev_obj)
	: _dev_obj(dev_obj), _buf(nullptr), _format(D3DFMT_INDEX16) {
}
...
lwStaticStreamMgr::lwStaticStreamMgr(lwIDeviceObject* dev_obj)
	...
	_entity_vb_seq = nullptr;
	_entity_ib_seq = nullptr;
...
#if (defined LW_USE_DX9)
	_dev_obj->SetVertexShader(nullptr);
#endif
	if (FAILED(_dev_obj->SetFVFX(fvf)))
		goto __ret;
...
LW_RESULT lwLockableStreamVB::Create(void* data, DWORD size, DWORD usage, DWORD fvf) {
	...
	if (LW_FAILED(_dev_obj->CreateVertexBuffer(size, usage, fvf, D3DPOOL_DEFAULT, &_buf, nullptr)))
		goto __ret;
...
LW_RESULT lwLockableStreamVB::Lock(UINT offset, UINT size, void** data, DWORD flag) {
	...
	if (_buf == nullptr || _lock_cnt > 0 || (offset + size) >= _size)
		goto __ret;
...
LW_RESULT lwLockableStreamVB::ResetDevice() {
	...
	if (_buf == nullptr && _size != 0) {
		if (LW_FAILED(_dev_obj->CreateVertexBuffer(_size, _usage, _fvf, D3DPOOL_DEFAULT, &_buf, nullptr)))
			goto __ret;
...
LW_RESULT lwLockableStreamIB::Create(void* data, DWORD size, DWORD usage, DWORD fvf) {
	...
	if (LW_FAILED(_dev_obj->CreateIndexBuffer(size, usage, (D3DFORMAT)fvf, D3DPOOL_DEFAULT, &_buf, nullptr)))
		goto __ret;
...
LW_RESULT lwLockableStreamIB::ResetDevice() {
	...
	if (_buf == nullptr && _size != 0) {
		if (LW_FAILED(_dev_obj->CreateIndexBuffer(_size, _usage, (D3DFORMAT)_fvf, D3DPOOL_DEFAULT, &_buf, nullptr)))
			goto __ret;
...
LW_RESULT lwSurfaceStream::CreateRenderTarget(UINT width, UINT height, D3DFORMAT format, D3DMULTISAMPLE_TYPE multi_sample, DWORD multi_sample_quality, BOOL lockable, HANDLE* handle) {
	...
	if (FAILED(dev_obj->CreateRenderTarget(&_surface, width, height, format, multi_sample, multi_sample_quality, lockable, nullptr)))
		goto __ret;
...
LW_RESULT lwSurfaceStream::ResetDevice() {
	...
	if (_type == SURFACESTREAM_RENDERTARGET) {
		ret = dev_obj->CreateRenderTarget(&_surface, _width, _height, (D3DFORMAT)_format, _multi_sample, _multi_sample_quality, _lockable, nullptr);
	} else if (_type == SURFACESTREAM_DEPTHSTENCIL) {
		ret = dev_obj->CreateDepthStencilSurface(&_surface, _width, _height, (D3DFORMAT)_format, _multi_sample, _multi_sample_quality, _discard, nullptr);
	}
...
LW_RESULT lwVertexBuffer::Release() {
	...
	if (LW_FAILED(Destroy()))
		goto __ret;
	{
		auto* devobj = reinterpret_cast<lwDeviceObject*>(_dev_obj);
...
lwVertexBuffer::lwVertexBuffer(lwIDeviceObject* dev_obj)
	: _dev_obj(dev_obj), _buf(nullptr), _next(nullptr), _prev(nullptr) {
	_dlock_pos = 0;
...
LW_RESULT lwVertexBuffer::LoadData(const void* data_seq, DWORD data_size, UINT offset, DWORD lock_flag) {
	...
	if (data_seq == nullptr || data_size == 0)
		goto __ret;

	if ((offset + data_size) > _buf_info.size)
		goto __ret;
	{
		D3DLOCK_TYPE* p_buf = nullptr;
...
LW_RESULT lwVertexBuffer::LoadDataDynamic(const void* data_seq, DWORD data_size) {
	...
	if (_buf == nullptr || ((_buf_info.usage & D3DUSAGE_DYNAMIC) == 0))
		goto __ret;
