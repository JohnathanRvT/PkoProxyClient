...
unsigned int lwThreadPool::_ThreadProc() {
	...
	lwThreadInfo* thread_info = nullptr;

	DWORD i = 0;
	for (i = 0; i < _thread_seq_size; i++) {
...
LW_RESULT lwThreadPool::Create(DWORD thread_seq_size, DWORD task_seq_size, DWORD suspend_flag) {
	...
	_event_signal = ::CreateEvent(nullptr, TRUE, FALSE, nullptr);
	if (_event_signal == nullptr)
		goto __ret;

	if (LW_FAILED(_event_pool.Create(nullptr, TRUE, TRUE, nullptr)))
		goto __ret;
	{
		...
		_thread_num = 0;
		DWORD i = 0;
		for (i = 0; i < thread_seq_size; i++) {
			_thread_seq[i].handle = ::_beginthreadex(nullptr, 0, __thread_proc, (void*)this, suspend_flag, &_thread_seq[i].id);
...
LW_RESULT lwThreadPool::FindTask(lwThreadProc proc, void* param) {
	return _FindTask(nullptr, proc, param);
}
