...
LW_RESULT lwDeviceObject::CreateDirect3D() {
	...
	if ((_d3d = Direct3DCreateX(D3D_SDK_VERSION)) == nullptr)
		goto __ret;
...
LW_RESULT lwDeviceObject::CreateCubeTexture(IDirect3DCubeTextureX** o_tex, UINT edge_length, UINT levels, DWORD usage, D3DFORMAT format, D3DPOOLX pool, HANDLE* handle) {
	return _dev->CreateCubeTextureX(edge_length, levels, usage, format, pool, o_tex, handle);
}
LW_RESULT lwDeviceObject::CreateOffscreenPlainSurface(IDirect3DSurfaceX** surface, UINT width, UINT height, D3DFORMAT format, D3DPOOLX pool, HANDLE* handle) {
	return _dev->CreateOffscreenPlainSurfaceX(width, height, format, pool, surface, handle);
}
#if (defined LW_USE_DX9)
BOOL lwDeviceObject::GetSoftwareVertexProcessing() {
...
LW_RESULT lwDeviceObject::SetFVFX(DWORD fvf) {
	if (_fvf_value != fvf) {
		_fvf_value = fvf;
		return _dev->SetFVFX(fvf);
	}
	return LW_RET_OK;
}
