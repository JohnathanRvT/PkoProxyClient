...

	{
		for (int i = 0; i < num; i++) {
			if ((pi[i].start > pi[i].end) || (pi[i].end >= _frame_num)) {
				...
				char buf[256];
				_snprintf_s(buf, _TRUNCATE, "CharacterAction.tx table error!\n"
											"Character Type: %d\n"
											"Action ID: %d\n"
											"Start: %d\n"
											"End: %d\n"
				...
				result += hint;
				MessageBox(nullptr, result.c_str(), "error", MB_OK);
				goto __ret;
			}
		}
...
LW_RESULT lwPoseCtrl::ReplacePose(DWORD id, const lwPoseInfo* pi) {
	...
	if ((id >= (DWORD)_pose_num) || (pi == nullptr))
		goto __ret;
