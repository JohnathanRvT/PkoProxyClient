...
lwPrimitive::lwPrimitive(lwIResourceMgr* mgr)
	: _res_mgr(mgr), _id(LW_INVALID_INDEX), _parent_id(LW_INVALID_INDEX), _helper_object(nullptr) {
	_anim_agent = nullptr;
...
LW_RESULT lwPrimitive::LoadRenderCtrl(const lwRenderCtrlCreateInfo* rcci) {
	...
	lwIRenderCtrl* render_ctrl = nullptr;
...
lwMatrix44* lwPrimitive::GetMatrixLocal() {
	return _render_agent ? _render_agent->GetLocalMatrix() : nullptr;
}
lwMatrix44* lwPrimitive::GetMatrixGlobal() {
	return _render_agent ? _render_agent->GetGlobalMatrix() : nullptr;
}
...
LW_RESULT lwPrimitive::ExtractGeomObjInfo(lwIGeomObjInfo* info) {
		...
		goto __ret;
	{
		...
		// mtltex
		{
			...
			//
			a->mtl_size = (DWORD)lwGetMtlTexInfoSize(&a->mtl_seq[0], a->mtl_num);
