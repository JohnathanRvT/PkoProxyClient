...
lwTex::lwTex(lwIResourceMgr* res_mgr)
	: _res_mgr(res_mgr), _reg_id(LW_INVALID_INDEX),
	  _stage(LW_INVALID_INDEX), _tex(nullptr), _colorkey_type(COLORKEY_TYPE_NONE),
...
LW_RESULT lwTex::EndSet() {
	...
	dev_obj->SetTexture(_stage, nullptr);
...
LW_RESULT lwTex::LoadTexInfo(const lwTexInfo* info, const char* tex_path) {
	...
	case TEX_TYPE_FILE:
		if (tex_path) {
			_snprintf_s(_file_name, _TRUNCATE, "%s%s", tex_path, info->file_name);
		} else {
...
//#undef USE_DDS_FILE
		...
		// 这个版本是为了兼容非dds文件检查
		lwDDSHeader* dds_header = nullptr;
		BOOL dds_flag = 1;
...
__ret:
	...
	if (LW_FAILED(ret)) {
		lwMessageBox("load texture file error with: (%d) %s\n", ret, this->_file_name);
	}
...
DWORD lwTex::SetLOD(DWORD level) {
	if ((_tex == nullptr) || (_pool != D3DPOOL_MANAGED))
		return 0;
...
LW_RESULT lwMesh::LoadVideoMemory() {
	...
	case STREAM_GENERIC:
		if (_data_info.vb_size > 0) {
			...
			if (LW_FAILED(_svb->Create(_data_info.vb_size, 0, _mesh_info_ptr->fvf, D3DPOOL_DEFAULT, _data_info.vb_stride, nullptr)))
				goto __ret;
		...
		if (_data_info.ib_size > 0) {
			...
			if (LW_FAILED(_sib->Create(_data_info.ib_size, 0, D3DFMT_INDEX16, D3DPOOL_DEFAULT, nullptr)))
				goto __ret;
...
LW_RESULT lwMesh::AddStateToSet(DWORD state, DWORD value) {
	for (int i = 0; i < LW_MESH_RS_NUM; i++) {
		if (_mesh_info.rs_set[i].state == LW_INVALID_INDEX || _mesh_info.rs_set[i].state == state) {
...
LW_RESULT lwMesh::BeginSet() {
	...
	dev_obj->SetVertexShader(nullptr);
#endif
	dev_obj->SetFVFX(_mesh_info_ptr->fvf);
...
lwMtlTexAgent::lwMtlTexAgent(lwIResourceMgr* mgr)
	...
	DWORD i = 0;
	for (i = 0; i < LW_MAX_MTL_TEX_NUM; i++) {
		_tex_seq[i] = nullptr;
	}
...
LW_RESULT lwMtlTexAgent::LoadMtlTex(lwMtlTexInfo* info, const char* tex_path) {
	...
	DWORD j = 0;
	for (j = 0; j < LW_MAX_MTL_TEX_NUM; j++) {
...
LW_RESULT lwMtlTexAgent::BeginSet() {
...

	{
		DWORD v = 129;
		lwRenderStateAtom* rsa;
		for (i = 0; i < _rsa_0.GetStateNum(); i++) {
	...
	if (_opacity != 1.0f) {
		...
		if (_transp_type != MTLTEX_TRANSP_FILTER) {
			if (_opacity_reserve_rs[0] != LW_INVALID_INDEX) {
				_rsa_0.ResetStateValue(D3DRS_SRCBLEND, _opacity_reserve_rs[0], nullptr);
				_opacity_reserve_rs[0] = LW_INVALID_INDEX;
			}
			if (_opacity_reserve_rs[1] != LW_INVALID_INDEX) {
				_rsa_0.ResetStateValue(D3DRS_DESTBLEND, _opacity_reserve_rs[1], nullptr);
				_opacity_reserve_rs[1] = LW_INVALID_INDEX;
			}
		}
	}
...
lwMeshAgent::lwMeshAgent(lwIResourceMgr* res_mgr)
	: _res_mgr(res_mgr), _mesh_obj(nullptr) {
	_mt_flag = 0;
...
LW_RESULT lwMeshAgent::BeginSet() {
		...
		goto __ret;
	{
		...
		// render state
		DWORD i = 0;
		for (i = 0; i < _rs_set.SEQUENCE_SIZE; i++) {
...
LW_RESULT lwThreadPoolMgr::Create() {
	...
	for (i = 0; i < CRITICALSECTION_SEQ_SIZE; i++) {
...
lwResourceMgr::lwResourceMgr(lwISysGraphics* sys)
	: _sys_graphics(sys), _mesh_size_sm(0), _mesh_size_vm(0), _tex_size_sm(0), _tex_size_vm(0),
	  _shader_mgr(nullptr), _thread_pool_mgr(nullptr) {
...
lwResourceMgr::~lwResourceMgr() {
	...
	for (auto& [str, pTexture] : mColorFilterTextureList) {
		if (pTexture) {
			pTexture->Release();
		}
	}
...
LW_RESULT lwResourceMgr::CreateMesh(lwIMesh** ret_obj) {
	...
	if ((o = LW_NEW(lwMesh(this))) == nullptr)
		goto __ret;
...
LW_RESULT lwResourceMgr::CreateTex(lwITex** ret_obj) {
	...
	if ((o = LW_NEW(lwTex(this))) == nullptr)
		goto __ret;
...
LW_RESULT lwResourceMgr::CreateMeshAgent(lwIMeshAgent** ret_obj) {
	...
	lwMeshAgent* o = nullptr;

	if ((o = LW_NEW(lwMeshAgent(this))) == nullptr)
		goto __ret;
...
LW_RESULT lwResourceMgr::CreateMtlTexAgent(lwIMtlTexAgent** ret_obj) {
	...
	lwMtlTexAgent* o = nullptr;

	if ((o = LW_NEW(lwMtlTexAgent(this))) == nullptr)
		goto __ret;
...
LW_RESULT lwResourceMgr::CreateRenderCtrlVS(lwIRenderCtrlVS** ret_obj, DWORD type) {
	...
	lwIRenderCtrlVS* o = nullptr;

	if (_render_ctrl_proc_seq[type] == nullptr)
		goto __ret;
...
LW_RESULT lwResourceMgr::CreatePrimitive(lwIPrimitive** ret_obj) {
	...
	lwPrimitive* o = nullptr;

	if ((o = LW_NEW(lwPrimitive(this))) == nullptr)
		goto __ret;
...
LW_RESULT lwResourceMgr::CreateHelperObject(lwIHelperObject** ret_obj) {
	...
	lwHelperObject* o = nullptr;

	if ((o = LW_NEW(lwHelperObject(this))) == nullptr)
		goto __ret;
...
LW_RESULT lwResourceMgr::CreatePhysique(lwIPhysique** ret_obj) {
	...
	lwIPhysique* o = nullptr;

	if ((o = LW_NEW(lwPhysique(this))) == nullptr)
		goto __ret;
...
LW_RESULT lwResourceMgr::CreateModel(lwIModel** ret_obj) {
	...
	if ((o = LW_NEW(lwModel(this))) == nullptr)
		goto __ret;
...
LW_RESULT lwResourceMgr::CreateItem(lwIItem** ret_obj) {
	...
	if ((o = LW_NEW(lwItem(this))) == nullptr)
		goto __ret;
...
LW_RESULT lwResourceMgr::CreateNodeObject(lwINodeObject** ret_obj) {
	...
	lwINodeObject* o = nullptr;

	if ((o = LW_NEW(lwNodeObject(this))) == nullptr)
		goto __ret;
...
IDirect3DTextureX* lwResourceMgr::_createMonochromaticTexture(
	...
	if (filterTexture.empty()) {
		IDirect3DTextureX* texture = nullptr;
		IDirect3DDeviceX* device = _dev_obj->GetDevice();

		HRESULT hr = device->CreateTextureX(
			width, height,
			0, D3DUSAGE_DYNAMIC,
			D3DFMT_A8R8G8B8,
			D3DPOOL_DEFAULT,
		...
		auto* writer = reinterpret_cast<size_t*>(lockedRect.pBits);
		size_t y = 0;
		for (y = 0; y < height; ++y) {
