...
// 加载高度图
//----------------------------------------------------
bool CHeightmap::LoadHeightmap(MPMap* pMap) {
	m_pMap = pMap;
...
int CHeightmap::GetHeightIndexOffset(int index, char offsetZ, char offsetX) {
	...
	if (retIndex - offsetX >= 0 && retIndex - offsetZ < m_SizeX * m_SizeY) {
		if (retIndex < 0)
			return retIndex + m_SizeX * m_SizeY;
		else
			return retIndex - m_SizeX * m_SizeY;
	} else {
		if (retIndex - offsetX < 0)
			num = index + (m_SizeY + offsetZ) * m_SizeX;
		else
			num = index + (m_SizeY - offsetZ) * m_SizeX;
	}
	if (num + offsetX >= 0 && num + offsetX < m_SizeX * m_SizeY) {
		return num + offsetX;
	} else if (num + offsetX < 0)
		return num + offsetX + m_SizeX * m_SizeY;
	else
		return num + offsetX - m_SizeX * m_SizeY;
}
...
bool CHeightmap::IsInFrustum(Boundary_3D* pBoundary) {
	if (IsInFrustum(pBoundary->minx, pBoundary->minz))
		return true;
	if (IsInFrustum(pBoundary->maxx, pBoundary->minz))
		return true;
	if (IsInFrustum(pBoundary->maxx, pBoundary->maxz))
		return true;
	if (IsInFrustum(pBoundary->minx, pBoundary->maxz))
		return true;
...
DWORD CHeightmap::GetColor(int nWorldX, int nWorldY) {
	...
	int y = InverseY % m_pMap->_nSectionHeight;
	SNewFileTile* pTile = (SNewFileTile*)(m_pMap->m_pMapData + dwPos);
	return LW_RGB565TODWORD(pTile[y * m_pMap->_nSectionWidth + x].sColor);
}