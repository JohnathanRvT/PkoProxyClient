...
MPConsole::MPConsole()
	...
	AddText("Begin of MindPower Engine");
...
BOOL MPConsole::OnKeyDownEvent(int nKeyCode) {
	...
	case VK_UP: {
		...
		if (_bShowCursor)
			_snprintf_s(_szInputTmp, 128, _TRUNCATE, "]%s_", _szInput);
		else
			_snprintf_s(_szInputTmp, 128, _TRUNCATE, "]%s", _szInput);
		break;
	}
	}
	return TRUE;
...
BOOL MPConsole::OnCharEvent(TCHAR iChar, DWORD dwParam) {
	...
	if (iChar == '\r') {
		...
		if (strcmp(_szInput, "programmer") == 0) {
			_AddText("                MindPower Stars                       ", false);
			_AddText("                                                        ", false);
