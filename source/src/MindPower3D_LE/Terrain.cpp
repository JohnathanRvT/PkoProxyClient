...
MINDPOWER_API CHeightmap g_Hmap;
MINDPOWER_API bool g_bLineMode = false;
MINDPOWER_API int g_nNoDrawLevel = 0;
#define WORLD_SCALE 1.0f

lwColorValue4b s_Ambient; //环境光
...
void HeightmapVertex::SetupVertex(int nMapX, int nMapY, int nMoveX, int nMoveY, int ofsX, int ofsY) {
	...
	{
		lwColorValue4b vert_amb, c;
		c.color = pFileTile ? (LW_RGB565TODWORD(pFileTile->sColor) | 0xff000000) : 0;
...
bool CTerrain::LoadTerrain(MPMap* pMap) {
	m_pMap = pMap;
	// height map
...
bool CTerrain::SaveLodmap(char* szFilename, int width, int height) {
	...
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++) {
			switch (m_LODmap[y][x]) { //Blue             //Green            //Red
			case 0:
				pBits[trg] = 0;
				pBits[trg + 1] = 0;
				pBits[trg + 2] = 0;
...
MINDPOWER_API int g_nPosX = 0;
MINDPOWER_API int g_nPosY = 0;
MINDPOWER_API int g_nPosZ = 0;
MINDPOWER_API DWORD g_dwRenderTerrainTime = 0;
MINDPOWER_API DWORD g_dwRenderTerrainCnt = 0;
...
void CTerrain::GenerateGroup(int nX, int nY, HeightmapVertex* pV, const float* uv, int nVertexCnt) {
	...
	if (pFileTile) {
		...
		for (int layer = 0; layer < 4; layer++) {
			MPTileTex* pLayer = &(mpTile.TexLayer[layer]);
