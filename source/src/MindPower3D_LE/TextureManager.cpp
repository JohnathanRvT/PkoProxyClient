...
MINDPOWER_API CTextureManager* g_pTextureManager = NULL; // 有效期: Application(Init-Destory)
...
bool CTextureManager::BatchLoad(const char* szIniFile) {
	...
	while (!feof(fp)) {
		...
		if (nInMem) {
			...
			if (bAMF) {
				if (nDesc)
					LoadAMFTextureDesc(nHandle, szFilename, nIndex, nMipmap);
				else
					LoadAMFTexture(nHandle, szFilename, nIndex, MM_AUTO, nMipmap);
			} else {
				if (nDesc)
					LoadTextureDesc(nHandle, szFilename, nMipmap);
				else
					LoadTexture(nHandle, szFilename, MM_AUTO, nMipmap);
			}
		}
	}
	fclose(fp);
...
void CTextureManager::Update(void) {
	...
	if (tm - m_UpdateTime > UPDATETIME) {
		std::map<TextureHandle, CTexture*>::iterator itor = m_TextureMap.begin();
		while (itor != m_TextureMap.end()) {
			if ((tm - (*itor).second->GetLastUseTime()) > MAXEXIST && !((*itor).second->IsRelease()) && (*itor).second->GetMemMode() == MM_AUTO) {
