...
bool CUdpServer::Read(ReceiveEvent pEvent, void* pParam) {
	...
	nBufferLength = nSpaceRemaining = sizeof(chIncomingDataBuffer);

	nBytesRead = recvfrom(m_Socket, (LPSTR)(chIncomingDataBuffer), nSpaceRemaining, 0, (LPSOCKADDR)&m_saFromAddr, &nLenght);
	...
	chIncomingDataBuffer[min(nBytesRead,4096-1)] = '\0';
	if (lstrlen(chIncomingDataBuffer) != 0) {
		if (pEvent) {
			pEvent(this, inet_ntoa(m_saFromAddr.sin_addr), ntohs(m_saFromAddr.sin_port), chIncomingDataBuffer, nBytesRead, pParam);
		}
	}
	return true;
}
